...
The upper map places one physical-route merchant against country outlines. This merchant has `28` site coordinates across `6` countries and `66,349` arrival rows in the arrival primitive. The visible markers show the footprint of that site estate at map scale; where several sites sit close together, the map reads them as a clustered location rather than as 28 perfectly separable dots. The important point is not dot-counting from the image, but the operating object being shown: these are physical site coordinates, not transaction rows. The visual therefore reads like an outlet geography, with concrete places where the platform can attach physical route context.

The country labels in the upper map help make the concept concrete. The merchant's site estate is not only a count of locations; it has spatial spread across a recognizable regional map. Most of the footprint sits across mainland Europe, with a northern extension into Finland. That matters because a physical `site_id` is not merely an arbitrary endpoint token. It represents a site-like geography that can later matter for local-time interpretation, country/site grouping, and endpoint-level exposure.
...
The left panel shows the distribution of physical site counts per physical merchant. Most physical merchants sit around the low tens of sites, with a median of `17`. The selected illustrative merchant has `28` sites, so it is above the median but still inside the ordinary body of the distribution rather than being a maximum-edge case. The full range runs from `1` to `124` physical sites per merchant.
