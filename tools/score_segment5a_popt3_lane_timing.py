#!/usr/bin/env python3
"""Emit Segment 5A S5 lane timing artifacts for POPT.3 from run logs/reports."""
...
MARKERS = {
    "input_load_schema_validation": re.compile(
        r"S5: phase input_load_schema_validation complete .*?\(elapsed=([0-9.]+)s, delta=([0-9.]+)s\)"
    ),
    "recomposition_checks": re.compile(
        r"S5: phase recomposition_checks complete .*?\(elapsed=([0-9.]+)s, delta=([0-9.]+)s\)"
    ),
    "issue_table_assembly": re.compile(
        r"S5: phase issue_table_assembly complete .*?\(elapsed=([0-9.]+)s, delta=([0-9.]+)s\)"
    ),
    "bundle_index_report_write": re.compile(
        r"S5: phase bundle_index_report_write complete .*?\(elapsed=([0-9.]+)s, delta=([0-9.]+)s\)"
    ),
    "output_write_idempotency": re.compile(
        r"S5: bundle complete .*?\(elapsed=([0-9.]+)s, delta=([0-9.]+)s\)"
    ),
}
...
def _parse_log_markers(run_log_path: Path) -> dict[str, dict[str, float] | None]:
    ...
    for line in run_log_path.read_text(encoding="utf-8", errors="ignore").splitlines():
        if "seg_5A.s5_validation_bundle" not in line:
            continue
...
def _write_md(path: Path, payload: dict[str, Any]) -> None:
    ...
    lines = [
        "# Segment 5A POPT.3 S5 Lane Timing",
        "",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Emit Segment 5A POPT.3 S5 lane timing artifact")
    ...
    for lane in (
        ...
        "recomposition_checks",
        "issue_table_assembly",
        "bundle_index_report_write",
        "output_write_idempotency",
    ):
        ...
        lane_rows.append(
            {
                ...
                "seconds": sec,
                "hms": _fmt_hms(sec),
                "share_of_s5_wall": (sec / s5_wall_s) if sec is not None and s5_wall_s > 0 else None,
            }
        )
    ...
    payload: dict[str, Any] = {
        ...
        "lane_timing": {
            ...
            "wall_minus_lane_delta_sum_s": s5_wall_s - lane_total,
        },
        "quality_checks": {
    ...
    out_json = (
        Path(args.out_json)
        if args.out_json
        else runs_root / "reports" / f"segment5a_popt3_lane_timing_{run_id}.json"
    )
    out_md = (
        Path(args.out_md)
        if args.out_md
        else runs_root / "reports" / f"segment5a_popt3_lane_timing_{run_id}.md"
    )
