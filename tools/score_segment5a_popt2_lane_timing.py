#!/usr/bin/env python3
"""Emit Segment 5A S4 lane timing artifacts for POPT.2 from run logs/reports."""
...
MARKERS = {
    "input_load_schema_validation": re.compile(
        r"S4: phase input_load_schema_validation complete .*?\(elapsed=([0-9.]+)s, delta=([0-9.]+)s\)"
    ),
    "domain_horizon_mapping": re.compile(
        r"S4: phase domain_horizon_mapping complete .*?\(elapsed=([0-9.]+)s, delta=([0-9.]+)s\)"
    ),
    "overlay_compute": re.compile(
        r"S4: phase overlay_compute complete .*?\(elapsed=([0-9.]+)s, delta=([0-9.]+)s\)"
    ),
    "output_schema_validation": re.compile(
        r"S4: phase output_schema_validation complete .*?\(elapsed=([0-9.]+)s, delta=([0-9.]+)s\)"
    ),
    "output_write": re.compile(
        r"S4: phase output_write complete \(elapsed=([0-9.]+)s, delta=([0-9.]+)s\)"
    ),
}
...
def _parse_log_markers(run_log_path: Path) -> dict[str, dict[str, float] | None]:
    ...
    for line in run_log_path.read_text(encoding="utf-8", errors="ignore").splitlines():
        if "seg_5A.s4_calendar_overlays" not in line:
            continue
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Emit Segment 5A POPT.2 S4 lane timing artifact")
    ...
    s4_report_path = _find_state_report(run_root, "S4")
    s4_report = _load_json(s4_report_path)
    s4_status = str(s4_report.get("status") or "")
    s4_wall_s = float(s4_report.get("durations", {}).get("wall_ms", 0.0)) / 1000.0
        ...
        lane_rows.append(
            {
                ...
                "seconds": sec,
                "hms": _fmt_hms(sec),
                "share_of_s4_wall": (sec / s4_wall_s) if sec is not None and s4_wall_s > 0 else None,
            }
        )
    ...
    payload: dict[str, Any] = {
        ...
        "lane_timing": {
            "table": lane_rows,
            "s4_wall_s": s4_wall_s,
            "s4_wall_hms": _fmt_hms(s4_wall_s),
            "lane_delta_sum_s": lane_total,
            "lane_delta_sum_hms": _fmt_hms(lane_total),
            "wall_minus_lane_delta_sum_s": s4_wall_s - lane_total,
        },
        "quality_checks": {
    ...
    out_json = (
        Path(args.out_json)
        if args.out_json
        else runs_root / "reports" / f"segment5a_popt2_lane_timing_{run_id}.json"
    )
    out_md = (
        Path(args.out_md)
        if args.out_md
        else runs_root / "reports" / f"segment5a_popt2_lane_timing_{run_id}.md"
    )
