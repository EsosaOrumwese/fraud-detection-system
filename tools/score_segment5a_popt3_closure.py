#!/usr/bin/env python3
"""Score Segment 5A POPT.3 closure gates from baseline/candidate artifacts."""
...
def _baseline_s5_report_path(baseline_payload: dict[str, Any], runs_root: Path, baseline_run_id: str) -> Path:
    baseline_report = (
        baseline_payload.get("run", {}).get("report_paths", {}).get("S5")
    ...
    return _find_state_report(baseline_run_root, "S5")
...
def _output_exists(run_root: Path, report: dict[str, Any], key: str) -> bool:
    output = report.get("output", {})
    rel = output.get(key)
    if not rel:
        return False
    return (run_root / str(rel)).exists()
...
def _write_md(path: Path, payload: dict[str, Any]) -> None:
    ...
    lines = [
        ...
        "",
        "## Runtime Gate",
        f"- S5 baseline: `{_fmt_hms(rt['s5']['baseline_s'])}` ({rt['s5']['baseline_s']:.3f}s)",
        f"- S5 candidate: `{_fmt_hms(rt['s5']['candidate_s'])}` ({rt['s5']['candidate_s']:.3f}s)",
        f"- S5 improvement: `{100.0 * rt['s5']['improvement_fraction']:.2f}%`",
        f"- gate rule: `{gates['runtime_gate_rule']}`",
        f"- runtime_gate_pass: `{gates['runtime_gate_pass']}`",
        "",
        "## Structural Veto",
        f"- s5_status_pass: `{veto['s5_status_pass']}`",
        f"- s5_counts_unchanged: `{veto['s5_counts_unchanged']}`",
        f"- s5_error_surface_clean: `{veto['s5_error_surface_clean']}`",
        f"- s5_required_outputs_present: `{veto['s5_required_outputs_present']}`",
        "",
        "## Verdict",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Score Segment 5A POPT.3 closure")
    parser.add_argument("--runs-root", default="runs/fix-data-engine/segment_5A")
    parser.add_argument("--candidate-run-id", required=True)
    parser.add_argument(
        "--baseline-json",
        default="runs/fix-data-engine/segment_5A/reports/segment5a_popt0_baseline_7f20e9d97dad4ff5ac639bbc41749fb0.json",
    )
    ...
    baseline_s5 = _state_elapsed_from_baseline_json(baseline_json_path, "S5")
    candidate_s5 = _state_elapsed_from_baseline_json(candidate_baseline_json, "S5")
    s5_impr = _improvement_fraction(baseline_s5, candidate_s5)

    s5_report = _load_json(_find_state_report(run_root, "S5"))

    baseline_s5_report_path = _baseline_s5_report_path(baseline_payload, runs_root, baseline_run_id)
    baseline_s5_report = _load_json(baseline_s5_report_path)

    count_keys = ("s1_rows", "s1_merchants", "s1_countries", "s1_tzids")
    baseline_counts = baseline_s5_report.get("counts", {})
    candidate_counts = s5_report.get("counts", {})
    s5_counts_unchanged = all(
        int(candidate_counts.get(key) or -1) == int(baseline_counts.get(key) or -2) for key in count_keys
    )

    required_outputs = (
        "validation_bundle_index_path",
        "validation_report_path",
        "validation_issue_table_path",
        "validation_passed_flag_path",
    )
    required_outputs_present = all(_output_exists(run_root, s5_report, key) for key in required_outputs)

    runtime_gate_pass = bool(candidate_s5 <= 180.0 or s5_impr >= 0.20)
    veto = {
        "s5_status_pass": str(s5_report.get("status") or "").upper() == "PASS",
        "s5_counts_unchanged": s5_counts_unchanged,
        "s5_error_surface_clean": not (s5_report.get("error_code") or s5_report.get("error_class")),
        "s5_required_outputs_present": required_outputs_present,
    }
    ...
    if all_veto_clear and all_runtime_clear:
        result = "UNLOCK_POPT4"
        reason = "POPT.3 runtime + structural gates passed."
    else:
        result = "HOLD_POPT3_REOPEN"
        reason = "One or more runtime/structural gates failed; continue bounded POPT.3 reopen."
    ...
    payload: dict[str, Any] = {
        ...
        "count_anchors": {
            "source": str(baseline_s5_report_path).replace("\\", "/"),
        ...
        "output_anchors": {
            "required_paths": list(required_outputs),
            "present": required_outputs_present,
        },
        "runtime": {
            "s5": {
        ...
        "veto": veto,
        "gates": {
            "runtime_gate_rule": "S5 <= 180s OR >=20% reduction vs baseline",
    ...
    out_json = (
        Path(args.out_json)
        if args.out_json
        else runs_root / "reports" / f"segment5a_popt3_closure_{candidate_run_id}.json"
    )
    out_md = (
        Path(args.out_md)
        if args.out_md
        else runs_root / "reports" / f"segment5a_popt3_closure_{candidate_run_id}.md"
    )
