...
[M] Upstream 1B egress & gates:
    - validation_bundle_1B              @ [manifest_fingerprint]
    - validation_passed_flag_1B         @ [manifest_fingerprint]  (_passed.flag; HashGate for 1B)
...
(M,R,P,N) --> (S0) Gate-in & sealed inputs (no RNG)
                    - Verify 1B’s fingerprint gate for this manifest_fingerprint:
                        * open validation_bundle_1B/ @ [manifest_fingerprint]
             ...
             (S2) Overrides & final tzid — per-site civil time egress  [NO RNG]
                inputs: s1_tz_lookup @ [seed, manifest_fingerprint],
                        tz_overrides (policy),
                        tz_world_2025a (tzid validity),
                        optional merchant_mcc_map (if sealed)
                -> site_timezones @ [seed, manifest_fingerprint]  (2A egress)
                     - PK/sort: (merchant_id, legal_country_iso, site_order)
             ...
             (S3) TZ timetable cache compile (tzdb → fingerprint cache)  [NO RNG]
                ...
                -> tz_timetable_cache @ [manifest_fingerprint]
                     ...
                     - write-once; partitioned [manifest_fingerprint]; no seed/run_id dimension
             ...
             (S4) TZ legality & DST windows (per-seed report)  [NO RNG]
                inputs: site_timezones[seed, manifest_fingerprint],
                        tz_timetable_cache[manifest_fingerprint],
                        s0_gate_receipt_2A
                -> s4_legality_report @ [seed, manifest_fingerprint]
                     ...
                     - read-only w.r.t. site_timezones & cache; no RNG
                                      ...
                                      |
                                      | tz_timetable_cache @ [manifest_fingerprint]
                                      | all s4_legality_report[seed, manifest_fingerprint] for this fingerprint
                                      v

             (S5) Validation bundle & PASS flag for 2A (fingerprint-scoped)  [NO RNG]
                inputs: s0_gate_receipt_2A,
                        tz_timetable_cache[manifest_fingerprint],
                        s4_legality_report[seed, manifest_fingerprint] for all seeds that have site_timezones
                -> validation_bundle_2A/      @ [manifest_fingerprint]
                     ...
                     - compute bundle digest:
                         ...
                         · SHA-256 → <hex64>
                -> validation_passed_flag_2A   @ [manifest_fingerprint]
...
- **Any consumer of 2A civil-time surfaces** (2B arrival mechanics, 3A/3B fraud posture, ingestion gate, scenario runner, model training):
    1) locate `data/layer1/2A/validation/manifest_fingerprint={manifest_fingerprint}/`,
