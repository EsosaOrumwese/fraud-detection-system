...
flowchart LR
  subgraph C["Black-box contract used by the platform"]
    direction TB
    C1["World identity<br/>manifest_fingerprint · parameter_hash · seed · scenario_id"]
    C2["Deterministic, immutable outputs<br/>for a fixed identity partition"]
    C3["Fail-closed readiness<br/>No PASS → No Read"]
  end

  subgraph L1["Layer 1 — Merchant operating world"]
    direction LR
    L1A["Merchant footprint<br/>outlets, sites, geography"]
    L1B["Civil-time + routing realism<br/>timezones, routing, virtual reach"]
  end

  subgraph L2["Layer 2 — Traffic world"]
    direction LR
    L2A["Demand intensity → realised arrivals<br/>weekly shape, overlays, routing evidence"]
  end

  subgraph L3["Layer 3 — Entity, behaviour, and supervision world"]
    direction LR
    L3A["Entity + graph world<br/>parties, products, credentials, devices, IPs"]
    L3B["Behaviour + supervision world<br/>flows, fraud overlays, labels, cases"]
  end

  L1A -->|site + geography basis| L1B
  L1A -->|merchant-world substrate| L2A
  L1B -->|time + routing context| L2A

  L1A -->|merchant + site world| L3A
  L1B -->|time/routing substrate| L3A
  L2A -->|sealed arrivals| L3A
  L2A -->|arrival + flow basis| L3B
  L3A -->|sealed entity world| L3B

  C1 -.-> L1A
  C2 -.-> L2A
  C3 -.-> L3B

  P["What the platform gets<br/>a governed black-box world for traffic, context, truth, and evidence"]
  L3B --> P
```
...
flowchart LR
  subgraph L1["Layer 1 — Merchant operating world"]
    direction LR
    A1["1A<br/>Outlet-world authority"]
    B1["1B<br/>Site-location authority"]
    A2["2A<br/>Civil-time authority"]
    B2["2B<br/>Routing realism authority"]
    A3["3A<br/>Zone allocation authority"]
    B3["3B<br/>Virtual-routing authority"]
  end

  subgraph L2["Layer 2 — Traffic world"]
    direction LR
    A5["5A<br/>Deterministic intensity authority"]
    B5["5B<br/>Arrival realisation authority"]
  end

  subgraph L3["Layer 3 — Entity, behaviour, and supervision world"]
    direction LR
    A6["6A<br/>Entity + static-risk authority"]
    B6["6B<br/>Behaviour + supervision authority"]
  end
  ...
  G["Every segment is a fail-closed authority boundary<br/>No PASS gate means no lawful downstream read"]
  G -.-> B3
  G -.-> B5
  G -.-> B6
```
...
flowchart LR
  ...
  R["Runtime replay"]
  S["Stress / readiness"]
  L["Supervision / labels"]
  C["Case history"]
  M["ML training / evaluation"]
  ...
  N["These are not five datasets.<br/>They are five uses of one truth path."]
  W --- N
```
