# Arval BNP Paribas Financial Data Analyst - Evidence-Led Resume Workbench

This workbench is reserved for building the Arval BNP Paribas Financial Data Analyst resume materials using:

`docs\experience_lake\outward-facing-assets\frameworks\evidence-led-resume-positioning-framework.md`

Source job ad:

`docs\experience_lake\outward-facing-assets\resume\job_ad_41_bnp_arval_financial_data_analyst\job_ad_source.md`

We are starting with **Step 1: 3 screens for Arval Financial Data Analyst** only.

The framework says the first move is to split the job ad into:
...
before creating proof buckets, evidence ledgers, evidence scoring, bullet portfolios, draft bullets, top sections, reviews, or placeholder values.

For **Arval BNP Paribas Financial Data Analyst**, the centre of gravity is:

> **finance data flows, SQL data modelling, large financial datasets, SAS/data governance awareness, finance reporting requests, and communication with non-technical finance stakeholders.**

This is **not** a fraud CV, not a payments/chargeback CV, and not a pure data science CV. The role sits inside the **finance team** and is about improving finance data processing, maintaining data flows, handling finance reporting requests, and upholding data quality/governance.

# 1. Business / problem screen

| Arval business problem | What the CV needs to prove |
| --- | --- |
| Finance team needs better data flows | You can build, maintain, and improve repeatable data flows for reporting or analysis |
| Existing SaaS data processing needs review | You can inspect current data processing, find inefficiencies or gaps, and improve how data is handled |
| Finance has recurring and ad-hoc data requests | You can respond to finance reporting needs with accurate datasets, reports, extracts, or summaries |
| Financial datasets need quality and governance | You can maintain clean, controlled, traceable datasets for reporting use |
| Finance users are non-technical | You can explain data issues, outputs, and changes clearly to finance stakeholders |
| Arval operates in vehicle leasing | Commercial/financial reporting should ideally connect to leasing, customer, contract, fleet, revenue, cost, or operational-finance data if we have evidence |

The strongest business positioning is probably:

```text
Financial Data Analyst | SQL Data Flows | Finance Reporting, Data Quality & Governance
```

or:

```text
Financial Data Analyst | SQL, SAS & Finance Data Modelling | Reporting Data Flows
```

Only include **SAS** in the headline if we can credibly support it.

# 2. Technical / capability screen

The ad's technical screen is clear: **SQL, data modelling, complex datasets, SAS, data governance, large financial datasets, reporting purposes**.

| Technical requirement | Concrete evidence to look for later |
| --- | --- |
| Strong SQL | SQL finance dataset query, SQL reporting table, SQL reconciliation query, SQL view for finance reporting |
| Data modelling | Finance reporting data model, fact/dimension table, dataset schema, reporting-ready table |
| Complex datasets | Large transaction/customer/contract/operational dataset, 2.35B-row data surface if relevant |
| Data flows | SQL data flow, ETL-style reporting workflow, scheduled extract, data pipeline, finance reporting table refresh |
| SaaS data processing review | SaaS export review, data-processing audit, field-mapping table, duplicate/missing-value check, data issue log |
| SAS programming | SAS data step, PROC SQL, SAS reporting dataset, SAS validation output - only if true |
| Data governance | data dictionary, validation checklist, access/definition controls, report definition log, data-quality rules |
| Finance reporting | finance reporting dataset, Excel finance workbook, Power BI finance dashboard, monthly finance pack |
| Data quality | reconciliation workbook, SQL QA checks, duplicate/missing-record checks, dashboard validation checks |

Important: **SQL and finance data flows must dominate**. Python/ML should not dominate this CV unless used to support data quality, automation, or large-dataset processing.

# 3. Working-style / stakeholder screen

The ad wants someone collaborative, finance-facing, process-improvement minded, and comfortable communicating with non-technical finance team members. It also emphasises data quality, governance, and working closely with team members.

| Working-style requirement | CV evidence to look for |
| --- | --- |
| Finance stakeholder communication | Finance report commentary, data request summary, metric definition note, reconciliation explanation |
| Collaboration | Worked with finance, operations, risk, data, or reporting stakeholders to resolve data issues |
| Process improvement | Reduced manual reporting time, improved refresh reliability, standardised data flow, automated checks |
| Data quality ownership | Built validation checks, reconciled source records, resolved duplicate/missing entries |
| Governance discipline | Maintained data definitions, controlled reporting logic, documented data lineage or KPI definitions |
| Handling general requests | Responded to ad-hoc data extracts, reporting queries, variance checks, finance summaries |
| Financial interest/commercial awareness | Linked analysis to revenue, cost, exposure, payment, transaction, operational or contract metrics |

The working-style story should be:

```text
I can build and maintain SQL-led finance reporting datasets, improve data-flow quality, respond to finance data requests, and explain data issues clearly to non-technical finance stakeholders.
```

# What this means for the CV direction

For this Arval role, the CV should probably pivot toward:
...
```text
SQL reporting datasets
data flows
large financial datasets
data modelling
SaaS data processing review
finance data requests
data-quality checks
governance documentation
Excel/Power BI finance reporting
SAS, if defensible
non-technical finance communication
```

Next step: **proof buckets for the Arval job ad**.

---

Now we move to **proof buckets** for the **Arval BNP Paribas Financial Data Analyst** role.

Per the framework, this step answers: **"What must this CV prove for this specific role?"** We are not building the evidence ledger yet, and we are not drafting bullets yet. The framework's sequence is: job ad -> proof buckets -> evidence ledger -> scoring -> bullet portfolio -> bullets.

Arval's ad is centred on joining the finance team to **build and maintain data flows**, review SaaS data processing, manage finance data requests/reporting needs, uphold data quality/governance, and demonstrate SQL, SAS, data modelling, complex datasets and finance stakeholder communication.

# Arval BNP Paribas - Proof Buckets

## 1. Finance data flows and reporting datasets

| What the CV must prove | Concrete artefacts to look for later |
| --- | --- |
| You can build and maintain repeatable data flows that support finance reporting. | SQL finance reporting table, SAS reporting dataset, finance data flow, Excel finance workbook, Power BI finance dashboard, scheduled reporting extract |

This is the core bucket. The role is not asking for modelling first; it is asking for someone who can improve the finance team's data IQ by maintaining the data flows behind reporting.

---

## 2. SQL data modelling and complex datasets

| What the CV must prove | Concrete artefacts to look for later |
| --- | --- |
| You can use SQL to model, join, maintain and query complex datasets for reporting purposes. | SQL reporting model, SQL views, SQL joins across finance/customer/transaction tables, fact/dimension-style reporting table, SQL reconciliation query |

This should be one of the strongest buckets because the ad explicitly asks for **strong SQL skills**, **data modelling**, and **maintaining complex datasets**.

---

## 3. SAS programming and governed reporting outputs

| What the CV must prove | Concrete artefacts to look for later |
| --- | --- |
| You can work with SAS programming and produce controlled reporting datasets or validation outputs. | SAS data step, PROC SQL query, SAS reporting dataset, SAS validation output, SAS data-quality check, SAS finance extract |

Since SAS is defendable, it should be a real proof bucket, not a throwaway skill. The role names SAS directly, so we should use it in the CV if the evidence supports it.
...
| What the CV must prove | Concrete artefacts to look for later |
| --- | --- |
| You can maintain trustworthy data through validation checks, reconciliation, controlled definitions and governance practices. | SQL validation checks, Excel reconciliation workbook, data-quality checklist, duplicate/missing-record check, data dictionary, KPI definition log, source-to-report reconciliation |

This is important because Arval explicitly mentions collaborating to ensure **data quality and governance practices** are upheld.

---

## 5. SaaS data processing review and process improvement

| What the CV must prove | Concrete artefacts to look for later |
| --- | --- |
| You can review existing data processing, find inefficiencies/gaps, and improve how data moves into reports. | SaaS export review, field-mapping table, process issue log, data-processing audit, reporting workflow redesign, manual-step reduction, refresh-time improvement |

This is Arval-specific. Many candidates will say "SQL" and "reporting"; fewer will show that they can review existing SaaS data processing and improve the workflow.

---

## 6. Finance team requests and reporting needs

| What the CV must prove | Concrete artefacts to look for later |
| --- | --- |
| You can respond to recurring and ad-hoc finance data requests with accurate outputs. | finance reporting pack, ad-hoc finance extract, variance analysis workbook, revenue/cost summary, monthly finance dataset, stakeholder data request log |

This bucket links the technical work to the finance team's day-to-day needs. It also prevents the CV from sounding like a pure data-engineering CV.
...
| What the CV must prove | Concrete artefacts to look for later |
| --- | --- |
| You can explain data flows, report outputs, data issues and metric definitions clearly to finance users. | finance data issue memo, reporting change note, reconciliation explanation, KPI definition note, stakeholder summary, dashboard commentary |

The ad specifically asks for communication with **non-technical finance team members**, so the CV needs one proof line later that shows clear explanation, not just technical delivery.

---

## 8. Financial concepts and commercial awareness

| What the CV must prove | Concrete artefacts to look for later |
| --- | --- |
| You understand finance-facing data enough to connect analysis to business needs. | revenue table, cost variance report, exposure summary, payment/transaction dataset, finance KPI dashboard, leasing/fleet/customer finance summary |

This bucket should not dominate unless we have strong evidence, but it matters because Arval asks for a keen interest in financial concepts and commercial process improvement.

# Priority order for Arval
...
3. SAS programming and governed reporting outputs
4. Data quality, reconciliation and governance
5. Finance team requests and reporting needs
6. SaaS data processing review and process improvement
7. Non-technical finance communication
8. Financial concepts and commercial awareness
```
...
```text
Financial Data Analyst | SQL Data Modelling, SAS & Finance Reporting | Data Quality & Governance
```

The concrete evidence we will later mine should look like:

```text
SQL finance reporting table
SAS reporting dataset
SQL data model
finance data flow
...
ad-hoc finance extract
KPI definition note
```

Next step: **build the evidence ledger for Arval** using these proof buckets.

---

Now we are building the **Arval evidence ledger**, not scoring yet and not drafting bullets yet.

The Arval role wants someone in the finance team who can **build and maintain data flows**, review SaaS data processing, manage finance data requests, uphold data quality/governance, use **SQL and SAS**, communicate with non-technical finance users, and maintain large financial datasets for reporting. The methodology says this ledger stage should capture artefact, action, data, method, metric, baseline, user, result and defence notes before scoring or bullet writing.

# Arval Evidence Ledger

## Experience sources we are mining

```text
1. Finance / Data Flow Analytics Platform
   Main source for SQL, SAS, data flows, reporting datasets, governance, large data, data quality and finance-style requests.

2. South Western Technologies - Data Analyst
   Strong support for Excel reconciliation, VBA automation, Power BI reporting, operational data quality, report validation and manager-facing summaries.

3. Business Analytics Project - University of Exeter
   Support for trend analysis, structured reporting, visual summaries and non-technical recommendations.

4. Behavioural Modelling Project - University of Exeter
   Secondary support for Python, modelling, validation and analytical discipline.
```

For Arval, I would probably rename the main platform to something like:
...
The first one is probably better because Arval's ad leads with **data flows**.
...
| --- | --- |
| Proof bucket | Finance data flows and reporting datasets |
| Concrete artefact | SQL finance reporting table; SAS reporting dataset; finance data flow; scheduled reporting extract; Excel finance workbook |
| Experience source | Main platform |
| What it proves | You can build and maintain repeatable data flows for finance reporting |
| Tools | SQL, SAS, Excel, Power BI, Google BigQuery if relevant |
| Dataset | 2.35B-row behavioural / transaction / financial data surface |
| Metric meaning | Number of maintained reporting tables, scheduled extracts, or reporting flows |
| Placeholder values | `[N] reporting tables`, `[N] scheduled extracts`, `[N] finance data flows`, `[N] reporting periods` |
| Baseline | Manual extracts; fragmented reporting files; ad-hoc SQL pulls; inconsistent report refresh |
| User / decision | Finance team uses the output for recurring reporting, variance checks, cost/revenue summaries or business performance review |
| Defence notes | This should become a core bullet later. It directly answers "building and maintaining data flows." |
...
| Ledger field | Evidence direction |
| --- | --- |
| Proof bucket | SQL data modelling and complex datasets |
| Concrete artefact | SQL reporting model; SQL views; SQL joins across customer/transaction/account/finance tables; reporting-ready fact table; SQL reconciliation query |
| Experience source | Main platform |
| What it proves | You can model and maintain complex datasets for reporting |
| Tools | SQL, Google BigQuery, SAS PROC SQL if relevant |
| Dataset | 2.35B-row data surface; `[N]` source tables |
| Metric meaning | Number of sources joined and reporting fields standardised |
| Placeholder values | `[N] source tables`, `[N] SQL views`, `[N] reporting fields`, `[N] records processed` |
| Baseline | Disconnected source tables; manual joins in Excel; unmodelled raw extracts |
| User / decision | Finance users get a reporting-ready dataset instead of raw extracts |
| Defence notes | Keep this concrete. Say "SQL reporting model," "SQL views," or "reporting-ready table," not "data modelling" alone. |
...
| Ledger field | Evidence direction |
| --- | --- |
| Proof bucket | SAS programming and governed reporting outputs |
| Concrete artefact | SAS data step; PROC SQL query; SAS reporting dataset; SAS validation output; SAS data-quality check |
| Experience source | Main platform / finance analytics work |
| What it proves | You can use SAS to produce controlled reporting datasets or validation outputs |
| Tools | SAS, PROC SQL, SQL |
| Dataset | Financial, customer, transaction or reporting dataset |
| Metric meaning | Number of SAS outputs or validation checks produced |
| Placeholder values | `[N] SAS reporting datasets`, `[N] PROC SQL queries`, `[N] SAS validation checks` |
| Baseline | Manual report preparation; SQL-only workflow; spreadsheet-only validation |
| User / decision | Finance/reporting stakeholders rely on controlled SAS outputs for reporting or validation |
| Defence notes | Since SAS is defendable, this should appear in the CV. Do not bury SAS in "Additional Tools." It is named in the ad and should have evidence behind it. |
...
| Ledger field | Evidence direction |
| --- | --- |
| Proof bucket | Data quality, reconciliation and governance |
| Concrete artefact | SQL validation checks; Excel reconciliation workbook; data-quality checklist; duplicate/missing-record check; data dictionary; KPI definition log |
| Experience source | Main platform; South Western Technologies |
| What it proves | You can maintain trustworthy reporting data through checks, reconciliation and controlled definitions |
| Tools | SQL, SAS, Excel, VBA, Power BI |
| Dataset | Finance reporting tables, operational records, transaction records, source-to-report outputs |
| Metric meaning | Reduction in reporting errors or reconciliation differences |
| Placeholder values | `[N] data-quality checks`, `[X]% reporting error reduction`, `[N] duplicate/missing records flagged`, `[N] KPI definitions documented` |
| Baseline | No checklist; manual checks; inconsistent KPI definitions; mismatched source/report totals |
| User / decision | Finance team can trust reported numbers and understand KPI definitions |
| Defence notes | This is a high-value Arval bucket because the ad explicitly names data quality and governance. It can draw strongly from South Western if the finance platform evidence is thinner. |
...
| Ledger field | Evidence direction |
| --- | --- |
| Proof bucket | SaaS data processing review and process improvement |
| Concrete artefact | SaaS export review; field-mapping table; process issue log; data-processing audit; reporting workflow redesign; manual-step reduction log |
| Experience source | Main platform; South Western Technologies as process-improvement support |
| What it proves | You can review existing processing and improve how data moves into reports |
| Tools | SQL, Excel, VBA, Power BI, SaaS export files |
| Dataset | SaaS exports, finance reporting extracts, operational records, dashboard source files |
| Metric meaning | Manual processing time reduced or recurring issues removed |
| Placeholder values | `[N] SaaS fields mapped`, `[N] processing issues logged`, `[X]% manual-step reduction`, review time from `[A]` to `[B]` |
| Baseline | Manual SaaS export review; repeated copy/paste steps; unclear field mappings; manual report refresh |
| User / decision | Finance/reporting users receive cleaner data faster with fewer manual corrections |
| Defence notes | This is a useful differentiator because many applicants will say SQL, but fewer will show SaaS processing review. Use only if we can name a real export, mapping, or workflow. |
...
| Ledger field | Evidence direction |
| --- | --- |
| Proof bucket | Finance team requests and reporting needs |
| Concrete artefact | Finance reporting pack; ad-hoc finance extract; variance analysis workbook; revenue/cost summary; monthly finance dataset; stakeholder request log |
| Experience source | Main platform; Frasers-style payments/finance outputs if relevant; South Western as manager-request support |
| What it proves | You can turn finance questions into accurate reporting outputs |
| Tools | SQL, SAS, Excel, Power BI |
| Dataset | Finance reporting tables, transaction data, cost/revenue metrics, operational finance records |
| Metric meaning | Number of finance requests answered or recurring reports maintained |
| Placeholder values | `[N] ad-hoc finance requests`, `[N] monthly reports`, `[N] variance checks`, `[M] finance stakeholders` |
| Baseline | Manual request handling; inconsistent response format; no standard reporting pack |
| User / decision | Finance team uses outputs for month-end review, variance checks, revenue/cost reporting or operational planning |
| Defence notes | This should become the "finance-facing" proof later so the CV does not sound like a back-end data engineering CV. |
...
| Ledger field | Evidence direction |
| --- | --- |
| Proof bucket | Non-technical finance communication |
| Concrete artefact | Finance data issue memo; reporting change note; reconciliation explanation; KPI definition note; dashboard commentary; finance summary note |
| Experience source | Main platform; Business Analytics Project; South Western Technologies |
| What it proves | You can explain data flows, report outputs, errors and metric definitions clearly to finance users |
| Tools / outputs | Excel summary, Power BI commentary, written memo, reporting pack notes |
| Dataset | Finance reporting outputs, reconciliation differences, KPI definitions, variance checks |
| Metric meaning | Number of notes/recommendations or stakeholder groups supported |
| Placeholder values | `[N] KPI definitions`, `[N] reporting-change notes`, `[N] finance summaries`, `[M] stakeholder groups` |
| Baseline | Verbal explanations only; unclear report definitions; repeated questions about metric meaning |
| User / decision | Non-technical finance stakeholders understand report changes, data issues and KPI definitions |
| Defence notes | This maps directly to Arval's non-technical finance communication requirement. Keep the artefact concrete: "KPI definition note," not "stakeholder communication." |
...
| Ledger field | Evidence direction |
| --- | --- |
| Proof bucket | Financial concepts and commercial awareness |
| Concrete artefact | Revenue table; cost variance report; exposure summary; transaction dataset; finance KPI dashboard; payment cost summary |
| Experience source | Main platform; Frasers payments/finance evidence; Business Analytics Project |
| What it proves | You can connect datasets to financial concepts like revenue, cost, variance, exposure or reporting value |
| Tools | SQL, SAS, Excel, Power BI, BigQuery |
| Dataset | Transaction data, revenue/cost fields, finance KPIs, payment/chargeback records if relevant |
| Metric meaning | Cost/revenue variance identified or finance KPI coverage |
| Placeholder values | `£[X] cost/revenue variance`, `[N] finance KPIs`, `[X]% variance explained`, `[N] reporting periods` |
| Baseline | Prior period; expected value; budget/actual view; manual finance review |
| User / decision | Finance users understand cost/revenue movements or reporting variances |
| Defence notes | Use this carefully. It should support commercial awareness, not pretend to be a qualified finance role. |

---

# Prioritised evidence pool before scoring

The strongest candidate evidence for Arval appears to be:

```text
1. SQL finance reporting table / SQL reporting model
2. SAS reporting dataset or SAS validation output
3. Finance data flow or scheduled reporting extract
4. SQL views joining multiple source tables
5. Excel reconciliation workbook
6. Data-quality checklist / validation checks
7. KPI definition log or data dictionary
8. SaaS export review / field-mapping table
9. Finance reporting pack or ad-hoc finance extract
10. Revenue/cost variance summary
11. Power BI finance/reporting dashboard
12. Non-technical finance summary note
```

# What this ledger tells us

The main CV should not be framed as fraud, payments, or ML for this role. It should be framed around:

```text
SQL-led finance data flows
SAS reporting outputs
large financial/reporting datasets
data modelling
data quality and governance
finance reporting requests
non-technical finance communication
process improvement
```
