...
- Identified customers/accounts likely to move into higher risk before escalation, using risk-band momentum, transaction volatility, activity drop-off, rising exposure/utilisation, repeated near-threshold behaviour, and recency/frequency changes to improve early-risk identification by 18% and reduce missed pre-escalation cases; XGBoost improved recall by 18% over a Logistic Regression baseline using time-based validation, with SHAP identifying 4 behavioural drivers across priority segments, by combining customer, account, transaction, risk, decision, and outcome data into point-in-time PySpark/SQL feature pipelines, training Logistic Regression and XGBoost models in Python, validating with time-based splits, and applying SHAP to explain behavioural drivers.

- Prioritised account reviews toward cases most likely to need action while supporting review-capacity planning around expected case load, using model score concentration, future risk-movement likelihood, review-need probability, outcome-linked labels, top-score bands, and expected case volumes to focus review effort by concentrating 42% of future-risk cases into the top 20% priority group; XGBoost scoring ranked accounts by risk-movement probability, with top-score band analysis showing risk concentration across scored accounts, by managing production-shaped scoring and data pipelines, refreshing PySpark/SQL feature inputs, converting XGBoost probabilities into priority bands and expected case volumes, and producing repeatable scoring, monitoring, and planning outputs through an AWS-hosted workflow.
