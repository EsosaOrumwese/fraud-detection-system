...
Keyed substreams (S0.3.3); labels:
...
Per **S0.3.6** draw accounting: `gamma_component` draws $= 3 \times$ attempts $+ \mathbf{1}[\phi_m<1]$; `poisson_component` draws are **variable** and reconciled via **envelope counters**; `nb_final` draws $=0$.
