...
2. Draw independent gamma components using the Marsaglia–Tsang method; any uniforms required by the sampler are drawn via the `u01` mapping in **S0.3.4** (open interval; one counter increment ⇒ one uniform):
