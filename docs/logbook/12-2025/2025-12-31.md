...
* 3:50pm
   - Authored `config/policy/3A/zone_mixture_policy.yaml` with theta_mix=0.35 and deterministic escalation rules for small, single-zone, and large multi-zone cases.
   - Built `scripts/build_zone_floor_policy_3a.py` and generated `config/allocation/zone_floor_policy.yaml` from tz_world_2025a tzid presence counts; lowered dominant-threshold to 0.60 to satisfy bump distribution realism floors.
   - Built `scripts/build_country_zone_alphas_3a.py` and generated `config/allocation/country_zone_alphas.yaml` from tz_world_2025a geometries + population_raster_2025 with area fallback only where pop mass is missing for most zones.
...
* 4:27pm
   - Authored `scripts/build_mcc_channel_rules_3b.py` and generated `config/virtual/mcc_channel_rules.yaml` (290 MCCs, 580 rules); CNP virtual share=0.1217 and overall share=0.02998 within corridors.
   - Authored `scripts/build_cdn_country_weights_3b.py` and generated `config/virtual/cdn_country_weights.yaml` with 235 ISO2s from iso3166/world_countries intersection; quantized weights to 12dp with sum fixed to 1.0.
   - Logged deviation: world_countries missing >5% ISO2 polygons, so the intersection set was used for the CDN country universe.
   - Built `config/virtual/virtual_validation.yml` and `config/virtual/cdn_key_digest.yaml` via new scripts, and created `config/logging/virtual_logging.yml` per v1 guide.
