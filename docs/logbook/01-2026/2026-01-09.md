# Logbook
### Date: 9th January 2026
### Project: Fraud Detection System
### Reference commits: Check commits on this date (if any)

* 12:02am
   - Checked the local system time with `Get-Date` before formalizing the 2A merchant_mcc_map contract.
   - Reviewed the 2A data-intake guide, the 2A dictionary/registry, and the ingress schema pack to determine where to register `merchant_mcc_map`.
   - Planned to add a `merchant_mcc_map` schema anchor in `schemas.ingress.layer1.yaml`, register it as optional reference_data in the 2A dictionary, and add a matching cross-layer pointer in the 2A registry.

* 12:05am
   - Added the `merchant_mcc_map` table schema to `docs/model_spec/data-engine/layer-1/specs/contracts/1A/schemas.ingress.layer1.yaml`.
   - Registered `merchant_mcc_map` as an optional ingress reference in `docs/model_spec/data-engine/layer-1/specs/contracts/2A/dataset_dictionary.layer1.2A.yaml` and added the matching cross-layer pointer to `docs/model_spec/data-engine/layer-1/specs/contracts/2A/artefact_registry_2A.yaml`.
   - Updated `docs/model_spec/data-engine/layer-1/specs/data-intake/2A/merchant_mcc_map_derivation-guide.md` and `docs/model_spec/data-engine/layer-1/specs/data-intake/layer-1_data.md` to point at the new schema anchor.

* 12:06am
   - Ran a repository search to confirm `merchant_mcc_map` references now point at the new schema anchor across layer-1 specs.

* 12:13am
   - Updated `docs/model_spec/data-engine/layer-1/specs/state-flow/2A/state.2A.s2.expanded.md` to name `merchant_mcc_map` (schema `schemas.ingress.layer1.yaml#/merchant_mcc_map`) as the optional MCC mapping anchor instead of a programme-specific placeholder.

* 12:17am
   - Read the 2B data-intake policy guides (alias_layout_policy_v1, day_effect_policy_v1, route_rng_policy_v1, virtual_edge_policy_v1) and compared them to `docs/model_spec/data-engine/layer-1/specs/contracts/2B/schemas.2B.yaml` to assess schema coverage.

* 12:19am
   - Recommended using `pad_byte_hex` as the canonical padding key and adopting a moderately strict schema posture (required keys + basic type/enum constraints, `extensions` allowed) for the 2B policy schemas.

* 12:22am
   - Added moderately strict schema shapes for `route_rng_policy_v1`, `alias_layout_policy_v1`, and `day_effect_policy_v1` in `docs/model_spec/data-engine/layer-1/specs/contracts/2B/schemas.2B.yaml`, keeping `extensions` as the expansion bucket.
   - Updated the 2B policy guides to reference schema anchors and aligned the alias layout padding key to `pad_byte_hex`.

* 12:23am
   - Sanity-checked 2B dictionary/registry policy schema refs and confirmed they point to the updated `schemas.2B.yaml#/policy/*` anchors.

* 12:26am
   - Tightened 3A policy schema anchors in `docs/model_spec/data-engine/layer-1/specs/contracts/3A/schemas.3A.yaml` to require rules for `zone_mixture_policy_v1`, enforce allowed metrics/decision reasons, and require positive `alpha` values for `country_zone_alphas_v1`.

* 12:36am
   - Audited 3B data-intake guides against `docs/model_spec/data-engine/layer-1/specs/contracts/3B/schemas.3B.yaml` and tightened schema anchors for external weights, virtual rules, CDN weights, validation policy, key digest, and settlement coordinates.
   - Added a new `virtual_logging_policy_v1` schema anchor and registered the policy in the 3B dictionary and registry.
   - Aligned 3B intake guides with contract paths and schema anchors (cdn weights external path, pelias cached dataset id, virtual logging policy schema reference).

* 12:44am
   - Recommended splitting the Pelias SQLite bundle metadata into its own contract entry (pelias_cached_bundle) and giving the sqlite file a minimal file-level schema anchor to avoid mismatched schema references.

* 12:50am
   - Added `pelias_cached_bundle_v1` and `pelias_cached_sqlite_v1` reference anchors in `docs/model_spec/data-engine/layer-1/specs/contracts/3B/schemas.3B.yaml`.
   - Updated `docs/model_spec/data-engine/layer-1/specs/contracts/3B/dataset_dictionary.layer1.3B.yaml` to point `pelias_cached_sqlite` at `pelias_cached_sqlite_v1` and registered `pelias_cached_bundle` with its own schema anchor.
   - Updated `docs/model_spec/data-engine/layer-1/specs/contracts/3B/artefact_registry_3B.yaml` to align schema refs and add the `pelias_cached_bundle` registry entry with a dependency on `pelias_cached_sqlite`.
   - Updated `docs/model_spec/data-engine/layer-1/specs/data-intake/3B/pelias_cached.sqlite_acquisition-guide.md` to document the sqlite + metadata bundle pair and their schema anchors.
   - Ran `rg` and `Get-Content` checks to confirm the new Pelias bundle entries are present in the contracts and guide.

* 12:53am
   - Reviewed the 3A data-intake guides (`zone_mixture_policy_3A.md`, `zone_floor_policy_3A.md`, `country_zone_alphas_3A.md`) and confirmed each already names the correct `schemas.3A.yaml#/policy/*` schema authority anchor.
   - No edits required for the 3A schema-anchoring pass.

* 12:56am
   - Reviewed the 5A data-intake guides in `docs/model_spec/data-engine/layer-2/specs/data-intake/5A` for schema anchors.
   - Added the missing schema anchor reference to `docs/model_spec/data-engine/layer-2/specs/data-intake/5A/scenario_calendar_5A_derivation-guide.md`.

* 1:02am
   - Tightened the 5A policy and scenario schemas in `docs/model_spec/data-engine/layer-2/specs/contracts/5A/schemas.5A.yaml` to match the 5A data-intake guides (merchant class, demand scale, baseline intensity, shape library, scenario horizon, scenario overlay, scenario calendar).
   - Left `scenario_metadata`, `overlay_ordering_policy_5A`, `scenario_overlay_validation_policy_5A`, and `shape_time_grid_policy_5A` permissive pending dedicated guide coverage.

* 1:21am
   - Reviewed 5A state-flow specs for references to additional policy/config artefacts and compared them to 5A contracts/guide coverage.
   - Confirmed which items are referenced in specs but lack data-intake guides or contract entries (e.g., shape_time_grid_policy_5A, baseline_validation_policy_5A, overlay_ordering_policy_5A, scenario_overlay_validation_policy_5A, validation_policy_5A, spec_compatibility_config_5A, class_shape_templates_5A, zone_shape_modifiers_5A, channel_shape_modifiers_5A).

* 1:41am
   - Re-checked `docs/model_spec/data-engine/layer-2/specs/state-flow/5A/state.5A.s2.expanded.md` to confirm how the specs describe time-grid config vs shape library policies for 5A.

* 1:53am
   - Removed the grid contract from `docs/model_spec/data-engine/layer-2/specs/data-intake/5A/shape_library_5A_authoring-guide.md` and updated section numbering to make `shape_time_grid_policy_5A` the sole grid authority.
   - Updated `docs/model_spec/data-engine/layer-2/specs/contracts/5A/schemas.5A.yaml` to drop the `shape_library_5A.grid` schema and add a strict schema for `shape_time_grid_policy_5A`.

* 1:55am
   - Scanned the 5A state-flow docs to check for any wording that assumes the local-week grid lives inside `shape_library_5A`.

* 1:59am
   - Added a one-line rule in `docs/model_spec/data-engine/layer-2/specs/state-flow/5A/state.5A.s2.expanded.md` clarifying that `shape_time_grid_policy_5A` is the sole grid authority and `shape_library_5A` must not redefine it.

* 3:23am
   - Reviewed the newly added 5A data-intake guides for zone modifiers, overlay ordering/validation, validation policy, and spec compatibility to assess alignment with contracts and schema anchors.

* 3:33am
   - Added strict schema definitions in docs/model_spec/data-engine/layer-2/specs/contracts/5A/schemas.5A.yaml for zone_shape_modifiers_5A, overlay_ordering_policy_5A, scenario_overlay_validation_policy_5A, alidation_policy_5A, and spec_compatibility_config_5A, plus a shared overlay_type_aggregation_v1  helper.
   - Added zone_shape_modifiers_5A, alidation_policy_5A, and spec_compatibility_config_5A entries to docs/model_spec/data-engine/layer-2/specs/contracts/5A/dataset_dictionary.layer2.5A.yaml and updated the shape_library_5A description to remove grid authority wording.
   - Updated docs/model_spec/data-engine/layer-2/specs/contracts/5A/dataset_dictionary.layer2.5A.yaml and docs/model_spec/data-engine/layer-2/specs/contracts/5A/artefact_registry_5A.yaml to point overlay_ordering_policy_5A at the new policy schema anchor.
   - Added registry entries for zone_shape_modifiers_5A, alidation_policy_5A, and spec_compatibility_config_5A in docs/model_spec/data-engine/layer-2/specs/contracts/5A/artefact_registry_5A.yaml.

* 3:40am
   - Added schema anchor bullets to docs/model_spec/data-engine/layer-2/specs/data-intake/5A/zone_shape_modifiers_5A-_authoring-guide.md, docs/model_spec/data-engine/layer-2/specs/data-intake/5A/scenario_overlay_validation_policy_5A_authoring-guide.md, and docs/model_spec/data-engine/layer-2/specs/data-intake/5A/spec_compatibility_config_5A_authoring-guide.md.

* 3:42am
   - Swept docs/model_spec/data-engine/layer-2/specs/data-intake/5A for schema anchor lines; found channel_shape_modifiers_5A_authoring-guide.md missing an anchor and noted that class_shape_templates_5A_authoring-guide.md and aseline_validation_policy_5A_authoring-guide.md point to anchors not present in docs/model_spec/data-engine/layer-2/specs/contracts/5A/schemas.5A.yaml.

* 3:48am
   - Added the missing schema anchor line to docs/model_spec/data-engine/layer-2/specs/data-intake/5A/channel_shape_modifiers_5A_authoring-guide.md.
   - Added schema definitions for aseline_validation_policy_5A and class_shape_templates_5A in docs/model_spec/data-engine/layer-2/specs/contracts/5A/schemas.5A.yaml to match the data-intake guides.

* 3:51am
   - Added the channel_shape_modifiers_5A schema and its shared channel_modifier_profile_v1 definition to docs/model_spec/data-engine/layer-2/specs/contracts/5A/schemas.5A.yaml to align with the 5A data-intake guide.

* 3:57am
   - Scanned docs/model_spec/data-engine/layer-2/specs/data-intake/5B guides for schema anchors and compared them to docs/model_spec/data-engine/layer-2/specs/contracts/5B/schemas.5B.yaml. All guides have schema anchor lines, but three point to #/policy/* while the schema pack exposes them under #/config/* (arrival_routing_policy_5B, arrival_time_placement_policy_5B, validation_policy_5B).

* 4:06am
   - Replaced the permissive 5B config schemas in docs/model_spec/data-engine/layer-2/specs/contracts/5B/schemas.5B.yaml with strict shapes for time_grid_policy_5B, grouping_policy_5B, arrival_lgcp_config_5B, arrival_count_config_5B, arrival_time_placement_policy_5B, arrival_routing_policy_5B, arrival_rng_policy_5B, and validation_policy_5B per the data-intake guides.
   - Updated the 5B guide schema anchors for arrival_routing_policy_5B, arrival_time_placement_policy_5B, and validation_policy_5B to use schemas.5B.yaml#/config/* to match the contract schema locations.

* 4:09am
   - Confirmed 5B schema_ref anchors in docs/model_spec/data-engine/layer-2/specs/contracts/5B/dataset_dictionary.layer2.5B.yaml and docs/model_spec/data-engine/layer-2/specs/contracts/5B/artefact_registry_5B.yaml resolve to entries present in docs/model_spec/data-engine/layer-2/specs/contracts/5B/schemas.5B.yaml.

* 4:12am
   - Scanned 5B state-flow docs for schema anchor references and found a single mismatch: schemas.5B.yaml#/control/sealed_inputs_5B in state.5B.s0.expanded.md (the schema pack exposes sealed_inputs_5B under alidation).

* 4:13am
   - Fixed the sealed-inputs schema anchor in docs/model_spec/data-engine/layer-2/specs/state-flow/5B/state.5B.s0.expanded.md to schemas.5B.yaml#/validation/sealed_inputs_5B.

* 4:19am
   - Listed 6A data-intake guides in docs/model_spec/data-engine/layer-3/specs/data-intake/6A and noted none currently include schema anchor lines; opened docs/model_spec/data-engine/layer-3/specs/contracts/6A/schemas.6A.yaml to prep for adding strict policy/prior/taxonomy shapes.

* 4:36am
   - Added shared schema defs (id_token, probability, range helpers) to `docs/model_spec/data-engine/layer-3/specs/contracts/6A/schemas.6A.yaml`.
   - Expanded 6A prior schemas in `docs/model_spec/data-engine/layer-3/specs/contracts/6A/schemas.6A.yaml` to cover population, segmentation, account_per_party, product_mix, instrument_per_account, instrument_mix, device_count, ip_count, and S5 role priors with required keys and typed sub-structures.
   - Expanded 6A taxonomy schemas in `docs/model_spec/data-engine/layer-3/specs/contracts/6A/schemas.6A.yaml` for party, account_types, instrument_types, devices, and ips to match the data-intake guides.
   - Added detailed policy schemas for `graph_linkage_rules_6A` and `device_linkage_rules_6A` in `docs/model_spec/data-engine/layer-3/specs/contracts/6A/schemas.6A.yaml`.

* 4:42am
   - Added missing schema anchor lines to the 6A taxonomy guides (account_types, instrument_types, devices, ips) to match `schemas.6A.yaml#/taxonomy/*`.

* 5:01am
   - Added shared schema defs (id_token, probability, range helpers) to docs/model_spec/data-engine/layer-3/specs/contracts/6B/schemas.6B.yaml.
   - Expanded the 6B policy schemas in docs/model_spec/data-engine/layer-3/specs/contracts/6B/schemas.6B.yaml to capture the data-intake guide structures for S1-S5 policies.

* 5:15am
   - Tightened nested schema shapes in docs/model_spec/data-engine/layer-3/specs/contracts/6B/schemas.6B.yaml (policy vocabularies, RNG policy substructures, delay/timing/campaign/fraud/truth/bank/case validation shapes, and stricter enums/required fields).
   - Added the missing schema_ref line to docs/model_spec/data-engine/layer-3/specs/data-intake/6B/fraud_campaign_catalogue_config_6B_authoring-guide.md and cleaned schema anchor notes in delay_models_6B and rng_policy_6B guides.

* 5:50am
   - Tightened remaining nested policy sections in docs/model_spec/data-engine/layer-3/specs/contracts/6B/schemas.6B.yaml (behaviour_config guardrails, behaviour_prior_pack submodels, flow_shape branching and enums, timing composition rules, delay models, fraud campaign/overlay structures, RNG policy substructures, and bank/truth/case vocab constraints).
   - Added/cleaned schema anchors in 6B guides and ran a focused anchor audit confirming all guide schema refs resolve.

* 5:58am
   - Added strict amount/distribution and delay distribution schema defs in docs/model_spec/data-engine/layer-3/specs/contracts/6B/schemas.6B.yaml (lognormal/lognormal mix/gamma/discrete price points for amounts; QF distributions for delays).
   - Tightened amount_model_6B and delay_models_6B policy shapes to bind to the new distribution schemas and require the guide-specified fields (rules, constraints, rounding, guardrails, realism targets).

* 6:05am
   - Read docs/tmp/artefact_list_review.md and compared layer-2/3 policy/config mentions against schemas.5A.yaml, schemas.5B.yaml, schemas.6A.yaml, and schemas.6B.yaml.
   - Identified which layer-2/3 policy IDs are present in their schema packs vs mismatched or upstream-only references.

* 6:12am
   - Updated docs/tmp/artefact_list_review.md to align layer-2/3 policy names with schema IDs and expanded 6A priors/taxonomies/policies into explicit contract IDs.

* 6:15am
   - Ran a consistency pass comparing policy/config/prior/taxonomy ids in docs/tmp/artefact_list_review.md against the layer-2/3 schema packs; confirmed all segment-local items resolve, with only upstream policy references and one report metric flagged.

* 6:39am
   - Generated 1A path trees for the artefact registry and dataset dictionary in docs/tmp/paths_tree_registry_1A.md and docs/tmp/paths_tree_dictionary_1A.md using templated paths.

* 6:43am
   - Generated registry and dictionary path trees for segments 1B, 2A, 2B, 3A, 3B, 5A, 5B, 6A, 6B in docs/tmp/paths_tree_registry_<seg>.md and docs/tmp/paths_tree_dictionary_<seg>.md using templated paths from each contract file.

* 6:46am
   - Built consolidated tree files for all segments (1A-6B) into docs/tmp/paths_tree_registry_all.txt and docs/tmp/paths_tree_dictionary_all.txt, preserving templated paths from each registry/dictionary.

* 6:48am
   - Removed per-segment path tree markdown files from docs/tmp after consolidating trees into the two .txt outputs.

* 6:52am
   - Verified docs/tmp/paths_tree_registry_all.txt and docs/tmp/paths_tree_dictionary_all.txt contain single combined trees (no per-segment sections) after latest request; no file edits needed.

* 6:55am
   - Regenerated docs/tmp/paths_tree_registry_all.txt and docs/tmp/paths_tree_dictionary_all.txt without segment prefixes, deduplicating identical (id,path) pairs for a single combined tree per file.

* 6:57am
   - Generated docs/tmp/paths_tree_collisions.txt to report ID→path collisions across combined registry/dictionary paths (0 registry collisions, 10 dictionary collisions).

* 7:00am
   - Rebuilt docs/tmp/paths_tree_collisions.txt with YAML block-scalar path handling; dictionary collisions reduced to 3 and block scalar placeholders resolved.

* 7:13am
   - Fixed indentation for hrsl_raster in docs/model_spec/data-engine/layer-1/specs/contracts/3B/artefact_registry_3B.yaml so its fields parse correctly in the registry tree.
   - Regenerated docs/tmp/paths_tree_registry_all.txt, docs/tmp/paths_tree_dictionary_all.txt, and docs/tmp/paths_tree_collisions.txt with dictionary parsing restricted to top-level id entries (indent <= 2) and block-scalar handling; crossborder_eligibility_flags no longer appears as a false collision.

* 7:18am
   - Moved sealed_inputs_v1 and segment_state_runs into an EXPECTED DUPLICATES section in docs/tmp/paths_tree_collisions.txt (collisions list now empty) and cleaned the registry-collisions header formatting.

* 10:17am
   - Reviewed scratch_files/scratch.md (ChatGPT storage layout proposal) to prepare feedback on path layout harmonization vs current contracts.

* 11:40am
   - Noted the sealed-inputs inconsistencies across segments and committed to standardizing sealed_inputs to JSON with per-segment ids, plus adding sealed_inputs for 1A/1B.
   - Queued contract/schema/state-flow/interface updates for sealed_inputs naming/paths, and a cleanup of 5A registry name labels to match dictionary ids.

* 12:18pm
   - Added `sealed_inputs_1A` to the 1A S0 outputs section (classes + paths) and documented its JSON inventory shape in `state.1A.s0.expanded.md`.
   - Updated 1B S0 outputs to include `sealed_inputs_1B`, documenting schema/path and publish rules in `state.1B.s0.expanded.md`.
   - Synced interface_pack `_harvest/dataset_outputs.json` sealed_inputs entries to segment-specific IDs and JSON paths, adding 1A/1B sealed_inputs outputs.

* 12:20pm
   - Regenerated docs/tmp/paths_tree_registry_all.txt, docs/tmp/paths_tree_dictionary_all.txt, and docs/tmp/paths_tree_collisions.txt via `python scripts/build_paths_tree.py` to reflect the sealed_inputs updates.

* 12:28pm
   - Updated docs/tmp crosscheck + artefact lists to replace legacy `sealed_inputs_v1` references with `sealed_inputs_2A`/`sealed_inputs_2B`.
   - Rewrote layer-1 2A/2B design DAG docs to use the new sealed_inputs ids/JSON paths and corrected the 2A format note to JSON.
* 12:53pm
   - Updated `docs/model_spec/data-engine/layer-1/specs/state-flow/2B/state.2B.s8.expanded.md` to treat `validation_bundle_2B` as the directory and add `validation_bundle_index_2B` as the index file (schema anchors, required IDs, registry mirroring).
   - Updated `docs/model_spec/data-engine/interface_pack/engine_outputs.catalogue.yaml` to add `validation_bundle_2B`, correct `validation_bundle_index_2B` schema/dictionary refs, and normalize `s0_gate_receipt_2B`, `s0_gate_receipt_6A`, `s0_gate_receipt_6B` paths.
   - Synced `docs/model_spec/data-engine/interface_pack/_harvest/dataset_outputs.json` to the new 2B validation bundle directory/index entries and the updated 2B/6A/6B gate receipt paths.
   - Updated `docs/model_spec/data-engine/layer-3/specs/state-flow/6A/state.6A.s0.expanded.md` and `docs/model_spec/data-engine/layer-3/specs/state-flow/6B/state.6B.s0.expanded.md` to use `data/layer3/<seg>/s0_gate_receipt/...` paths.
   - Updated 2B design DAGs (`docs/design/data-engine/layer-1/2B/2B-S8-dag.md`, `docs/design/data-engine/layer-1/2B/2B-overview-dag.md`) to reflect `validation_bundle_2B` directory + `validation_bundle_index_2B` index and the new schema anchor.
   - Regenerated docs/tmp path trees via `python scripts/build_paths_tree.py` after the contract/layout updates.
* 2:24pm
   - Reviewed the RDV rails constitution section header plan in `scratch_files/scratch.md`.
   - Created placeholder doc for the shared RDV constitution at `docs/model_spec/data-engine/interface_pack/rdv_rails_constitution_v1.md`.
* 2:27pm
   - Moved `docs/model_spec/data-engine/interface_pack/rdv_rails_constitution_v1.md` to `docs/model_spec/data-engine/rdv_rails_constitution_v1.md` per placement decision.

* 5:02pm
   - Cleaned `docs/model_spec/data-engine/rdv_rails_constitution_v1.md` to keep the State Manifest requirement sentence out of the bullet list and clarify optional `scenario_id`/`run_id` inclusion in the Run Anchor list.

* 5:07pm
   - Softened unconditional State Manifest requirements in `docs/model_spec/data-engine/rdv_rails_constitution_v1.md` by making recording/serialization conditional on "when required or emitted" and aligning anchor bullets and SC-E guidance to the same rule.

* 5:10pm
   - Ran a targeted RDV conflict scan across state-flow docs focusing on State Manifest language and run_id/scenario_id requirements to identify any remaining constitution mismatches.

* 5:18pm
   - Replaced State Manifest language in `docs/model_spec/data-engine/rdv_rails_constitution_v1.md` with the S0 sealing ledger (`s0_gate_receipt_<SEG>` + `sealed_inputs_<SEG>`), updated determinism evidence rules, reuse logging, execution protocol, and checklists to align with state-expanded docs.

* 5:20pm
   - Updated the Segment RDV Profile contract/template in `docs/model_spec/data-engine/rdv_rails_constitution_v1.md` to require a sealing ledger declaration (S0 sealing IDs + schema refs or an equivalent ledger).

* 5:37pm
   - Added a Cross-Layer Inputs block and Contract Card for 1A.S0 in `docs/model_spec/data-engine/layer-1/specs/state-flow/1A/state.1A.s0.expanded.md`.
   - Added a Contract Card for 1A.S3 in `docs/model_spec/data-engine/layer-1/specs/state-flow/1A/state.1A.s3.expanded.md` using existing input/output and gate facts from the state spec and contracts.

* 5:46pm
   - Added Contract Card blocks to the remaining 1A state-expanded docs (S1, S2, S4, S5, S6, S7, S8, S9) in `docs/model_spec/data-engine/layer-1/specs/state-flow/1A`.

* 5:56pm
   - Clarified the Contract Card sealing note to apply to external inputs only across 1A S1-S8.
   - Expanded the 1A.S7 Contract Card inputs to cover `rng_event_gumbel_key` (fallback) and optional `merchant_currency`.
   - Expanded the 1A.S8 Contract Card inputs to enumerate `rng_event_nb_final`, `rng_event_residual_rank`, and optional `s3_integerised_counts`.

* 6:07pm
   - Added Contract Cards to 1B state-expanded docs (S0-S9) with inputs/outputs/authority summaries aligned to each state’s sealed input and output sections.

* 6:14pm
   - Adjusted 1B Contract Cards to reserve `sealed_inputs` tags for external S0-sealed inputs and list internal artefacts by source state.
   - Added `tile_bounds` to the S1 Contract Card outputs and updated S1 scope/non-goals to include the companion geometry surface.

* 6:19pm
   - Expanded the 1B S1 spec anchors, identity/partition law, and output dataset section to cover `tile_bounds` alongside `tile_index`, with shared write/immutability rules noted.

* 6:27pm
   - Added Contract Card sections to 2A state-expanded docs (S0-S5), summarizing authoritative inputs, outputs, and ordering posture.
   - Marked external inputs as sealed via `sealed_inputs_2A` and internal artefacts by source state; flagged optional sealed inputs (iso/world tables and merchant_mcc_map) and S5 gate failure posture.

* 9:23pm
   - Swept 2A Contract Cards against `dataset_dictionary.layer1.2A.yaml` and `artefact_registry_2A.yaml`; all referenced IDs resolve in both contracts.
   - Noted that scope labels in the cards are informational; unpartitioned references (e.g., `tz_world_2025a`, `iso3166_canonical_2024`) are still present as sealed inputs in S0 and catalogued without fingerprint partitions.

* 9:35pm
   - Updated 2A Contract Card input scope labels to align with RDV rails: unpartitioned references/configs now marked as sealed/unpartitioned, and version-partitioned `merchant_mcc_map` labeled VERSION_SCOPED.

* 9:45pm
   - Added Cross-Layer Inputs blocks to `state.1B.s0.expanded.md` and `state.2A.s0.expanded.md` to document upstream segment gates and sealed external references per the per-segment S0 rule.

* 9:53pm
   - Added the Cross-Layer Inputs block and Contract Card to `state.2B.s0.expanded.md`.
   - Added Contract Cards to 2B state-expanded docs S1-S8, aligning inputs/outputs and scope labels to the dictionary and RDV rails.

* 9:54pm
   - Adjusted the 2B S0 Cross-Layer Inputs gate note to avoid introducing a 2A PASS requirement that is not currently declared in the 2B contracts.

* 10:00pm
   - Normalized 2B Contract Card scope labels to RDV enum usage by replacing SEED+FINGERPRINT with EGRESS_SCOPED plus explicit `scope_keys: [seed, manifest_fingerprint]`.
   - Marked `s0_gate_receipt_2B` as a non-gate receipt (`gate emitted: none`) and clarified RNG core logs in S5/S6 as shared append-only logs.

* 10:09pm
   - Added Contract Cards to the 3A state-expanded docs S1-S7 with inputs/outputs/authority summaries aligned to the 3A dictionary and RDV scope labels.
   - Included RNG log/event outputs in 3A.S3 and validation bundle inputs in 3A.S7 to reflect the current 3A state specifications.

* 10:11pm
   - Ran a consistency sweep comparing 3A Contract Card IDs to the 3A dictionary/registry and noted only the upstream validation bundles/flags missing from the 3A dictionary (cards otherwise align).

* 10:17pm
   - Added cross-layer upstream validation bundle and passed-flag entries (1A/1B/2A) to `dataset_dictionary.layer1.3A.yaml` so S0 Contract Card gate inputs resolve via dictionary-only lookup.

* 10:18pm
   - Re-ran the 3A Contract Card consistency sweep to confirm no remaining dictionary or registry gaps.

* 10:25pm
   - Added a Cross-Layer Inputs block and Contract Card to `state.3B.s0.expanded.md`.
   - Added Contract Cards to 3B state-expanded docs S1-S5 with inputs/outputs/authority summaries aligned to the 3B contracts and RDV scope labels.

* 10:30pm
   - Added upstream validation bundle and passed-flag entries (1A/1B/2A/3A) to `dataset_dictionary.layer1.3B.yaml` for dictionary-only gate resolution in 3B.S0.
   - Ran a 3B Contract Card consistency sweep; only run-report IDs are dictionary/registry-only, no card gaps remain.

* 10:40pm
   - Added the Cross-Layer Inputs block and Contract Card to `state.5A.s0.expanded.md`.
   - Added Contract Cards to 5A state-expanded docs S1-S5 with inputs/outputs/authority summaries aligned to the 5A contracts and RDV scope labels.

* 10:42pm
   - Ran a 5A Contract Card consistency sweep against the 5A dictionary/registry and noted only run-report surfaces and two 3B policy outputs are dictionary/registry-only (no card gaps).

* 10:46pm
   - Marked `virtual_routing_policy_3B` / `virtual_validation_contract_3B` mentions in 5A S0/S1 as context-only (not read) and removed the routing policy from the S0 minimal sealed-input requirements list.

* 10:58pm
   - Added the Cross-Layer Inputs block and Contract Card to state.5B.s0.expanded.md.
   - Added Contract Cards to 5B state-expanded docs S1-S5 with inputs/outputs/authority summaries aligned to the 5B contracts and RDV scope labels.

* 11:02pm
   - Ran a 5B Contract Card vs dictionary/registry sweep; no missing IDs in either contracts file.

* 11:10pm
   - Added a Cross-Layer Inputs block and Contract Card to state.6A.s0.expanded.md.
   - Added Contract Cards to 6A state-expanded docs S1-S5 with inputs/outputs/authority summaries aligned to the 6A contracts and RDV scope labels.

* 11:12pm
   - Ran a 6A Contract Card vs dictionary/registry sweep using registry name parsing; no missing IDs on either side.

* 11:18pm
   - Added a Cross-Layer Inputs block and Contract Card to state.6B.s0.expanded.md.
   - Added Contract Cards to 6B state-expanded docs S1-S5 with inputs/outputs/authority summaries aligned to the 6B contracts and RDV scope labels.

* 11:21pm
   - Ran a 6B Contract Card vs dictionary/registry sweep; no missing IDs in either contracts file.

* 11:30pm
   - Reviewed 1A state-expanded docs against RDV rails; identified missing Segment RDV Profile and missing State Class declarations in 1A state docs.

* 11:32pm
   - Relaxed Segment RDV Profile and State Class requirements in the RDV constitution: profiles are now optional unless enforcement tooling is in scope, S0 Cross-Layer Inputs + Contract Cards are the minimal binding fallback, and State Class can be inferred from Contract Cards when not declared.

* 11:35pm
   - Re-checked 1A state-expanded docs against the revised RDV constitution, focusing on S0 Cross-Layer Inputs, Contract Cards, and state classification requirements.
   - Noted gaps: no explicit State Class/RNG posture declarations in 1A Contract Cards (currently only inferable from narrative/IO), S0 Cross-Layer Inputs lacks the required alias note for manifest_fingerprint path labels, and S0 gate receipt language does not explicitly claim the full Run Anchor (seed/parameter_hash/manifest_fingerprint/run_id).

* 11:36pm
   - Ran an automated uniformity sweep across segments 1A-6B for S0 sealing, RNG evidence, validation bundles, and Contract Card/Cross-Layer Inputs coverage.
   - Findings: 1A lacks s0_gate_receipt_1A in dictionary/registry; 1A/1B/2A lack validation_bundle_index_<SEG> entries; other segments appear aligned on these checks.

* 11:42pm
   - Added s0_gate_receipt_1A and validation_bundle_index_{1A,1B,2A} definitions across schemas, dictionaries, and registries.
   - Updated 1A dataset_count and validation_passed_flag dependencies to include the bundle index where applicable.

* 11:45pm
   - Regenerated the combined dictionary and registry path tree reports to include the new s0_gate_receipt_1A and validation_bundle_index entries.

* 11:47pm
   - Checked state-flow docs for s0_gate_receipt_1A and validation_bundle_index entries; found s0_gate_receipt_1A in 1A.S0 but no validation_bundle_index references in state-expanded docs.
