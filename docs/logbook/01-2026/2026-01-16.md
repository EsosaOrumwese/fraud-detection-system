...
* 07:10pm
   - Added a new in-progress 2B.S6 reasoning entry in
     `docs/model_spec/data-engine/implementation_maps/segment_2B.impl_actual.md`
     (Entry: 2026-01-16 19:10) covering the country_weights interpretation and
     the s6_edge_log join strategy.

* 07:22pm
   - Implemented `2B.S6` virtual-edge routing in
     `packages/engine/src/engine/layers/l1/seg_2B/s6_edge_router/runner.py`
     (policy validation, arrival roster loop, RNG evidence, optional edge log).
   - Added CLI entrypoint `packages/engine/src/engine/cli/s6_edge_router_2b.py`.
   - Updated `makefile` with `segment2b-s6` command wiring and aggregate target.
...
Read S6 expanded spec and related contracts to start 2B.S6 review:
...
Next: append a detailed S6 pre-implementation planning entry in
`docs/model_spec/data-engine/implementation_maps/segment_2B.impl_actual.md`
with open confirmations (policy shape, optional s6_edge_log gating, run-report
handling) before touching code.

## 18:12

Recorded S6 confirmations and decisions (user-approved) in the 2B implementation
map: extend `virtual_edge_policy_v1` schema to allow default_edges +
merchant_overrides + geo_metadata; fallback to default_edges when overrides are
missing; gate `s6_edge_log` behind policy flag + dictionary; emit STDOUT
run-report and persist a diagnostic copy. Noted spec deviation (authoring guide
edges[] strictness) and the plan to log it explicitly.

## 18:26

Added further S6 implementation notes in `segment_2B.impl_actual.md`:
- Extend `s5_arrival_roster_row` schema to allow optional `is_virtual` so S6 can
  consume it when present while keeping backward compatibility.
- Implement append-capable atomic publish for `rng_trace_log` so S6 can append
  its trace rows to the existing run-scoped file.
- Leave `rng_audit_log` intact if already present; emit only when missing.

## 18:34

Recorded S6 decision to require `s5_selection_log` only when `s6_edge_log` is
enabled, and to join it by `(merchant_id, utc_timestamp, utc_day)` to supply
`tz_group_id` and `site_id` for the edge log. If edge log is disabled, S6 will
skip loading selection log entirely.

## 18:38

Appended a detailed S6 pre-implementation plan in
`docs/model_spec/data-engine/implementation_maps/segment_2B.impl_actual.md`
covering runner structure, policy validation, RNG draw flow, trace/audit log
append strategy, optional edge log gating, CLI wiring, and Makefile updates.

## 23:24

Observed 2B S6 failure (2B-S6-020) due to `virtual_edge_policy_v1` sealed
digest mismatch. Logged the remediation plan in
`docs/model_spec/data-engine/implementation_maps/segment_2B.impl_actual.md`
Entry 2026-01-16 23:24: delete run-local S0 outputs for the current run_id
(`s0_gate_receipt` + `sealed_inputs` for the manifest), rerun S0 to reseal,
then rerun S6.

## 23:27

S6 failed again with 2B-S6-080 because `rng_audit_log` already exists from S5.
Logged a new plan entry in `segment_2B.impl_actual.md` (Entry 2026-01-16 23:27)
to switch S5/S6 to append-style audit logging (skip if row exists, append if
missing, create if absent), mirroring the 1B approach.

## 23:31

Ran `make segment2b-s0 RUN_ID=2b22ab5c8c7265882ca6e50375802b26` to reseal
inputs after policy digest updates. Then re-ran
`make segment2b-s6 RUN_ID=2b22ab5c8c7265882ca6e50375802b26` and confirmed S6
completed successfully with audit log append/no-op behaviour.

## 23:33

Implemented append-style rng_audit_log handling in
`packages/engine/src/engine/layers/l1/seg_2B/s5_router/runner.py` and
`packages/engine/src/engine/layers/l1/seg_2B/s6_edge_router/runner.py`.
Replaced atomic publish for audit logs with `_ensure_rng_audit` and preserved
audit payload validation. Next: rerun S6 for the current run_id to confirm
green.

## 23:38

Planned a roster normalization step to add `is_virtual` to the run-scoped
s5_arrival_roster (default false until 5B supplies real flags). Logged a
detailed plan in `segment_2B.impl_actual.md` Entry 2026-01-16 23:38, including
adding a helper script + Makefile target and re-sealing via S0.

## 23:40

S0 reseal failed with 2B-S0-080 after roster normalization because
`s0_gate_receipt_2B` already existed and differed. Logged the decision to
delete run-local S0 outputs (receipt + sealed_inputs partitions) and rerun S0
in `segment_2B.impl_actual.md` Entry 2026-01-16 23:40.

## 23:41

Ran `make segment2b-arrival-roster RUN_ID=2b22ab5c8c7265882ca6e50375802b26`
to inject `is_virtual=false` into all roster rows, then removed the run-local
S0 receipt/sealed_inputs partitions and reran
`make segment2b-s0 RUN_ID=2b22ab5c8c7265882ca6e50375802b26` to reseal the
updated roster digest.

## 23:48

Updated `scripts/normalize_arrival_roster.py` to build a missing
s5_arrival_roster from upstream `site_locations` (unique merchant_id) with a
fixed UTC day/timestamp and `is_virtual=false`. Logged the plan in
`segment_2B.impl_actual.md` Entry 2026-01-16 23:48 so S0 can reseal and S5 can
run green even if the roster was deleted.

## 23:49

Roster build failed due to Polars streaming engine panic on Parquet. Updated
`scripts/normalize_arrival_roster.py` to use non-streaming
`read_parquet(...).unique()` for merchant_id extraction and logged the
decision in `segment_2B.impl_actual.md` Entry 2026-01-16 23:49.

## 23:55

S5 failed with 2B-S5-086 (created_utc mismatch) after resealing S0. Logged the
plan in `segment_2B.impl_actual.md` Entry 2026-01-16 23:55 to rerun S1–S4 for
the same run_id so their created_utc aligns with the new S0 receipt.
