...
* 1:30am
   - Raised S2 CUSUM threshold in `config/layer1/1A/policy/validation_policy.yaml` to `threshold_h=250.0` (semver 1.3.2, version 2026-01-03) to allow debugging the observed cusum_max (~201.85) without failing early.
...
* 4:40am
   - Investigated Segment 3A S2 `E_ZONE_DOMAIN` on country key `False`; found `NO:` in `config/layer1/3A/allocation/country_zone_alphas.yaml` was parsed as boolean (YAML 1.1). Quoted the Norway key to `"NO"` to keep it a string and align with ISO2 expectations.

* 4:41am
   - Attempted to restage `runs/local_full_run-2/config/layer1/3A/allocation/country_zone_alphas.yaml` after the Norway key fix; PowerShell reported the run copy is the same file (hardlink), so the update is already reflected in the run folder.
...
* 7:40am
   - Updated `config/layer1/1A/ingress/transaction_schema_merchant_ids.bootstrap.yaml` to `n_merchants=10000` (from 50000) and bumped semver to 1.0.1 with version 2026-01-03 so the merchant universe matches the 10k target for the 90-day dev run.
...
* 7:44am
   - Reverted `config/layer1/1A/ingress/transaction_schema_merchant_ids.bootstrap.yaml` to the prior 50k merchant scale and version/semver (1.0.0, 2024-12-31) because the user did not request edits to that bootstrap file.
...
* 5:49pm
   - Bumped `config/layer1/1A/models/hurdle/hurdle_simulation.priors.yaml` semver to 1.2.1 and version to 2026-01-03 so hurdle exports land under the new simulation_version.
...
* 9:56pm
   - Buffered RNG logging to cut per-draw I/O overhead: keep JSONL handles open per family, batch summary writes (every 10k events), and flush handles on cadence instead of per draw. Added a close() to flush + close handles deterministically (packages/engine/src/engine/layers/l1/seg_1A/s0_foundations/l2/rng_logging.py).
   - Ensured 5B RNG writers are closed after S2/S3/S4 to flush summaries and buffered logs while keeping determinism intact (packages/engine/src/engine/layers/l2/seg_5B/s2_intensity/runner.py, packages/engine/src/engine/layers/l2/seg_5B/s3_counts/runner.py, packages/engine/src/engine/layers/l2/seg_5B/s4_arrivals/runner.py).

* 10:05pm
   - Fixed an indentation regression in 5B.S2 grouping map construction (introduced during the RNG logger close refactor) so the grouping select + iter_rows loop is properly nested again (packages/engine/src/engine/layers/l2/seg_5B/s2_intensity/runner.py).

* 10:17pm
   - Added a lightweight count-law context in 5B.S3 to avoid repeated config dict lookups during bucket-count sampling (precomputed fields + helper builder), preserving spec semantics while reducing per-row overhead (packages/engine/src/engine/layers/l2/seg_5B/s3_counts/runner.py).

* 10:50pm
   - Extended RNGLogWriter with event-only mode + configurable filenames (emit_trace/emit_summary + event/trace/summary filenames) so S3 can write per-chunk event logs and generate trace/summary later without duplicating RNG records (packages/engine/src/engine/layers/l1/seg_1A/s0_foundations/l2/rng_logging.py).
   - Refactored 5B.S3 to chunk work by Parquet row-group ranges, run chunks in parallel, and then merge counts + RNG event logs deterministically in row-group order while preserving the required writer ordering and RNG accounting semantics (packages/engine/src/engine/layers/l2/seg_5B/s3_counts/runner.py).
   - Added S3 merge step that writes the final `part-00000.jsonl` event log plus a reconstructed `rng_trace_log.jsonl` and `rng_totals.json` from merged events, keeping full-fidelity outputs at 90-day/1-hour resolution without changing the S3 domain or count law.

* 10:59pm
   - Documented the 5B.S3 full-fidelity runtime refactor pattern (chunked row-group processing, deterministic merge, RNG event-only logs + trace/summary reconstruction) for downstream reuse in `docs/engineering-decisions/state5B_s3_counts_runtime_refactor.md`.

* 11:10pm
   - Increased 5B.S3 visibility by logging each chunk start/schedule and tightening progress cadence (log interval 60s; log every ~1%/100k rows) so long runs show activity sooner without flooding the console (`packages/engine/src/engine/layers/l2/seg_5B/s3_counts/runner.py`).

* 11:56pm
   - Added a true `virtual_mode` column to 3B `virtual_classification_3B` output (NON_VIRTUAL/HYBRID/VIRTUAL_ONLY) derived from the policy decision so downstream 5B logic can rely on explicit mode rather than inferring from `is_virtual` alone (`packages/engine/src/engine/layers/l1/seg_3B/s1_virtuals/l2/runner.py`).
   - Updated 3B schemas to include `virtual_mode` in the output contract and spec (`contracts/schemas/layer1/schemas.3B.yaml`, `docs/model_spec/data-engine/layer-1/specs/contracts/3B/schemas.3B.yaml`).

* 7:33pm
   - Applied scratch review updates to ingestion_gate section header plan (envelope identity line, IG5 decision list/closures, indexing/overload/retention headings, and new contract map appendix).
