...
## 09:20 - 2A.S2 output schema harden
...
## 09:24 - 2A.S2 output dir reuse on empty
...
## 09:28 - 2A.S3 method binding fix
...
## 09:33 - 2A.S3 tzdb digest alignment
- Added tzdb_release metadata parsing so S3 validates archive bytes against 	zdb_release.json.archive_sha256 (spec-defined) instead of the sealed aggregate digest.
- Added sealed asset digest verification for 	zdb_release and propagated the metadata digest into TimetableAssets/run report.
- Added hex64 validation helper for the metadata digest.

## 09:33 - Segment2A rerun (resume S0/S1/S2)
...
- Summary written to 
uns/local_full_run-0/summaries/segment2a_result.json.
...
## 10:01 - 2B.S0 partition args fix
- Added ingerprint alongside manifest_fingerprint when sealing alidation_bundle_1B and _passed.flag so partition key resolution passes in 2B S0.

## 10:03 - 2B.S0 fingerprint partition fix (site_locations + optional pins)
- Added ingerprint template arg for site_locations and optional 2A pins so partition resolution matches dictionary partitioning in 2B S0.

## 10:07 - 2B.S0 sealed_inputs partition format
...
## 10:08 - 2B.S0 receipt payload schema alignment
- Added required receipt fields (segment, state, alidation_bundle_path, lag_sha256_hex) and emit seed as string to match the current 2B schema bundle; optionally include notes.

## 10:09 - 2B.S0 run report partition sample
...
## 10:11 - 2B schema + receipt alignment
...
## 10:14 - 2B.S3 optional pin fingerprint
...
## 10:16 - 2B.S1 weight normalisation scope
...
## 10:17 - 2B outputs reset for S1 regroup
- Removed 
uns/local_full_run-0/data/layer1/2B so S1-S8 can rebuild after changing S1 normalisation grouping.

## 10:21 - 2B.S5 site_id collision fix
...
## 13:14 - 2A.S0 merchant_mcc_map manifest rehome
...
## 13:38 - 2A.S2 merchant_mcc_map path alignment
...
## 14:00 - 2B policy formatting TODO
- [TODO] After the current run completes, pretty-format the 2B policy JSON files (alias_layout, route_rng, day_effect, virtual_edge) and bump their version tags/semver to reflect the byte-change.

## 14:02 - 2B policy formatting (executed)
