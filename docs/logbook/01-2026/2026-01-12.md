...
* 8:01pm
   - Plan: add a convenience Make target `SEG_1A` as an alias to `segment1a` so `make SEG_1A` runs the full Segment 1A pipeline without changing behavior or inputs.

* 8:02pm
   - Created `docs/model_spec/data-engine/implementation_maps/segment_1B.impl_actual.md` and logged the initial 1B readiness plan (read S0–S9 expanded specs + 1B contracts, identify sealed inputs like population raster, then draft per-state plans).

* 8:27pm
   - Verified S0 sealed-input paths from the 1B dictionary exist locally: `reference/iso/iso3166_canonical/2024-12-31/iso3166.parquet`, `reference/spatial/world_countries/2024/world_countries.parquet`, `reference/spatial/population/2025/population.tif`, and `reference/spatial/tz_world/2025a/tz_world.parquet`.
