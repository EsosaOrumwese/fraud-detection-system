...
* 9:05am
  - Re-ran SR CLI reuse run with new equivalence key; READY achieved (run_id=870056d6aaa95c99e1d770a484469563) with run_facts_view written under artefacts/fraud-platform/<platform_run_id>/sr/. Recorded results in scenario_runner.impl_actual.md.
...
* 6:22pm
  - Wired Postgres READY lease config into local/dev/prod profiles.
  - Added env placeholder resolution in IG wiring loader to resolve `${VAR}` at runtime.
  - Created scratch smoke profile + synthetic run_facts/control_bus artifacts and ran two‑instance READY consumer smoke test with Postgres leases.
  - Result: for each READY message, one instance processed and the other returned SKIPPED_LEASED; synthetic run completed with expected schema‑policy quarantines.

* 8:20pm
  - Added local workflow automation: scripts/platform.ps1 (stack up/down, SR reuse run, IG READY once/dual, IG audit), scripts/README.md, and .env.example placeholder file.
  - Documented the workflow entrypoint in README.md and logged the decision trail in platform.impl_actual.md.

* 8:31pm
  - Fixed local IG profile to use `object_store.root: artefacts` so READY run_facts paths resolve correctly for local SR runs.

* 9:07pm
  - Migrated platform runtime artifacts to `runs/fraud-platform/` and updated SR/IG wiring defaults, control bus roots, and smoke test paths.
  - Added SR/IG file logging to a shared `runs/fraud-platform/platform.log` (override via `PLATFORM_LOG_PATH`).
  - Replaced the PowerShell platform workflow with Makefile targets and updated README; removed `scripts/platform.ps1` and `scripts/README.md`.
  - Moved on-disk runtime artifacts from `artefacts/fraud-platform` → `runs/fraud-platform` and updated synthetic run_facts locator paths.

* 9:10pm
  - Removed remaining PowerShell helpers (`scripts/run_sr_tests.ps1`, `scripts/localstack.ps1`) and added Make targets for SR test tiers + LocalStack lifecycle; updated SR README accordingly.

* 9:49pm
  - Fixed SR CLI crash during `make platform-sr-run-reuse` by restoring the missing `datetime` import.

* 9:52pm
  - Added `make platform-bus-clean` to clear stale READY messages from the file control bus before running IG.

* 10:02pm
  - Ran SR Tier 0 tests: `make sr-tests-tier0` → 40 passed, 17 deselected.
  - Brought up parity stack: `make platform-stack-up` (MinIO + Postgres running).
  - Ran SR parity tier: `make sr-tests-parity` → 1 passed, 3 skipped, 53 deselected.
  - Ran IG core suite in venv: `python -m pytest tests/services/ingestion_gate/test_admission.py tests/services/ingestion_gate/test_ops_index.py tests/services/ingestion_gate/test_health_governance.py tests/services/ingestion_gate/test_phase4_ready_consumer.py tests/services/ingestion_gate/test_phase6_hash_chain.py -q` → 12 passed.
  - Cleared control bus: `make platform-bus-clean`.
  - SR reuse run returned `LEASE_BUSY` for run_id `9142f6d3ed4ca6375f388946e0e4ea4d` (existing lease on fixed equivalence key); IG READY once returned `[]` (no READY messages after bus clean).

* 10:11pm
  - Investigated IG pull `INTERNAL_ERROR` during SR→IG smoke run; reproduced pyarrow schema merge error on engine parquet.
  - Switched IG parquet reads to `pyarrow.parquet.ParquetFile(path).read()` to avoid dataset schema merge issues.

* 10:47pm
  - Ran a single SR→IG READY ingestion with a 10‑minute cap after the parquet fix. No new `INTERNAL_ERROR` events were emitted, but the full pull did not complete within the cap (large dataset).
  - Stopped stray READY consumer processes to avoid lease contention before the bounded run.

* 11:13pm
  - Implemented platform run session logging (platform_runtime helper, SR/IG services use platform_log_paths) and documented in platform/scenario_runner/ingestion_gate impl_actual maps.
  - Added IG pull time budget wiring + enforcement with TIME_BUDGET_EXCEEDED, documented in ingestion_gate impl_actual.
  - Added `make platform-run-new`, updated .env.example and service READMEs with session log notes, and set local profile pull_time_budget_seconds=600.
  - Added IG unit test for time budget and ran: `\.\.venv\Scripts\python.exe -m pytest tests/services/ingestion_gate/test_phase6_time_budget.py -q` → 1 passed.

* 11:15pm
  - Updated ingestion_gate.build_plan.md to include Phase 6.4 (local smoke time budget guardrail) and documented the DoD checklist.

* 11:27pm
  - Ran `make platform-sr-run-reuse SR_RUN_EQUIVALENCE_KEY=local_full_run_5_reuse_20260125_2316` → READY committed (run_id=40dfb540e134f8bb8eb3585da3aeee7a, message_id=27342594a254d6ccaed98c618587f884303b37d676897e21dd8be1a0e3eb6997).
  - Ran `make platform-ig-ready-once` with local profile time budget; pull completed as PARTIAL with `TIME_BUDGET_EXCEEDED` on output `arrival_events_5B` (status saved under runs/fraud-platform/<platform_run_id>/ig/pull_runs/run_id=40dfb540e134f8bb8eb3585da3aeee7a.json). Process was stopped after the 10-minute cap.

* 11:28pm
  - Updated IG build plan status line to note time-budget guard completed while Phase 6 remains in progress.

* 11:30pm
  - Planned local sharded pulls (locator_range, shard_size=1) and SR READY reemit loop to advance IG checkpoints under the 10‑minute time budget.
  - Added impl_actual entries outlining the workflow + Makefile target change.

* 11:33pm
  - Enabled local sharded pulls (locator_range, shard_size=1) and added `make platform-sr-reemit` for READY re‑emission.
  - Updated SR README to document the new Make target.

* 11:35pm
  - Fixed `platform-sr-reemit` default kind to `READY_ONLY` (CLI accepted values); updated SR README example.

* 11:56pm
  - Ran `make platform-sr-reemit SR_REEMIT_RUN_ID=40dfb540e134f8bb8eb3585da3aeee7a SR_REEMIT_KIND=READY_ONLY` to publish a new READY message.
  - Ran `make platform-ig-ready-once` with sharding enabled; run still ended PARTIAL with `TIME_BUDGET_EXCEEDED` on `arrival_events_5B` shard 0, indicating a single parquet shard exceeds the 10‑minute budget.

* 11:59pm
  - Added `dev_local.yaml` profile for uncapped completion runs; kept `local.yaml` for time‑budgeted smoke.
  - Added Make target `platform-ig-ready-once-dev` and documented local vs dev testing policy in profile/IG docs and platform build plan.
