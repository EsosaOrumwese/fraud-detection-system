...
* 8:45pm
   - Read S2 expanded spec and supporting contracts/guides for planning:
     - `docs/model_spec/data-engine/layer-1/specs/state-flow/3B/state.3B.s2.expanded.md`
     - `docs/model_spec/data-engine/layer-1/specs/contracts/3B/schemas.3B.yaml`
     - `docs/model_spec/data-engine/layer-1/specs/contracts/3B/dataset_dictionary.layer1.3B.yaml`
     - `docs/model_spec/data-engine/layer-1/specs/contracts/3B/artefact_registry_3B.yaml`
     - `docs/model_spec/data-engine/layer-1/specs/data-intake/3B/cdn_country_weights_authoring-guide.md`
     - `docs/model_spec/data-engine/layer-1/specs/data-intake/3B/cdn_weights_ext_yaml_acquisition-guide.md`
     - `docs/model_spec/data-engine/layer-1/specs/data-intake/3B/cdn_key_digest_authoring-guide.md`
     - `docs/model_spec/data-engine/layer-1/specs/data-intake/3B/virtual_logging_policy_authoring-guide.md`
     ...
     - `docs/model_spec/data-engine/layer-1/specs/contracts/2B/schemas.2B.yaml`
     - `docs/model_spec/data-engine/layer-1/specs/contracts/1A/schemas.layer1.yaml`
   - Inspected current sealed inputs for 3B to see which S2 inputs are already sealed:
     `runs/local_full_run-5/970b0bd6833be3a0f08df8e8abf0364c/data/layer1/3B/sealed_inputs/manifest_fingerprint=35c89fb31f5d034652df74c69ffbec7641b2128375ba5dd3582fb2e5a4ed2e08/sealed_inputs_3B.json`.
   - Preparing S2 plan + questions in `docs/model_spec/data-engine/implementation_maps/segment_3B.impl_actual.md`.

* 8:48pm
   - Captured approved S2 decisions (RNG policy extension, sealed spatial/tz assets, uniform edge weights, edge_id ordering, tz resolution, digest law) in `docs/model_spec/data-engine/implementation_maps/segment_3B.impl_actual.md`.

* 9:21pm
   - Refined S2 implementation plan (precompute budgets/tiles, tile_bounds jitter, edge_jitter-only RNG, tz overrides country-only, RNG derivation domains).
   - Logged the detailed decisions in `docs/model_spec/data-engine/implementation_maps/segment_3B.impl_actual.md`.

* 9:54pm
   - Reviewed S2 contracts/schema anchors for edge catalogue + RNG events:
     `docs/model_spec/data-engine/layer-1/specs/contracts/3B/schemas.3B.yaml`,
     `docs/model_spec/data-engine/layer-1/specs/contracts/2B/schemas.2B.yaml`,
     `docs/model_spec/data-engine/layer-1/specs/contracts/1A/schemas.layer1.yaml`.
   - Decided S2 determinism details (rng_event_id hash law, uniform edge_weight, tz_source mapping,
     digest ordering, provenance fields) and logged them in
     `docs/model_spec/data-engine/implementation_maps/segment_3B.impl_actual.md`.

* 10:23pm
   - Implemented 3B.S2 edge catalogue runner with sealed-input validation, deterministic
     edge allocation + jitter loop, tz resolution (polygon/nudge/override), RNG event
     logging, index/digest assembly, and run-report emission:
     `packages/engine/src/engine/layers/l1/seg_3B/s2_edge_catalogue/runner.py`.
   - Added S2 CLI entrypoint:
     `packages/engine/src/engine/cli/s2_edge_catalogue_3b.py`.
   - Wired Makefile target/args for `segment3b-s2`:
     `makefile`.

* 10:24pm
   - Ran `make segment3b-s2`; failed with `E3B_S2_005_SEALED_INPUT_DIGEST_MISMATCH`
     because `route_rng_policy_v1` was modified after the S0 seal for this
     manifest (sealed digest no longer matches).
   - Logged decision to re-run 3B.S0 to reseal the policy before retrying S2 in
     `docs/model_spec/data-engine/implementation_maps/segment_3B.impl_actual.md`.

* 10:25pm
   - Fixed schema regex for `route_rng_policy_v1.streams.virtual_edge_catalogue.rng_stream_id`
     so `3B.edge_catalogue` validates:
     `docs/model_spec/data-engine/layer-1/specs/contracts/2B/schemas.2B.yaml`.
   - Logged the contract fix rationale in
     `docs/model_spec/data-engine/implementation_maps/segment_3B.impl_actual.md`.

* 10:26pm
   - Re-ran `make segment3b-s0`; failed with `E3B_S0_009_IMMUTABILITY_VIOLATION`
     because `sealed_inputs_3B` already exists for the current manifest and the
     updated RNG policy would change its contents.
   - Logged the decision point/options in
     `docs/model_spec/data-engine/implementation_maps/segment_3B.impl_actual.md`.

* 10:37pm
   - Documented dev-mode plan to move existing `sealed_inputs_3B.json` and
     `s0_gate_receipt_3B.json` out of the canonical paths to unblock S0 reseal,
     with backup under `runs/.../dev_overrides/`.

* 10:38pm
   - Moved existing S0 blockers for reseal into
     `runs/local_full_run-5/970b0bd6833be3a0f08df8e8abf0364c/dev_overrides/3B_S0_reseal_2026-01-18_2237/`:
     - `runs/local_full_run-5/970b0bd6833be3a0f08df8e8abf0364c/data/layer1/3B/sealed_inputs/manifest_fingerprint=35c89fb31f5d034652df74c69ffbec7641b2128375ba5dd3582fb2e5a4ed2e08/sealed_inputs_3B.json`
     - `runs/local_full_run-5/970b0bd6833be3a0f08df8e8abf0364c/data/layer1/3B/s0_gate_receipt/manifest_fingerprint=35c89fb31f5d034652df74c69ffbec7641b2128375ba5dd3582fb2e5a4ed2e08/s0_gate_receipt_3B.json`

* 10:38pm
   - Re-ran `make segment3b-s0` after moving blockers; S0 reseal completed
     successfully for run_id `970b0bd6833be3a0f08df8e8abf0364c`.

* 10:39pm
   - Removed the S0 reseal blockers again by moving the current
     `sealed_inputs_3B.json` and `s0_gate_receipt_3B.json` into
     `runs/local_full_run-5/970b0bd6833be3a0f08df8e8abf0364c/dev_overrides/3B_S0_reseal_2026-01-18_2239/`
     so the manifest can be resealed.

* 10:52pm
   - Investigated `E3B_S2_019_INFRASTRUCTURE_IO_ERROR` from `make segment3b`
     and found S2 failed to locate partitioned parquet files under
     `data/layer1/1B/tile_index/...` because `_resolve_parquet_files` only
     scanned the top-level directory.
   - Updated `_resolve_parquet_files` in
     `packages/engine/src/engine/layers/l1/seg_3B/s2_edge_catalogue/runner.py`
     to use `rglob("*.parquet")` so partitioned datasets are discovered.

* 10:56pm
   - Re-ran `make segment3b-s2`; S2 now fails with
     `E3B_S2_TILE_SURFACE_INVALID` for country `AQ` because the CDN policy
     includes countries that have no `tile_index`/`tile_weights` surfaces.
   - Removed `AQ` and `VA` from
     `config/layer1/3B/virtual/cdn_country_weights.yaml`, renormalised the
     remaining weights to sum to 1, and rebuilt
     `config/layer1/3B/virtual/cdn_key_digest.yaml` with
     `scripts/build_cdn_key_digest_3b.py` so the policy remains valid.

* 10:57pm
   - Moved the current S0 seal artefacts to unblock resealing after the policy
     change:
     - `runs/local_full_run-5/970b0bd6833be3a0f08df8e8abf0364c/data/layer1/3B/sealed_inputs/manifest_fingerprint=35c89fb31f5d034652df74c69ffbec7641b2128375ba5dd3582fb2e5a4ed2e08/sealed_inputs_3B.json`
     - `runs/local_full_run-5/970b0bd6833be3a0f08df8e8abf0364c/data/layer1/3B/s0_gate_receipt/manifest_fingerprint=35c89fb31f5d034652df74c69ffbec7641b2128375ba5dd3582fb2e5a4ed2e08/s0_gate_receipt_3B.json`
     -> `runs/local_full_run-5/970b0bd6833be3a0f08df8e8abf0364c/dev_overrides/3B_S0_reseal_2026-01-18_2256/`.

* 11:00pm
   - Re-ran `make segment3b-s0`; S0 reseal completed successfully after the
     CDN policy update.

* 11:09pm
   - Re-ran `make segment3b-s2`; S2 ran for ~213s and failed with
     `E3B_S2_019_INFRASTRUCTURE_IO_ERROR` in `tile_surfaces` with empty
     exception detail (run_report updated at 23:01).
   - Observed high memory usage during the tile-surface phase and updated
     `packages/engine/src/engine/layers/l1/seg_3B/s2_edge_catalogue/runner.py`
     to store tile bounds only for tiles that receive edges (while still
     validating that all tile_ids in `tile_weights` are covered), to reduce
     per-run memory pressure before retrying S2.

* 11:17pm
   - Attempted another `make segment3b-s2`; the run again stalled in
     `tile_surfaces` with rising memory usage, so I terminated the
     `engine.cli.s2_edge_catalogue_3b` and `make` processes to apply another
     memory reduction pass.
   - Updated S2 to load only the `tile_id` column for full coverage checks and
     then load full bounds only for the tiles that actually receive edges,
     further reducing memory pressure in the `tile_surfaces` phase.

* 11:22pm
   - Re-ran `make segment3b-s2`; it reached `tile_surfaces` and then failed
     with `E3B_S2_019_INFRASTRUCTURE_IO_ERROR` because `_StepTimer.info()` was
     called with formatting args (expects a single message string).
   - Fixed the log line in
     `packages/engine/src/engine/layers/l1/seg_3B/s2_edge_catalogue/runner.py`
     to preformat the message via f-string.

* 11:30pm
   - Re-ran `make segment3b-s2`; it progressed into edge placement and failed
     with `E3B_S2_TZ_RESOLUTION_FAILED` for a TW point (no tz polygon match
     after nudge/overrides).
   - Added a `scope: country` override for `TW` -> `Asia/Taipei` in
     `config/layer1/2A/timezone/tz_overrides.yaml` so the override path can
     resolve these cases on the next run.
   - Moved current 3B S0 seal artefacts to
     `runs/local_full_run-5/970b0bd6833be3a0f08df8e8abf0364c/dev_overrides/3B_S0_reseal_2026-01-18_2330/`
     to allow resealing with the updated tz override policy.
...
* 11:33pm
   - Checked `reference/spatial/tz_world/2025a/tz_world.parquet` with pandas
     (GeoParquet columns) to confirm `TW` maps to `Asia/Taipei`; this supports
     the TW override decision for polygon gaps during jitter.

* 11:40pm
   - Re-ran `make segment3b-s2`; the CLI timed out locally, but the S2 run
     completed in the background and wrote a PASS run_report at
     `runs/local_full_run-5/970b0bd6833be3a0f08df8e8abf0364c/reports/layer1/3B/state=S2/seed=42/manifest_fingerprint=35c89fb31f5d034652df74c69ffbec7641b2128375ba5dd3582fb2e5a4ed2e08/run_report.json`.
