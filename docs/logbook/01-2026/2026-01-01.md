...
* 1:18am
   ...
   - [TODO] Residual quantisation ignores `config/layer1/1A/numeric/residual_quantisation.yaml` and hard-codes `DP_RESID=8`; no reference to the authored policy exists (spec requires policy-driven quantisation) (`packages/engine/src/engine/layers/l1/seg_1A/s7_integer_allocation/kernel.py`).
...
* 7:15pm
   - Implemented 5B.S5 RNG accounting against event logs + trace totals, plus routing and civil-time checks using sealed upstream surfaces (site_timezones, edge_catalogue_3B, virtual_settlement_3B, universe hashes).
   - Added validation of per-arrival routing hash coherence and tzid↔local time offsets in the S5 validator.
   - [TODO] S5 still does not validate full schema/PK/partition constraints for `s4_arrival_events_5B` or read `tz_timetable_cache` for civil-time checks (ZoneInfo-only).
   - [TODO] S5 bundle membership is limited to report + issue table; consider adding additional evidence files if required by the bundle layout policy.

* 7:28pm
   - Implemented 5B.S5 schema/partition/PK validation for `arrival_events_5B` (required columns, partition/value checks, PK uniqueness) and wired failures into the issue table + report flag.
   - Added `tz_timetable_cache` validation in S5 (manifest fields, fingerprint match, index digest, rle_cache_bytes) and tzid coverage check against `site_timezones`; report now includes `tz_cache_ok`.
   - Added missing-column guards so routing/civil/count/time/RNG checks fail closed when required arrival columns are absent.

* 9:35pm
   - Decision: keep `validation_report_5B.bundle_sha256` unset to avoid self-referential hashing; rely on `_passed.flag` bundle digest until spec clarifies a non-recursive echo strategy.

* 9:36pm
   - Copied Layer-3 contracts for 6A/6B from specs into `contracts/` (schemas.layer3/6A/6B, dataset dictionaries, artefact registries) to align engine resolution with the registry authority.
   - Decision: layer3 dictionaries are list-form in specs; will add layer3 dictionary helpers that can resolve list-based entries instead of the sectioned mapping used by layer1/2.

* 10:31pm
   - Started 6A/6B implementation pass: inspected layer-3 state-flow docs and dataset dictionaries to scope required outputs and sealed inputs.
   - Decision: implement minimal deterministic 6A/6B pipeline aligned to dataset dictionary and required fields; defer full spec run-report surface due to missing dictionary entries (log deviations as they occur).

* 10:37pm
   - Reviewed state specs for 6A.S4 (devices/IP graph) and 6B.S1 (arrival attachment/sessionisation) plus dataset dictionaries for 6A/6B to confirm required outputs and partition keys.
   - Decision: implement deterministic minimal attachments/graph/labels that preserve required identity fields and partitioning, with partial prior parsing (fallback defaults where priors are incomplete).

* 10:38pm
   - Added deterministic normal inverse CDF approximation in packages/engine/src/engine/layers/l3/shared/deterministic.py to support account/instrument weight construction without external libs.

* 10:39pm
   - Updated packages/engine/src/engine/layers/l3/seg_6A/s2_accounts/runner.py to derive account counts from product_mix priors and allocate accounts to parties using account_per_party priors (deterministic lognormal weights via new normal_icdf helper).
   - Decision: ignore optional context_scaling/merchant_mode in product_mix for now; log as deviation if encountered in configs.

* 10:40pm
   - Added upstream PASS checks and required-hints enforcement in packages/engine/src/engine/layers/l3/seg_6A/s1_parties/runner.py based on 6A S0 receipt + population priors inputs_allowed.

* 10:41pm
   - Added packages/engine/src/engine/layers/l3/seg_6A/s3_instruments/__init__.py to export S3 instrument runner types.

* 10:42pm
   - Implemented 6A.S4 device/IP graph generator in packages/engine/src/engine/layers/l3/seg_6A/s4_network/runner.py with deterministic device/ip creation, link tables, and summary outputs; added exports in .../s4_network/__init__.py.
   - Decision: use simplified priors (base lambdas + optional mix weights) and deterministic hash sampling for types; ignore advanced linkage rules beyond max_devices_per_party for now.

* 10:45pm
   - Implemented 6A.S5 posture + validation runner in packages/engine/src/engine/layers/l3/seg_6A/s5_validation/runner.py, emitting fraud role tables and a minimal validation bundle/index/flag; added exports in .../s5_validation/__init__.py.
   - Decision: use simplified role probability parsing with deterministic hash selection; default to CLEAN-heavy distribution when priors omit explicit probabilities.

* 10:46pm
   - Added packages/engine/src/engine/layers/l3/seg_6A/__init__.py to export S0-S5 runners and IO dataclasses.

* 10:47pm
   - Added upstream PASS guardrails to 6A.S2 and 6A.S3 runners (enforce s0_gate_receipt_6A.upstream_segments status checks).

* 10:48pm
   - Implemented 6B.S1 arrival attachment + sessionisation in packages/engine/src/engine/layers/l3/seg_6B/s1_arrivals/runner.py with deterministic entity selection and session_id hashing; added exports in .../s1_arrivals/__init__.py.

* 10:50pm
   - Implemented 6B.S2 baseline flow/event generation in packages/engine/src/engine/layers/l3/seg_6B/s2_baseline/runner.py and added exports in .../s2_baseline/__init__.py (amounts derived deterministically from amount_model bounds).

* 10:51pm
   - Implemented 6B.S3 fraud overlay in packages/engine/src/engine/layers/l3/seg_6B/s3_fraud/runner.py with deterministic fraud assignment + campaign catalogue; added exports in .../s3_fraud/__init__.py.

* 10:52pm
   - Implemented 6B.S4 labelling in packages/engine/src/engine/layers/l3/seg_6B/s4_labels/runner.py with deterministic truth/bank-view labels + case timeline; added exports in .../s4_labels/__init__.py.

* 11:08pm
   - Added Segment 6B exports (`seg_6B/__init__.py`) and S5 validation exports (`seg_6B/s5_validation/__init__.py`) to mirror 6A module structure.
   - Implemented Segment 6A/6B scenario runners and CLI wrappers (`engine/scenario_runner/l3_seg_6A.py`, `engine/scenario_runner/l3_seg_6B.py`, `engine/cli/segment6a.py`, `engine/cli/segment6b.py`) for S0-S5 orchestration and summary output.
   - Updated `engine/scenario_runner/__init__.py` and `engine/cli/__init__.py` to export 5B/6A/6B entry points for automation.
   - Extended the makefile with Segment 5B/6A/6B targets and arguments; these targets derive manifest/parameter hashes from upstream summaries and select validation bundles via glob to keep run orchestration repeatable.

* 11:24pm
   - Organized makefile execution defaults by adding unbuffered Python wrappers (`PY_ENGINE`, `PY_SCRIPT`) and switching segment/profile/build commands to use them so console output is responsive during long runs.
   - Added explicit segment-start echo lines for 1A/1B/2A/2B/3A/3B/5A/5B/6A/6B targets to avoid "silent" runs.

* 11:26pm
   - Updated the makefile `all` target to include Segment 5B, 6A, and 6B so the default pipeline runs through the full implemented stack.

* 11:44pm
   - Fixed 6B S0 sealing to label config/contract rows with `owner_segment=6B`, look up manifest keys in the 6B registry, and emit accurate error text for missing 6B configs.
   - Updated 6B S5 scenario discovery to scan actual `s1_arrival_entities_6B` outputs (dictionary path glob) instead of sealed-input inventory, avoiding false “baseline-only” validation failures.
   - Adjusted 6B S5 bundle hashing to include `index.json` in the digest so `_passed.flag` reflects the full bundle contents (report + issue table + index).

* 11:49pm
   - Clarified scenario responsibility: data-intake guides (scenario overlay policy, horizon config, scenario calendar) define possible scenarios and timing inputs, while the scenario runner chooses which scenario IDs to materialize for a run (baseline-only vs baseline+overlays).
   - Decision for future runner work: ensure the scenario runner explicitly selects non-baseline scenarios (or accepts a list of scenario IDs) if non-baseline partitions must always exist.

* 11:55pm
   - Identified upstream bundle format mismatch risk for 6A/6B S0 when verifying 5A/5B validation bundles (layer2 uses JSON pass flags + `entries` index format).
   - Updated 6A/6B S0 to parse layer2 bundle indices and pass flags directly, and compute bundle digests using the same algorithm as 5A/5B to avoid false gate failures.
