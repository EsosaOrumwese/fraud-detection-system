...
* 6:27am
  - Aligned local file‑bus root to `runs/fraud-platform/event_bus` for EB smoke runs (updated local profile).
...
* 7:13pm
  - Aligned SR local parity wiring to use `s3://fraud-platform/sr` (single MinIO bucket) to remove extra bucket friction.
