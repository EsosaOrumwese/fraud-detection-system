...
* 11:03pm
  - Fixed stream-sort CLI JSON output to serialize receipt payloads (avoids StreamSortStats serialization error).

* 11:10pm
  - Added WSP log detail for STREAM_VIEW_ID_MISMATCH (expected vs manifest id) to make diagnostics explicit.
...
* 11:08PM
  - Assessed 2A outputs for run c25a2675... (site_timezones, s1_tz_lookup, legality report, cache) and added detailed findings + realism grade to eports/eda/segment_2A/segment_2A_published_report.md.

* 11:47PM
  - Embedded 2A realism plots and added detailed plot-by-plot assessments plus visual takeaway in eports/eda/segment_2A/segment_2A_published_report.md.

* 11:51PM
  - Added 2A realism grade and prioritized improvement areas to eports/eda/segment_2A/segment_2A_published_report.md.

* 11:57pm
  - WSP READY now prefers explicit oracle wiring (`ORACLE_ENGINE_RUN_ROOT`/`ORACLE_ROOT`) to prevent stream_view_id mismatches when SR uses local engine roots.
  - Exported ORACLE_* vars into WSP Make targets; updated parity runbook with stream view verification + mismatch guidance.
