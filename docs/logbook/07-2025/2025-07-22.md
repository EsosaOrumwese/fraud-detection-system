...
* 8:09pm
  ...
  * Summary:
    - Govern MCC→is_virtual policy ledger in `config/virtual/mcc_channel_rules.yaml` (`semver` + `sha256_digest` → `virtual_rules_digest`) with CI `test_virtual_rules.py`
    ...
    - Govern CDN edge weights in `config/virtual/cdn_country_weights.yaml` (`semver` + `sha256_digest` → `cdn_weights_digest`) with CI `test_cdn_weights.py`
    ...
    - Govern virtual‑merchant validation thresholds in `config/virtual/virtual_validation.yml` (`semver` + `sha256_digest` → `virtual_validation_digest`) with CI `test_virtual_validation.py`
    - Specify crash recovery logging to `logs/edge_progress.log` (`config/logging/virtual_logging.yml` govern rotation + retention → `virtual_logging_digest`) with CI `test_log_config.py`
