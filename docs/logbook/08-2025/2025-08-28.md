...
* 7:11pm
  * Require `blocks` in rng_envelope to close S1/S0 budget contract
    * Rationale
      - Align the frozen layer-1 schema with the S1 + S0 contracts where the
        uniform budget identity is:
          `u128(after) − u128(before) = parse_u128(draws) = blocks`
      - Remove ambiguity from S1 text where `blocks` was treated as normative
        (and used by the trace as `blocks_total`) but not required by schema.
    * Changes
      - schemas.layer1.yaml.frozen.txt:
        - Add `blocks` (uint64) to `$defs.rng_envelope.properties`.
        - Add `blocks` to `$defs.rng_envelope.required`.
        - Document that `blocks` equals `parse_u128(draws)` (count of 64-bit uniforms).
