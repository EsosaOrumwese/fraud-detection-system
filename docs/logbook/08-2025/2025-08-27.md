...
* 10:03pm
  * Finalize S1.V validator & CI; lock replay, budgeting, gating, and partition lint
    - Purpose & scope: schema → numeric replay → budget/trace → partition → gating → cardinality.
    - Inputs: registry-bound hurdle dataset + cumulative trace; gated streams discovered via registry filter;
      frozen encoders and single-YAML β; run lineage keys.
    - Schema conformance: envelope complete (ts_utc, module/substream_label, before/after_hi/lo, blocks, draws);
      payload minimal & authoritative (merchant_id string→u64, pi round-trip, is_multi boolean, deterministic derived, u number|null).
    - Numeric replay: fixed-order binary64 dot (Neumaier) + thresholded two-branch logistic; assert 0≤pi≤1 and finiteness.
    - RNG accounting: reconstruct base counter; enforce budget identity
      (after−before = blocks = parse_u128(draws) ∈ {0,1}); lane policy; regenerate u for stochastic rows.
    - Trace model: reconcile cumulative trace totals per (module, substream_label, merchant) to Σ(event blocks) and counter delta.
    - Partition authority: path ↔ embed equality on {seed, parameter_hash, run_id, module, substream_label};
      ensure no fingerprint in path; registry literals exact.
    - Gating: presence-based—gated 1A RNG streams exist iff hurdle is_multi=true (discovered via registry filter).
    - Cardinality & uniqueness: exactly one hurdle row per merchant; count equals ingress set.
    - Failure objects: canonical JSON with registry dataset_id, module/substream_label, lineage, counters,
      blocks/draws, expected_delta/trace_totals, and precise failure_code (maps to S1.6).
    - CI: seed/parameter-bundle matrix; schema/partition/replay/gating/cardinality steps; any failure blocks merge.
