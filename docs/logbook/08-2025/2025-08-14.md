...
* 3:42pm
  ...
  * Reinforce outlet_catalogue ordering contract for Batch 4.3
    - `schemas.1A.yaml#/egress/outlet_catalogue.notes`:
      - Added explicit note that inter-country order is NOT represented in `outlet_catalogue`.
      - Stated consumers MUST join `alloc/country_set.rank` (0=home; foreigns follow Gumbel order).
    - `data_dictionary:outlet_catalogue.description`:
      - Retained warning but strengthened to “1B MUST join country_set.rank” for inter-country order.
      - Preserved existing hand-off gate language for `_passed.flag` verification (see S9.9).
    - No schema structure changes; purely normative clarifications to remove ambiguity for 1B.

* 4:06pm
  * Batch 5.A — Pin numeric baseline for ordering-sensitive math
    - S0.8: Made numeric policy normative for S5/S7:
      - IEEE-754 binary64 with roundTiesToEven
      - FMA disabled on ordering/integerisation code paths
      - Serial, single-thread reductions only (fixed order)
      - Honour subnormals (no flush-to-zero)
      - Changes treated as artefacts; flip manifest_fingerprint on toggle changes
    - Left RNG envelope and partitioning invariants unchanged.

* 4:27pm
  * Batch 5.B — Standardize ordering-sensitive sums with Neumaier compensation
    - S5.3: Replace plain serial sum with fixed-order Neumaier compensated sum (binary64)
      over ISO-lex indices; renormalize if |S−1|>1e−12 and recheck via sum_comp; forbid BLAS/parallel reductions.
    - S7: Apply the same compensated sum for Dirichlet normalization in country_set.rank order;
      tighten sum-to-one tolerance to 1e−12 and include algorithm block; prohibit BLAS/GPU reductions.
    - No schema/registry/dictionary changes. Complements Batch 5.A numeric baseline.
  * Batch-5.C: Make residual quantisation in S7.3 mathematically exact
    - Updated S7.3 step (4) to explicitly define 8-decimal-place quantisation using ties-to-even rounding.
    - Added formal definition:
        q = roundToEven(10^8 * u) ∈ ℤ,  r_i = q / 10^8,
      with .5 cases rounded to the even integer.
    - Mandated that sorting keys use this quantised r_i to ensure platform-independent tie behaviour.
    - No other algorithmic changes; all invariants and downstream steps remain intact.
  * Batch 5.D — Inline “no-BLAS/no-GPU” prohibitions at summation sites
    - S5.3 step (5): Added explicit one-line rule: “Do not call BLAS, GPU, or parallel
      reductions here; this is a single-thread loop.”
    - S7.2: Added matching one-line prohibition: “No BLAS/GPU; this normalisation is a
      single-thread loop.”
    - No schema/registry/dictionary changes; purely contextual guardrails where sums occur.

* 4:55pm
  * docs(1A): harmonise “sum-to-one” tolerances across S5/S7 (Batch 5.E)
    - S5.3: after fixed-order compensated sum renormalisation, specify dual tolerances:
      internal target |Σw−1| ≤ 1e-12 (binary64); schema/event checks permit ≤ 1e-6
      for emit-time rounding/downstream reader variance.  
    - S7.2: same tolerance statement added after Dirichlet weight normalisation.  
    - Note that validator records both tolerances in `metrics.csv` for traceability.

* 5:27pm
  * Batch 5.F — Add numeric-determinism validation hooks to S9
    - New §S9.10: validator recomputes S5 currency→country sums with fixed-order
      Neumaier compensation (binary64) and asserts |Σw−1| ≤ 1e−12; if sparse_flag,
      enforces equal-split within 1e−12.
    - For S7, recompute sum via Neumaier in country_set.rank order, assert
      |Σw−1| ≤ 1e−6 for recorded weights, and verify quantised residuals
      (8 dp, ties-to-even) reproduce published residual_rank order.
    - Failures are hard-fail; diagnostics in bundle; _passed.flag withheld.
    - No schema/registry/dictionary changes; results summarised in
      rng_accounting.json and metrics.csv inside validation_bundle_1A.
