...
* 11:41pm:
   ...
   - Outlined the S1 data plumbing: scenario runner will hydrate sealed paths from the S0 receipt, stream `site_locations` in partitions to a point-in-polygon engine fed by the sealed `tz_world` parquet, call a deterministic lookup kernel (including ε-nudge metadata from `config/timezone/tz_nudge.yml`), and persist results + diagnostics to `runs/<root>/data/layer1/2A/s1_tz_lookup/seed=<seed>/fingerprint=<fp>/`.
