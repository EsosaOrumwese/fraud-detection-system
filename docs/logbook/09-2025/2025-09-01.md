...
* 6:16pm
  * spec(s0): close medium issues M1–M4 for State-0
    * Key Changes
      * M1 — Trace/audit lineage & envelope scope
        - S0.3.9: Audit rows explicitly marked as **core logs (not events)**.
        - Added **Lineage binding (normative)** for `rng_trace_log`: consumers and
          producers MUST bind `{seed, parameter_hash, run_id}` from the enclosing
          partition path; these fields are not duplicated as columns; drift is a
          hard **F5** failure.
      * M2 — Ordering notes (JSONL vs Parquet)
        - S0.10.3 (Log-scoped RNG): Replaced the misplaced **Parquet** note with
          the correct **JSONL line-order** rule — append order **within a file**,
          **no** ordering guarantees across files/parts; equality by **row set**.
        - S0.2.2 (Parameter-scoped storage effect): Added the **Parquet row/row-group
          unspecified** caveat; consumers must treat equality as **row-set**
          equality (no reliance on physical order).
      * M3 — Open-interval uniform mapping
        - After `# u01 mapping (open interval)`: Added a normative note that the
          `u==1.0` remap is **measure-zero**, required by binary64 rounding, and
          **does not** affect PRNG budgets or counter deltas. Reaffirmed schema
          type `u01` enforces the open interval **(0,1)**.
      * M4 — Constants policy (generalised)
        - S0.8.1: Added a **constants** policy: all decision-critical constants
          (e.g., `TAU`) MUST be encoded as **binary64 hex literals**; recomputation
          (e.g., `2*pi`) is forbidden to avoid cross-platform drift.
    * Files changed
      - state.1A.s0.expanded.v4-yellow.txt
        - §S0.3.9: audit vs event clarification; lineage binding rule for trace.
        - §S0.10.3 (Log-scoped RNG): JSONL line-order rule.
        - §S0.2.2 (Parameter-scoped storage effect): Parquet row/row-group note.
        - `u01` mapping block: measure-zero / no-budget-impact note.
        - §S0.8.1: constants policy bullet.

* 7:15pm
  * Added the first draft for the L3 validation spec for state-1 expanded doc.
  * spec(s0): close low issues L1–L6 for State-0
    * Summary of closures
      - L1 — Canonical RNG algorithm string
        - Under “PRNG (fixed)”, keep prose name “Philox 2×64 with 10 rounds”
          and add the wire token: `philox2x64-10` (lowercase). Validators MUST
          expect this exact token on the wire (e.g., rng_audit_log.algorithm).
      - L2 — Budget table completeness
        - Extend the normative Budget table to explicitly include all
          **non-consuming** events with `(blocks=0, draws="0")` and `before==after`:
          `stream_jump`, `sequence_finalize`, `site_sequence_overflow`.
      - L3 — Single authority for paths/partitions
        - After the Log-scoped (RNG) path template, add a normative note:
          paths and partition columns are authoritative in the **dataset dictionary**;
          strings in S0 are examples only.
      - L4 — Duplicated `uint64` totals note
        - Verified deduplication: the “saturating uint64; fail before overflow”
          rule appears once under §S0.3.9 for `rng_trace_log.blocks_total`.
          No redundant repeats remain.
      - L5 — Neumaier dot-product pseudocode
        - Verified the reference kernel uses the correct half-open loop
          `for i in 0..len(a)-1:` and proper compensation; no further edits.
      - L6 — Schema anchors for audit/trace logs
        - Fix remaining audit reference to:
          `schemas.layer1.yaml#/rng/core/rng_audit_log`.
        - Add explicit trace schema pointer:
          `schemas.layer1.yaml#/rng/core/rng_trace_log`.
    * Files changed
      - state.1A.s0.expanded.v4-yellow.L.APPLIED.txt
        - Added RNG wire token line.
        - Added non-consuming rows in the Budget table.
        - Inserted dictionary authority note under Log-scoped path template.
        - Corrected audit schema anchor; added trace schema anchor line.
        - (Verified: no duplicate uint64 rule; Neumaier kernel already correct.)
