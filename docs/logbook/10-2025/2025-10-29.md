...
* 1:45pm:
   - Converted S6 tile-bound/centroid caches to hydrate directly from `pyarrow.dataset` into NumPy arrays and wired searchsorted lookups, wiping out the 400M `iter_rows` hotspot (packages/engine/src/engine/layers/l1/seg_1B/s6_site_jitter/l2/aggregate.py).
