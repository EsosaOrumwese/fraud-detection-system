...
* 10:34pm:
   - Patched S7 tile-bounds loader to handle both legacy min_* columns (test harness) and the new west/east naming coming out of S1B. It now joins per-ISO centroids from the tile index only when needed, so downstream synthesis no longer crashes once S6 finishes.

* 11:13pm:
   - Adjusted S7 tile-bounds loader to handle current west/east/south/north parquet schema without pulling in the entire tile_index. When centroid columns are missing we compute them directly from min/max edges, which avoids the heavy join that was crashing Polars and keeps S7 streaming-friendly.
   - I reimplemented load_tile_bounds to stream the parquet data in manageable Arrow batches, so we never try to materialise all 800+ fragments at once.
   - The loader now works in two modes:
      - Legacy schema: If the parquet already exposes the old min_*/centroid_* columns (true for unit fixtures), we read them directly.
      - Current schema: If only west_lon/east_lon/south_lat/north_lat exist (the real run), we rename those and derive centroids via (min + max) / 2. No more joining on the massive tile_index partition.
   - After batching, we concatenate the per‑ISO slices into a Polars DataFrame and hand it to the synthesis kernel just like before.

* 11:56pm:
   - Reworked TileBoundsPartition.collect_country to stream via pyarrow.dataset and intersect the requested ISO with the exact tile IDs needed, while normalising both legacy (min_lon_deg/centroid_*) and current (west_lon/east_lon) schemas and surfacing a deterministic error if geometry columns are missing.
   - Updated the S7 synthesis kernel to request per-country geometry using the new filtered loader, preventing the join from materialising millions of unused tiles and keeping the original validation guard in place.
   - Adjusted the S7 validator to reuse the streaming tile loader lazily by ISO, so geometry parity checks run without relying on the removed .frame attribute and stay consistent with the deterministic checks.

* 11:58pm:
   - Rebuilt the S7 synthesis frame so every column is materialised via typed polars.Series (pl.UInt64/pl.Float64), which removes the signed-64 overflow we were hitting when a tile id exceeded 2^63-1. The rest of the logic is untouched, and the targeted suite passes again