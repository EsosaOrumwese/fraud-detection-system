...
* 7:19pm:
   ...
   - Reworked the S3 integerisation kernel so bounded Hamilton follows the spec: floors come from policy, unbounded ceilings are honoured, shares are recomputed on the eligible set, residuals are quantised at `dp_resid=8`, and the bump order/stored `residual_rank` is deterministic and capacity-aware.
   - Tightened the base-weight DSL evaluator: predicates like `mcc == 5411` now coerce strings/numbers safely, and we keep a full weight vector (zero for excluded countries) so integerisation can still consider uniform fallback when every score is suppressed.
   - Added unit coverage for the new behaviour—tests assert that excluded countries carry zero weight, that “deny-all” policies trigger uniform fallback, and that we correctly handle policy sets with expanded ISO coverage.

* 7:28pm:
   - Expanded _compute_priors so it always produces a weight slot per candidate (zero for excluded rows) and distinguishes between “no score anywhere” (uniform fallback) and partial exclusions. Added targeted tests that exercise rule exclusion and deny-all policies.
   - Tightened predicate evaluation: mixed string/number comparisons (e.g., `mcc == 5411`) now coerce safely before evaluation, keeping the governed DSL spec-compliant.
   - Upgraded `validate_s3_outputs` into a spec-aligned L3 harness: it now enforces partition lineage, schema/NULL constraints, key uniqueness, prior DP consistency, integerisation reconstruction (including bounds), sequencing checks, and cross-dataset coherence. Metrics remain published for orchestration.

* 8:06pm:
   - Removed the duplicate dataclass annotation so the validator class can be constructed during import.
   - Added a shared Segment 1A dictionary resolver and wired hurdle gating to read through it, eliminating the old bespoke loader.
   - Updated the S3 runner to cache the dictionary, resolve every output via `resolve_dataset_path`, and provide consistent template arguments for all egress artefacts.
   - Pointed S3 validation publishing at the dictionary-defined bundle location so the sealed gate stays aligned with contract paths

* 11:59am:
   - Hardened validate_s3_outputs with JSON-Schema enforcement, richer metrics (schema flag, floor/ceiling usage, diagnostic row counts), and a per-merchant diagnostics feed that is written to `validation_bundle/.../s3_crossborder_universe/integerisation_diagnostics.jsonl`. The validation result now carries diagnostics so the Segment 1A orchestrator can surface them, and the bundle publisher accepts/serialises the new payload.
   - Extended the loader to accept optional `score_value` literals and a governed `normalisation` block; added `PolicyNormalisation` and support in `_compute_priors` to rescale deterministic scores (`sum_to_target`/`l1`), while keeping legacy behaviour as default. Runner/kernel plumbing now consumes the broader rule forms.
   - Updated the Segment 1A README highlight and the dataset preview blueprint to document the new toggles/policy options. Added tests that cover the new validation paths (schema failure, diagnostics) plus score-value and normalisation flows for priors.