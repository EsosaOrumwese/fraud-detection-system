...
* 10:07pm:
   - add Polars dependency, reload each outlet shard to enforce writer sort and noise-free hashes (with new budget-aware error codes), emit seed/fingerprint metadata, and ensure composite SHA covers raw bytes.
   - introduce reusable budget descriptors for all RNG families, log schema versions (via cached schema resolution) into the S9 summary, and expose budget metadata inside rng_accounting
   - extend the harness with helpers and negative cases (unsorted bundle, lineage tampering, missing trace coverage, plus unit checks for every RNG budget path) and assert the new schema-version bookkeeping.