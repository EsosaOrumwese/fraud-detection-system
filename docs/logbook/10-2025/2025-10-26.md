   ...
   - What's Blocking
      1. The CLI is still failing immediately after S4. Because the Codex console truncates stdout, we can’t see the stack trace; we only know the process stops right after S4 and before the S5 “complete” log appears. Outside this environment you should capture the full output with something like:
         ...
         PYTHONPATH=packages/engine/src python3 -m engine.cli.segment1a \
          ...
          --param crossborder_hyperparams.yaml=config/layer1/1A/policy/crossborder_hyperparams.yaml \
          --param hurdle_coefficients.yaml=config/layer1/1A/models/hurdle/exports/version=2025-10-09/20251009T120000Z/hurdle_coefficients.yaml \
          --param nb_dispersion_coefficients.yaml=config/layer1/1A/models/hurdle/exports/version=2025-10-24/20251024T234923Z/nb_dispersion_coefficients.yaml \
...
eference/iso/iso_legal_tender/2024/iso_legal_tender.parquet (155 currency tokens) now has a settlement and usage share vector. Steps: auto-expanded config/layer1/1A/policy/legal_tender_2024Q4.yaml from the parquet, then ran scripts/build_settlement_shares.py --version 2025-10-26 --iso-table reference/layer1/iso_canonical/v2025-10-09/iso_canonical.parquet --gdp-table reference/economic/world_bank_gdp_per_capita/2025-10-07/gdp.parquet --legal-tender config/layer1/1A/policy/legal_tender_2024Q4.yaml followed by scripts/build_ccy_country_shares.py --version 2025-10-26 --iso-table reference/layer1/iso_canonical/v2025-10-09/iso_canonical.parquet --settlement-shares reference/network/settlement_shares/2025-10-26/settlement_shares.parquet. Updated the dataset dictionary and S5 loader defaults to reference the new 2025-10-26 artefacts.
...
* 5:18pm:
   - Segment 1A now runs clean end-to-end (seed 2025102501). I just re-executed your exact CLI, captured the run in `runs/segment1a_full.log`, and everything finished through S9 with `_passed.flag` in `runs/local_layer1_regen4/data/layer1/1A/validation/fingerprint=962dda0d3387953d34b2e528019506110cddcda94b4a06c302b4778b2f942f96`. The only remaining warning in the log is the long-standing S3 notice about not republishing a per-state validation bundle, which is expected. S6 no longer throws the “missing rng_event_ztp_final entries” warning—the loader now ingests the `crossborder_eligibility_flags` surface, keeps ineligible merchants as domestic-only (k_target=0), and logs their count at INFO level. S7/S8 happily consume those merchants (S7 metrics show 4,265 merchants in scope; S8 validation passed).
   - Key fixes that made this work:
      - `packages/engine/src/engine/layers/l1/seg_1A/s6_foreign_selection/loader.py`: loads the eligibility flags table, keeps ineligible merchants in the context with `k_target=0`, and only logs a warning when a truly eligible merchant lacks a `ztp_final` row.
      - `contracts/dataset_dictionary/l1/seg_1A/layer1.1A.yaml`: points the `crossborder_eligibility_flags` dataset at the parameter-scoped parquet S0 actually writes.
      - `tests/engine/layers/l1/seg_1A/s6_foreign_selection/test_runner.py`: helper now writes that eligibility file, and there’s coverage to ensure ineligible merchants stay in the S6/S7/S8 pipeline without spurious warnings.
