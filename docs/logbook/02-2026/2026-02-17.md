...
* 9:31PM
  - Extended `Segment 2A` build plan for post-P3 closure pathing.
  - Reworked `P4` from a generic closure block into an explicit branch framework:
    - `P4.A` freeze-now closure,
    - `P4.B` reopen-1B (recommended),
    - `P4.B1` witness protocol + runtime budgets,
    - `P4.B2` integrated `1B -> 2A` recertification,
    - `P4.B3` final decision gate.
  - Preserved non-negotiables in plan:
...
* 9:47PM
  - Started full `P4` execution (`P4.A -> P4.B/B1/B2/B3`) per user direction.
  - Execution design locked:
    - close `P4.A` as baseline freeze snapshot,
    - run `P4.B` upstream reopen with witness-first gate.
  - Upstream reopen scope pinned to `1B` representativeness policies (`S2/S4`) only; no 2A cap/threshold edits.
  - Candidate lane root selected:
...
* 11:18PM
  - Completed full `P4` execution on candidate root:
    - `runs/fix-data-engine/segment_2A_p4b_r1`.
  - `P4.B1` witness (`42,7`) results:
    - witness scorer:
      - `runs/fix-data-engine/segment_2A_p4b_r1/reports/segment2a_p2_certification_42-b65bfe6e_7-07891eca.json`
    - gate: `HOLD_REWORK` (seed `42` improved but seed `7` regressed).
  - `P4.B2` full seed-pack (`42,7,101,202`) results:
    - seed `101` failed 2A `S1` with `2A-S1-091` (`fallback_country_cap_exceeded`, `KZ`),
    - seed `202` failed 2A `S1` with `2A-S1-092` (`override_rate_cap_exceeded`).
  ...
  - Final `P4.B3` disposition:
    - `FREEZE_WITH_BLOCKERS`.
    - retained `2A` authority remains `runs/fix-data-engine/segment_2A_p3`.
  - Restored rejected `1B` candidate policies back to authority content:
    - `config/layer1/1B/policy/policy.s2.tile_weights.yaml`
    - `config/layer1/1B/policy/policy.s4.alloc_plan.yaml`
  - Storage hygiene:
    - pruned superseded partial lane `runs/fix-data-engine/segment_2A_p4b`
    - retained evidence lane `runs/fix-data-engine/segment_2A_p4b_r1`.
  - Updated implementation/build docs:
    - `docs/model_spec/data-engine/implementation_maps/segment_2A.build_plan.md`
    - `docs/model_spec/data-engine/implementation_maps/segment_2A.impl_actual.md`
    - `docs/model_spec/data-engine/implementation_maps/segment_1B.impl_actual.md`
