...
* 2:51PM
  ...
  - Decision implemented:
    - S8 now iterates all candidate merchants (not only `candidate ∩ ztp_final`),
...
* 5:33PM
  ...
  - Transition captured:
    - moving from “works on laptop” documentation stage to migration design stage.
...
* 5:19PM
  ...
  - Structural scope change:
    - merchant scope now uses `candidate_set ∩ nb_final` (instead of `candidate_set ∩ ztp_final`).
...
* 7:37PM
  - Implemented Segment `1A` P1.1 run-loop hardening in `makefile`:
    - added `segment1a-p1` to run `S0 -> S1 -> S2` only,
    - added `segment1a-p1-check` to assert required S1/S2 output surfaces,
    - added alias `engine-seg1a-p1`.
  - Added verifier tool:
    - `tools/verify_segment1a_p1_outputs.py`
    - validates presence of `rng_event_hurdle_bernoulli`, `rng_event_nb_final`, `rng_event_gamma_component`, `rng_event_poisson_component` for a run id.
  - Validation executed:
  ...
  - P1.1 proof run:
    - `run_id=e97b15d23d61dde3ae2c416721f271f2`
    - pre-clean ran first,
    - state-scoped execution completed through S2,
    - `segment1a-p1-check` passed all four required output surfaces.

* 7:46PM
  - Executed Segment `1A` P1.2 closure checks using state-scoped loop `make segment1a-p1` (S0->S2 only).
  - Multi-seed runs completed and passed output-surface checks:
    - seed `42` -> `e97b15d23d61dde3ae2c416721f271f2`
    - seed `43` -> `bc3cd75be277994a13a8d435d249ee4b`
    - seed `44` -> `099bce7800f7e53d88b5aaf368e22c06`
  - P1.2 metric results (from S1/S2 authorities):
    - single-site share: `0.4138`, `0.4166`, `0.4153` (all inside `B` and `B+` bands),
    - branch purity: no `S2` rows for hurdle-false merchants; no missing `nb_final` for hurdle-true merchants across all three runs.
  ...
  - Build-plan status updated:
    - `P1.1` DoD checked complete,
    - `P1.2` DoD checked complete.
  - Decision:
    - no additional hurdle coefficient edits required for P1.2 at this stage;
    - proceed to `P1.3` (NB mean/dispersion count-shape calibration).

* 8:15PM
  - Executed fresh P1.3 baseline remeasure under current sampler patch using state-scoped loop `S0->S2` only:
    - seed `42` -> `80051a088f94efb8ee21ab65cbcbd6ce`
    - seed `43` -> `2d6621e142e1fe8424f1ef8e9c672ff0`
    - seed `44` -> `871e986a5523bcc872d12e9fe0dd5b89`
  - Baseline posture:
    - median outlet count remains on-band (`6.0`),
    - gini remains on-band (`~0.506 to 0.511`),
    - dispersion gates remain on-band (`phi_cv~0.14`, `phi_p95/p05~1.58`),
    - concentration is under-target with top-decile share below B lower bound on all seeds (`~0.294 to 0.299` vs B min `0.35`).
  - Decision:
    - keep hurdle and `beta_phi` fixed,
    - open a controlled `beta_mu` non-intercept uplift iteration to restore concentration into the B corridor without destabilizing other P1.3 metrics.
  - Captured full reasoning trail in:
    - `docs/model_spec/data-engine/implementation_maps/segment_1A.impl_actual.md`

* 8:24PM
  - Completed P1.3 NB-mean/dispersion calibration iterations with staged `beta_mu` spread uplifts (hurdle + `beta_phi` fixed):
    - `20260212T200543Z` (`x1.15`) -> under target concentration.
    - `20260212T200704Z` (`x1.40`) -> still under B top-decile floor.
    - `20260212T200823Z` (`x2.00`) -> selected candidate.
  - Multi-seed validation for selected bundle:
    - seed `42` -> `292e136bd6b58beafe0d81755dac2fb2`
    - seed `43` -> `e22f339fd496dfb2508ea33949907d54`
    - seed `44` -> `411b7f4e5466109e7007043180ed794b`
  - P1.3 score posture (all seeds):
    - median outlets `9.0`,
    - top10 share `~0.382 to 0.385`,
    - gini `~0.600 to 0.602`,
    - `phi_cv ~0.139 to 0.140`,
    - `phi_p95/p05 ~1.575 to 1.585`.
  - Outcome:
    - P1.3 DoD passes at `B` level across seeds,
    - `B+` miss remains only on gini tight band,
    - branch purity unchanged and still clean.
  - Updated artifacts/docs:
    - `runs/fix-data-engine/segment_1A/reports/p1_3_nb_multiseed_scorecard.json`
    - `docs/model_spec/data-engine/implementation_maps/segment_1A.build_plan.md` (P1.3 DoD checkboxes marked complete)
    - `docs/model_spec/data-engine/implementation_maps/segment_1A.impl_actual.md` (full decision trail)

* 8:52PM
  - Executed P1.4 joint reconciliation + lock verification for Segment `1A` using two consecutive state-scoped pass sets (`S0->S2`, no coefficient edits between passes).
  - Consecutive pass run IDs:
    - pass1: `e48afa8eb791c839f36d59c34020ca66`, `326d1abec0aeb9e9643ee541b5eb4334`, `95d192a8ae8cae271da7083108e583ab`
    - pass2: `798476f5603c06f499be7ac76b13150b`, `45fd5a38414b705f64c3f7ee09bdbee4`, `1288cf0b8d4ee8e17a55a814a63260d7`
  - Added scoring/lock helper:
    - `tools/score_segment1a_p1_4_lock.py`
  - Generated lock scorecard:
    - `runs/fix-data-engine/segment_1A/reports/p1_4_lock_scorecard.json`
  - P1.4 outcome:
    - two consecutive runs meet all P1 B metrics: PASS,
    - same-seed replay stability (tol=`1e-12`): PASS (`delta=0.0` on tracked metrics),
    - locked bundle paths + SHA256 recorded from sealed inputs: PASS.
  - Locked bundle recorded:
    - hurdle coeffs: `.../version=2026-02-12/20260212T200823Z/hurdle_coefficients.yaml`
    - dispersion coeffs: `.../version=2026-02-12/20260212T200823Z/nb_dispersion_coefficients.yaml`
  - Updated plan/notes:
    - `docs/model_spec/data-engine/implementation_maps/segment_1A.build_plan.md` (P1.4 DoD checkboxes marked complete)
    - `docs/model_spec/data-engine/implementation_maps/segment_1A.impl_actual.md` (P1.4 closure entry)

* 8:57PM
  - Pruned unnecessary run folders under `runs/fix-data-engine/segment_1A` in preparation for `P2`.
  - Keep policy used:
    - retain only run IDs referenced by active scorecards:
      - `p1_2_hurdle_multiseed_scorecard.json`
      - `p1_3_nb_multiseed_scorecard.json`
      - `p1_4_lock_scorecard.json`
    - remove all other run-id folders in that runs root.
  - Cleanup result:
    - run folders reduced from `26` to `12` (deleted `14`).
    - runs-root size reduced from `~0.934 GB` to `~0.344 GB` (reclaimed `~0.590 GB`).

* 9:02PM
  - Per user clarification, performed an additional run-id folder-only trim to keep just P1.3 + P1.4 evidence runs (drop P1.2-only run IDs).
  - Additional cleanup result:
    - run folders reduced from `12` to `9` (deleted `3`).
    - runs-root size reduced from `~0.344 GB` to `~0.258 GB` (additional `~0.086 GB` reclaimed).

* 9:06PM
  - Executed emergency storage trim for `runs/fix-data-engine/segment_1A` per user directive.
  - Removed all remaining run-id folders (`9` deleted) and kept only report artifacts under `runs/fix-data-engine/segment_1A/reports/`.
  - Post-trim status:
    - run-id folders: `0`
    - runs-root size: approximately `0 GB` (reports-only footprint).

* 9:07PM
  - Opened docs scaffold task for migration planning.
  - Target:
    - `docs/model_spec/platform/migration_to_dev/`
  - Planned creation:
    - `dev_min_spine_green_v0_run_process_flow.md` (empty scaffold)
    - `dev_min_handles.registry.v0.md` (empty scaffold)
    - `README.md` describing file intent and explicit mapping to `docs/design/platform/local-parity` flow.

* 9:11PM
  - Completed migration-to-dev scaffold creation under:
    - `docs/model_spec/platform/migration_to_dev/`
  - Created files:
    - `dev_min_spine_green_v0_run_process_flow.md` (empty)
    - `dev_min_handles.registry.v0.md` (empty)
    - `README.md` (index + mapping statement)
  - Verification:
    - both scaffold files are zero-length,
    - README is present and non-empty,
    - README maps these docs to canonical local-parity flow docs and phase-id posture.

* 9:12PM
  - Recorded explicit phase-boundary assumption for Segment `1A` remediation:
    - `P1` is frozen as statistically realistic baseline while executing `P2+`.
  - Build-plan updates:
    - added `P1 freeze contract` under Section `3` with locked surfaces and reopen conditions.
    - added `P2` precondition binding `P2` to the freeze contract.
  - Implementation notes updated with the same decision and constraints in:
    - `docs/model_spec/data-engine/implementation_maps/segment_1A.impl_actual.md`

* 9:18PM
  - Expanded Segment `1A` `P2` build-plan sections with the agreed execution posture:
    - strict P2 run mode (`S0->S6`), frozen `P1` enforcement,
    - authoritative P2 scoring datasets,
    - explicit metric formulas (`C_m`, `R_m`, `rho_m`, Spearman coupling),
    - hard pathology caps and stratified checks,
    - phased `P2.1 -> P2.4` DoD blocks,
    - constrained calibration method (band-distance + pathology penalty),
    - storage-retention/pruning policy for iterative P2 loops.
  - Added matching implementation-notes entry capturing the decision rationale and planning choices in:
    - `docs/model_spec/data-engine/implementation_maps/segment_1A.impl_actual.md`

* 10:14PM
  - Applied migration-doc drift closure per user directive under:
  ...
  - Enforced hard constraint for `P3`:
    - local seeding/bootstrap from laptop/MinIO/local FS is forbidden,
    - seed/sync must source from managed object-store locations only.
  - Normalized runbook handle references to registry naming:
    - `S3_ORACLE_RUN_PREFIX_PATTERN`, `S3_STREAM_VIEW_MANIFEST_KEY_PATTERN`, `S3_STREAM_SORT_RECEIPT_KEY_PATTERN`,
    - `S3_QUARANTINE_RUN_PREFIX_PATTERN`, `S3_EVIDENCE_RUN_ROOT_PATTERN`, `S3_ARCHIVE_RUN_PREFIX_PATTERN`,
    - networking refs updated to `SUBNET_IDS_PUBLIC` + `SECURITY_GROUP_ID_APP` where applicable.
  - Added/updated IG handles in registry:
    - `IG_LISTEN_ADDR`, `IG_PORT`, `IG_INGEST_PATH=/v1/ingest/push`, `IG_HEALTHCHECK_PATH=/v1/ops/health`,
    - `IG_AUTH_MODE=api_key`, `IG_AUTH_HEADER_NAME=X-IG-Api-Key`, `SSM_IG_API_KEY_PATH`.
  - Updated runbook IG checks:
    - auth smoke checks now explicitly hit `IG_INGEST_PATH`.
  - Clarified phase identity:
    - `P(-1)` explicitly marked preflight-only (not part of Spine Green closure acceptance).

* 10:16PM
  - Opened `P2.1` implementation planning for Segment `1A` (engine remediation track).
  - Logged pre-implementation decision trail in:
    - `docs/model_spec/data-engine/implementation_maps/segment_1A.impl_actual.md`
  - Planned `P2.1` mechanics:
    - add repeatable `S0->S6` make runner (`segment1a-p2`) with run-id handoff,
    - add run-surface verifier for P2 (`tools/verify_segment1a_p2_outputs.py`),
    - add baseline scorer (`tools/score_segment1a_p2_1_baseline.py`) with global + stratified metrics and pathology caps.
  - Explicit policy/contract handling pinned before coding:
    - `rng_event_ztp_retry_exhausted` treated as optional-presence diagnostic,
    - `s3_integerised_counts` required only when sealed `policy.s3.integerisation.yaml` has `emit_integerised_counts=true` (Variant-B owner posture otherwise reports intentional absence).

* 10:22PM
  - Implemented `P2.1` run/scoring harness for Segment `1A`.
  - Added make targets:
    - `segment1a-p2` (state chain `S0->S6`),
    - `segment1a-p2-check`,
    - `engine-seg1a-p2`.
  - Added tools:
    - `tools/verify_segment1a_p2_outputs.py` (policy-aware surface verifier),
    - `tools/score_segment1a_p2_1_baseline.py` (global + stratified baseline scorecard).
  - Executed:
  ...
  - Key global metrics:
    - `median(C_m)=37.0` (fails B band),
    - `Spearman(C_m,R_m)=0.2977` (slightly below threshold),
    - `median(rho_m)=0.0` (fails threshold),
    - pathology rails pass (`share_exhausted=0`, `share_high_reject_gt16=0`).
  - Build plan updated:
    - marked all `P2.1` DoD checkboxes complete in
      `docs/model_spec/data-engine/implementation_maps/segment_1A.build_plan.md`.
  - Storage cleanup applied:
    - removed obsolete run folder `54602dac43ab522a7100b47c66ea824b`,
    - retained active baseline run + report artifacts.

* 10:23PM
  - Applied a build-plan consistency patch for `P2.1` surface semantics in
    `docs/model_spec/data-engine/implementation_maps/segment_1A.build_plan.md`:
    - `s3_integerised_counts` explicitly policy-gated by sealed `emit_integerised_counts`,
    - `rng_event_ztp_retry_exhausted` explicitly optional-presence diagnostic.

* 10:24PM
  - Opened `P2.3` diagnostic pass before edits.
  - Baseline causal readout (run `203d3c33f5fbe060184ad845a86b9e6c`):
    - `S4` target generation is active (`ztp_final=4316`, `K_target` median `3`),
    - `S6` realization loss is dominant (`K_target_sum=15334`, `K_realized_sum=6486`),
    - empties dominated by `ZERO_WEIGHT_DOMAIN=2189` with `CAPPED_BY_MAX_CANDIDATES=1885`.
  - Pinned first `P2.3` hypothesis:
    - relax `S6` `max_candidates_cap` first (policy-only) to test cap-induced zero domains before any S4 theta adjustments.

* 10:39PM
  - Executed `P2.3` trial-1 (`S6` cap relaxation) and re-ran `S0->S6`:
    - policy patch: `config/layer1/1A/policy.s6.selection.yaml`
  ...
  - Result summary:
    - `Spearman(C_m,R_m)` improved to `0.3129` (now above threshold),
    - `median(C_m)` unchanged at `37`,
    - `median(rho_m)` unchanged at `0.0`,
    - pathology rails remain pass.
  - S6 diagnostics:
    - `CAPPED_BY_MAX_CANDIDATES` collapsed `1885 -> 0`,
    - `ZERO_WEIGHT_DOMAIN` remained dominant (`2139`), so realization remains structurally constrained.
  - Decision:
    - keep cap-relaxation patch,
    - note that `P2.3` cannot close `rho` alone while upstream candidate-domain shape remains unresolved (dependency on `P2.2`).
  - Storage hygiene:
    - removed superseded run folder `203d3c33f5fbe060184ad845a86b9e6c`,
    - kept active run `09e24520babd99de27b05d5c1b11ea7c` + reports.

* 10:41PM
  - Started ordered remediation per user directive: `P2.2` before further `P2.3`.
  - Recorded P2.2 policy-only plan in implementation notes:
    - replace binary near-global S3 allow posture with size-tiered candidate sets,
    - keep deny rails unchanged,
    - enforce mutually constrained `N_GE` predicate ranges to avoid union blow-up.

* 10:58PM
  - Completed first `P2.2` S3 tiered-candidate patch run:
    - run id: `726b32b705420828b6c5ca0d054117ab`
    - scorecard: `runs/fix-data-engine/segment_1A/reports/segment1a_p2_1_baseline_726b32b705420828b6c5ca0d054117ab.json`
  - Key movement:
    - `median(C_m)` contracted to `6.0` (target-band hit),
    - `Spearman(C_m,R_m)` improved to `0.4007`,
    - `median(rho_m)` remained `0.0`.
  - Transitioned to `P2.3` next:
    - planned S4 ZTP theta retune (`crossborder_hyperparams.yaml`) to lift realization intensity while keeping pathology rails green.

* 10:39PM
  - Closed remaining migration-doc ambiguity rails under:
    - `docs/model_spec/platform/migration_to_dev/dev_min_spine_green_v0_run_process_flow.md`
    - `docs/model_spec/platform/migration_to_dev/dev_min_handles.registry.v0.md`
  - Wildcard handle references removed from runbook phase requirement blocks:
    - replaced `SSM_CONFLUENT_*`, `RDS_*`, `SVC_*`, `FP_BUS_*` family refs with explicit keys.
  - Added referenced-but-undefined keys to handles registry:
    - run pinning (`CONFIG_DIGEST_*`, scenario derivation),
    - WSP/READY knobs,
    - RTDL catch-up knobs,
    - reporter lock keys,
    - idempotency/subject-key keys,
    - evidence path patterns for ingest/rtdl/decision/case/obs closure outputs.
  - P3 seed contract tightened:
    - registry now has `ORACLE_SEED_SOURCE_*` handles,
    - runbook now states first-time local parity upload (if any) is operator pre-step outside `dev-min-run`; P3 seed tasks are S3-only.
  - DB backend pinned for v0:
    - `DB_BACKEND_MODE = rds_instance`, active handles are `RDS_INSTANCE_ID` + `RDS_ENDPOINT`, Aurora explicitly out-of-scope.
  - Naming and de-dup cleanup:
    - P12 tag keys aligned to `TAG_*_KEY`,
    - observability section now references logging handles defined in ECS section (no duplicate value declarations).
  - Verification:
    - wildcard family grep checks clean,
    - key-presence checks for newly added handles pass,
    - runbook handle-like tokens resolve to registry keys except non-handle phase labels/outcome enum names.

* 10:49PM
  - Added migration-bridge closure note to platform implementation map:
    - `docs/model_spec/platform/implementation_maps/dev_substrate/platform.impl_actual.md`
  - Note records that the local-parity -> dev bridge is now sealed by:
    - `docs/model_spec/platform/migration_to_dev/dev_min_spine_green_v0_run_process_flow.md`
    - `docs/model_spec/platform/migration_to_dev/dev_min_handles.registry.v0.md`
  - Captured why these docs prevent careless migration drift:
    - phase/gate/proof/rollback translation clarity,
    - single-source handle authority,
    - pinned writer-boundary + evidence + teardown rails,
    - prevention of hidden laptop coupling and false-green closure.
  - Captured explicit counterfactual risks if migration proceeded without this bridge contract.

* 11:05PM
  - Executed dev_substrate fresh-start cleanse per user directive.
  - Purged legacy implementation surfaces to prevent adaptation around faulty migration paths:
    - removed `scripts/dev_substrate/*`,
    - removed `infra/terraform/envs/dev_min/*` and local terraform residue dirs,
    - removed `config/platform/dev_substrate/*`, `config/platform/profiles/dev_min.yaml`, `config/platform/sr/wiring_dev_min.yaml`,
    - removed legacy component build plans/impl docs under `docs/model_spec/platform/implementation_maps/dev_substrate/`.
  - Re-baselined implementation maps folder:
    - rewrote `docs/model_spec/platform/implementation_maps/dev_substrate/README.md` with explicit fresh-start authority stance,
    - created new `docs/model_spec/platform/implementation_maps/dev_substrate/platform.build_plan.md` baseline,
    - appended emphatic authoritative reset entry in `docs/model_spec/platform/implementation_maps/dev_substrate/platform.impl_actual.md`.
  - Reset authority pinned:
    - `docs/model_spec/platform/migration_to_dev/dev_min_spine_green_v0_run_process_flow.md`
    - `docs/model_spec/platform/migration_to_dev/dev_min_handles.registry.v0.md`

* 11:08PM
  - Removed legacy migration make command surface from `makefile`:
    - deleted `DEV_MIN_*` defaults block tied to removed dev_substrate tooling,
    - deleted all `platform-dev-min-phase*` targets that referenced removed scripts/configs.
  - Verification after patch:
    - no executable legacy dev-min make targets remain in `makefile`.
    - legacy mentions persist only as historical text in docs (implementation history/challenge archive), not runnable command targets.

* 11:15PM
  - Opened P2.2 bridge diagnostics on latest run `50e974a99ee544735a6a09942923c447` before any new edits.
  - Confirmed dominant blocker remains `S6` zero-support empties (`ZERO_WEIGHT_DOMAIN=2223`).
  - Quantified zero-domain concentration by S3 rule buckets:
    - `ALLOW_GLOBAL_LARGE=1146`, `ALLOW_REGIONAL_MID=955`, `ALLOW_LOCAL_SMALL_HUB=114`.
  - Identified smallest high-impact missing support-country bridge groups from S5 support overlap:
    - alpine/nordic (`LI/BV/SJ/FO/GL`),
    - USD-support territories (`PR/PA/EC/SV/HT`),
    - commonwealth/oceania (`GG/IM/JE/CX/CK`).
  - Logged plan in implementation notes to run one constrained S3 bridge patch pass, then re-score and decide whether P2 scope must explicitly include S5 support broadening.

* 11:22PM
  - Executed constrained P2.2 bridge patch on `config/layer1/1A/policy/s3.rule_ladder.yaml` (bridge-set additions only; predicates unchanged).
  - Ran fresh `S0->S6` loop:
    - run id: `fd7a0b9d3b9b2b8b7fa9bcd2e9d355cd`
    - scorecard: `runs/fix-data-engine/segment_1A/reports/segment1a_p2_1_baseline_fd7a0b9d3b9b2b8b7fa9bcd2e9d355cd.json`
  - Key movement:
    - `median(C_m): 12.0` (still within B band),
    - `Spearman(C_m,R_m): 0.4805` (improved),
    - `median(rho_m): 0.0` (still failed).
  - S6 diagnostics moved materially:
    - `selected: 1670 -> 2424`,
    - `ZERO_WEIGHT_DOMAIN: 2223 -> 1436`,
    - `NO_CANDIDATES: 453 -> 452` (flat).
  - Post-run ceiling check recorded in implementation notes:
    - remaining blocker is S5 support sparsity (many merchants still foreign-impossible under current currency support),
    - next step recommendation is explicit S5 support broadening scope if we want realistic closure on realization ratio.

* 11:30PM
  - Opened P2 scope-extension step to S5 after confirming residual `rho` blocker is structural support sparsity, not S3-only posture.
  - Locked next implementation direction in notes:
    - minimal S5 runner extension to honor override ISO keys in support union,
    - policy-only foreign spillover via `ccy_smoothing_params.yaml` `min_share_iso` for high-impact singleton/home-only currencies,
    - no change to S6 selection law or PASS gates.

* 11:31PM
  - Reviewed run `90545f47fb3a3993080b6483eab1bdd8` and confirmed remaining P2 blocker moved from support overlap to eligibility gate volume.
  - Next test locked in notes: policy-only eligibility expansion by setting `crossborder_hyperparams.yaml` `default_decision=allow` while retaining explicit deny rules.

* 11:33PM
  - Reviewed eligibility-expansion run `124ff492f2bb4faa0bebfc5132d59cd6`.
  - Observed expected gate lift (`S6 gated_in=5734`) but large candidate-empty spill (`NO_CANDIDATES=666`).
  - Logged next step in implementation notes: add constrained S3 fallback allow rule (low priority) to cut no-candidate empties while preserving deny precedence.

* 11:35PM
  - Profiled run `aac21c227858f925c5dcdddb42902e96` tail blockers.
  - We are close to global `rho` movement threshold (`selected=4922`), with residual empties entirely `ZERO_WEIGHT_DOMAIN` (`814`) and `NO_CANDIDATES=0`.
  - Logged targeted next patch in implementation notes: add tail-currency support overrides (`XCD/XCG/MOP/XPF/...`) in S5 policy.

* 11:38PM
  - Captured run `0c6ab3ebd659c7176d65dceef3f39109` as first non-zero global realization-ratio movement (`median_rho_m=0.0435`).
  - Locked next action in implementation notes: tighten S3 bridge set cardinality to improve `R/C` ratio without reintroducing empty domains.

* 11:47PM
  - Continued Segment `1A` `P2` diagnostics from latest run `c83b1c5110b3d1c2803b7f01de959d5d` before new edits.
  - Confirmed global blocker is denominator-heavy ratio posture, not S4 target scarcity:
    - scorecard: `median(C_m)=10.0`, `Spearman=0.7846`, `median(rho_m)=0.0417`.
    - S6 receipt: `K_target_sum=119680` vs `K_realized_sum=18135`, `shortfall_merchants=4078`.
  - Isolated dominant low-rho branch from S3 candidate policy:
    - `ALLOW_GLOBAL_LARGE` merchants (`n=2676`) have `median(C)=25`, `median(rho)=0.0833`, `share(rho>=0.1)=0.417`.
  - Logged pre-edit design decision in implementation notes:
    - apply S3-only candidate-breadth compression pass (tier threshold + admit-set cardinality tightening),
    - keep deny rails/fallback/eligibility unchanged,
    - then rerun `S0->S6` and rescore.

* 11:52PM
  - Captured P2 continuation diagnostics from run `ecfbd48ee04ccf5d965556bbf8c9266c`.
  - S3 compression pass improved global realization ratio but did not close B threshold:
    - `median(rho_m): 0.0417 -> 0.0625`, with `rho>=0.1` merchants at `4290/10000`.
  - Confirmed residual blocker is support-density in specific currency cohorts, not S4 target generation.
  - Locked next action in implementation notes:
    - targeted `ccy_smoothing_params.yaml` `min_share_iso` expansion for high-impact currencies (`AUD/NZD/MOP/GBP/CHF/USD/QAR/HKD/KRW`),
    - no S3/S4/S6 logic change in this pass,
    - rerun `S0->S6` and rescore immediately.

* 11:58PM
  - Executed targeted S5 support-density pass for high-impact sparse currencies in
    `config/layer1/1A/allocation/ccy_smoothing_params.yaml`:
    - added/expanded `min_share_iso` for `USD, GBP, AUD, NZD, CHF, MOP, QAR, HKD, KRW`.
  - Fresh run `9901b537de3a5a146f79365931bd514c` closed the global P2 hard gate:
    - `median(C_m)=8.0`, `Spearman=0.7891`, `median(rho_m)=0.1176` (pass), pathology rails pass.
    - S6 movement: `selected=5415`, `ZERO_WEIGHT_DOMAIN=271`.
  - Ran a same-config replay `d6e04d5dc57b9dc3f41ac59508cafd3f` for P2.4 stability.
    - Key global P2 metrics and S6 summary are identical to the prior run.
  - Recorded full decision + evidence trail in
    `docs/model_spec/data-engine/implementation_maps/segment_1A.impl_actual.md`.

* 11:59PM
  - Pruned superseded run-id folders under `runs/fix-data-engine/segment_1A` to control storage growth.
  - Retained only:
    - `c83b1c5110b3d1c2803b7f01de959d5d` (pre-closure anchor),
    - `9901b537de3a5a146f79365931bd514c` (closure run),
    - `d6e04d5dc57b9dc3f41ac59508cafd3f` (stability replay run).
