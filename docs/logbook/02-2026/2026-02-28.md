...
## 2026-02-28 18:05:41 +00:00 - Compact post-run realism snapshot generated for fresh run `a3bd...`
- User requested compact PASS/B/B+ summary tied to fresh full-chain run-id `a3bd8cac9a4284cd36072c6b9624a0c1` under `runs/local_full_run-7`.
- Executed segment scorers into:
  - `runs/local_full_run-7/reports/postrun_snapshot_a3bd8cac9a4284cd36072c6b9624a0c1`
- Scorers executed:
  - `tools/score_segment1a_p3_1_baseline.py`
  - `tools/score_segment1b_p0_baseline.py`
  - `tools/score_segment2a_p2_certification.py` (compact required seed set `{42}`)
  - `tools/score_segment2b_p3_candidate.py` (same run used as lock/candidate for compact lane)
  - `tools/score_segment3a_p0_baseline.py`
  - `tools/score_segment3b_p0_baseline.py` (compact seed map `{42}`)
  - `tools/score_segment5a_p0_realism.py` (phase `P5`, compact seed set `{42}`)
...
- Authority-dependent scorer attempts encountered missing local authority files for legacy 1A/1B freeze-integrated scorers; compact direct-run alternatives used and noted in snapshot.
- Consolidated outputs emitted:
  - `runs/local_full_run-7/reports/postrun_snapshot_a3bd8cac9a4284cd36072c6b9624a0c1/postrun_realism_snapshot_a3bd8cac9a4284cd36072c6b9624a0c1.json`
  - `runs/local_full_run-7/reports/postrun_snapshot_a3bd8cac9a4284cd36072c6b9624a0c1/postrun_realism_snapshot_a3bd8cac9a4284cd36072c6b9624a0c1.md`

## 2026-02-28 19:39:41 +00:00 - Dev-full oracle onboarding standard repin (planning-only)
- Trigger:
  - USER requested repin + plan update before execution so oracle onboarding is explicitly:
    1) upload raw to S3,
    2) run managed distributed sort,
    3) verify parity,
    4) then proceed to certification.
- Decision:
  - active dev_full oracle standard is now `raw -> managed sort -> parity` with no local sort runtime path.
- Files updated:
...
- Key repins:
  ...
  - managed sort contract handles/evidence paths pinned (`ORACLE_STREAM_SORT_*`, raw-upload/sort/parity receipts).
- Run-process gate update:
  - P3 now requires raw upload receipt + managed sort receipt before required-output/materialization checks.
  - added blocker `DFULL-RUN-B3.2` for upload/sort receipt/parity failures.
- Planning update:
  - added `M5.R1..M5.R4` refresh lane in M5 orchestration plan.
  - added `P3.A1` deep lane in M5.P3 plan.
  - legacy dev-min copy-remediation retained as history only, explicitly not active standard.
- Execution boundary for this entry:
  - documentation/planning only; no runtime actions, no upload/sort execution, no certification rerun.
