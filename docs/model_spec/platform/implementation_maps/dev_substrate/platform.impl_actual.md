...
## Entry: 2026-02-12 12:30PM - Corrective pre-change note for main-doc depth expansion (RTDL + Case/Labels machinery)

### Context
USER rejected high-level treatment in main narrative for RTDL and Case/Labels and requested parity with Control/Ingress process/machinery detail.

### Corrective note
This entry is added as corrective planning capture because the doc edit was executed immediately after runtime pack verification. The intent and scope were:
1. keep main narrative canonical,
2. expand sections 8/9/10 from conceptual component bullets to explicit runtime machinery,
3. pin process ids, entrypoints, key state/checkpoint stores, and commit evidence paths per component,
4. avoid changing phase order or semantic scope.

### Planned/implemented target
`docs/design/platform/local-parity/spine_green_v0_run_process_flow.txt` sections:
- `8) RTDL core plane`
- `9) RTDL decision lane`
- `10) Case + Labels plane`

### Drift sentinel checkpoint
Docs-only depth expansion; no runtime semantics changed.

## Entry: 2026-02-12 12:30PM - Applied closure: main-doc machinery detail parity across planes

### Trigger
USER flagged unacceptable imbalance: Control/Ingress was machinery-explicit while RTDL and Case/Labels were too high-level in main narrative.

### Changes applied
Expanded `docs/design/platform/local-parity/spine_green_v0_run_process_flow.txt` so sections 8, 9, 10 now include explicit runtime machinery blocks.

1. Section 8 (RTDL core) now includes per-process runtime mapping:
- `archive_writer_worker`, `ieg_projector`, `ofp_projector`, `csfb_intake`
- each with entrypoint module, run-pin/store context, and commit evidence path.

2. Section 9 (RTDL decision lane) now includes per-process runtime mapping:
- `dl_worker`, `df_worker`, `al_worker`, `dla_worker`
- explicit state/checkpoint stores and commit evidence paths, plus closure expectation on health artifacts.

3. Section 10 (Case + Labels) now includes per-process runtime mapping:
- `case_trigger_worker`, `case_mgmt_worker`, `label_store_worker`
- explicit run pins/state stores and durable commit evidence paths,
- LS authority rule retained and made explicit at runtime boundary.

### Why this closes the concern
Main narrative now treats all in-scope planes as operational machinery, not abstract services. This reduces migration blind spots around:
- checkpoint ownership,
- replay/startup behavior,
- commit evidence expectations,
- per-worker operational failure domains.

### Validation
- Verified referenced commit-evidence paths exist on active run baseline (where applicable).
- Readability check confirms process ids and entrypoints are explicit in main doc sections 8-10.

### Drift sentinel assessment
Docs-only hardening; no runtime behavior changes.
