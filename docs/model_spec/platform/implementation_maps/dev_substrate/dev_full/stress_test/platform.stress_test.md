...
| M8 | Spine Obs/Gov (`P11`) | Stress observability/governance paths so evidence remains complete under high event rates | Evidence completeness + low-overhead telemetry proven | IN_PROGRESS (`S4_GREEN`) |
...
3. Dedicated phase files:
   ...
   - `stress_test/platform.M8.stress_test.md` (`S4_GREEN`).
4. Next step: execute `M8-ST-S5` fail-closed (closure sync + attributable cost-outcome receipt + final gate), with targeted rerun on first blocker.
