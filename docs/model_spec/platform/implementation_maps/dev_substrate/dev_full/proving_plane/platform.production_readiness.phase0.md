...
That mattered because the table is not supposed to be a large receipt archive. It is supposed to be the hot admission / dedupe ledger. The old posture was paying production-write cost for receipt detail that was not needed in the hot row.

The obvious alternative was tested first: switch Lambda receipt persistence to `object_store` mode and keep DynamoDB small. That diagnostic was run as `phase0_20260310T164435Z`.
...
### Further Phase 0.B planning corrections before closure
...
That forced one more `Phase 0.B` planning correction:
...
A cleaner steady-state baseline was then tested with:
...
### Current accepted Phase 0 closeout verdict
`Phase 0.D` is green.

The decisive bounded envelope rerun was `phase0_20260310T223332Z`, generated by `phase0-envelope-common-rateplan-bypass-replay-burst2s-r61-lanes50-igpush1`, with the following accepted production-shaped posture:

- `lane_count = 50`
- `ig_push_concurrency = 1`
- `target_burst_seconds = 0.25`
- replay-delay bypass enabled for scheduled rate-plan replay
- WSP runtime pinned to `fraud-platform-dev-full-wsp-ephemeral:61`
- exact APIGW access-log scoring on the declared steady / burst / recovery windows
...
- dispatcher blockers `= 0`

This closes the last open `Phase 0.C` blocker without changing the declared production envelope and without relying on any diagnostic-only synchronized or unstable proof shape.

`Phase 0` verdict:

- `Phase 0.A` green: active AWS boundary and telemetry set pinned explicitly enough to avoid blind bounded runs
- `Phase 0.B` green: truthful steady-state correctness proven on the exact APIGW access-log boundary, not on stale minute-bin approximations
- `Phase 0.C` green: declared `3000 steady / 6000 burst` envelope and bounded recovery proven on AWS-real telemetry
- `Phase 0.D` green: evidence is explainable, attributable, auditable, and sufficient to reaffirm the working-platform base

Working-platform judgment:

- `Control + Ingress` remains the confirmed working-platform base under the proving method now in force
- later red posture in `Phase 1` and beyond may now be attributed to the newly introduced plane or coupled path, not to unresolved ingress ambiguity

Immediate follow-through required:

- refresh the three readiness graphs so they reflect the green `Phase 0` verdict rather than the earlier open burst blocker
- carry the updated authority state into `Phase 1`
...
- all four closure conditions are satisfied
- `Phase 0` is closed green
