...
**Traffic policy (v0):** SR publishes READY for **one behavioural stream per run** (baseline **or** fraud). `arrival_events_5B` and flow anchors are **context** surfaces (not traffic) and are streamed separately by WSP.
