...
- S5 (RNG): site-to-tile assignment
...
- Evidence: `s0_gate_receipt_1B` (manifest_fingerprint-scoped JSON)
...
- Partition families:
  ...
  - manifest_fingerprint-only: `[manifest_fingerprint]` (e.g., `s0_gate_receipt_1B`, `sealed_inputs_1B`, validation bundles)
...
## 8) Review flags (status)

1) **Token naming standardization** (resolved in current specs)
- All tokens use `manifest_fingerprint`; keep as a regression check.

2) **Mirror 1A validation path everywhere** (resolved in current specs)
- All references use `.../validation/manifest_fingerprint={manifest_fingerprint}/`.

3) **Gate-in receipt enforcement** (resolved in current specs)
- States that read 1A egress require `s0_gate_receipt_1B` (FAIL_CLOSED if missing).
