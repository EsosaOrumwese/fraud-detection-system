...
This section fixes the **logical shapes**, **schema anchors**, and **catalogue wiring** for the three datasets produced by 6B.S3:

* `s3_campaign_catalogue_6B`
* `s3_flow_anchor_with_fraud_6B`
* `s3_event_stream_with_fraud_6B`

JSON-Schema packs remain the **single source of truth** for shapes. The Layer-3 dataset dictionary and artefact registry **MUST** match those schemas; where they diverge, schemas win.

---

### 5.1 Schema anchors in `schemas.6B.yaml`

The 6B segment schema pack **MUST** define the following anchors for S3:
...
* Flow anchor with overlay:

```text
schemas.6B.yaml#/s3/flow_anchor_with_fraud_6B
```

* Event stream with overlay:
...
### 5.2 `s3_campaign_catalogue_6B` shape & catalogue links

#### 5.2.1 Row model (schema expectations)

`schemas.6B.yaml#/s3/campaign_catalogue_6B` **MUST** define a table-shaped schema with one row per realised campaign instance.
...
* Identity / axes:
  ...
  * `seed: integer|string`
  * `campaign_id: string|integer` (unique per `(seed, manifest_fingerprint)`)

* Campaign identity:

  * `campaign_type: string`

    * Enum of campaign types S3 supports (e.g. `CARD_TESTING`, `ATO`, `REFUND_ABUSE`, `COLLUSION`, etc.).
  * `template_id: string`

    * Identifier of the source template in the campaign config pack.

* Scope:

  * `scenario_scope: string|array`

    * Either a single `scenario_id` or description of scenario set (depending on design).
  * `start_ts_utc: string` (ISO-8601)
  * `end_ts_utc: string` (ISO-8601)

* Targeting summary:

  * `target_mode: string`

    * E.g. `ENTITY_CENTRIC`, `FLOW_CENTRIC`, `SESSION_CENTRIC`.
  * `target_segment_descriptor: object|string`

    * Compact description of filter (segment name, conditions).

* Realised intensity (minimally):

  * `target_entity_count: integer` (how many parties/accounts/devices/merchants targeted).
  * `target_flow_count: integer` (how many flows touched).
  * `target_event_count: integer` (how many events touched).

Optional fields MAY include parameter snapshots, tuning knobs, or additional metrics; they MUST be defined as optional so older consumers can ignore them.
...
- id: s3_campaign_catalogue_6B
  ...
  owner_segment: 6B
  description: >
    Realised fraud/abuse campaign instances for a world/run; one row per
    campaign instance with type, parameters, scope, and intensity statistics.
  version: '{seed}.{manifest_fingerprint}'
  format: parquet
  path: data/layer3/6B/s3_campaign_catalogue_6B/seed={seed}/fingerprint={manifest_fingerprint}/s3_campaign_catalogue_6B.parquet
  partitioning: [seed, fingerprint]
  primary_key: [seed, manifest_fingerprint, campaign_id]
  ordering: [seed, manifest_fingerprint, campaign_id]
  schema_ref: schemas.6B.yaml#/s3/campaign_catalogue_6B
  produced_by: [6B.S3]
  consumed_by: [6B.S4, 6B.S5]
```

#### 5.2.3 Artefact registry entry

`artefact_registry_6B.yaml` **MUST** register the dataset consistently, e.g.:
...
- manifest_key: s3_campaign_catalogue_6B
  ...
  path_template: data/layer3/6B/s3_campaign_catalogue_6B/seed={seed}/fingerprint={manifest_fingerprint}/s3_campaign_catalogue_6B.parquet
  partitioning: [seed, fingerprint]
  final_in_layer: false
```
...
### 5.3 `s3_flow_anchor_with_fraud_6B` shape & catalogue links

#### 5.3.1 Relationship to baseline flow anchor

`schemas.6B.yaml#/s3/flow_anchor_with_fraud_6B` MUST be defined as an **extension** of the S2 flow anchor schema:

* It MUST include all required fields from `schemas.6B.yaml#/s2/flow_anchor_baseline_6B` (either via `$ref` or inlined) so that each row can be interpreted as a flow-level view (identity, entity context, baseline timestamps/amounts/outcomes).
* It MUST add overlay fields described below, without changing types or semantics of baseline fields.

#### 5.3.2 Additional overlay fields

At minimum, S3’s flow anchor schema MUST add:

* Identity / provenance:

  * `origin_flow_id: string|integer|null`

    * For baseline-derived flows: MUST equal the original `flow_id` from S2.
    * For pure-fraud flows: MUST be null or set according to overlay semantics.
  * `origin_type: string`

    * Enum: e.g. `BASELINE_FLOW`, `PURE_FRAUD_FLOW`, `BASELINE_FLOW_MUTATED`.

* Campaign tagging:

  * `campaign_id: string|integer|null`

    * FK into `s3_campaign_catalogue_6B` for this `(seed, fingerprint)`; null if not attached to any campaign.
  * `fraud_pattern_type: string`

    * Enum of fraud/abuse pattern types (e.g. `CARD_TESTING`, `ATO`, `REFUND_ABUSE`, `COLLUSION`, `NONE`).

* Overlay flags & metrics (examples; exact set defined in schema):

  * `amount_modified_flag: boolean`
  * `routing_anomalous_flag: boolean`
  * `extra_auths_flag: boolean`
  * `overlay_event_count: integer` (how many events differ from baseline)

Additional fields for diagnostics (e.g. “score” or “intensity index”) MAY be added as optional fields.

#### 5.3.3 Dictionary entry
...
- id: s3_flow_anchor_with_fraud_6B
  ...
  owner_segment: 6B
  description: >
    Flow-level view after fraud/abuse overlay; one row per flow with baseline
    anchor fields plus campaign/fraud overlay metadata.
  version: '{seed}.{manifest_fingerprint}.{scenario_id}'
  format: parquet
...
#### 5.3.4 Artefact registry entry

`artefact_registry_6B.yaml` **MUST** have a matching entry:
...
### 5.4 `s3_event_stream_with_fraud_6B` shape & catalogue links

#### 5.4.1 Relationship to baseline event stream

`schemas.6B.yaml#/s3/event_stream_with_fraud_6B` MUST be defined as an **extension** of S2’s baseline event stream schema:

* It MUST include all required fields from `schemas.6B.yaml#/s2/event_stream_baseline_6B` (via `$ref` or inlined), so each row still has identity, event type/timestamp, and baseline entity/routing context.
* It MUST add overlay metadata fields without altering baseline field semantics.

#### 5.4.2 Additional overlay fields

Required new fields include:

* Provenance:

  * `origin_flow_id: string|integer|null`

    * For mutated baseline events: flow_id from S2.
    * For newly created events (pure fraud), may be null or a related flow id as per policy.
  * `origin_event_seq: integer|null`

    * For mutated baseline events: event_seq from S2.
    * Null for purely new events.

* Campaign & pattern tagging:

  * `campaign_id: string|integer|null`
  * `fraud_pattern_type: string` (same enum family as flow-level).
  * `is_fraud_event: boolean`

    * Indicates whether this event is considered part of the fraudulent/abusive behaviour (as opposed to a purely legit event in a campaign-touched flow).

* Overlay flags (per event):

  * `amount_modified_flag: boolean`
  * `timestamp_modified_flag: boolean`
  * `device_swapped_flag: boolean`
  * `routing_anomalous_flag: boolean`

Additional metrics (e.g. “distance from baseline”, pattern-specific scores) MAY be added as optional fields.

#### 5.4.3 Dictionary entry
...
- id: s3_event_stream_with_fraud_6B
  ...
  owner_segment: 6B
  description: >
    Event-level view after fraud/abuse overlay; one row per event with 
    baseline event fields plus campaign/fraud overlay metadata.
  version: '{seed}.{manifest_fingerprint}.{scenario_id}'
  format: parquet
...
#### 5.4.4 Artefact registry entry

`artefact_registry_6B.yaml` **MUST** register the dataset, e.g.:
...
### 5.5 Cross-dataset relationships (FK expectations)

While JSON-Schema does not enforce foreign keys, these shapes are deliberately wired so that:

* For each `(seed, manifest_fingerprint)`:

  * Every non-null `campaign_id` in `s3_flow_anchor_with_fraud_6B` and `s3_event_stream_with_fraud_6B` MUST exist in `s3_campaign_catalogue_6B` for the same axes.

* For each `(seed, manifest_fingerprint, scenario_id)`:

  * Every `flow_id` in `s3_event_stream_with_fraud_6B` MUST exist in `s3_flow_anchor_with_fraud_6B`.
  * `event_seq` MUST be unique per `(seed, fingerprint, scenario_id, flow_id)` and form a contiguous monotone sequence.

* For flows derived from baseline:

  * `origin_flow_id` in S3 anchors MUST match `flow_id` from S2 anchors.
  * For events derived from baseline, `origin_flow_id` and `origin_event_seq` MUST match the corresponding event in S2.

These relationships will be checked by S3 and S5 validation logic. The schema anchors and catalogue links defined here ensure that those checks are straightforward and unambiguous.
