...
This section fixes the **logical shapes**, **schema anchors**, and **catalogue wiring** for the two datasets produced by 6B.S0:

* `s0_gate_receipt_6B`
* `sealed_inputs_6B`

It also declares which upstream schema anchors 6B.S0 relies on when reading HashGates and sealed-inputs manifests.

Schema files are the **single source of truth** for shapes. The dataset dictionary and artefact registry **MUST** match those schemas; where they diverge, schemas win and the catalogue MUST be corrected.

---

### 5.1 Schema anchors for 6B.S0 outputs

The shapes for the two S0 outputs **MUST** be defined in the shared Layer-3 schema pack:
...
* **Sealed inputs manifest**

  ```text
  schemas.layer3.yaml#/gate/6B/sealed_inputs_6B
  ```

The 6B dataset dictionary and artefact registry **MUST** reference these anchors via `schema_ref` for the corresponding dataset ids (see §5.4).

These schema definitions are binding for all implementations of 6B.S0 and for all downstream consumers of its outputs.

---

### 5.2 `s0_gate_receipt_6B` shape

#### 5.2.1 Row model

`schemas.layer3.yaml#/gate/6B/s0_gate_receipt_6B` **MUST** define a single JSON object per `manifest_fingerprint` with:

* `manifest_fingerprint: string`

  * Pattern: 64-char lower-case hex (SHA-256), or the engine’s agreed fingerprint format.
  * This field is the **primary key** and **MUST** equal the partition token `fingerprint={manifest_fingerprint}`.

* `parameter_hash: string`

  * Pattern: lower-case hex SHA-256.
  * Identifies the 6B parameter/config pack used by this run.

* `spec_version_6B: string`

  * Semantic version or opaque tag; identifies the 6B spec/contract version.

* `upstream_segments: object`
  An object keyed by upstream segment id (e.g. `"1A"`, `"1B"`, `"2A"`, `"2B"`, `"3A"`, `"3B"`, `"5A"`, `"5B"`, `"6A"`). Each value **MUST** be an object with at least:

  * `status: string`

    * Enum: `"PASS"`, `"FAIL"`, `"MISSING"`.
    * 6B.S0 **MUST** set `"PASS"` only if it successfully verified the segment’s HashGate for this fingerprint.

  * `bundle_path: string`

    * Relative path to the validation bundle directory under the owning segment’s root, as resolved via the owning segment’s dictionary/registry.

  * `bundle_sha256: string`

    * The digest 6B.S0 recomputed for the upstream bundle index; 64-char lower-case hex.

  * `flag_path: string`

    * Relative path to the `_passed.flag_*` file.

  Additional optional fields (e.g. `validation_report_path`, `notes`) MAY be included but MUST NOT be required.

* `contracts_6B: object`
  An object keyed by logical contract group (e.g. `"schemas"`, `"dictionary"`, `"registry"`, `"behaviour_priors"`, `"campaign_config"`, `"labelling_policy"`, `"validation_policy"`). Each value **MUST** contain at least:

  * `logical_id: string` — matches the `id` in the 6B dictionary/registry.
  * `schema_ref: string|null` — `$ref` into `schemas.layer3.yaml` or `schemas.6B.yaml` where applicable.
  * `path: string` — relative path template under the layer-3 root.
  * `sha256_hex: string` — digest of the artefact as used in S0.

* `sealed_inputs_digest_6B: string`

  * 64-char lower-case hex.
  * Deterministic digest over the `sealed_inputs_6B` table content for this `manifest_fingerprint` (hashing law defined in §6).

* `created_utc: string`

  * ISO-8601 timestamp in UTC (`YYYY-MM-DDThh:mm:ssZ`).
  * **MUST** be derived from a deterministic source (e.g. orchestration metadata or S0 invocation envelope), not from wall-clock time at execution.

The schema **MUST** prohibit additional top-level required fields and **MAY** allow optional metadata fields under an `"extra"` object (or similar) to avoid schema churn.

#### 5.2.2 Cardinality & partition law

The schema for `s0_gate_receipt_6B` **MUST** enforce:

* **Exactly one** row per `manifest_fingerprint` and `spec_version_6B`.
* No `seed`, `scenario_id` or `run_id` fields in the primary key or partitioning columns.
* `manifest_fingerprint` is **required** and unique in the dataset.

The dataset dictionary entry **MUST** set:

* `primary_key: [manifest_fingerprint]`
* `partitioning: [fingerprint]`

Downstream 6B states and 4A/4B **MUST** treat absence of a receipt row for a given fingerprint as “6B.S0 has not successfully run”.

---

### 5.3 `sealed_inputs_6B` shape

#### 5.3.1 Row model

`schemas.layer3.yaml#/gate/6B/sealed_inputs_6B` **MUST** define a tabular schema with one row per artefact that 6B is authorised to read.

Each row **MUST** contain at least:

* `manifest_fingerprint: string`

  * As above; partition key and first PK component.

* `owner_layer: integer`

  * Enum: `1`, `2`, `3`.
  * The layer that owns the artefact.

* `owner_segment: string`

  * Enum drawn from the known segments (`"1A"`, `"1B"`, `"2A"`, `"2B"`, `"3A"`, `"3B"`, `"5A"`, `"5B"`, `"6A"`, `"6B"`).
  * 6B.S0 MUST reject rows with unknown segment ids.

* `manifest_key: string`

  * A logical key unique within `(owner_layer, owner_segment)` that corresponds to the `manifest_key` or dataset id used in the owning segment’s artefact registry.
  * Combined with `(manifest_fingerprint, owner_layer, owner_segment)` this serves as the **primary key** for this table.

* `path_template: string`

  * The resolved path template as per the owning segment’s dictionary/registry, including partition placeholders (e.g. `seed={seed}`, `fingerprint={manifest_fingerprint}`, `scenario_id={scenario_id}`).
  * **MUST NOT** contain hard-coded literal fingerprints, parameter hashes, or seeds.

* `partition_keys: array<string>`

  * Ordered list of the logical partition columns required to read this artefact (e.g. `["seed","fingerprint"]`, `["fingerprint"]`, `["seed","fingerprint","scenario_id"]`).
  * 6B.S0 **MUST** ensure this list matches the owning segment’s dictionary.

* `schema_ref: string`

  * JSON Schema `$ref` into the owning layer’s schema pack (e.g. `schemas.5B.yaml#/s4/arrival_events_5B`, `schemas.6A.yaml#/s1/party_base_6A`), or into `schemas.layer3.yaml` / `schemas.6B.yaml` for 6B-local artefacts.
  * 6B.S0 **MUST** validate the presence and resolvability of this reference.

* `role: string`

  * Enum defined by 6B (e.g. `arrival_stream`, `entity_graph`, `static_posture`, `sealed_inputs_upstream`, `behaviour_prior`, `campaign_config`, `labelling_policy`, `validation_policy`, `other`).
  * The role classification is a 6B concern only; it does not modify upstream semantics.

* `status: string`

  * Enum: `REQUIRED`, `OPTIONAL`, `IGNORED`.
  * `REQUIRED` artefacts **MUST** exist and be readable for 6B.S0 to pass.
  * `OPTIONAL` artefacts are nice-to-have; missing artefacts **MUST NOT** cause S0 failure, but downstream states **MUST** be prepared to degrade gracefully.
  * `IGNORED` artefacts are listed only for completeness and **MUST NOT** be read by 6B.

* `read_scope: string`

  * Enum: `ROW_LEVEL`, `METADATA_ONLY`.
  * `ROW_LEVEL` indicates that data-plane states (6B.S1–S4) may read full rows from this dataset.
  * `METADATA_ONLY` indicates that 6B may use only metadata (e.g. existence, size, digest) and MUST NOT scan rows (typical for some upstream sealed-inputs or validation bundles).

* `sha256_hex: string`

  * 64-char lower-case hex digest of the artefact’s canonical serialisation, as defined by the owning segment/policy.
  * 6B.S0 **MUST** compute or verify this digest before recording it.

The schema MAY add optional descriptive fields, for example:

* `upstream_bundle_id: string|null` — which upstream validation bundle this artefact is covered by.
* `notes: string|null` — human-oriented commentary.
  But these MUST NOT be required for all rows.

#### 5.3.2 Cardinality & PK/partition rules

The schema **MUST** enforce:

* `manifest_fingerprint`, `owner_layer`, `owner_segment`, `manifest_key`, `path_template`, `schema_ref`, `role`, `status`, `read_scope`, and `sha256_hex` are **required** fields.
* The composite key `(manifest_fingerprint, owner_layer, owner_segment, manifest_key)` is unique.

The dataset dictionary entry for `sealed_inputs_6B` **MUST** use:

* `partitioning: [fingerprint]`
* `primary_key: [manifest_fingerprint, owner_layer, owner_segment, manifest_key]`
* `ordering: [owner_layer, owner_segment, manifest_key]`

Writers **MUST** respect this ordering when producing the parquet file for a given fingerprint.

---

### 5.4 Catalogue links

#### 5.4.1 Dataset dictionary entries

`dataset_dictionary.layer3.6B.yaml` **MUST** include entries for:
...
* `s0_gate_receipt_6B`:
  ...
  - id: s0_gate_receipt_6B
    ...
    owner_layer: 3
    owner_segment: 6B
    description: Behavioural gate receipt for 6B.
    version: '{manifest_fingerprint}'
    format: json
    path: data/layer3/6B/gate_receipt/fingerprint={manifest_fingerprint}/s0_gate_receipt_6B.json
    partitioning: [fingerprint]
...
* `sealed_inputs_6B`:
  ...
  - id: sealed_inputs_6B
    ...
    owner_layer: 3
    owner_segment: 6B
    description: Sealed list of all artefacts 6B is allowed to read.
    version: '{manifest_fingerprint}'
    format: parquet
    path: data/layer3/6B/sealed_inputs/fingerprint={manifest_fingerprint}/sealed_inputs_6B.parquet
    partitioning: [fingerprint]
...
Field names and exact YAML layout may differ, but the semantics (ids, partitioning, PK, schema_ref, consumed_by) are binding.

#### 5.4.2 Artefact registry entries

`artefact_registry_6B.yaml` **MUST** register the same artefacts with consistent metadata, e.g.:

* For `s0_gate_receipt_6B`:
...
The registry entries for **upstream** artefacts that 6B will consume (e.g. `arrival_events_5B`, `s1_party_base_6A`, `s2_account_base_6A`, etc.) MUST already exist in their owning segment registries and may add `"consumed_by": ["6B"]` or equivalent annotations as a non-binding convenience. 6B.S0 does **not** own those entries; it only references them via `sealed_inputs_6B`.

---

This section fully binds the shapes, anchors, and catalogue wiring for 6B.S0’s outputs, and fixes how they relate to upstream schemas and catalogues. Subsequent sections build on this to define the deterministic algorithm and identity/merge discipline for producing them.
