...
This section fixes the **logical shapes**, **schema anchors**, and **catalogue wiring** for the two datasets produced by 6B.S2:

* `s2_flow_anchor_baseline_6B`
* `s2_event_stream_baseline_6B`

JSON-Schema packs remain the **single source of truth** for shapes. The Layer-3 dataset dictionary and artefact registry **MUST** reflect those schemas exactly; if they diverge, schemas win.

---

### 5.1 Schema anchors in `schemas.6B.yaml`

The 6B segment schema pack **MUST** define the following anchors for S2:
...
* `dataset_dictionary.layer3.6B.yaml` **MUST** use them as `schema_ref` for the matching dataset ids.
* `artefact_registry_6B.yaml` **MUST** point its `schema` fields at the same anchors.

---

### 5.2 `s2_flow_anchor_baseline_6B` shape

#### 5.2.1 Core identity & linkage

`schemas.6B.yaml#/s2/flow_anchor_baseline_6B` **MUST** define an object with at least these **required** fields:
...
* Identity axes:
  ...
  * `flow_id: string|integer` (using a shared `id` format, e.g. `id64`)

* Linkage to S1:

  * `session_id` — FK into `s1_session_index_6B.session_id` for the same `(seed, fingerprint, scenario_id)`.
  * A way to link back to originating arrivals, either:

    * a scalar `primary_arrival_key` (e.g. `(merchant_id, arrival_seq)`), or
    * a repeated/array field `arrival_keys` if flows can aggregate multiple arrivals.
      The exact representation is up to the schema, but **MUST** be unambiguous and documented.

* Entity context:

  * `party_id` — FK to 6A party base.
  * `account_id` — FK to 6A account base.
  * `instrument_id` — FK to 6A instrument base (nullable when no instrument is involved).
  * Optional `device_id`, `ip_id` for the primary device/IP of the flow.

These FKs are not enforced by JSON-Schema but are enforced by S2 and 6B validation.

#### 5.2.2 Amounts, currencies & outcomes

The schema MUST define fields for **core financial characteristics** under baseline assumptions, for example:

* Amounts:

  * `auth_amount: number` (or fixed-decimal encoded as integer + scale).
  * `clear_amount: number` (may equal `auth_amount` or differ due to tips / partial clears).
  * `refund_amount: number` (nullable/zero if no refund occurs).

* Currency:

  * `transaction_currency: string` (e.g. ISO 4217 code).
  * Optional `settlement_currency: string`.

* Baseline outcome flags:

  * `auth_outcome: string` (e.g. `"APPROVED"`, `"DECLINED"`).
  * `clearing_outcome: string` (e.g. `"SETTLED"`, `"NOT_SETTLED"`).
  * `refund_outcome: string` (e.g. `"NONE"`, `"FULL"`, `"PARTIAL"`).

The exact enums are defined in the schema; this spec only requires that:

* amounts and currencies are clearly typed,
* baseline outcomes can be expressed without reference to fraud (no fraud/dispute labels here).

#### 5.2.3 Timestamps & temporal shape

The schema MUST define:

* `first_auth_ts_utc: string` (ISO-8601 UTC)
* `final_auth_ts_utc: string` (ISO-8601 UTC; may equal first_auth)
* `clear_ts_utc: string|null` (nullable if not settled)
* `refund_ts_utc: string|null` (nullable if no refund)

Optional derived fields (duration between stages, time from arrival to auth, etc.) MAY be added as optional fields.

These timestamps MUST be consistent with:

* the session window in `s1_session_index_6B`, and
* any underlying arrival timestamps that seeded the flow.

#### 5.2.4 Dictionary entry
...
- id: s2_flow_anchor_baseline_6B
  ...
  owner_segment: 6B
  description: >
    Baseline (all-legit) flow anchor table; one row per flow/transaction with
    entity context, amounts, key timestamps, and baseline outcomes.
  version: '{seed}.{manifest_fingerprint}.{scenario_id}'
  format: parquet
  ...
  partitioning: [seed, fingerprint, scenario_id]
  primary_key: [seed, manifest_fingerprint, scenario_id, flow_id]
...
The exact YAML shape can vary, but the semantics (id, partitioning, PK, ordering, schema_ref) are binding.

#### 5.2.5 Artefact registry entry

`artefact_registry_6B.yaml` MUST contain a matching entry, e.g.:
...
Additional registry metadata (retention, encryption, etc.) MAY be added but MUST NOT change these core properties.

---

### 5.3 `s2_event_stream_baseline_6B` shape

#### 5.3.1 Core identity & sequence

`schemas.6B.yaml#/s2/event_stream_baseline_6B` MUST define at least:
...
* Identity axes:
  ...
  * `seed: integer|string`
  * `scenario_id: string|integer`
  * `flow_id: string|integer` (FK into `s2_flow_anchor_baseline_6B`)
  * `event_seq: integer` (non-negative or positive; defines strict order within a flow)

The schema MUST document that `(seed, manifest_fingerprint, scenario_id, flow_id, event_seq)` is unique and that `event_seq` forms a contiguous sequence for each flow.

#### 5.3.2 Event type & timestamps

The event schema MUST include:

* `event_type: string`

  * Enum describing the kind of event, e.g.:
...
* `event_ts_utc: string`
  ...
  * MUST be consistent with the flow-level timestamps in `s2_flow_anchor_baseline_6B` and fit within the session window from S1.

For some event types, the schema SHOULD also define:

* `event_amount` and `event_currency` (e.g. on CLEARING or REFUND).
* `auth_result_code` / `auth_response_code` (for response events).
* Any other event-specific fields needed to model baseline behaviour.

The exact fields and their types live in `schemas.6B.yaml`; this spec only requires they be well-defined and consistent with flow-level aggregates.

#### 5.3.3 Entity & routing context

Per event, the schema SHOULD (and MAY require) fields for:

* Entity context:

  * `party_id`, `account_id`, `instrument_id` mirroring the flow anchor,
  * optional `device_id`, `ip_id` if relevant.

* Routing context:

  * If relevant at the event level (e.g. for auth), fields referencing:

    * `site_id` (for physical POS) or
    * `edge_id` (for virtual/CDN),
    * `is_virtual`, `routing_universe_hash`, etc.,

  copied from or derived consistently with the originating arrival(s).

Whether entity/routing context is required on every event or only certain event types is defined in the schema; this spec only requires that context, when present, is consistent with S1 and upstream surfaces.

#### 5.3.4 Dictionary entry

`dataset_dictionary.layer3.6B.yaml` MUST define:
...
- id: s2_event_stream_baseline_6B
  ...
  owner_segment: 6B
  description: >
    Baseline (all-legit) event stream; one row per event in each flow, with
    event type, timestamp, entity context and routing.
  version: '{seed}.{manifest_fingerprint}.{scenario_id}'
  format: parquet
  ...
  partitioning: [seed, fingerprint, scenario_id]
  primary_key: [seed, manifest_fingerprint, scenario_id, flow_id, event_seq]
...
#### 5.3.5 Artefact registry entry

`artefact_registry_6B.yaml` MUST have a matching entry, e.g.:
...
### 5.4 Cross-dataset relationships (FK expectations)

Although JSON-Schema does not enforce referential integrity, the S2 shapes are designed so that:

* For any `(seed, manifest_fingerprint, scenario_id)`:

  * Every `flow_id` in `s2_event_stream_baseline_6B` MUST exist in `s2_flow_anchor_baseline_6B`.
  * For each `flow_id`, `event_seq` MUST form a contiguous, ordered sequence.
  * `session_id` in `s2_flow_anchor_baseline_6B` MUST exist in `s1_session_index_6B`.
  * Any entity IDs present in S2 outputs MUST exist in 6A bases and be consistent with S1 attachments.

These relationships are enforced by S2’s algorithm and validated by the 6B validation state; the schema + dictionary wiring here simply makes those checks possible.
...
* The schema anchors for S2 outputs in `schemas.6B.yaml`.
* Their partitioning, primary keys and ordering in `dataset_dictionary.layer3.6B.yaml`.
* Their artefact-level registration in `artefact_registry_6B.yaml`.

Any implementation of 6B.S2 MUST produce `s2_flow_anchor_baseline_6B` and `s2_event_stream_baseline_6B` that conform to these shapes and catalogue links, so that:

* S3 can treat them as the canonical baseline flows to corrupt/overlay,
* S4 can label them, and
* S5 can validate them against upstream and S1 invariants.
