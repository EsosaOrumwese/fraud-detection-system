...
This section fixes the **logical shapes**, **schema anchors**, and **catalogue wiring** for the four datasets produced by 6B.S4:
...
### 5.2 `s4_flow_truth_labels_6B` — shape & catalogue links

#### 5.2.1 Row model (schema expectations)

`schemas.6B.yaml#/s4/flow_truth_labels_6B` **MUST** define a table schema with one row per flow in S3 overlays for a given partition.
...
* Identity / axes:
  ...
  * `flow_id: string|integer`  *(FK into `s3_flow_anchor_with_fraud_6B`)*

* Truth classification:

  * `truth_label: string`

    * Enum, e.g. `LEGIT`, `FRAUD`, `ABUSE`.
  * `truth_subtype: string`

    * Enum or structured type, e.g. `CARD_TESTING`, `ATO`, `REFUND_ABUSE`, `MULE_ACTIVITY`, `FRIENDLY_FRAUD`, `NONE`.

* Provenance:

  * `label_policy_id: string`

    * Identifier of the `truth_labelling_policy_6B` version used.
  * `campaign_id: string|integer|null`

    * If truth label is driven directly by a S3 campaign; null for purely heuristic labels.
  * `pattern_source: string`

    * Brief classification of what drove the label, e.g. `CAMPAIGN`, `COLLATERAL`, `HEURISTIC_ONLY`.

Optional fields MAY include:

* Confidence scores, per-flow notes, or additional diagnostic flags, but MUST be optional.
...
- id: s4_flow_truth_labels_6B
  ...
  owner_segment: 6B
  description: >
    Flow-level ground truth labels for overlaid flows; one row per flow with
    truth classification and provenance.
  version: '{seed}.{manifest_fingerprint}.{scenario_id}'
  format: parquet
...
`artefact_registry_6B.yaml` **MUST** register it consistently, e.g.:
...
### 5.3 `s4_flow_bank_view_6B` — shape & catalogue links

#### 5.3.1 Row model (schema expectations)

`schemas.6B.yaml#/s4/flow_bank_view_6B` MUST define one row per flow with bank-view outcomes.

Required fields (minimum):

* Identity / axes:

  * `manifest_fingerprint`, `parameter_hash`, `seed`, `scenario_id`, `flow_id` (same constraints as above).

* Bank-view classification:

  * `bank_view_label: string`

    * Enum, e.g. `BANK_CONFIRMED_FRAUD`, `BANK_CONFIRMED_LEGIT`, `NO_CASE_OPENED`, `DISPUTE_REJECTED`, etc.
  * `auth_decision: string`

    * Enum: `APPROVE`, `DECLINE`, `REVIEW`, `CHALLENGE`, etc.
  * `detection_outcome: string`

    * Enum: `DETECTED_AT_AUTH`, `DETECTED_POST_AUTH`, `NOT_DETECTED`.

* Lifecycle timestamps (nullable as appropriate):

  * `auth_ts_utc: string` (optional if already implied by S3 but recommended)
  * `detection_ts_utc: string|null`
  * `dispute_ts_utc: string|null`
  * `chargeback_ts_utc: string|null`
  * `case_opened_ts_utc: string|null`
  * `case_closed_ts_utc: string|null`

* Provenance:

  * `bank_view_policy_id: string`
  * `delay_model_id: string` (if multiple delay models exist)

Optional fields MAY include additional bank metrics (e.g. risk score, reason codes) but MUST be optional.
...
- id: s4_flow_bank_view_6B
  ...
  owner_segment: 6B
  description: >
    Flow-level bank-view outcomes and lifecycle; one row per flow with auth,
    detection, dispute/chargeback, and case outcomes.
  version: '{seed}.{manifest_fingerprint}.{scenario_id}'
  format: parquet
...
`artefact_registry_6B.yaml` MUST include:
...
### 5.4 `s4_event_labels_6B` — shape & catalogue links

#### 5.4.1 Row model

`schemas.6B.yaml#/s4/event_labels_6B` MUST define one row per event in the S3 overlay stream.
...
* Identity / axes:
  ...
  * `flow_id`, `event_seq`  *(FK into `s3_event_stream_with_fraud_6B`)*

* Truth-level flags:

  * `is_fraud_event_truth: boolean`
  * `truth_event_role: string`

    * Enum like: `PRIMARY_FRAUD_ACTION`, `SUPPORTING_EVENT`, `LEGIT_CONTEXT`, `DETECTION_ACTION`, `CASE_EVENT`, `NONE`.

* Bank-view flags:

  * `is_detection_action: boolean`
  * `is_case_event: boolean`
  * Optional `bank_event_label: string` (more detailed bank-view classification for this event).

Optional fields MAY include extra diagnostics or reason codes.
...
- id: s4_event_labels_6B
  ...
  owner_segment: 6B
  description: >
    Event-level truth and bank-view flags for overlaid events; one row per
    event keyed to s3_event_stream_with_fraud_6B.
  version: '{seed}.{manifest_fingerprint}.{scenario_id}'
  format: parquet
...
### 5.5 `s4_case_timeline_6B` — shape & catalogue links

#### 5.5.1 Row model

`schemas.6B.yaml#/s4/case_timeline_6B` MUST define a cases + events schema with:
...
* Identity / axes:
  ...
  * `parameter_hash: string`
  * `seed: integer|string`
  * `case_id: string|integer`
  * `case_event_seq: integer`  *(sequence of events within a case)*

* Case event fields:

  * `case_event_type: string`

    * Enum, e.g. `CASE_OPENED`, `DETECTION_ATTACHED`, `CUSTOMER_DISPUTE_RECEIVED`, `CHARGEBACK_FILED`, `CHARGEBACK_DECISION`, `CASE_CLOSED`.
  * `case_event_ts_utc: string` (ISO-8601 UTC).

* Linkage:

  * `flow_id: string|integer|null`

    * Flow to which this case event primarily relates (if any).
  * Optional `flow_ids` array if events can attach to multiple flows.
  * Optional `event_seq: integer|null` if the case event is tied to a particular event.

Optional fields MAY describe case-level state (e.g. case_status after this event) or notes but MUST be optional.
...
- id: s4_case_timeline_6B
  ...
  owner_segment: 6B
  description: >
    Case-level timeline of disputes/chargebacks/investigations; one row per 
    case event, keyed by case_id and case_event_seq.
  version: '{seed}.{manifest_fingerprint}'
  format: parquet
  path: data/layer3/6B/s4_case_timeline_6B/seed={seed}/fingerprint={manifest_fingerprint}/part-*.parquet
  partitioning: [seed, fingerprint]
  primary_key: [seed, manifest_fingerprint, case_id, case_event_seq]
  ordering: [seed, manifest_fingerprint, case_id, case_event_seq]
...
- manifest_key: s4_case_timeline_6B
  ...
  path_template: data/layer3/6B/s4_case_timeline_6B/seed={seed}/fingerprint={manifest_fingerprint}/part-*.parquet
  partitioning: [seed, fingerprint]
  final_in_layer: true
```
...
### 5.6 Cross-dataset relationships (FK expectations)

Though JSON-Schema does not enforce FKs, these shapes are wired to make the following relationships obvious and checkable:

* For each `(seed, fingerprint, scenario_id)`:

  * Every `flow_id` in `s3_flow_anchor_with_fraud_6B` MUST appear exactly once in:

    * `s4_flow_truth_labels_6B`, and
    * `s4_flow_bank_view_6B`.

  * Every `(flow_id, event_seq)` in `s3_event_stream_with_fraud_6B` MUST appear exactly once in `s4_event_labels_6B`.

* For each `(seed, fingerprint)`:

  * Every `case_id` used in `s4_case_timeline_6B` MUST be unique in that partition.
  * Any flow that S4 declares as “in a case” in `s4_flow_bank_view_6B` MUST appear in at least one row of `s4_case_timeline_6B` via the linkage fields.

These FK expectations are enforced by S4’s algorithm and by the 6B validation/HashGate state (S5). The schema anchors and catalogue wiring defined here ensure those checks are straightforward and unambiguous.
