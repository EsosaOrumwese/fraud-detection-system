...
This section fixes **where the shapes for S0’s outputs live**, how they are **exposed to the catalogue**, and how **later 6A states** (and tooling) must discover them. S0 does **not** re-encode column types or constraints inline; JSON-Schema + the dataset dictionary remain the **sole shape authorities**.

---

### 5.1 JSON-Schema as the sole shape authority

For 6A.S0, all dataset shapes are defined in **Layer-3 schemas**, not in this document:

* `schemas.layer3.yaml` holds shared Layer-3 primitives and gate-level structures.
* `schemas.6A.yaml` may refine or import those primitives but **must not** redefine S0 datasets; S0 outputs live under the *layer-wide* schema so 6B and any Layer-3 tooling can see them without segment coupling.

The following anchors are **binding** and must be present:

* `schemas.layer3.yaml#/gate/6A/s0_gate_receipt_6A`
* `schemas.layer3.yaml#/gate/6A/sealed_inputs_6A`

Any consumer that needs the shape of S0 outputs **must** resolve these anchors through the schema catalogue, not rely on ad-hoc redefinitions.

---

### 5.2 Schema anchors — required fields (informal overview)

This spec does *not* embed the JSON-Schema text, but it does constrain what those schemas must express.

#### 5.2.1 `s0_gate_receipt_6A` schema

`schemas.layer3.yaml#/gate/6A/s0_gate_receipt_6A` must:

* Model a **single logical row per `manifest_fingerprint`**, even if physically represented as a 1-row table or JSON file.

* Include, at minimum, the following **required fields** (names can be bikeshedded, but semantics may not):
  ...
  * `spec_version_6A: string`  (or equivalent version struct)

  * `upstream_gates: object`

    * keys in `{ "1A","1B","2A","2B","3A","3B","5A","5B" }`
    * each value an object with at least:

      * `segment_id: string`
      * `gate_status: string` with enum `{ "PASS","FAIL","MISSING" }`
      * `bundle_path: string`
      * `bundle_digest_sha256: string | null`
      * `flag_path: string | null`

  * `schemas_layer3_version: string`

  * `schemas_6A_version: string`

  * `dictionary_6A_version: string`

  * `registry_6A_version: string`

  * `prior_packs: array` of objects, each with:

    * `prior_id: string`
    * `prior_version: string`
    * `prior_role: string` (e.g. `POPULATION_PRIOR`, `PRODUCT_PRIOR`, etc.)

  * `sealed_inputs_digest_6A: string` (exactly 64 hex chars)

  * `sealed_inputs_row_count: integer`

  * `created_utc: string` (RFC 3339 with micros)

* Forbids additional top-level properties **by default** (`additionalProperties: false`) unless explicitly allowed under a documented extension mechanism.

#### 5.2.2 `sealed_inputs_6A` schema

`schemas.layer3.yaml#/gate/6A/sealed_inputs_6A` must:

* Model a **tabular dataset** where each row is one sealed artefact.

* Require at least the following fields:

  * `manifest_fingerprint: string`

  * `owner_layer: integer`

  * `owner_segment: string`  (e.g. `"1B"`, `"5A"`, `"6A"`)

  * `manifest_key: string`  (or `logical_id` if that is your established name)

  * `schema_ref: string`

  * `path_template: string`

  * `partition_keys: array` of strings (ordered)

  * `role: string` with a closed enum including at least:
  ...
  * `status: string` with enum `{ "REQUIRED","OPTIONAL","IGNORED" }`

  * `read_scope: string` with enum `{ "ROW_LEVEL","METADATA_ONLY" }`

  * `sha256_hex: string` (64 hex chars)

  * optional `size_bytes: integer`

  * optional `row_count_estimate: integer`

  * optional `discovered_from: string`

  * `created_utc: string` (RFC 3339 with micros)

* Enforce uniqueness of `(manifest_fingerprint, owner_layer, owner_segment, manifest_key)` at the **schema or constraint** level where supported (e.g. via PK declaration in the 6A schemas file).

* Again, default to `additionalProperties: false` to discourage silent drift.

The **exact JSON-Schema** definitions live in `schemas.layer3.yaml`; this section fixes the required semantic content and constraints that the schema must capture.

---

### 5.3 Dataset dictionary entries for S0 outputs

The dataset dictionary for Layer-3 / 6A (`dataset_dictionary.layer3.6A.yaml`) must contain explicit entries for both S0 outputs.

#### 5.3.1 Dictionary entry: `s0_gate_receipt_6A`

Required properties in the dictionary (informal YAML):
...
* Partition key is **only** `fingerprint`, using the standard token `fingerprint={manifest_fingerprint}`.
* `version` is tied to `manifest_fingerprint`; there is no `parameter_hash` or `seed` in the version string.
* `columns_strict: true` ensures shape drift cannot sneak in outside the schema.

#### 5.3.2 Dictionary entry: `sealed_inputs_6A`
...
* Again partitioned only by `fingerprint={manifest_fingerprint}`.
* Writer ordering is fixed as `(owner_layer, owner_segment, manifest_key)`; later states must rely on the *content*, not the physical order, except when recomputing the digest law defined in §6.
* Dataset is clearly marked as control-plane (no PII, separate retention).

---

### 5.4 Artefact registry entries for S0 outputs

`artefact_registry_6A.yaml` must define registry entries for S0 outputs so CI/CD and orchestration can wire them up deterministically.
...
- manifest_key: engine.layer3.6A.sealed_inputs
  ...
  produced_by: 6A.S0
  dependencies:
    - engine.layer3.6A.s0_gate_receipt   # implied by digest relation
    - engine.layer3.6A.contracts
    - engine.layer1.*.egress             # as appropriate
    - engine.layer2.*.egress             # as appropriate
  retention_class: engine_control_plane
```

These registry entries:

* Make it explicit that S0’s artefacts *depend* on upstream HashGates and 6A contracts/prior packs.
* Provide a stable `manifest_key` that tools can use to locate S0 outputs without hard-coding paths.
* Link S0 to its producing state (`6A.S0`) and downstream consumers (`6A.S1–S5`, `6B.S0`).

---

### 5.5 Catalogue discovery & authority rules

Finally, S0 and all later 6A states must obey the following **catalogue rules**:

* **Shape discovery:**
  Any consumer that needs column-level or type-level information for S0 outputs must follow:

  1. Look up the dataset entry in `dataset_dictionary.layer3.6A.yaml` via `id`.
  2. Resolve `schema_ref` into `schemas.layer3.yaml`.
  3. Treat that JSON-Schema as the **only** shape authority.

* **Location discovery:**
  Path, partitions, and version are discovered from the dataset dictionary and registry:

  1. Use `dataset_id` → dictionary entry for `path` + `partitioning`.
  2. Optionally use registry `manifest_key` for environment-specific resolution.
  3. Never hard-code filesystem or object-store paths in the implementation.

* **No shadow schemas:**
  6A implementation code **must not** introduce inline or ORM-derived schemas for S0 datasets that contradict `schemas.layer3.yaml`. If a mismatch is detected, the schema file wins and the implementation must be treated as buggy.

* **No out-of-catalogue datasets:**
  S0 and downstream states may not read or write datasets that are not present in the relevant dataset dictionary and registry. Any such behaviour is out of spec and risks breaking the sealed-world guarantee.

Together, these constraints ensure that **S0’s datasets are discoverable, uniquely shaped, and consistently located** across the engine, and that all later 6A/6B states can treat them as stable, binding control-plane artefacts for the entity & product world.
