...
- `s4_device_base_6A` — Deterministic device universe built from sealed device priors and S3 links.
- `s4_ip_base_6A` — Network/IP endpoint universe derived from device telemetry.
- `s4_device_links_6A` — Edges tying devices to parties/accounts/instruments.
- `s4_ip_links_6A` — Edges tying IP addresses to devices or entities.
- `s4_entity_neighbourhoods_6A` — Graph features summarising per-entity neighbourhoods across devices/IPs.
- `s4_network_summary_6A` — Segment-level network diagnostics for QA gates.
