...
This section fixes **where the shapes for S3’s datasets live**, how they are wired into the **catalogue**, and how S3 and downstream states must discover them.

As everywhere else in this engine:

* **JSON-Schema + dataset dictionary are the only shape authorities.**
* This document defines semantics and constraints, not an alternative schema.

S3 has:

* **Required:**

  * `s3_instrument_base_6A` — instrument / credential universe.
  * `s3_account_instrument_links_6A` — per-account instrument linkage surface (derived from base).
* **Optional (strictly derived):**

  * `s3_party_instrument_holdings_6A` — per-party instrument holdings.
  * `s3_instrument_summary_6A` — aggregate instrument counts for diagnostics.

---

### 5.1 JSON-Schema anchors (shape authority)

All S3 dataset shapes live under **Layer-3 / 6A schemas**, not in this text.

Binding anchors:

* `schemas.6A.yaml#/s3/instrument_base`
* `schemas.6A.yaml#/s3/account_instrument_links`
* `schemas.6A.yaml#/s3/party_instrument_holdings` *(if implemented)*
* `schemas.6A.yaml#/s3/instrument_summary` *(if implemented)*

The schema files **must**:

* Reuse shared primitives and enums from `schemas.layer3.yaml` where appropriate (ID types, ISO codes, enums for regions, currencies, risk tiers, etc.).
* Fully define columns, types, nullability, and per-field constraints.
* Mark required fields as `required` and use `additionalProperties: false` at record level, unless a field is explicitly designed to be extensible.

Below is an informal summary of what those schemas must encode; the actual JSON-Schema definitions in `schemas.6A.yaml` are the source of truth.

#### 5.1.1 `s3_instrument_base_6A` — required schema content (informal)

`schemas.6A.yaml#/s3/instrument_base` must include the fields described in §4.1:
...
* Identity & axes:
  ...
  * `seed: integer` / `id64`
  * `instrument_id: string` / `id64` — unique per `(mf, seed)`

* Owner references:

  * `account_id: string` / `id64` — FK into `s2_account_base_6A`.
  * optional `owner_party_id: string` / `id64` — FK into `s1_party_base_6A`.
  * optional `owner_merchant_id: string` / `id64` — FK into merchant universe.

* Instrument classification:

  * `instrument_type: string` — enum from S3 instrument taxonomy.
  * optional `token_type: string` — enum (PAN vs network token vs alias).
  * `scheme: string` — enum (e.g. VISA, domestic scheme codes).
  * optional `brand_tier: string` — enum (STANDARD, GOLD, etc.).

* Identifier & display fields:

  * `masked_identifier: string`
  * optional structured parts, e.g.:

    * `bin_prefix: string`,
    * `last4: string`,
    * `issuer_country_iso: string`.

* Expiry & static flags:

  * optional `expiry_month: integer`, `expiry_year: integer` (if applicable).
  * static flags, e.g.:

    * `contactless_enabled: boolean`,
    * `virtual_only: boolean`,
    * `card_present_capable: boolean`,
    * `card_not_present_capable: boolean`.

The schema SHOULD encode PK metadata (if your schema flavour supports it):

* `primary_key: ["manifest_fingerprint","seed","instrument_id"]`.

#### 5.1.2 `s3_account_instrument_links_6A` — required schema content (informal)

`schemas.6A.yaml#/s3/account_instrument_links` must encode the linkage surface from accounts to instruments.
...
* Instrument keys / grouping:

  Depending on chosen shape:

  * **Variant A (per-instrument rows):**

    * `instrument_id: string` / `id64` (FK into `s3_instrument_base_6A`).
...
* Metrics (for grouped variant):

  * `instrument_count: integer` ≥ 0.

Schema must clearly define which variant is in play, as that becomes part of the contract.

Logical PK examples:

* Variant A: `["manifest_fingerprint","seed","account_id","instrument_id"]`
* Variant B: `["manifest_fingerprint","seed","account_id","instrument_type","scheme?"]`

#### 5.1.3 `s3_party_instrument_holdings_6A` — optional schema content (informal)

If implemented, `schemas.6A.yaml#/s3/party_instrument_holdings` must capture per-party holdings:
...
* Keys:
  ...
  * `party_id` — FK into `s1_party_base_6A`.
  * grouping keys — typically:
...
* Metrics:
  ...
  * `instrument_count: integer` ≥ 0.
  * optional splits (e.g. `card_instrument_count`, `non_card_instrument_count` if grouped differently).

Logical PK example:

* `["manifest_fingerprint","seed","party_id","instrument_type","scheme?"]`.

#### 5.1.4 `s3_instrument_summary_6A` — optional schema content (informal)

If implemented, `schemas.6A.yaml#/s3/instrument_summary` must encode diagnostics/aggregates:
...
* Grouping keys:
  ...
  * subset of `{region_id, country_iso, party_type, segment_id, account_type, instrument_type, scheme}`; the chosen grouping is part of the contract.

* Metrics:

  * `instrument_count: integer` ≥ 0.
  * optional `account_count_with_instrument` or similar.

PK is the tuple of axis keys chosen for the grouping.

---

### 5.2 Dataset dictionary entries (catalogue links)

The 6A dataset dictionary (`dataset_dictionary.layer3.6A.yaml`) MUST contain entries for:
...
#### 5.2.1 `s3_instrument_base_6A` (required)

Informal YAML sketch:
...
- id: s3_instrument_base_6A
  ...
  path: data/layer3/6A/s3_instrument_base_6A/seed={seed}/fingerprint={manifest_fingerprint}/s3_instrument_base_6A.parquet
...
* `version` encodes `(seed, mf, ph)`.
* `schema_ref` is the only shape authority; `columns_strict: true` must be set.

#### 5.2.2 `s3_account_instrument_links_6A` (required)
...
- id: s3_account_instrument_links_6A
  ...
  version: '{seed}.{manifest_fingerprint}.{parameter_hash}'
  path: data/layer3/6A/s3_account_instrument_links_6A/seed={seed}/fingerprint={manifest_fingerprint}/s3_account_instrument_links_6A.parquet
  partitioning: [seed, fingerprint]
  ordering: [account_id, instrument_type, scheme, instrument_id]   # or grouped keys
...
The exact `ordering` depends on whether you choose per-instrument rows or grouped rows; the dictionary entry must reflect that choice.

#### 5.2.3 `s3_party_instrument_holdings_6A` (optional)
...
- id: s3_party_instrument_holdings_6A
  ...
  version: '{seed}.{manifest_fingerprint}.{parameter_hash}'
  path: data/layer3/6A/s3_party_instrument_holdings_6A/seed={seed}/fingerprint={manifest_fingerprint}/s3_party_instrument_holdings_6A.parquet
  partitioning: [seed, fingerprint]
...
#### 5.2.4 `s3_instrument_summary_6A` (optional)
...
- id: s3_instrument_summary_6A
  ...
  version: '{seed}.{manifest_fingerprint}.{parameter_hash}'
  path: data/layer3/6A/s3_instrument_summary_6A/seed={seed}/fingerprint={manifest_fingerprint}/s3_instrument_summary_6A.parquet
  partitioning: [seed, fingerprint]
...
`artefact_registry_6A.yaml` MUST register S3 datasets with stable `manifest_key`s and explicit dependencies.

#### 5.3.1 Instrument base registry entry
...
- manifest_key: engine.layer3.6A.s3.instrument_base
  ...
  dependencies:
    ...
    - engine.layer3.6A.prior.instrument_mix          # example keys
...
#### 5.3.2 Account–instrument links registry entry
...
Optional registry entries for party holdings and summary follow the same pattern, with:

* `produced_by: 6A.S3`,
* dependencies including `s3_instrument_base_6A` (and S1/S2 surfaces for joins).

These entries give orchestration / CI:

* a stable key for resolving S3 outputs,
* explicit dependency graphs for change impact analysis.

---

### 5.4 Catalogue discovery rules (for S3 and downstream consumers)

All S3 implementations and downstream consumers (S4/S5, 6B) MUST use the **catalogue**, not hard-coded paths or ad-hoc schemas.

**Shape discovery:**

1. Given a dataset ID (e.g. `s3_instrument_base_6A`):

   * Look up the entry in `dataset_dictionary.layer3.6A.yaml`.
   * Take its `schema_ref`.
   * Resolve `schema_ref` in `schemas.6A.yaml` (and `schemas.layer3.yaml` for common defs).
   * Treat the resolved JSON-Schema as the **only** shape authority.

2. Do NOT derive schemas from existing data, ORMs, or library models; those must conform to the JSON-Schema, not vice versa.

**Location discovery:**

1. From the same dictionary entry (and registry if needed):

   * Read `path` / `path_template` and `partitioning`.
   * Substitute `seed={seed}`, `fingerprint={manifest_fingerprint}` to get the physical partition path.

2. Do NOT hard-code filesystem or object-store paths.

**No shadow datasets or shapes:**

* S3 MUST NOT write additional, undocumented datasets for instruments or links that are not in the dictionary/registry.
* Downstream states MUST NOT read “sidecar” copies of S3 outputs with altered schemas or paths.

**No hidden merge rules:**

* The dictionary/registry definitions (IDs, partitioning, path templates) are binding for how S3 outputs may be partitioned and merged.
* Any behaviour that contradicts §7 (identity, partitions, merge discipline) is out-of-spec.

With these anchors, dictionary entries, and registry manifests in place, S3’s datasets are:

* **discoverable** via the catalogue,
* **uniquely shaped** by JSON-Schema,
* and correctly scoped to `(seed, manifest_fingerprint)` for consistent use throughout Layer-3 and 6B.
