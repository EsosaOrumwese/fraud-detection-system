...
This section fixes **where the shapes for S2’s datasets live**, how they are wired into the **catalogue**, and how S2 and downstream states must discover them. As everywhere else, **JSON-Schema + the dataset dictionary remain the only shape authorities**; this doc describes semantics and constraints, not an alternative schema.

S2 has:

* **Required:**

  * `s2_account_base_6A` — account universe.
  * `s2_party_product_holdings_6A` — per-party holdings, derived from the base.

* **Optional, derived from base:**

  * `s2_merchant_account_base_6A` — merchant-focused view (if you choose to expose one).
  * `s2_account_summary_6A` — aggregate counts by region/segment/type.

---

### 5.1 JSON-Schema anchors (shape authority)

All S2 dataset shapes live under **Layer-3 / 6A schemas**, not in this text.

Binding anchors:

* `schemas.6A.yaml#/s2/account_base`
* `schemas.6A.yaml#/s2/party_product_holdings`
* `schemas.6A.yaml#/s2/merchant_account_base` *(if implemented)*
* `schemas.6A.yaml#/s2/account_summary` *(if implemented)*

The schema files **must**:

* Reuse shared primitives and enums from `schemas.layer3.yaml` where appropriate (IDs, iso codes, enums, monetary bands).
* Fully define column sets, types, nullability, and per-field constraints.
* Mark required fields as such, and set `additionalProperties: false` to prevent silent drift.

Below is an *informal* summary of what those schemas must express; the actual JSON-Schema is the source of truth.

#### 5.1.1 `s2_account_base_6A` — required schema content (informal)

`schemas.6A.yaml#/s2/account_base` must include at least:

* **Identity & linkage**

  * `manifest_fingerprint: string`
  * `parameter_hash: string`
  * `seed: integer` (or `id64`/`uint64` as per your primitives)
  * `account_id: string` / `id64` — unique per `(mf, seed)`

* **Owner references**

  * `owner_party_id: string` / `id64` — FK into `s1_party_base_6A`.
  * optional `owner_merchant_id: string` / `id64` — FK into the merchant universe, if merchant accounts are modelled.
  * optional `ownership_role: string` — enum for `PRIMARY_HOLDER`, `JOINT_HOLDER`, `MERCHANT_SETTLEMENT`, etc.

* **Classification**

  * `account_type: string` — enum over account taxonomy.
  * optional `product_family: string`.
  * optional `product_tier` / `brand`: string encoded, constrained by taxonomy.

* **Currency & geo**

  * `currency_iso: string` — ISO 4217 code (where applicable).
  * optional `country_iso: string` and/or `region_id: string`.

* **Static account attributes**

  Examples (all optional or required according to the account spec):

  * `overdraft_enabled: boolean`
  * `credit_limit_band: string` (banded)
  * `account_risk_tier: string` (enum)
  * `channel_profile: string` (enum)
  * flags such as `eligible_for_overdraft`, `eligible_for_loans`.

The schema should also encode PK metadata if your schema flavour supports it:

* `primary_key: ["manifest_fingerprint","seed","account_id"]`.

#### 5.1.2 `s2_party_product_holdings_6A` — required schema content (informal)

`schemas.6A.yaml#/s2/party_product_holdings` must include:
...
* Grouping keys:
  ...
  * either `account_type` or `product_family` (or both), as chosen for the spec.

* Metrics:

  * `account_count: integer` (non-negative).
  * optional splits, e.g.:

    * `primary_account_count`, `secondary_account_count`,
    * simple risk counters (e.g. `high_risk_account_count`).

Again, the schema should enforce a PK such as:

* `primary_key: ["manifest_fingerprint","seed","party_id","account_type"]` (or corresponding group).

#### 5.1.3 Optional schemas

If you implement them:

* `schemas.6A.yaml#/s2/merchant_account_base`:

  * identity fields as in account base,
  * subset focused on accounts with `owner_merchant_id` non-null,
  * PK typically `[manifest_fingerprint, seed, account_id]`.

* `schemas.6A.yaml#/s2/account_summary`:

  * identity: `manifest_fingerprint`, `parameter_hash`, `seed`,
  * grouping keys: e.g. `country_iso`, `segment_id`, `account_type`,
  * metrics: `account_count`, optional `party_count_with_account`, etc.
...
The 6A dataset dictionary (`dataset_dictionary.layer3.6A.yaml`) must contain explicit entries for all S2 datasets.

#### 5.2.1 `s2_account_base_6A` (required)
...
- id: s2_account_base_6A
  ...
  path: data/layer3/6A/s2_account_base_6A/seed={seed}/fingerprint={manifest_fingerprint}/s2_account_base_6A.parquet
...
* `version` ties together `(seed, mf, ph)` as the natural identity triple.
* `schema_ref` is the only shape authority; `columns_strict: true` is required.

#### 5.2.2 `s2_party_product_holdings_6A` (required)
...
- id: s2_party_product_holdings_6A
  ...
  version: '{seed}.{manifest_fingerprint}.{parameter_hash}'
  path: data/layer3/6A/s2_party_product_holdings_6A/seed={seed}/fingerprint={manifest_fingerprint}/s2_party_product_holdings_6A.parquet
  partitioning: [seed, fingerprint]
...
* Must be explicitly marked `status: required` because it is a key contract for later states, even though it is derived.
* Partitioning and identity follow the same `[seed, fingerprint]` pattern.

#### 5.2.3 `s2_merchant_account_base_6A` (optional)

If you decide to expose a merchant-focused view:
...
- id: s2_merchant_account_base_6A
  ...
  version: '{seed}.{manifest_fingerprint}.{parameter_hash}'
  path: data/layer3/6A/s2_merchant_account_base_6A/seed={seed}/fingerprint={manifest_fingerprint}/s2_merchant_account_base_6A.parquet
  partitioning: [seed, fingerprint]
  ...
  lineage:
    produced_by: [ '6A.S2' ]
    consumed_by:
      - '6A.S2'   # internal QA / joins
      - '6B.S0'
  pii_class: synthetic_customer
  retention_class: diagnostics
```

This dataset is **not** a new source of truth; it must be derived from the base.

#### 5.2.4 `s2_account_summary_6A` (optional)
...
- id: s2_account_summary_6A
  ...
  version: '{seed}.{manifest_fingerprint}.{parameter_hash}'
  path: data/layer3/6A/s2_account_summary_6A/seed={seed}/fingerprint={manifest_fingerprint}/s2_account_summary_6A.parquet
  partitioning: [seed, fingerprint]
...
Again, this is strictly derived from the base table.

---

### 5.3 Artefact registry entries (runtime manifests)

`artefact_registry_6A.yaml` must register S2 outputs with stable `manifest_key`s and explicit dependencies. Informal examples:

#### 5.3.1 Account base registry entry
...
- manifest_key: engine.layer3.6A.s2.account_base
  ...
  dependencies:
    ...
    - engine.layer3.6A.prior.product_mix           # example keys
...
#### 5.3.2 Party holdings registry entry
...
Optional entries for merchant base and summary follow the same pattern, with dependencies on `s2_account_base_6A`.

These registry entries:

* give CI/orchestration a stable key for S2 artefacts,
* explicitly encode which upstream and prior/config artefacts S2 depends on.

---

### 5.4 Catalogue discovery rules (for S2 and downstream consumers)

All S2 implementations and consumers (S3–S5, 6B) must be written to use the catalogue rather than hard-coded paths or ad-hoc schemas.

**Shape discovery:**

1. Given a dataset ID (e.g. `s2_account_base_6A`):

   * Look up the entry in `dataset_dictionary.layer3.6A.yaml`.
   * Read its `schema_ref`.
   * Resolve `schema_ref` into `schemas.6A.yaml` (and `schemas.layer3.yaml` for shared defs).
   * Treat that JSON-Schema as the **only** shape authority.

2. Do *not* derive schemas from ORMs, hand-written class definitions, or existing data; those must conform to the schema, not the other way around.

**Location discovery:**

1. From the same dictionary entry and, if needed, the registry:

   * read `path` / `path_template` and `partitioning`,
   * substitute `seed={seed}`, `fingerprint={manifest_fingerprint}` to construct the partition path.

2. Do *not* hard-code filesystem or object-store paths.

**No shadow datasets / shapes:**

* S2 must not write extra, undocumented datasets for accounts or holdings that are not in the dictionary/registry.
* Downstream states must not read “sidecar” copies of S2 outputs with altered schemas.

**No hidden merging rules:**

* The dictionary/registry definitions (partitioning, path templates, dataset IDs) are binding for how S2 outputs may be partitioned and merged.
* Any behaviour that conflicts with §7 (identity, partitions, merge discipline) is out-of-spec.

With these anchors, dictionary entries, and registry manifests defined, S2’s datasets are:

* globally **discoverable** via the catalogue,
* **uniquely shaped** by schema,
