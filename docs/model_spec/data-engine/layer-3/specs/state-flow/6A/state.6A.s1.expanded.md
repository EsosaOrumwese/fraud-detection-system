...
This section fixes **where the shapes for S1’s datasets live**, how they are exposed via the **catalogue**, and how S1 and downstream states must discover and use them. As with the rest of the engine, **JSON-Schema + the dataset dictionary are the only shape authorities**; this document must not be treated as a competing schema.

S1 has:

* one **required** dataset: `s1_party_base_6A`, and
* one **optional** dataset: `s1_party_summary_6A` (diagnostic / aggregate).

All references below use those logical names; you can rename them mechanically as long as the semantics stay identical.

---

### 5.1 JSON-Schema anchors (shape authority)

All S1 dataset shapes are defined under **Layer-3 schemas**, not in this text.

Binding anchors:

* `schemas.6A.yaml#/s1/party_base`
* `schemas.6A.yaml#/s1/party_summary` (if the summary table is implemented)

The schema files **must**:

* Reference common primitives (IDs, enums, geo codes, etc.) from `schemas.layer3.yaml` where applicable.
* Define complete column sets, types, nullability, and per-field constraints for S1 datasets.
* Mark fields that S1 owns as **required**, and use `additionalProperties: false` for S1 records to avoid shape drift.

#### 5.1.1 `s1_party_base_6A` — required schema content (informal)

The schema at `schemas.6A.yaml#/s1/party_base` must encode at least the following logical fields (from §4):

* Identity & linkage:

  * `manifest_fingerprint: string`
  * `parameter_hash: string`
  * `seed: integer` (or `uint64` / `id64` as per layer-3 primitives)
  * `party_id: string` (or `id64` / similar) — unique per `(mf, seed)`

* Type & segmentation (enums from taxonomies):

  * `party_type: string` (enum: `RETAIL`, `BUSINESS`, …)
  * `segment_id: string` (segment code)
  * optional `segment_group` / `segment_family`

* Home geography:

  * `country_iso: string` (ISO code)
  * optional `region_id: string` / `region_group: string`
  * optional `zone_representation: string` (if used)

* Static attributes (only those S1 owns):

  * e.g. `lifecycle_stage`, `income_band`, `turnover_band`, `customer_tenure_band`, boolean eligibility flags.

Additionally, the schema should:

* Declare the **logical primary key** as `(manifest_fingerprint, seed, party_id)` via `primary_key` metadata if your schema flavour supports it.
* Forbid extra columns unless explicitly versioned in.

#### 5.1.2 `s1_party_summary_6A` — optional schema content (informal)

If you implement a summary, `schemas.6A.yaml#/s1/party_summary` must encode:

* Identity & linkage:

  * `manifest_fingerprint`, `parameter_hash`, `seed`

* Grouping keys (matching the chosen summary grain), e.g.:

  * `country_iso`, `segment_id`, `party_type`
  * optionally `region_id` / `region_group`

* Metrics:

  * `party_count: integer` (non-negative, no upper bound in schema)
  * optional `population_prior_id: string` or `prior_reference` if you want to echo which prior record governed that cell.

The schema must state that `party_count` is **derived** (non-authoritative) and the authoritative source remains `s1_party_base_6A`.

---

### 5.2 Dataset dictionary entries (catalogue links)

The dataset dictionary for Layer-3 / 6A (`dataset_dictionary.layer3.6A.yaml`) must contain explicit entries for S1 outputs.

#### 5.2.1 Dictionary entry — `s1_party_base_6A` (required)
...
- id: s1_party_base_6A
  ...
  path: data/layer3/6A/s1_party_base_6A/seed={seed}/fingerprint={manifest_fingerprint}/s1_party_base_6A.parquet
...
* `owner_layer: 3`, `owner_segment: 6A`, `status: required`.
* `version` ties together `seed`, `manifest_fingerprint`, and `parameter_hash` as the natural identity triple.
* `partitioning: [seed, fingerprint]` matches the standard engine pattern (seed first, then fingerprint).
* `schema_ref` points to the S1 base schema anchor.
* `columns_strict: true` ensures the schema file remains the only shape authority.

#### 5.2.2 Dictionary entry — `s1_party_summary_6A` (optional)
...
- id: s1_party_summary_6A
  ...
  version: '{seed}.{manifest_fingerprint}.{parameter_hash}'
  path: data/layer3/6A/s1_party_summary_6A/seed={seed}/fingerprint={manifest_fingerprint}/s1_party_summary_6A.parquet
  partitioning: [seed, fingerprint]
  ...
  lineage:
    produced_by: [ '6A.S1' ]
    consumed_by:
      - '6A.S1'   # for internal QA / validation
      - '6A.S2'
      - '6B.S0'
  pii_class: synthetic_customer
...
The summary is **not** required for correctness; if present, it must be declared as `status: optional` and treated as strictly derived from the base table.

---

### 5.3 Artefact registry entries (runtime manifest)

`artefact_registry_6A.yaml` must register S1 outputs with stable `manifest_key`s and explicit dependencies.

#### 5.3.1 Registry — party base
...
- manifest_key: engine.layer3.6A.s1.party_base
  ...
  produced_by: 6A.S1
  dependencies:
    - engine.layer3.6A.s0_gate_receipt     # S0 gate
    - engine.layer3.6A.sealed_inputs       # S0 sealed inputs
    - engine.layer3.6A.prior.population    # population priors (example key)
    - engine.layer3.6A.prior.segmentation  # segmentation priors
    - engine.layer3.6A.taxonomy.party      # party/segment taxonomies
  retention_class: core_entity
  pii_class: synthetic_customer
```
...
* `produced_by: 6A.S1` ties the dataset to the state.
* `dependencies` explicitly list the S0 artefacts and priors required to reproduce the population.

#### 5.3.2 Registry — party summary (optional)
...
This makes it explicit the summary is derived from the base.

---

### 5.4 Catalogue discovery rules (for S1 and downstream consumers)

All S1 implementations and downstream consumers (S2–S5, 6B) must follow the catalogue, not hard-coded paths or shapes:

1. **Finding shapes**

   * Given a dataset ID (`s1_party_base_6A` or `s1_party_summary_6A`):

     1. Look it up in `dataset_dictionary.layer3.6A.yaml`.
     2. Read its `schema_ref`.
     3. Resolve `schema_ref` in `schemas.6A.yaml` (and `schemas.layer3.yaml` for shared defs).
     4. Treat that JSON-Schema as the **only** shape authority.

2. **Finding locations**

   * From the same dictionary entry, and optionally via the registry:

     1. Read `path` / `path_template` and `partitioning`.
     2. Substitute `seed={seed}`, `fingerprint={manifest_fingerprint}` to get the physical partition.
     3. Do **not** hard-code filesystem or object-store paths.

3. **No shadow schemas**

   * No implementation or tool may introduce an independently-maintained schema for S1 outputs (e.g. ORM models) that diverges from `schemas.6A.yaml`.
   * If a mismatch is detected, the JSON-Schema wins; the implementation is considered buggy.

4. **No out-of-catalogue datasets**

   * S1 must not write additional tables for party base or summary that aren’t in `dataset_dictionary.layer3.6A.yaml`.
   * Downstream states must not read “sidecar” versions of these datasets outside the catalogue.

With these anchors, dictionary entries, and registry hooks in place, S1’s datasets are:

* **discoverable** by any engine component,
* **uniquely shaped** by schema,
* and **correctly scoped** by `seed` and `manifest_fingerprint` across the Layer-3 stack.
