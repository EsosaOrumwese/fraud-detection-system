...
* **`world_countries_shapefile_2024.shp`** – Country polygons (reference, cross‑layer for TZ mapping).  
* **`tz_world_shapefile_2025.shp`** – IANA time‑zone polygons consumed by 1C.  
* **`population_raster_2025.tif`** – Population‑density raster feeding spatial priors in 1B. 
...
* **`crossborder_allocation.py`** – Houses the λ_extra (θ₀ + θ₁·log N) computation **and** the Dirichlet + Gumbel‑top‑k logic; θ₁ sub‑linearity (`p < 1 e‑5` in `theta1_stats`) is enforced inside this module.
...
* **`ranking_residual_cache_1A.parquet`** – Deterministic cache of residuals so downstream steps don’t re‑roll RNG.  
* **`residual_quantisation_policy.yaml`** – Locks the 8‑decimal‑place rounding rule in a governed config.  
...
* **`validation_bundle_1A/`** – Persisted metrics + drift plots produced by the post‑write validator.  
* **`_passed.flag`** – Single‑byte flag emitted inside the bundle when **all** validation checks succeed.  
...
* **Numeric environment:** enforce IEEE‑754 binary64; disable fused‑multiply‑add; quantise residuals to 8 d.p. before sorting. Policy captured in **`residual_quantisation_policy.yaml`**.  * **Output storage policy:** ZSTD level 3; path pattern includes `seed=` and `fingerprint=`; publish the pattern string here to prevent accidental drift.
