...
```
Note: The instance receipt path above is for engine-emitted receipts. If the engine is a black box and does not emit receipts, Scenario Runner writes verifier receipts in its own object store under `fraud-platform/<platform_run_id>/sr/instance_receipts/...`.
