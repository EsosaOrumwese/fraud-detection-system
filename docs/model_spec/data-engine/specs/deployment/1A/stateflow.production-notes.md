...
> This note sketches how the Layer‑1 / Segment 1A states (pre-ingestion, S0‑S3) are expected to run inside the enterprise environment once the engine is promoted. It is **conceptual only** – all contracts remain in the authoritative spec folders.
...
- **Governed artefact pack:** versioned bundle in a private object store (e.g., `s3://cw-fraud-engine/artifacts/<version>/`). Contents:
  ...
  - Policy YAMLs / numeric manifests (`policy.s3.rule_ladder.yaml`, `policy.s3.base_weight.yaml`, `policy.s3.thresholds.yaml`, `numeric_policy.json`, `math_profile_manifest.json`, …).
