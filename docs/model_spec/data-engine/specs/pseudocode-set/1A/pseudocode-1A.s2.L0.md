...
| `event_poisson_nb(...)`           | merchant_id, lineage, `s_before`, λ, `prev_totals`         | (k, s_after, new_totals) | samples; writes **`rng_event_poisson_component`**; payload `{merchant_id, context:"nb", lambda, k, attempt:int≥1}`; updates trace | L1/L2   | `schemas.layer1.yaml#/rng/events/poisson_component` |
