...
$$
\mathrm{merchant\_ids}\subset\{(\mathrm{merchant\_id},\ \mathrm{mcc},\ \mathrm{channel},\ \mathrm{home\_country\_iso})\},
$$
...
$$
t(m):=\big(\mathrm{mcc}_m,\ \mathrm{channel}_m\in\{\mathrm{CP},\mathrm{CNP}\},\ \mathrm{home\_country\_iso}_m,\ \mathrm{merchant\_u64}_m\big)\in\mathcal{K}\times\mathcal{C}\times\mathcal{I}\times\mathbb{U}_{64}.
$$
...
> - `blocks` = $(\mathrm{after\_hi},\mathrm{after\_lo}) - (\mathrm{before\_hi},\mathrm{before\_lo})$ in unsigned 128-bit arithmetic. (authoritative equality check).
...
2. **`rng_trace_log`** (**one row per** $(\mathrm{module},\mathrm{substream\_label})$; cumulative **blocks** (unsigned 64-bit), with the *current* `(counter_before, counter_after)`.  
...
For $m\in\mathcal M$ with $c=\mathrm{home\_country\_iso}(m)\in\mathcal I$,
...
For merchant $m$ with $c=\mathrm{home\_country\_iso}(m)$, $g_c>0$, $b_m\in\{1,\dots,5\}$:

### Hurdle (logit) design

$$
\boxed{\,x_m=\big[1,\ \phi_{\text{mcc}}(\mathrm{mcc}_m),\ \phi_{\text{ch}}(\mathrm{channel\_sym}_m),\ \phi_{\text{dev}}(b_m)\big]^\top\,}\in\mathbb R^{1+C_{\text{mcc}}+2+5}.
$$
...
$$
\boxed{\,x^{(\mu)}_m=\big[1,\ \phi_{\text{mcc}}(\mathrm{mcc}_m),\ \phi_{\text{ch}}(\mathrm{channel\_sym}_m)\big]^\top\,}\in\mathbb R^{1+C_{\text{mcc}}+2},
$$

$$
\boxed{\,x^{(\phi)}_m=\big[1,\ \phi_{\text{mcc}}(\mathrm{mcc}_m),\ \phi_{\text{ch}}(\mathrm{channel\_sym}_m),\ \ln g_c\big]^\top\,}\in\mathbb R^{1+C_{\text{mcc}}+2+1}.
$$
...
* **Merchant tuple** $t(m)=(\mathrm{mcc}_m,\mathrm{channel\_sym}_m,\mathrm{home\_country\_iso}_m)$ from `merchant_ids` (S0.1), where `channel_sym ∈ {CP,CNP}` (S0.1 mapping is authoritative).
...
* **Match:** $r$ matches $m$ iff $ \mathrm{mcc}_m\in S_{\rm mcc} \land \mathrm{channel\_sym}_m\in S_{\rm ch} \land \mathrm{home\_country\_iso}_m\in S_{\rm iso}$.
...
1. Fetch $t(m) = (\mathrm{mcc}, \mathrm{channel\_sym}, \mathrm{home\_iso})$.
...
\mathbf 1\{\mathrm{default\_decision}=\text{"allow"}\},& \text{otherwise.}
\end{cases}
$$
...
$$
(\mathrm{merchant\_id},\ \eta_m,\ \pi_m),\quad \eta_m=\beta^\top x_m,\ \pi_m=\sigma(\eta_m)\in[0,1],
$$
