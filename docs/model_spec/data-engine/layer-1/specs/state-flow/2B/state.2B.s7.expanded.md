...
* **Prior state evidence (2B.S0):** `s0_gate_receipt_2B` + `sealed_inputs_2B` at `[manifest_fingerprint]` prove the sealed inputs; S7 **does not** re-hash 1B. 
* **S2 — Alias mechanics (read-only):** `s2_alias_index` (`#/plan/s2_alias_index`) and `s2_alias_blob` (`#/binary/s2_alias_blob`) at `[seed, manifest_fingerprint]`; policy echo via `alias_layout_policy_v1`. 
* **S3 — Day effects (read-only):** `s3_day_effects` (`#/plan/s3_day_effects`) at `[seed, manifest_fingerprint]`. 
* **S4 — Group mixes (read-only):** `s4_group_weights` (`#/plan/s4_group_weights`) at `[seed, manifest_fingerprint]`. 
...
* **Emit** a fields-strict **`s7_audit_report`** (JSON) at **`[seed, manifest_fingerprint]`** containing: catalogue/inputs digests, per-validator PASS/WARN/FAIL, key metrics (merchant/group/day counts, decode max|Δ|, Σ-errors, draws expected/observed if logs), deterministic samples, and a summary. (Shape owned by a 2B validation anchor.) 

**Out of scope.** S7 **does not**: re-run routing (S5/S6); re-encode or mutate S2/S3/S4; read via literal paths or network; or publish the segment PASS bundle (that’s S8). Evidence logs remain **read-only**; the only S7 write is the audit report under `[seed, manifest_fingerprint]`.

**Determinism posture.** All checks are **RNG-free** and **replayable** under the same sealed inputs. Inputs are resolved **by Dataset Dictionary ID only** (S2/S3/S4 at `[seed, manifest_fingerprint]`; S5/S6 logs, if present, at `[seed,parameter_hash,run_id,utc_day]`; policies are token-less, selected by the S0-sealed path+digest). **No PASS → No read** is enforced via the 2B.S0 receipt and sealed inventory.
...
* **S2 — Alias mechanics (plan + binary):**
  `s2_alias_index@seed={seed}/manifest_fingerprint={manifest_fingerprint}` (**shape:** `schemas.2B.yaml#/plan/s2_alias_index`),
  `s2_alias_blob@seed={seed}/manifest_fingerprint={manifest_fingerprint}` (**shape:** `#/binary/s2_alias_blob`). 

* **S3 — Day effects:**
  `s3_day_effects@seed={seed}/manifest_fingerprint={manifest_fingerprint}` (**shape:** `#/plan/s3_day_effects`). 

* **S4 — Group mixes:**
  `s4_group_weights@seed={seed}/manifest_fingerprint={manifest_fingerprint}` (**shape:** `#/plan/s4_group_weights`). 
...
* **Exact partitions:** S2/S3/S4 reads use **exactly** `[seed, manifest_fingerprint]`; optional S5/S6 logs use **exactly** `[seed, parameter_hash, run_id, utc_day]`. Policies are token-less and carry `partition = {}` in receipts; selection is by the **exact S0-sealed** `path` **and** `sha256_hex`. **Path↔embed equality** MUST hold wherever lineage is embedded.   
...
* **No network I/O.**
* **No literal paths.**
* **No mutation** of any input artefact; S7 writes only the audit report under `[seed, manifest_fingerprint]`, **write-once** and **atomic publish** (idempotent re-emit requires byte-identical bytes). 
...
* **Gate law (S0-evidence):** S7 reads **only** cross-layer/policy assets sealed in S0; all within-segment reads are **Dictionary-only** at exactly **`[seed, manifest_fingerprint]`**. S7 does **not** re-hash 1B. *(Gate evidence is checked in §3/§9.)*
...
**Partition law (Dictionary authority):** `[seed, manifest_fingerprint]` (see §5). Path↔embed equality is **binding**. 
...
* **S2 alias mechanics:**
  `schemas.2B.yaml#/plan/s2_alias_index` · `#/binary/s2_alias_blob` — both at `[seed, manifest_fingerprint]`. 
* **S3 day effects:** `#/plan/s3_day_effects` — `[seed, manifest_fingerprint]`. 
* **S4 group mixes:** `#/plan/s4_group_weights` — `[seed, manifest_fingerprint]`. 
...
* **`s7_audit_report`**:
  Path → `data/layer1/2B/s7_audit_report/seed={seed}/manifest_fingerprint={manifest_fingerprint}/s7_audit_report.json` · **Partitioning:** `[seed, manifest_fingerprint]` · **Format:** `json` · **Schema-ref:** `schemas.2B.yaml#/validation/s7_audit_report_v1`.
* **Referenced inputs:** `s2_alias_index`, `s2_alias_blob`, `s3_day_effects`, `s4_group_weights` at `[seed, manifest_fingerprint]`; policies are token-less files selected by **S0-sealed** path+digest.  
...
> S7 performs **no RNG**. All checks are pure reads over sealed artefacts resolved by **Dataset Dictionary ID** at the exact partitions (S2/S3/S4 at `[seed, manifest_fingerprint]`; optional S5/S6 logs at `[seed,parameter_hash,run_id,utc_day]`). Shapes come from the 2B pack (plan/binary/policies/trace), and—if logs are present—the **Layer-1** envelope/core logs are read for reconciliation.
...
* S7 **MUST** resolve all inputs **by Dataset Dictionary ID** with **exact partitions**:
  `…/seed={seed}/manifest_fingerprint={manifest_fingerprint}` for S2/S3/S4; token-less **policies** (e.g., `alias_layout_policy_v1`, `route_rng_policy_v1`, `virtual_edge_policy_v1`) are selected by the **exact S0-sealed** `path` + `sha256_hex`. **No literal paths. No network I/O.** 
...
**Check:** `s0_gate_receipt_2B` **and** `sealed_inputs_2B` exist at `[manifest_fingerprint]` and are schema-valid.
...
**Check:** S2/S3/S4 are read at **`[seed, manifest_fingerprint]`**; S5/S6 logs (if present) at **`[seed,parameter_hash,run_id,utc_day]`**; token-less policies are selected by **S0-sealed** `path+sha256_hex`.
...
* **2B-S7-070 — PARTITION_SELECTION_INCORRECT** · *Abort*
  **Trigger:** Partitioned reads not **exactly** `[seed, manifest_fingerprint]` (S2/S3/S4) or optional logs not `[seed,parameter_hash,run_id,utc_day]`; token-less policies not selected by **S0-sealed** `path+sha256_hex`.
...
* **2B-S7-501 — IMMUTABLE_OVERWRITE** · *Abort*
  **Trigger:** Target `[seed, manifest_fingerprint]` already has a report and bytes differ.
...
* `target.audit_report_path` MUST match the Dictionary path family for `s7_audit_report` at `[seed, manifest_fingerprint]`. 
...
* Adding `s7_audit_report` to the **Dictionary** (ID, path family, partition law `[seed, manifest_fingerprint]`, format `json`, schema-ref) and to the **Registry** (metadata, write_once, atomic) is required for go-live.
...
* **S2/S3/S4 (read-only):** `s2_alias_index`, `s2_alias_blob`, `s3_day_effects`, `s4_group_weights` @ **`[seed, manifest_fingerprint]`**. 
...
* **S7 output (authoritative):** `s7_audit_report` @ **`[seed, manifest_fingerprint]`** *(add entry if not already present: json; schema_ref `schemas.2B.yaml#/validation/s7_audit_report_v1`)*. 
