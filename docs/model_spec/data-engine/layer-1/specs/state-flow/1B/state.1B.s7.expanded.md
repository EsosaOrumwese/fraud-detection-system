...
* **Upstream datasets (1B):** `s5_site_tile_assignment` and `s6_site_jitter` with partitions `[seed, manifest_fingerprint, parameter_hash]`, writer-sorts fixed by Schema/Dictionary, and write-once/atomic-move posture.  
...
* **Identity tokens:** exactly one `{seed, manifest_fingerprint, parameter_hash}` for the entire S7 publish; path token `manifest_fingerprint=…` MUST byte-equal any embedded `manifest_fingerprint` field where present (path↔embed equality). This mirrors S3–S6 identity law.  
* **Partition law:** S7 outputs (see §5/§6 later) follow the 1B pattern `[seed, manifest_fingerprint, parameter_hash]`; logs are **not** introduced by S7 (deterministic; RNG-free). Upstream S5/S6 and their RNG logs retain their existing partition families (`[seed, parameter_hash, run_id]` for RNG).  
...
* **Prepare for egress.** Produce rows suitable for S8 **`site_locations`** (order-free) which publishes under partitions **`[seed, manifest_fingerprint]`**; S7 must not impose order beyond its writer sort. 
...
* **No egress publish.** SHALL NOT publish or mutate S8 **`site_locations`** (that happens in S8 under `[seed, manifest_fingerprint]`). 
...
S7 SHALL execute under a fixed lineage tuple **`{seed, manifest_fingerprint, parameter_hash, run_id}`**. Any embedded lineage fields in rows (e.g., `manifest_fingerprint`) MUST byte-equal their path tokens (`manifest_fingerprint=…`). This mirrors the 1B partition law used by S5/S6.  
...
S7 SHALL read **only** the following sealed surfaces for **this** `{seed, manifest_fingerprint, parameter_hash}` (or `{parameter_hash}` where noted). Shapes are governed by Schema; paths/partitions/writer policy by the Dictionary.

* **S5 — `s5_site_tile_assignment`** (authoritative site→tile keyset)
  Path family: `…/s5_site_tile_assignment/seed={seed}/manifest_fingerprint={manifest_fingerprint}/parameter_hash={parameter_hash}/`
  Partitions: `[seed, manifest_fingerprint, parameter_hash]` · Writer sort: `[merchant_id, legal_country_iso, site_order]` · Shape: `schemas.1B.yaml#/plan/s5_site_tile_assignment`.  

* **S6 — `s6_site_jitter`** (effective in-pixel deltas per site)
  Path family: `…/s6_site_jitter/seed={seed}/manifest_fingerprint={manifest_fingerprint}/parameter_hash={parameter_hash}/`
  Partitions: `[seed, manifest_fingerprint, parameter_hash]` · Writer sort: `[merchant_id, legal_country_iso, site_order]` · Shape: `schemas.1B.yaml#/plan/s6_site_jitter`.  
...
Any S7 output dataset (see §5/§6) MUST publish under `[seed, manifest_fingerprint, parameter_hash]` with writer sort `[merchant_id, legal_country_iso, site_order]`, and MUST satisfy path↔embed equality for lineage fields. This keeps S7 aligned with S5/S6 identity law.  
...
S7 SHALL read **only** these inputs for the fixed `{seed, manifest_fingerprint, parameter_hash}` (or `{parameter_hash}` where noted):

* **S5 — `s5_site_tile_assignment`** (site→tile keyset)
  Path family `…/seed={seed}/manifest_fingerprint={manifest_fingerprint}/parameter_hash={parameter_hash}/`; partitions `[seed, manifest_fingerprint, parameter_hash]`; writer sort `[merchant_id, legal_country_iso, site_order]`; schema `schemas.1B.yaml#/plan/s5_site_tile_assignment`. 
...
* **1A egress — `outlet_catalogue`** (coverage parity check)
  Path family `…/seed={seed}/manifest_fingerprint={manifest_fingerprint}/`; partitions `[seed, manifest_fingerprint]`; order-free; **read only after validating** `s0_gate_receipt_1B` for this `manifest_fingerprint` (schema-valid; no bundle re-hash). 
...
**No PASS → No read.** S7’s read of `outlet_catalogue` is permitted only when `s0_gate_receipt_1B` is present and schema-valid for the same `manifest_fingerprint` (no bundle re-hash). 

## 4.5 Partition/identity posture for S7 outputs (preview)

Any S7 dataset SHALL publish under `[seed, manifest_fingerprint, parameter_hash]` with writer sort `[merchant_id, legal_country_iso, site_order]`, mirroring S5/S6 identity law (see §5/§6 for S7’s own anchor). 
...
**Partitions (binding):** `[seed, manifest_fingerprint, parameter_hash]` · **Writer sort:** `[merchant_id, legal_country_iso, site_order]` · **Format:** parquet · **Write-once; atomic move; file order non-authoritative**.
...
**Path↔embed equality (binding):** wherever `manifest_fingerprint` appears as a column, its value MUST byte-equal the `manifest_fingerprint=` path token. Mirrors S5/S6 law.  
...
**Partitions:** `[seed, manifest_fingerprint]` · **Writer sort:** `[merchant_id, legal_country_iso, site_order]` (file order non-authoritative).
...
**Identity:** partitions **`[seed, manifest_fingerprint, parameter_hash]`**; **PK** `[merchant_id, legal_country_iso, site_order]`; **writer sort** `[merchant_id, legal_country_iso, site_order]`; **columns_strict: true**.
...
* **S5 assignment:** `schemas.1B.yaml#/plan/s5_site_tile_assignment`
  *(Dictionary binds path/partitions `[seed, manifest_fingerprint, parameter_hash]`, writer sort `[merchant_id, legal_country_iso, site_order]`.)* 
...
* **S8 egress:** `schemas.1B.yaml#/egress/site_locations`
  *(Dictionary binds path/partitions `[seed, manifest_fingerprint]`; order-free; writer sort `[merchant_id, legal_country_iso, site_order]`.)* 
...
1. **Join S5 → S6 (1:1).** Inner-join `s5_site_tile_assignment` to `s6_site_jitter` on the site key; there MUST be **exactly one** S6 row per S5 row. (Both datasets are partitioned `[seed, manifest_fingerprint, parameter_hash]` and share the same writer sort.) 
...
* **1A coverage (read-side parity):** the site key MUST join to `outlet_catalogue` for this `manifest_fingerprint`; `site_order` continuity preserved. (**No PASS → No read** of 1A per S0.)  
...
Write exactly one `s7_site_synthesis` row for the site with fields per the S7 schema anchor (PK `[merchant_id, legal_country_iso, site_order]`; partitions `[seed, manifest_fingerprint, parameter_hash]`; writer sort as in §7.1). Publish via stage → fsync → **single atomic move**; file order is non-authoritative. 

## 7.6 Identity & order discipline (run-wide)

* **Path↔embed equality:** where lineage is embedded (e.g., `manifest_fingerprint`), its value SHALL byte-equal the path token `manifest_fingerprint=…`. 
...
Where lineage appears in both **path** and **rows**, values **MUST** be byte-identical (e.g., `manifest_fingerprint` path token equals embedded `manifest_fingerprint`). This naming/equality rule is part of the 1B gate contract and is binding here as well. 
...
* **Receipt parity (fingerprint):** any S7 publish for `manifest_fingerprint=f` implies the S0 gate receipt for `f` exists and is valid (“No PASS → No read” discipline). 
...
**Rule.** S7 lives at
`…/s7_site_synthesis/seed={seed}/manifest_fingerprint={manifest_fingerprint}/parameter_hash={parameter_hash}/` with partitions `[seed, manifest_fingerprint, parameter_hash]`; any embedded lineage (e.g., `manifest_fingerprint`) byte-equals path tokens (path↔embed equality).
...
**Rule.** For this `manifest_fingerprint`, every S7 site key joins **1:1** to `outlet_catalogue`; `site_order` continuity preserved. S7 reads 1A only if the 1A bundle for the same `manifest_fingerprint` **PASS**es; **No PASS → No read**.
**Detection.** Validate `s0_gate_receipt_1B` (schema-valid) and confirm it matches the target `manifest_fingerprint`; then join coverage. 
...
* S7 not under `…/seed={seed}/manifest_fingerprint={manifest_fingerprint}/parameter_hash={parameter_hash}/`, or
...
**Trigger:** For this `manifest_fingerprint`, any S7 site key fails to join **1:1** to `outlet_catalogue`, or the 1A consumer gate is not honoured.
**Detection:** Validate `s0_gate_receipt_1B` (schema-valid for the target `manifest_fingerprint`), then assert 1:1 join coverage. **No PASS → No read.** 
...
* `path_embed_mismatches` — any lineage field ≠ path token (`manifest_fingerprint` ↔ `manifest_fingerprint`). 
...
If S7 reads 1A `outlet_catalogue` for coverage checks, it **MUST** first validate `s0_gate_receipt_1B` (schema-valid) and confirm it matches the target `manifest_fingerprint`. **No PASS → No read.** Record the receipt’s `flag_sha256_hex` in the run summary.  

## 11.3 Sources S7 SHALL use for the summary

* **S5** `s5_site_tile_assignment` — partitions `[seed, manifest_fingerprint, parameter_hash]`; writer sort `[merchant_id, legal_country_iso, site_order]`. 
* **S6** `s6_site_jitter` — same partitions/sort. 
* **S1** geometry (**`tile_bounds`**) — parameter-scoped `[parameter_hash]`; sort `[country_iso, tile_id]`. 
* **1A** `outlet_catalogue` (read only after PASS) — partitions `[seed, manifest_fingerprint]`; order-free. 
...
* **Row groups aligned to sort.** Write parquet in **writer-sort** with balanced row groups (tens of MB) to improve downstream range scans over `[merchant_id, legal_country_iso, site_order]`. (Identity partitions for S7 mirror S5/S6: `[seed, manifest_fingerprint, parameter_hash]`.) 
...
* **Gate discipline up front.** Verify the 1A PASS receipt once per `manifest_fingerprint` before any 1A read (coverage parity stage), then reuse the result across shards. (S7 mirrors the layer’s **No PASS → No read** discipline.) 
...
* **No egress publish.** S7 prepares rows only; S8 writes `site_locations` under `[seed, manifest_fingerprint]` and remains order-free (downstreams join 1A S3 for inter-country order). 
...
1. **Identity / path law**
   ...
   * Changing S7 partitions from **`[seed, manifest_fingerprint, parameter_hash]`** or its path family.
...
6. **Downstream egress law**
   ...
   * Changing S8 `site_locations` partitions from **`[seed, manifest_fingerprint]`** would require S7 re-ratification (S7 prepares rows for that law). 
...
* **Dictionary (1B):**
  – `s5_site_tile_assignment` and `s6_site_jitter` → partitions **`[seed, manifest_fingerprint, parameter_hash]`**, writer sort **`[merchant_id, legal_country_iso, site_order]`**. 
  – `tile_bounds` → partitions **`[parameter_hash]`**, sort **`[country_iso, tile_id]`**. 
  – **Egress** `site_locations` → partitions **`[seed, manifest_fingerprint]`**; order-free.
...
* **Downstream neutrality:** S7 does **not** publish egress or RNG logs; S8 packages `site_locations` under `[seed, manifest_fingerprint]`. 
...
* **manifest_fingerprint** — hex64 fingerprint of the run manifest; path token is `manifest_fingerprint={manifest_fingerprint}`. S5/S6 (and S7) use it in partitions; egress uses `[seed, manifest_fingerprint]`.  
...
* **S7 — `s7_site_synthesis`**
  `data/layer1/1B/s7_site_synthesis/seed={seed}/parameter_hash={parameter_hash}/manifest_fingerprint={manifest_fingerprint}/`
  Partitions `[seed, manifest_fingerprint, parameter_hash]` · Writer sort `[merchant_id, legal_country_iso, site_order]`. *(Matches S5/S6.)*
...
* **S8 egress — `site_locations`**
  `data/layer1/1B/site_locations/seed={seed}/manifest_fingerprint={manifest_fingerprint}/`
  Partitions `[seed, manifest_fingerprint]` · Order-free; consumers join 1A S3 for inter-country order. 
...
Where lineage appears both as **path tokens** and **embedded columns** (e.g., `manifest_fingerprint`), values MUST byte-equal; the path token is always `manifest_fingerprint=…`. (Applied throughout 1B planning tables and egress.)  
...
* S5: `…/s5_site_tile_assignment/seed={seed}/manifest_fingerprint={manifest_fingerprint}/parameter_hash={parameter_hash}/` · partitions `[seed, manifest_fingerprint, parameter_hash]` · writer sort `[merchant_id, legal_country_iso, site_order]`. 
* S6: `…/s6_site_jitter/seed={seed}/manifest_fingerprint={manifest_fingerprint}/parameter_hash={parameter_hash}/` · same partitions/sort. 
* S1 geometry (parameter-scoped): `…/tile_bounds/parameter_hash={parameter_hash}/` · partitions `[parameter_hash]`. 
* Egress reference (for S8): `…/site_locations/seed={seed}/manifest_fingerprint={manifest_fingerprint}/` · partitions `[seed, manifest_fingerprint]`. 
...
*(S6 table lives under `[seed, manifest_fingerprint, parameter_hash]` with the same PK and writer-sort.)* 
...
**1A coverage parity gate:** before joining `outlet_catalogue` for coverage, validate `s0_gate_receipt_1B` and use its recorded `flag_sha256_hex`. **No PASS → No read.** 
...
* **Path↔embed equality:** embedded `manifest_fingerprint` == path token `manifest_fingerprint=…`. *(Same lineage law used across 1B planning tables.)* 
...
* **A703 Partition & identity law:** path partitions `[seed, manifest_fingerprint, parameter_hash]`; embedded lineage equals path tokens. 
...
* **A706 1A coverage parity:** after PASS check, 1:1 join to `outlet_catalogue` for this `manifest_fingerprint`. 
...
*(Counters derive from S5/S6 under `[seed, manifest_fingerprint, parameter_hash]`, S1 geometry under `[parameter_hash]`, and 1A egress **after** the PASS check.)*   
...
* Egress `site_locations` is `[seed, manifest_fingerprint]` and order-free: Dictionary. 
