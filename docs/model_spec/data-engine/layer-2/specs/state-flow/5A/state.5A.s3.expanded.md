...
This section fixes **where** the 5A.S3 outputs live in the schema hierarchy, **what their row shapes are**, and **how the dataset dictionary and artefact registry must refer to them**. All rules here are **binding**.
...
All 5A.S3 row shapes MUST be defined in the Segment 5A schema file:
...
* `schemas.5A.yaml#/model/class_zone_baseline_local_5A` *(optional)*
* `schemas.5A.yaml#/model/merchant_zone_baseline_utc_5A` *(optional)*

Each anchor MUST:

* fully describe the row-level schema (columns, types, nullability),
* be compatible with the primary keys and partitioning declared in the dataset dictionary, and
* reuse Layer-1 `$defs` for shared primitives where applicable (e.g. `id64`, `iso2`, `iana_tzid`, `dec_u128`).

No anchor defined for another segment/layer may be re-used or overloaded for these datasets.

---

### 5.2 `merchant_zone_baseline_local_5A` — schema anchor & row shape (required)
...
* Table-like schema: one row per `(merchant, zone[, channel], local_bucket)` for a given `(parameter_hash, manifest_fingerprint, scenario_id)`.

**Required columns (minimum)**

* **Identity / world / pack**

  * `manifest_fingerprint` — string; non-null; world identity.
  * `parameter_hash` — string; non-null; parameter pack identity.
  * `scenario_id` — string; non-null; scenario identity (e.g. `"baseline"`, `"stress_X"`).
...
* **Merchant & zone keys**
  ...
  * Zone representation — one of:
    ...
    * `legal_country_iso` — string; non-null; `$ref` Layer-1 ISO definition (e.g. `schemas.ingress.layer1.yaml#/iso3166_canonical_2024/...`).
    * `tzid` — string; non-null; `$ref` Layer-1 IANA tzid (`schemas.layer1.yaml#/iana_tzid`).

    **Option B (derived zone_id)**

    * `zone_id` — string or integer; non-null; MUST be a deterministic encoding of `(legal_country_iso, tzid)`; its `$ref` MUST describe or point to that encoding.

  * Optional (if S3 tracks channel dimension):

    * `channel` or `channel_group` — string enum or constrained string (e.g. `"POS"`, `"ECOM"`, `"HYBRID"`).

* **Time key**

  * `bucket_index` — integer; non-null; FK to `shape_grid_definition_5A` for `(parameter_hash, scenario_id)`; MUST be in `[0, T_week-1]`.

* **Baseline intensity**

  * `lambda_local_base` — numeric; non-null; MUST use a precise numeric type (e.g. `$ref: schemas.layer1.yaml#/$defs/dec_u128` or equivalent); MUST be ≥ 0.

* **Spec version**

  * `s3_spec_version` — string; non-null; MAJOR.MINOR.PATCH spec version for S3 (see §12).

**Recommended optional fields**

* `scale_source` — short string; which base scale field/interpretation was used (e.g. `"weekly_volume_expected"`, `"scale_factor"`).
* `weekly_volume_expected` — numeric echo of S1 base scale if that is the governing contract; MAY be nullable if not used.
* `scale_factor` — numeric echo if an additional factor is used.
* `baseline_clip_applied` — boolean indicating if any clipping was applied.

**Key & uniqueness**

The schema MUST be compatible with the dictionary-declared PK:

* If using explicit country+tz:
...
### 5.3 `class_zone_baseline_local_5A` — schema anchor & row shape (optional)

**Anchor**

* `schemas.5A.yaml#/model/class_zone_baseline_local_5A`

**Type**

* Table-like schema: one row per `(demand_class, zone[, channel], local_bucket)` for a given `(parameter_hash, manifest_fingerprint, scenario_id)`.
...
* **Class & zone keys**
  ...
  * `demand_class` — string; non-null; same label space as S1/S2.
  * Zone representation — as in S3.2: `(legal_country_iso, tzid)` or `zone_id`, with the same `$ref` types.
  * Optional `channel` / `channel_group` — if S3 aggregates by channel.

* **Time key**

  * `bucket_index` — integer; non-null; FK into `shape_grid_definition_5A`.

* **Aggregated baseline intensity**

  * `lambda_local_base_class` — numeric; non-null; semantics MUST be documented (sum vs mean vs other policy-defined aggregate).

* **Spec version**

  * `s3_spec_version` — string; non-null.

**Key & uniqueness**

If this dataset is materialised, the schema MUST support a PK like:
...
or, with `zone_id`, replace `legal_country_iso, tzid` by `zone_id`.

Optional `channel` / `channel_group` MUST be added to the PK if used as a dimension.

This table MUST be derivable as a deterministic aggregate of `merchant_zone_baseline_local_5A`.

---

### 5.4 `merchant_zone_baseline_utc_5A` — schema anchor & row shape (optional)

If you choose to pre-compute UTC baselines in S3, the schema MUST be anchored as:

* `schemas.5A.yaml#/model/merchant_zone_baseline_utc_5A`

**Type**

* Table-like schema: one row per `(merchant, zone[, channel], utc_bucket)` for a given `(parameter_hash, manifest_fingerprint, scenario_id)` and specified UTC horizon.
...
* **Merchant & zone keys**
  ...
  * `merchant_id` — same as S3.2.
  * Zone representation — same as S3.2 (explicit `(legal_country_iso, tzid)` or `zone_id`).
  * Optional `channel` / `channel_group`.
...
* **Time key (UTC)**
  ...
  * `utc_bucket_index` — integer; non-null; FK into a horizon-specific UTC grid, **or**
  * `(utc_date, utc_bucket_within_date)` pair; non-null; semantics tied to Layer-2 UTC grid (if defined in a shared schema).

  The spec MUST define or point to the UTC grid schema (often shared with S4).

* **Baseline intensity (UTC)**

  * `lambda_utc_base` — numeric; non-null; MUST be ≥ 0.
  * Units MUST be consistent with the local baseline (`lambda_local_base`) and the mapping performed.
...
primary_key:
  ...
  - merchant_id
  - legal_country_iso   # or zone_id
  - tzid                # omit if zone_id used
  - utc_bucket_index    # or (utc_date, utc_bucket_within_date)
```

This dataset is optional and only required if there is a clear consumer needing pre-computed UTC baselines.

---

### 5.5 Dataset dictionary entries (5A)

The 5A dataset dictionary (e.g. `dataset_dictionary.layer2.5A.yaml`) MUST define entries for S3 outputs.

#### 5.5.1 `merchant_zone_baseline_local_5A`
...
- id: merchant_zone_baseline_local_5A
  ...
  path: data/layer2/5A/merchant_zone_baseline_local/
...
#### 5.5.2 `class_zone_baseline_local_5A` (optional)
...
- id: class_zone_baseline_local_5A
  ...
  path: data/layer2/5A/class_zone_baseline_local/
...
#### 5.5.3 `merchant_zone_baseline_utc_5A` (optional)
...
- id: merchant_zone_baseline_utc_5A
  ...
  path: data/layer2/5A/merchant_zone_baseline_utc/
...
* mirror the schema anchors,
* use the partition/PK contracts defined in §7, and
* obey `fingerprint={manifest_fingerprint}` token convention.

---

### 5.6 Artefact registry entries (5A)

The 5A artefact registry (e.g. `artefact_registry_5A.yaml`) MUST register S3 outputs as manifest-visible artefacts.

#### 5.6.1 `merchant_zone_baseline_local_5A` artefact

Example entry:

```yaml
- artifact_id: "merchant_zone_baseline_local_5A"
  name: "Layer-2 / 5A merchant×zone baseline local-week intensities"
  type: "dataset"
  category: "model"
  owner_subsegment: "5A"
  ...
  primary_key:
    ...
    - bucket_index
  produced_by: ["5A.S3"]
  consumed_by: ["5A.S4","5A.validation_*","5B","6A"]
  dependencies:
...
#### 5.6.2 `class_zone_baseline_local_5A` artefact (optional)

If present:

```yaml
- artifact_id: "class_zone_baseline_local_5A"
  name: "Layer-2 / 5A class×zone baseline local-week intensities"
  type: "dataset"
  category: "model"
  owner_subsegment: "5A"
...
#### 5.6.3 `merchant_zone_baseline_utc_5A` artefact (optional)

If present:

```yaml
- artifact_id: "merchant_zone_baseline_utc_5A"
  ...
  dependencies:
    - merchant_zone_baseline_local_5A
    - site_timezones      # via 2A
    - tz_timetable_cache  # via 2A
  cross_layer: true
```

Registry entries MUST tie S3 outputs to:

* their schema anchors,
* their dictionary paths,
* and their dependencies (S1/S2 outputs and S3 policies).

---

### 5.7 Foreign-key & compatibility constraints

Finally, the schemas and dictionary entries MUST support:

* FK from `merchant_zone_baseline_local_5A` to:

  * S1 `merchant_zone_profile_5A` on `(manifest_fingerprint, merchant_id, zone_representation)`, and
  * S2 `shape_grid_definition_5A` on `(parameter_hash, scenario_id, bucket_index)`.

* FK from `class_zone_baseline_local_5A` (if present) to `merchant_zone_baseline_local_5A` (or S1/S2) on appropriate class/zone keys.

* FK from `merchant_zone_baseline_utc_5A` (if present) to:

  * `merchant_zone_baseline_local_5A` (or S2 grid) and
  * Layer-1 civil-time assets (via 2A) as needed for mapping.

These constraints ensure S3 outputs are:

* **schema-governed**,
* **catalogue-visible**, and
* **cleanly joinable** with S1/S2 and the rest of Layer-2, without extra ambiguity or bloat.
