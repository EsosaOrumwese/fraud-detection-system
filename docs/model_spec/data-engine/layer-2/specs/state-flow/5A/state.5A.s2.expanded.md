...
This section fixes **where** the 5A.S2 outputs live in the schema hierarchy, **what their row shapes are**, and **how the dataset dictionary and artefact registry must refer to them**. All rules here are **binding**.

5A.S2 defines:

* **required** modelling tables:

  * `shape_grid_definition_5A`
  * `class_zone_shape_5A`
* **optional** modelling table:
...
All S2 row shapes MUST be defined in the 5A segment schema file:
...
* `schemas.5A.yaml#/model/class_shape_catalogue_5A` *(only if implemented)*

Each anchor MUST:

* fully describe the row-level schema (types, nullability),
* be compatible with the primary keys and partitioning declared in the dataset dictionary,
* reuse Layer-1 `$defs` for shared primitives where applicable (e.g. `iana_tzid`, `iso2`, `dec_u128`).

S2 MUST NOT define conflicting anchors with other segments or layers.

---

### 5.2 `shape_grid_definition_5A` — schema anchor & row shape
...
* Table-like schema: one row per time bucket in the local week, for a given `(parameter_hash, scenario_id)`.
...
* **Identity**
  ...
  * `scenario_id` — string, non-null (if S2 is scenario-aware; a baseline scenario ID may still be used when there is only one).

* **Time-grid key**

  * `bucket_index` — integer, non-null, constrained to `[0, T_week-1]` for this `(parameter_hash, scenario_id)`.

* **Local time semantics**

  * `local_day_of_week` — integer or enum; recommended:

    * integer 1–7 with a clear convention (e.g. 1=Monday,…,7=Sunday).
  * `local_minutes_since_midnight` — integer, non-null, giving the offset (in minutes) from local midnight.

    * Alternative representations (`hour_of_day` + `minute_of_hour`) MAY be used, but MUST be consistent and non-null.

* **Bucket duration**

  * `bucket_duration_minutes` — integer, non-null, ≥ 1, MUST match the time-grid policy.

**Recommended optional fields**

* `is_weekend` — boolean, optional.
* `is_nominal_open_hours` — boolean, optional (e.g. indicates “typical business hours” bucket).
* `time_grid_version` — string, optional; version of the underlying time-grid policy.

**Key & uniqueness**

The schema MUST be compatible with:

* `primary_key: ["parameter_hash","scenario_id","bucket_index"]`

i.e. all three fields are required and non-null.

---

### 5.3 `class_zone_shape_5A` — schema anchor & row shape
...
* Table-like schema: one row per `(class, zone[, channel], bucket_index)` for a given `(parameter_hash, scenario_id)`.
...
* **Class & zone identifiers**

  * `demand_class` — string, non-null; MUST be consistent with S1 class labels and the shape-policy config.

  * Zone representation — one of:

    **Option A (explicit country + tzid):**

    * `legal_country_iso` — string, non-null; MUST `$ref` the Layer-1 ISO-2 definition (`schemas.ingress.layer1.yaml#/iso3166_canonical_2024/...`).
    * `tzid` — string, non-null; MUST `$ref` `schemas.layer1.yaml#/iana_tzid`.

    **Option B (derived zone_id):**

    * `zone_id` — string or integer, non-null; MUST be a deterministic, reversible encoding of `(legal_country_iso, tzid)`, and the schema MUST describe it or reference an appropriate `$defs/zone_id` type.

  * Optional `channel_group` or `channel` — string, if S2 maintains separate shapes per channel group.

* **Time-grid key**

  * `bucket_index` — integer, non-null, FK into `shape_grid_definition_5A` for `(parameter_hash, scenario_id)`.

* **Shape value**

  * `shape_value` — numeric, non-null; MUST:

    * be ≥ 0, and
    * use a numeric type with sufficient precision (e.g. reuse `dec_u128` or a bounded `float` from Layer-1 `$defs`).

* **Spec version**

  * `s2_spec_version` — string, non-null; spec version for S2 (MAJOR.MINOR.PATCH, see §12).

**Recommended optional fields**

* `template_id` — string, cross-ref to `class_shape_catalogue_5A` (if present).
* `adjustment_flags` — array of strings (or a compact bitmask) indicating which policy adjustments were applied (e.g. `"weekend_shift"`, `"late_night_peak"`).
* `notes` — optional short free-text for ops/debugging (non-binding).

**Key & uniqueness**

Depending on zone representation:

* If using `(legal_country_iso, tzid)`:
...
The schema MUST treat PK fields as required and non-null.

---

### 5.4 `class_shape_catalogue_5A` — schema anchor & row shape (optional)

**Anchor**

* `schemas.5A.yaml#/model/class_shape_catalogue_5A`

**Type**

* Table-like schema: one row per `demand_class` (and optional `channel_group`) for a given `(parameter_hash, scenario_id)`.

**Required columns (minimum)**

* `parameter_hash` — string, non-null.
* `scenario_id` — string, non-null.
* `demand_class` — string, non-null.

Optional but recommended:

* `channel_group` — string, nullable, if templates differ per channel.
* `template_id` — string, non-null; stable identifier for the base template used for this class.
* `template_type` — string, non-null; short descriptor like `"office_hours"`, `"nightlife"`, `"online_flat"`.
* `template_params` — structured field (object/JSON) describing parameters (e.g. peak positions, peak magnitudes, weekday/weekend weights).
* `policy_version` — string, optional; version of the shape-policy artefact used.

**Key & uniqueness**

If implemented, the schema MUST support:

* `primary_key: ["parameter_hash","scenario_id","demand_class"]`
  (and include `channel_group` if the catalogue is per class×channel, e.g. `["parameter_hash","scenario_id","demand_class","channel_group"]`).

The dataset MUST be derivable from S2’s policies; any mismatch with `class_zone_shape_5A` is a configuration error.

---

### 5.5 Dataset dictionary entries (5A)

The 5A dataset dictionary (e.g. `dataset_dictionary.layer2.5A.yaml`) MUST declare S2 outputs with concrete IDs, schemas, paths, and partitioning.

#### 5.5.1 `shape_grid_definition_5A`

Example dictionary entry (field names are illustrative but semantics are binding):

```yaml
- id: shape_grid_definition_5A
  owner_subsegment: "5A"
  schema_ref: schemas.5A.yaml#/model/shape_grid_definition_5A
  path: data/layer2/5A/shape_grid_definition/
  ...
  status: required
  produced_by: ["5A.S2"]
  consumed_by: ["5A.S3","5A.S4","5A.validation_*","5B","6A"]
  ordering: ["bucket_index"]
```

#### 5.5.2 `class_zone_shape_5A`
...
- id: class_zone_shape_5A
  ...
  path: data/layer2/5A/class_zone_shape/
...
(If `zone_id` is used, adjust `primary_key` and `ordering` accordingly.)

#### 5.5.3 `class_shape_catalogue_5A` (optional)
...
- id: class_shape_catalogue_5A
  ...
  path: data/layer2/5A/class_shape_catalogue/
  ...
  primary_key:
    - parameter_hash
    - scenario_id
    - demand_class        # and channel_group if applicable
  status: optional
  produced_by: ["5A.S2"]
  consumed_by: ["5A.S3","5A.S4","5A.validation_*","5B","6A"]
  ordering:
...
These dictionary entries MUST:

* match the schema anchors,
* use the partition and PK constraints defined above, and
* obey the engine’s path/token conventions (e.g. `{parameter_hash}`, `{scenario_id}`).

---

### 5.6 Artefact registry entries (5A)

The 5A artefact registry (e.g. `artefact_registry_5A.yaml`) MUST register S2 outputs as manifest-visible artefacts tied to the parameter pack and scenario.

#### 5.6.1 `shape_grid_definition_5A` artefact

Example entry:

```yaml
- artifact_id: "shape_grid_definition_5A"
...
#### 5.6.2 `class_zone_shape_5A` artefact

Example entry:

```yaml
- artifact_id: "class_zone_shape_5A"
...
#### 5.6.3 `class_shape_catalogue_5A` artefact (optional)

If present:

```yaml
- artifact_id: "class_shape_catalogue_5A"
...
These registry entries ensure:

* S2 outputs are discoverable by the manifest,
* downstream segments can refer to them via stable `artifact_id` / `manifest_key`, and
* they are properly tied to parameter pack + scenario, not to any specific manifest_fingerprint.

---

Within this structure, S2’s outputs have **clear schema anchors**, **catalogue definitions**, and **registry entries**, aligned with the rest of your engine’s conventions, while staying lean and focused on the class/zone shape library.
