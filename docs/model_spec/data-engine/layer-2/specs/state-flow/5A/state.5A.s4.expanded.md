...
This section fixes **where** the 5A.S4 outputs live in the schema hierarchy, **what their row shapes are**, and **how the dataset dictionary and artefact registry must refer to them**. All rules here are **binding**.

5A.S4 defines:

* one **required** modelling table:

  * `merchant_zone_scenario_local_5A`
* two **optional** modelling tables:
...
All S4 row shapes MUST be defined in the 5A segment schema file:

* **File:** `schemas.5A.yaml`

This file MUST contain a top-level `model` section with at least these anchors:

* `schemas.5A.yaml#/model/merchant_zone_scenario_local_5A`
* `schemas.5A.yaml#/model/merchant_zone_overlay_factors_5A` *(optional)*
* `schemas.5A.yaml#/model/merchant_zone_scenario_utc_5A` *(optional)*

Each anchor MUST:

* fully describe the row-level schema (columns, types, nullability),
* be compatible with the primary keys and partitioning defined in the dataset dictionary, and
* reuse Layer-1 `$defs` for shared primitives where applicable (e.g. `id64`, `iso2`, `iana_tzid`, `dec_u128`).

S4 MUST NOT reuse anchors intended for other segments or layers.

---

### 5.2 `merchant_zone_scenario_local_5A` — schema anchor & row shape *(required)*
...
* Table-like schema: one row per `(merchant, zone[, channel], local_horizon_bucket)` for a given `(parameter_hash, manifest_fingerprint, scenario_id)`.
...
* **Identity / world / pack**
  ...
  * `scenario_id` — string; non-null.

* **Merchant & zone keys**

  * `merchant_id` — MUST reuse the Layer-1 merchant ID type (e.g. `$ref: schemas.layer1.yaml#/$defs/id64`).

  * Zone representation — one of:

    **Option A — explicit `(country, tzid)`**

    * `legal_country_iso` — string; non-null; `$ref` a Layer-1 ISO-2 definition, e.g.
      `schemas.ingress.layer1.yaml#/iso3166_canonical_2024/properties/country_iso`.
    * `tzid` — string; non-null; `$ref` `schemas.layer1.yaml#/iana_tzid`.

    **Option B — derived `zone_id`**

    * `zone_id` — string or integer; non-null; MUST be a deterministic encoding of `(legal_country_iso, tzid)`. The schema MUST either:

      * define `zone_id` in `$defs` (e.g. `schemas.layer2.yaml#/$defs/zone_id`), or
      * reference an existing agreed type.

  * Optional channel dimension:

    * `channel` or `channel_group` — string (enum or constrained), non-null if you model per-channel baselines.

* **Local horizon time key**

  The schema MUST define one *canonical* representation of local horizon buckets. For example:

  * `local_horizon_bucket_index` — integer, non-null, with a separate config mapping to local date/time, **or**
  * `local_date` — string (e.g. `YYYY-MM-DD`) +
    `local_bucket_within_date` — integer, non-null.

  The chosen representation MUST be:

  * sufficient to reconstruct `(local_day_of_week, local_minutes_since_midnight)` so you can map back to S2’s local-week grid, and
  * used consistently across all S4 local datasets.

* **Scenario intensity**

  * `lambda_local_scenario` — numeric; non-null; MUST be ≥ 0 and finite.

    * SHOULD reuse a high-precision numeric type from Layer-1, e.g. `$ref: schemas.layer1.yaml#/$defs/dec_u128` (or a well-defined float type, if you’ve standardised that).

* **Spec version**

  * `s4_spec_version` — string; non-null; MAJOR.MINOR.PATCH version of the S4 spec (see §12).

**Recommended optional fields**

* `overlay_factor_total` — numeric, optional; combined overlay factor applied at this bucket.
* `lambda_base_local` — optional echo from S3 baseline for validation/debug (not authoritative).
* `scale_source` — optional short string indicating which base-scale interpretation was used.
* `created_utc` — timestamp of S4 run (informational only).

**Key & uniqueness**

The schema MUST be compatible with the dictionary-declared PK. For explicit `(country, tzid)`:
...
primary_key:
  ...
  - legal_country_iso
  - tzid
  - local_horizon_bucket_index   # or (local_date, local_bucket_within_date)
```

If `zone_id` is used, `legal_country_iso` and `tzid` are replaced with `zone_id`. If channel is part of the domain, it MUST be included in the PK.

All PK fields MUST be required and non-null.

---

### 5.3 `merchant_zone_overlay_factors_5A` — schema anchor & row shape *(optional)*

**Anchor**

* `schemas.5A.yaml#/model/merchant_zone_overlay_factors_5A`
...
* **Merchant & zone keys**
  ...
  * Same as `merchant_zone_scenario_local_5A`:
...
* **Local horizon time key**
  ...
  * The same representation used in `merchant_zone_scenario_local_5A` (e.g. `local_horizon_bucket_index` or `local_date` + `local_bucket_within_date`); non-null.

* **Overlay factor**

  * `overlay_factor_total` — numeric; non-null; MUST be ≥ 0 and finite.

    * This is the composite multiplicative factor such that:
...
* **Spec version**
  ...
  * `s4_spec_version` — string; non-null.

**Recommended optional fields**

* Per-event-type factors, e.g.:
...
* `overlays_applied` — array of strings or a compact bitmask describing which event categories contributed.

**Key & uniqueness**

If present, the schema MUST support a PK that matches the domain of S4 local intensities:
...
Again, add `channel` / `channel_group` to the PK if you model per-channel overlays.

This dataset MUST be a pure deterministic function of S3 baselines, scenario calendar, and overlay policy; no new randomness or unsealed inputs may be used.

---

### 5.4 `merchant_zone_scenario_utc_5A` — schema anchor & row shape *(optional)*
...
* Table-like schema: one row per `(merchant, zone[, channel], utc_horizon_bucket)` for a given `(parameter_hash, manifest_fingerprint, scenario_id)`.
...
* **Merchant & zone keys**
  ...
  * `merchant_id` — as before.
  * zone representation — same choice as above (explicit `(legal_country_iso, tzid)` or `zone_id`).
  * optional `channel` / `channel_group`.

* **UTC horizon time key**

  You MUST choose a canonical UTC grid representation. Common patterns:

  * `utc_horizon_bucket_index` — integer, non-null, plus separate UTC-horizon config mapping to actual timestamps, **or**
  * `(utc_date, utc_bucket_within_date)` — non-null (date + intra-day bucket index).

  The schema MUST make it possible to map between UTC buckets and actual UTC time windows deterministically.

* **Scenario intensity (UTC)**

  * `lambda_utc_scenario` — numeric; non-null; MUST be ≥ 0, finite.

    * Units MUST be consistent with S4’s local λ and horizon bucket duration.
...
* Mapping hints to the original local bucket/horizon (e.g. “local bucket index” or a compact mapping code), for debugging DST edge cases.

**Key & uniqueness**

Typical PK (for explicit `(country, tz)` representation):
...
primary_key:
  ...
  - legal_country_iso   # or zone_id
  - tzid                # omit if zone_id is used
  - utc_horizon_bucket_index   # or (utc_date, utc_bucket_within_date)
```

All PK fields MUST be required and non-null. PK semantics MUST ensure exactly one row per domain element, with no duplicates.

---

### 5.5 Dataset dictionary entries (5A)

The 5A dataset dictionary (e.g. `dataset_dictionary.layer2.5A.yaml`) MUST define entries for S4 outputs.

#### 5.5.1 `merchant_zone_scenario_local_5A` *(required)*

Example dictionary entry (field names illustrative; semantics binding):

```yaml
- id: merchant_zone_scenario_local_5A
  owner_subsegment: "5A"
  schema_ref: schemas.5A.yaml#/model/merchant_zone_scenario_local_5A
  path: data/layer2/5A/merchant_zone_scenario_local/
...
#### 5.5.2 `merchant_zone_overlay_factors_5A` *(optional)*
...
- id: merchant_zone_overlay_factors_5A
  ...
  path: data/layer2/5A/merchant_zone_overlay_factors/
...
#### 5.5.3 `merchant_zone_scenario_utc_5A` *(optional)*
...
- id: merchant_zone_scenario_utc_5A
  ...
  path: data/layer2/5A/merchant_zone_scenario_utc/
...
* match schema anchors,
* use partition/PK contracts from §4/§7,
* obey `fingerprint={manifest_fingerprint}` and `scenario_id={scenario_id}` token conventions.

---

### 5.6 Artefact registry entries (5A)

The 5A artefact registry (e.g. `artefact_registry_5A.yaml`) MUST register S4 outputs as manifest-visible artefacts.

#### 5.6.1 `merchant_zone_scenario_local_5A` artefact

Example registry entry:

```yaml
- artifact_id: "merchant_zone_scenario_local_5A"
  ...
  primary_key:
    ...
    - local_horizon_bucket_index   # or (local_date, local_bucket_within_date)
  produced_by: ["5A.S4"]
  consumed_by: ["5A.S5","5B","6A","5A.validation_*"]
  dependencies:
    - merchant_zone_baseline_local_5A
...
#### 5.6.2 `merchant_zone_overlay_factors_5A` artefact *(optional)*

```yaml
- artifact_id: "merchant_zone_overlay_factors_5A"
...
#### 5.6.3 `merchant_zone_scenario_utc_5A` artefact *(optional)*

```yaml
- artifact_id: "merchant_zone_scenario_utc_5A"
...
* tie S4 outputs to their schema anchors and dictionary paths,
* declare the correct partition keys and PKs,
* record dependencies on S3 and scenario/overlay artefacts.

---

### 5.7 Foreign-key & compatibility constraints

Finally, schema/dictionary/registry definitions MUST support the following **joinability** and alignment requirements:

* `merchant_zone_scenario_local_5A` MUST join to:

  * `merchant_zone_baseline_local_5A` on `(manifest_fingerprint, scenario_id, merchant_id, zone_representation[,channel])`, and
  * scenario calendar/event/horizon configs via time keys, as defined in S4’s algorithm.

* `merchant_zone_overlay_factors_5A` (if present) MUST share a 1:1 domain with `merchant_zone_scenario_local_5A`, so that:

  ```text
  lambda_local_scenario = lambda_base_local × overlay_factor_total
  ```

  can be reconstructed exactly for every row.

* `merchant_zone_scenario_utc_5A` (if present) MUST be consistent with `merchant_zone_scenario_local_5A` and 2A civil-time mapping; for every `(m,z,h_utc)` there MUST be a corresponding set of local buckets whose intensities sum to the same horizon bucket within tolerance, as defined in your mapping rules.

Within this structure, S4’s outputs have **clear schema anchors**, **catalogue definitions**, and **registry entries**, aligned with the rest of your engine’s conventions, while staying lean and focused on the calendar/scenario overlay role.
