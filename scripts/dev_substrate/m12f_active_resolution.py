#!/usr/bin/env python3
"""Dispatch M12.F managed lane and materialize local evidence artifacts."""
...
def main() -> int:
    env = dict(os.environ)
    execution_id = env.get("M12F_EXECUTION_ID", "").strip()
    run_dir_raw = env.get("M12F_RUN_DIR", "").strip()
    ...
    upstream_m12f_execution = env.get("M12_UPSTREAM_M12F_EXECUTION", "M12_S3_NOT_APPLICABLE").strip() or "M12_S3_NOT_APPLICABLE"
    ...
    if not execution_id:
        raise SystemExit("M12F_EXECUTION_ID is required.")
    if not run_dir_raw:
        raise SystemExit("M12F_RUN_DIR is required.")
    ...
    if rc != 0:
        blockers.append({"code": "M12-B6", "message": f"M12.F workflow dispatch failed ({err[:140]})."})
    ...
    if rc == 0:
        ...
        while time.time() < find_deadline:
            run_id, run_url = find_workflow_run(repo, branch, dispatch_start - timedelta(seconds=120), "M12.F")
            if run_id > 0:
                break
            time.sleep(5)
        if run_id <= 0:
            workflow_advisories.append("M12.F workflow run id not discoverable after dispatch.")
    ...
    if run_id > 0:
        ...
        if run_status.lower() != "completed" or run_conclusion.lower() != "success":
            workflow_advisories.append(f"M12.F workflow did not report success ({run_status}/{run_conclusion}).")

    s3 = boto3.client("s3", region_name=aws_region)
    snapshot_key = f"evidence/dev_full/run_control/{execution_id}/m12f_active_resolution_snapshot.json"
    observation_key = f"evidence/dev_full/run_control/{execution_id}/m12_post_promotion_observation_snapshot.json"
    register_key = f"evidence/dev_full/run_control/{execution_id}/m12f_blocker_register.json"
    summary_key = f"evidence/dev_full/run_control/{execution_id}/m12f_execution_summary.json"
    ...
    for key, target in (
        ...
        try:
            ...
            blockers.append({"code": "M12-B6", "message": f"M12.F artifact unreadable: {target}."})
    ...
    if register is not None and isinstance(register.get("blockers"), list):
        for row in register.get("blockers", []):
            ...
            if msg:
                blockers.append({"code": "M12-B6", "message": f"M12.F managed blocker: {msg}"})

    if summary is not None:
        if not bool(summary.get("overall_pass")):
            blockers.append({"code": "M12-B6", "message": "M12.F summary overall_pass is false."})
        if str(summary.get("next_gate", "")).strip() != "M12.G_READY":
            blockers.append({"code": "M12-B6", "message": "M12.F summary next_gate is not M12.G_READY."})
        if str(summary.get("verdict", "")).strip() != "ADVANCE_TO_M12_G":
            blockers.append({"code": "M12-B6", "message": "M12.F summary verdict is not ADVANCE_TO_M12_G."})

    lane_gate_pass = bool(
        summary
        and bool(summary.get("overall_pass"))
        and str(summary.get("next_gate", "")).strip() == "M12.G_READY"
        and str(summary.get("verdict", "")).strip() == "ADVANCE_TO_M12_G"
    )
    ...
    if snapshot is None:
        snapshot = {
            ...
            "execution_id": execution_id,
            "overall_pass": False,
            "note": "ACTIVE resolution snapshot missing from managed output.",
            "upstream_refs": {
                "m11j_execution_id": upstream_m11j_execution,
                "m12a_execution_id": upstream_m12a_execution,
                "m12b_execution_id": upstream_m12b_execution,
    ...
    if observation is None:
        observation = {
            ...
            "execution_id": execution_id,
            "overall_pass": False,
            "note": "Post-promotion observation snapshot missing from managed output.",
            "source_refs": {
                "m12e_execution_id": upstream_m12e_execution,
                "m12d_execution_id": upstream_m12d_execution,
            },
    ...
    write_local_artifacts(
        run_dir,
        {
            ...
            "m12_post_promotion_observation_snapshot.json": observation,
            "m12f_blocker_register.json": register,
