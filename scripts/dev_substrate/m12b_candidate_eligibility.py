#!/usr/bin/env python3
"""Dispatch M12.B managed lane and materialize local evidence artifacts."""
...
def main() -> int:
    env = dict(os.environ)
    execution_id = env.get("M12B_EXECUTION_ID", "").strip()
    run_dir_raw = env.get("M12B_RUN_DIR", "").strip()
    ...
    upstream_m12b_execution = env.get("M12_UPSTREAM_M12B_EXECUTION", "M12_S1_NOT_APPLICABLE").strip() or "M12_S1_NOT_APPLICABLE"
    upstream_m12c_execution = env.get("M12_UPSTREAM_M12C_EXECUTION", "M12_S1_NOT_APPLICABLE").strip() or "M12_S1_NOT_APPLICABLE"
    upstream_m12d_execution = env.get("M12_UPSTREAM_M12D_EXECUTION", "M12_S1_NOT_APPLICABLE").strip() or "M12_S1_NOT_APPLICABLE"
    upstream_m12e_execution = env.get("M12_UPSTREAM_M12E_EXECUTION", "M12_S1_NOT_APPLICABLE").strip() or "M12_S1_NOT_APPLICABLE"
    upstream_m12f_execution = env.get("M12_UPSTREAM_M12F_EXECUTION", "M12_S1_NOT_APPLICABLE").strip() or "M12_S1_NOT_APPLICABLE"
    upstream_m12g_execution = env.get("M12_UPSTREAM_M12G_EXECUTION", "M12_S1_NOT_APPLICABLE").strip() or "M12_S1_NOT_APPLICABLE"
    upstream_m12h_execution = env.get("M12_UPSTREAM_M12H_EXECUTION", "M12_S1_NOT_APPLICABLE").strip() or "M12_S1_NOT_APPLICABLE"
    upstream_m12i_execution = env.get("M12_UPSTREAM_M12I_EXECUTION", "M12_S1_NOT_APPLICABLE").strip() or "M12_S1_NOT_APPLICABLE"

    if not execution_id:
        raise SystemExit("M12B_EXECUTION_ID is required.")
    if not run_dir_raw:
        raise SystemExit("M12B_RUN_DIR is required.")
    ...
    if rc != 0:
        blockers.append({"code": "M12-B2", "message": f"M12.B workflow dispatch failed ({err[:140]})."})
    ...
    if rc == 0:
        ...
        while time.time() < find_deadline:
            run_id, run_url = find_workflow_run(repo, branch, dispatch_start - timedelta(seconds=120), "M12.B")
            if run_id > 0:
                break
            time.sleep(5)
        if run_id <= 0:
            workflow_advisories.append("M12.B workflow run id not discoverable after dispatch.")
    ...
    if run_id > 0:
        ...
        if run_status.lower() != "completed" or run_conclusion.lower() != "success":
            workflow_advisories.append(f"M12.B workflow did not report success ({run_status}/{run_conclusion}).")

    s3 = boto3.client("s3", region_name=aws_region)
    snapshot_key = f"evidence/dev_full/run_control/{execution_id}/m12b_candidate_eligibility_snapshot.json"
    register_key = f"evidence/dev_full/run_control/{execution_id}/m12b_blocker_register.json"
    summary_key = f"evidence/dev_full/run_control/{execution_id}/m12b_execution_summary.json"
    ...
    for key, target in (
        ...
        try:
            ...
            blockers.append({"code": "M12-B2", "message": f"M12.B artifact unreadable: {target}."})

    if summary is not None:
        if not bool(summary.get("overall_pass")):
            blockers.append({"code": "M12-B2", "message": "M12.B summary overall_pass is false."})
        if str(summary.get("next_gate", "")).strip() != "M12.C_READY":
            blockers.append({"code": "M12-B2", "message": "M12.B summary next_gate is not M12.C_READY."})
        if str(summary.get("verdict", "")).strip() != "ADVANCE_TO_M12_C":
            blockers.append({"code": "M12-B2", "message": "M12.B summary verdict is not ADVANCE_TO_M12_C."})

    lane_gate_pass = bool(
        summary
        and bool(summary.get("overall_pass"))
        and str(summary.get("next_gate", "")).strip() == "M12.C_READY"
        and str(summary.get("verdict", "")).strip() == "ADVANCE_TO_M12_C"
    )
    if workflow_advisories:
        if not lane_gate_pass:
            for msg in workflow_advisories:
                blockers.append({"code": "M12-B2", "message": msg})
    ...
    if snapshot is None:
        snapshot = {
            ...
            "execution_id": execution_id,
            "overall_pass": False,
            "note": "Candidate eligibility snapshot missing from managed output.",
            "upstream_refs": {
                "m11j_execution_id": upstream_m11j_execution,
                "m12a_execution_id": upstream_m12a_execution,
            },
    ...
    write_local_artifacts(
        run_dir,
        {
            "m12b_candidate_eligibility_snapshot.json": snapshot,
            "m12b_blocker_register.json": register,
            "m12b_execution_summary.json": summary,
        },
    )
