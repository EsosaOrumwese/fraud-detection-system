#!/usr/bin/env python3
"""Dispatch M11.E managed lane and materialize local evidence artifacts."""
...
def main() -> int:
    env = dict(os.environ)
    execution_id = env.get("M11E_EXECUTION_ID", "").strip()
    run_dir_raw = env.get("M11E_RUN_DIR", "").strip()
    evidence_bucket = env.get("EVIDENCE_BUCKET", "").strip()
    upstream_m11d_execution = env.get("UPSTREAM_M11D_EXECUTION", "").strip()
    ...
    if not execution_id:
        raise SystemExit("M11E_EXECUTION_ID is required.")
    if not run_dir_raw:
        raise SystemExit("M11E_RUN_DIR is required.")
    if not evidence_bucket:
        raise SystemExit("EVIDENCE_BUCKET is required.")
    if not upstream_m11d_execution:
        raise SystemExit("UPSTREAM_M11D_EXECUTION is required.")
    ...
    rc, _, err = run_cmd(
        [
            ...
            "m11_subphase=E",
            "-f",
            f"upstream_m11d_execution={upstream_m11d_execution}",
            "-f",
            f"m11e_execution_id={execution_id}",
        ],
        timeout=150,
    )
    if rc != 0:
        blockers.append({"code": "M11-B5", "message": f"M11.E workflow dispatch failed ({err[:140]})."})
    ...
    if rc == 0:
        ...
        while time.time() < find_deadline:
            run_id, run_url = find_workflow_run(repo, branch, dispatch_start - timedelta(seconds=120), "M11.E")
            if run_id > 0:
                break
            time.sleep(5)
        if run_id <= 0:
            workflow_advisories.append("M11.E workflow run id not discoverable after dispatch.")
    ...
    if run_id > 0:
        ...
        if run_status.lower() != "completed" or run_conclusion.lower() != "success":
            workflow_advisories.append(f"M11.E workflow did not report success ({run_status}/{run_conclusion}).")

    s3 = boto3.client("s3", region_name=aws_region)
    baseline_key = f"evidence/dev_full/run_control/{execution_id}/m11_eval_vs_baseline_report.json"
    prov_key = f"evidence/dev_full/run_control/{execution_id}/m11_leakage_provenance_check.json"
    snapshot_key = f"evidence/dev_full/run_control/{execution_id}/m11e_eval_gate_snapshot.json"
    register_key = f"evidence/dev_full/run_control/{execution_id}/m11e_blocker_register.json"
    summary_key = f"evidence/dev_full/run_control/{execution_id}/m11e_execution_summary.json"
    ...
    for key, target in (
        ...
        try:
            ...
            blockers.append({"code": "M11-B5", "message": f"M11.E artifact unreadable: {target}."})

    if summary is not None:
        if not bool(summary.get("overall_pass")):
            blockers.append({"code": "M11-B5", "message": "M11.E summary overall_pass is false."})
        if str(summary.get("next_gate", "")).strip() != "M11.F_READY":
            blockers.append({"code": "M11-B5", "message": "M11.E summary next_gate is not M11.F_READY."})
    ...
    if register and isinstance(register.get("blockers"), list):
        for row in register.get("blockers", []):
            ...
            if any(token in text for token in ("resourcelimit", "quota", "throttl", "accessdenied", "access denied")):
                blockers.append({"code": "M11-B19", "message": "M11.E indicates quota/capacity/access boundary pressure."})
                break

    lane_gate_pass = bool(summary and bool(summary.get("overall_pass")) and str(summary.get("next_gate", "")).strip() == "M11.F_READY")
    ...
    if workflow_advisories:
        if lane_gate_pass:
            ...
            for msg in workflow_advisories:
                blockers.append({"code": "M11-B5", "message": msg})
    ...
    if baseline is None:
        baseline = {
            ...
            "execution_id": execution_id,
            "overall_pass": False,
            "note": "Eval-vs-baseline report missing from managed output.",
        }
    if provenance is None:
        provenance = {
            ...
            "execution_id": execution_id,
            "overall_pass": False,
            "note": "Leakage provenance report missing from managed output.",
        }
    ...
    write_local_artifacts(
        run_dir,
        {
            "m11_eval_vs_baseline_report.json": baseline,
            "m11_leakage_provenance_check.json": provenance,
            "m11e_eval_gate_snapshot.json": snapshot,
            "m11e_blocker_register.json": register,
