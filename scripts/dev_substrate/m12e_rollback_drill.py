#!/usr/bin/env python3
"""Dispatch M12.E managed lane and materialize local evidence artifacts."""
...
def main() -> int:
    env = dict(os.environ)
    execution_id = env.get("M12E_EXECUTION_ID", "").strip()
    run_dir_raw = env.get("M12E_RUN_DIR", "").strip()
    ...
    if not execution_id:
        raise SystemExit("M12E_EXECUTION_ID is required.")
    if not run_dir_raw:
        raise SystemExit("M12E_RUN_DIR is required.")
    ...
    if rc != 0:
        blockers.append({"code": "M12-B5", "message": f"M12.E workflow dispatch failed ({err[:140]})."})
    ...
    if rc == 0:
        ...
        while time.time() < find_deadline:
            run_id, run_url = find_workflow_run(repo, branch, dispatch_start - timedelta(seconds=120), "M12.E")
            if run_id > 0:
                break
            time.sleep(5)
        if run_id <= 0:
            workflow_advisories.append("M12.E workflow run id not discoverable after dispatch.")
    ...
    if run_id > 0:
        ...
        if run_status.lower() != "completed" or run_conclusion.lower() != "success":
            workflow_advisories.append(f"M12.E workflow did not report success ({run_status}/{run_conclusion}).")

    s3 = boto3.client("s3", region_name=aws_region)
    snapshot_key = f"evidence/dev_full/run_control/{execution_id}/m12e_rollback_drill_snapshot.json"
    register_key = f"evidence/dev_full/run_control/{execution_id}/m12e_blocker_register.json"
    summary_key = f"evidence/dev_full/run_control/{execution_id}/m12e_execution_summary.json"
    ...
    for key, target in (
        ...
        try:
            ...
            blockers.append({"code": "M12-B5", "message": f"M12.E artifact unreadable: {target}."})
    ...
    if register is not None and isinstance(register.get("blockers"), list):
        for row in register.get("blockers", []):
            ...
            if msg:
                blockers.append({"code": "M12-B5", "message": f"M12.E managed blocker: {msg}"})

    if summary is not None:
        if not bool(summary.get("overall_pass")):
            blockers.append({"code": "M12-B5", "message": "M12.E summary overall_pass is false."})
        if str(summary.get("next_gate", "")).strip() != "M12.F_READY":
            blockers.append({"code": "M12-B5", "message": "M12.E summary next_gate is not M12.F_READY."})
        if str(summary.get("verdict", "")).strip() != "ADVANCE_TO_M12_F":
            blockers.append({"code": "M12-B5", "message": "M12.E summary verdict is not ADVANCE_TO_M12_F."})

    lane_gate_pass = bool(
        summary
        and bool(summary.get("overall_pass"))
        and str(summary.get("next_gate", "")).strip() == "M12.F_READY"
        and str(summary.get("verdict", "")).strip() == "ADVANCE_TO_M12_F"
    )
    ...
    if workflow_advisories:
        if lane_authority_available:
            ...
            for msg in workflow_advisories:
                blockers.append({"code": "M12-B5", "message": msg})
    ...
    if snapshot is None:
        snapshot = {
            ...
            "execution_id": execution_id,
            "overall_pass": False,
            "note": "Rollback drill snapshot missing from managed output.",
            "upstream_refs": {
                "m11j_execution_id": upstream_m11j_execution,
                "m12a_execution_id": upstream_m12a_execution,
                "m12b_execution_id": upstream_m12b_execution,
    ...
    write_local_artifacts(
        run_dir,
        {
            ...
            "m12e_blocker_register.json": register,
