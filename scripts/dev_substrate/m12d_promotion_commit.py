#!/usr/bin/env python3
"""Dispatch M12.D managed lane and materialize local evidence artifacts."""
...
def main() -> int:
    env = dict(os.environ)
    execution_id = env.get("M12D_EXECUTION_ID", "").strip()
    run_dir_raw = env.get("M12D_RUN_DIR", "").strip()
    ...
    upstream_m12d_execution = env.get("M12_UPSTREAM_M12D_EXECUTION", "M12_S2_NOT_APPLICABLE").strip() or "M12_S2_NOT_APPLICABLE"
    upstream_m12e_execution = env.get("M12_UPSTREAM_M12E_EXECUTION", "M12_S2_NOT_APPLICABLE").strip() or "M12_S2_NOT_APPLICABLE"
    upstream_m12f_execution = env.get("M12_UPSTREAM_M12F_EXECUTION", "M12_S2_NOT_APPLICABLE").strip() or "M12_S2_NOT_APPLICABLE"
    upstream_m12g_execution = env.get("M12_UPSTREAM_M12G_EXECUTION", "M12_S2_NOT_APPLICABLE").strip() or "M12_S2_NOT_APPLICABLE"
    upstream_m12h_execution = env.get("M12_UPSTREAM_M12H_EXECUTION", "M12_S2_NOT_APPLICABLE").strip() or "M12_S2_NOT_APPLICABLE"
    upstream_m12i_execution = env.get("M12_UPSTREAM_M12I_EXECUTION", "M12_S2_NOT_APPLICABLE").strip() or "M12_S2_NOT_APPLICABLE"

    if not execution_id:
        raise SystemExit("M12D_EXECUTION_ID is required.")
    if not run_dir_raw:
        raise SystemExit("M12D_RUN_DIR is required.")
    ...
    if rc != 0:
        blockers.append({"code": "M12-B4", "message": f"M12.D workflow dispatch failed ({err[:140]})."})
    ...
    if rc == 0:
        ...
        while time.time() < find_deadline:
            run_id, run_url = find_workflow_run(repo, branch, dispatch_start - timedelta(seconds=120), "M12.D")
            if run_id > 0:
                break
            time.sleep(5)
        if run_id <= 0:
            workflow_advisories.append("M12.D workflow run id not discoverable after dispatch.")
    ...
    if run_id > 0:
        ...
        if run_status.lower() != "completed" or run_conclusion.lower() != "success":
            workflow_advisories.append(f"M12.D workflow did not report success ({run_status}/{run_conclusion}).")

    s3 = boto3.client("s3", region_name=aws_region)
    snapshot_key = f"evidence/dev_full/run_control/{execution_id}/m12d_promotion_commit_snapshot.json"
    lifecycle_key = f"evidence/dev_full/run_control/{execution_id}/m12d_registry_lifecycle_event.json"
    transport_key = f"evidence/dev_full/run_control/{execution_id}/m12d_broker_transport_proof.json"
    publication_key = f"evidence/dev_full/run_control/{execution_id}/m12d_learning_registry_publication_receipt.json"
    register_key = f"evidence/dev_full/run_control/{execution_id}/m12d_blocker_register.json"
    summary_key = f"evidence/dev_full/run_control/{execution_id}/m12d_execution_summary.json"

    snapshot: dict[str, Any] | None = None
    lifecycle_event: dict[str, Any] | None = None
    transport_proof: dict[str, Any] | None = None
    publication_receipt: dict[str, Any] | None = None
    register: dict[str, Any] | None = None
    summary: dict[str, Any] | None = None
    for key, target in (
        (snapshot_key, "snapshot"),
        (lifecycle_key, "lifecycle_event"),
        (transport_key, "transport_proof"),
        (publication_key, "publication_receipt"),
        ...
        try:
            ...
            if target == "snapshot":
                snapshot = payload
            elif target == "lifecycle_event":
                lifecycle_event = payload
            elif target == "transport_proof":
                transport_proof = payload
            elif target == "publication_receipt":
            ...
            blockers.append({"code": "M12-B4", "message": f"M12.D artifact unreadable: {target}."})
    ...
    if register is not None and isinstance(register.get("blockers"), list):
        for row in register.get("blockers", []):
            ...
            if msg:
                blockers.append({"code": "M12-B4", "message": f"M12.D managed blocker: {msg}"})

    if summary is not None:
        if not bool(summary.get("overall_pass")):
            blockers.append({"code": "M12-B4", "message": "M12.D summary overall_pass is false."})
        if str(summary.get("next_gate", "")).strip() != "M12.E_READY":
            blockers.append({"code": "M12-B4", "message": "M12.D summary next_gate is not M12.E_READY."})
        if str(summary.get("verdict", "")).strip() != "ADVANCE_TO_M12_E":
            blockers.append({"code": "M12-B4", "message": "M12.D summary verdict is not ADVANCE_TO_M12_E."})

    lane_gate_pass = bool(
        summary
        and bool(summary.get("overall_pass"))
        and str(summary.get("next_gate", "")).strip() == "M12.E_READY"
        and str(summary.get("verdict", "")).strip() == "ADVANCE_TO_M12_E"
    )
    ...
    if workflow_advisories:
        if lane_authority_available:
            ...
            for msg in workflow_advisories:
                blockers.append({"code": "M12-B4", "message": msg})
    ...
    if snapshot is None:
        snapshot = {
            ...
            "execution_id": execution_id,
            "overall_pass": False,
            "note": "Promotion commit snapshot missing from managed output.",
            "upstream_refs": {
                "m11j_execution_id": upstream_m11j_execution,
                "m12a_execution_id": upstream_m12a_execution,
                "m12b_execution_id": upstream_m12b_execution,
    ...
    if lifecycle_event is None:
        lifecycle_event = {
            ...
            "execution_id": execution_id,
            "overall_pass": False,
            "note": "Registry lifecycle event missing from managed output.",
            "upstream_refs": {
                "m11j_execution_id": upstream_m11j_execution,
                "m12a_execution_id": upstream_m12a_execution,
                "m12b_execution_id": upstream_m12b_execution,
    ...
    if transport_proof is None:
        transport_proof = {
            ...
            "execution_id": execution_id,
            "overall_pass": False,
            "note": "Broker transport proof missing from managed output.",
            "upstream_refs": {
                "m11j_execution_id": upstream_m11j_execution,
                "m12a_execution_id": upstream_m12a_execution,
                "m12b_execution_id": upstream_m12b_execution,
    ...
    if publication_receipt is None:
        publication_receipt = {
            ...
            "execution_id": execution_id,
            "overall_pass": False,
            "note": "Learning registry publication receipt missing from managed output.",
            "upstream_refs": {
                "m11j_execution_id": upstream_m11j_execution,
                "m12a_execution_id": upstream_m12a_execution,
                "m12b_execution_id": upstream_m12b_execution,
    ...
    write_local_artifacts(
        run_dir,
        {
            "m12d_promotion_commit_snapshot.json": snapshot,
            ...
            "m12d_blocker_register.json": register,
            "m12d_execution_summary.json": summary,
        },
    )
