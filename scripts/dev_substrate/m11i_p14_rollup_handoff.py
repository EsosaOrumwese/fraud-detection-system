#!/usr/bin/env python3
"""Dispatch M11.I managed lane and materialize local evidence artifacts."""
...
def main() -> int:
    env = dict(os.environ)
    execution_id = env.get("M11I_EXECUTION_ID", "").strip()
    run_dir_raw = env.get("M11I_RUN_DIR", "").strip()
    evidence_bucket = env.get("EVIDENCE_BUCKET", "").strip()
    upstream_m11h_execution = env.get("UPSTREAM_M11H_EXECUTION", "").strip()
    ...
    if not execution_id:
        raise SystemExit("M11I_EXECUTION_ID is required.")
    if not run_dir_raw:
        raise SystemExit("M11I_RUN_DIR is required.")
    if not evidence_bucket:
        raise SystemExit("EVIDENCE_BUCKET is required.")
    if not upstream_m11h_execution:
        raise SystemExit("UPSTREAM_M11H_EXECUTION is required.")
    ...
    rc, _, err = run_cmd(
        [
            ...
            "m11_subphase=I",
            "-f",
            f"upstream_m11h_execution={upstream_m11h_execution}",
            "-f",
            f"m11i_execution_id={execution_id}",
        ],
        timeout=150,
    )
    if rc != 0:
        blockers.append({"code": "M11-B9", "message": f"M11.I workflow dispatch failed ({err[:140]})."})
    ...
    if rc == 0:
        ...
        while time.time() < find_deadline:
            run_id, run_url = find_workflow_run(repo, branch, dispatch_start - timedelta(seconds=120), "M11.I")
            if run_id > 0:
                break
            time.sleep(5)
        if run_id <= 0:
            workflow_advisories.append("M11.I workflow run id not discoverable after dispatch.")
    ...
    if run_id > 0:
        ...
        if run_status.lower() != "completed" or run_conclusion.lower() != "success":
            workflow_advisories.append(f"M11.I workflow did not report success ({run_status}/{run_conclusion}).")

    s3 = boto3.client("s3", region_name=aws_region)
    verdict_key = f"evidence/dev_full/run_control/{execution_id}/m11i_p14_gate_verdict.json"
    handoff_key = f"evidence/dev_full/run_control/{execution_id}/m12_handoff_pack.json"
    register_key = f"evidence/dev_full/run_control/{execution_id}/m11i_blocker_register.json"
    summary_key = f"evidence/dev_full/run_control/{execution_id}/m11i_execution_summary.json"
    ...
    for key, target in (
        ...
        try:
            ...
            code = "M11-B10" if target == "handoff" else "M11-B9"
            blockers.append({"code": code, "message": f"M11.I artifact unreadable: {target}."})

    if summary is not None:
        if not bool(summary.get("overall_pass")):
            blockers.append({"code": "M11-B9", "message": "M11.I summary overall_pass is false."})
        if str(summary.get("verdict", "")).strip() != "ADVANCE_TO_P15":
            blockers.append({"code": "M11-B9", "message": "M11.I summary verdict is not ADVANCE_TO_P15."})
        if str(summary.get("next_gate", "")).strip() != "M11.J_READY":
            blockers.append({"code": "M11-B9", "message": "M11.I summary next_gate is not M11.J_READY."})

    if verdict_payload is not None:
        if str(verdict_payload.get("verdict", "")).strip() != "ADVANCE_TO_P15":
            blockers.append({"code": "M11-B9", "message": "M11.I gate verdict is not ADVANCE_TO_P15."})
        if str(verdict_payload.get("next_gate", "")).strip() != "M11.J_READY":
            blockers.append({"code": "M11-B9", "message": "M11.I gate verdict next_gate is not M11.J_READY."})

    if handoff_payload is not None:
        if str(handoff_payload.get("p14_verdict", "")).strip() != "ADVANCE_TO_P15":
            blockers.append({"code": "M11-B10", "message": "M12 handoff p14_verdict is not ADVANCE_TO_P15."})
        if not bool(handoff_payload.get("m12_entry_ready")):
            blockers.append({"code": "M11-B10", "message": "M12 handoff m12_entry_ready is false."})
        entry_gate = handoff_payload.get("m12_entry_gate", {})
        if not isinstance(entry_gate, dict) or str(entry_gate.get("next_gate", "")).strip() != "M12_READY":
            blockers.append({"code": "M11-B10", "message": "M12 handoff m12_entry_gate.next_gate is not M12_READY."})
    ...
    if register and isinstance(register.get("blockers"), list):
        for row in register.get("blockers", []):
            ...
            if any(token in text for token in ("resourcelimit", "quota", "throttl", "accessdenied", "access denied")):
                blockers.append({"code": "M11-B19", "message": "M11.I indicates quota/capacity/access boundary pressure."})
                break

    lane_gate_pass = bool(summary and bool(summary.get("overall_pass")) and str(summary.get("next_gate", "")).strip() == "M11.J_READY")
    handoff_gate_pass = bool(handoff_payload and bool(handoff_payload.get("m12_entry_ready")) and isinstance(handoff_payload.get("m12_entry_gate"), dict) and str(handoff_payload.get("m12_entry_gate", {}).get("next_gate", "")).strip() == "M12_READY")
    notes: list[str] = []
    if workflow_advisories:
        if lane_gate_pass and handoff_gate_pass:
            notes.extend(workflow_advisories)
        else:
            for msg in workflow_advisories:
                blockers.append({"code": "M11-B9", "message": msg})
    ...
    if verdict_payload is None:
        verdict_payload = {
            ...
            "verdict": "HOLD_REMEDIATE",
            "next_gate": "HOLD_REMEDIATE",
            "note": "M11.I gate verdict missing from managed output.",
        }
    if handoff_payload is None:
        handoff_payload = {
            ...
            "phase_id": "P14",
            "execution_id": execution_id,
            "m12_entry_ready": False,
            "m12_entry_gate": {"next_gate": "HOLD_REMEDIATE"},
            "note": "M12 handoff pack missing from managed output.",
            "notes": notes,
        }
    else:
        existing_notes = handoff_payload.get("notes", [])
        if isinstance(existing_notes, list):
            existing_notes.extend(notes)
            handoff_payload["notes"] = [str(x) for x in existing_notes]
        elif notes:
            handoff_payload["notes"] = notes
    ...
    if blockers:
        ...
        verdict_payload["verdict"] = "HOLD_REMEDIATE"
        handoff_payload["m12_entry_ready"] = False
        if isinstance(handoff_payload.get("m12_entry_gate"), dict):
            handoff_payload["m12_entry_gate"]["next_gate"] = "HOLD_REMEDIATE"
    ...
    write_local_artifacts(
        run_dir,
        {
            ...
            "m11i_blocker_register.json": register,
            "m11i_execution_summary.json": summary,
        },
    )
