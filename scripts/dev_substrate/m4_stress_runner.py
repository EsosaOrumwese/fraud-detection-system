...
def load_latest_stage_snapshot(out_root: Path, run_prefix: str) -> dict[str, Any]:
    runs = sorted(out_root.glob(f"{run_prefix}*/stress"))
    for d in reversed(runs):
        sp = d / "m4_execution_summary.json"
        if not sp.exists():
        ...
        try:
            ...
            continue
        br = {}
        bp = d / "m4_blocker_register.json"
        if bp.exists():
            br = load_json_safe(bp)
        return {"path": d.as_posix(), "summary": summ, "blocker_register": br}
    return {}
...
def run_s3(phase_id: str, out_root: Path) -> int:
    ...
    copy_errs = copy_stagea_from_stage(out_root, out, "m4_stress_s2_", "M4-ST-S2")
    if copy_errs:
        blockers.append({"id": "M4-ST-B9", "severity": "S3", "status": "OPEN", "details": {"copy_errors": copy_errs}})

    s2 = load_latest_successful_stage(out_root, "m4_stress_s2_", "M4-ST-S2")
    if not s2:
        blockers.append({"id": "M4-ST-B9", "severity": "S3", "status": "OPEN", "details": {"missing_dependency": "successful M4 S2"}})
    ...
    if flink_runtime_active == "MSF_MANAGED":
        ...
        probes.append(
            {
                "id": "s3_flink_eks_virtual_cluster",
                "group": "runtime",
                "cmd": ["aws", "emr-containers", "describe-virtual-cluster", "--id", emr_vc_id, "--region", region, "--query", "virtualCluster.state", "--output", "text"],
                "timeout": 30,
                "required": bool(emr_vc_id),
    ...
    def run_probe_set(tag: str) -> list[dict[str, Any]]:
        set_rows: list[dict[str, Any]] = []
        with cf.ThreadPoolExecutor(max_workers=max(1, len(probes))) as ex:
            futs = []
            for p in probes:
                if p["required"]:
                    futs.append(ex.submit(run_named_probe, str(p["id"]), str(p["group"]), list(p["cmd"]), int(p["timeout"])))
            ...
            for f in cf.as_completed(futs):
                set_rows.append(f.result())
        for r in set_rows:
            r["window_mode"] = tag
        return set_rows

    pre_rows = run_probe_set("precheck")
    checks.extend(pre_rows)
    pre_failed = [x for x in pre_rows if str(x.get("status", "FAIL")) != "PASS"]
    precheck_fail_closed = len(pre_failed) > 0
    precheck_issues: list[str] = []
    if precheck_fail_closed:
        precheck_issues.append(f"critical precheck failures: {len(pre_failed)}")

    inj_results: list[dict[str, Any]] = []
    inj_issues: list[str] = []
    if not precheck_fail_closed:
        missing_path = f"/fraud-platform/dev_full/stress/m4/s3/noncritical-missing-{phase_id}"
        dep_probe = run_cmd(
            ["aws", "ssm", "get-parameter", "--name", missing_path, "--region", region, "--query", "Parameter.ARN", "--output", "text"],
            timeout=20,
        )
        dep_text = f"{dep_probe.get('stdout', '')} {dep_probe.get('stderr', '')}".lower()
        dep_detected = dep_probe.get("status") != "PASS" and ("parameternotfound" in dep_text or "parameter not found" in dep_text)
        inj_results.append(
            {
                "injection_id": "inj_dependency_path_unavailable",
                "expected_classification": "DETECTED_MISSING_DEPENDENCY_PATH",
                "observed_classification": "DETECTED_MISSING_DEPENDENCY_PATH" if dep_detected else "UNCLASSIFIED_DEPENDENCY_PATH",
                "detected": dep_detected,
                "evidence": dep_probe,
                "cause": "forced missing SSM parameter path",
                "impact": "dependency unavailable branch must fail-closed",
                "remediation_hint": "preserve dependency-path fail-closed guards",
            }
        )

        simulated_fields = sorted(req_corr_fields - {"event_id"})
        missing_corr = sorted(req_corr_fields - set(simulated_fields))
        bind_detected = len(missing_corr) > 0
        inj_results.append(
            {
                "injection_id": "inj_runtime_binding_correlation_mismatch",
                "expected_classification": "DETECTED_CORRELATION_BINDING_MISMATCH",
                "observed_classification": "DETECTED_CORRELATION_BINDING_MISMATCH" if bind_detected else "UNEXPECTED_BINDING_ACCEPT",
                "detected": bind_detected,
                "evidence": {"required_fields": sorted(req_corr_fields), "observed_fields": simulated_fields, "missing_fields": missing_corr},
                "cause": "simulated runtime binding payload missing required field",
                "impact": "must reject malformed binding fail-closed",
                "remediation_hint": "retain strict required-field enforcement",
            }
        )

        lock_state = {"platform_run_id": datetime.now(timezone.utc).strftime("platform_%Y%m%dT%H%M%SZ"), "status": "RUNNING", "stale": True}
        lock_detected = lock_state["status"] == "RUNNING"
        inj_results.append(
            {
                "injection_id": "inj_stale_lock_duplicate_activation",
                "expected_classification": "DETECTED_STALE_LOCK_DUPLICATE",
                "observed_classification": "DETECTED_STALE_LOCK_DUPLICATE" if lock_detected else "UNEXPECTED_DUPLICATE_ALLOW",
                "detected": lock_detected,
                "evidence": {"lock_state": lock_state},
                "cause": "simulated duplicate activation while stale running lock exists",
                "impact": "duplicate activation must be rejected",
                "remediation_hint": "enforce single-active lock semantics",
            }
        )

        inj_issues = [f"{x.get('injection_id')}:{x.get('observed_classification')}" for x in inj_results if not bool(x.get("detected"))]

    recovery_budget_seconds = int(pkt.get("M4_STRESS_RECOVERY_BUDGET_SECONDS", 300))
    rec_start = time.monotonic()
    rec_rows = run_probe_set("recovery_immediate")
    checks.extend(rec_rows)
    rec_failed = [x for x in rec_rows if str(x.get("status", "FAIL")) != "PASS"]
    recovery_issues: list[str] = []
    if rec_failed:
        recovery_issues.append(f"immediate recovery failures: {len(rec_failed)}")

    rec_final_rows = run_probe_set("recovery_final")
    checks.extend(rec_final_rows)
    rec_final_failed = [x for x in rec_final_rows if str(x.get("status", "FAIL")) != "PASS"]
    if rec_final_failed:
        recovery_issues.append(f"final recovery failures: {len(rec_final_failed)}")
    recovery_elapsed_seconds = max(1, int(round(time.monotonic() - rec_start)))
    if recovery_elapsed_seconds > recovery_budget_seconds:
        recovery_issues.append(f"recovery budget exceeded: elapsed={recovery_elapsed_seconds}s budget={recovery_budget_seconds}s")
    ...
    control_issues.extend(corr_issues)
    control_issues.extend(precheck_issues)
    control_issues.extend(recovery_issues)
    fail_counts: dict[str, int] = {}
    for r in checks:
        pid = str(r.get("probe_id", ""))
        if str(r.get("status", "FAIL")) != "PASS":
    ...
    s2_path = Path(str(s2.get("path", ""))) if s2 else Path("")
    s2_ctrl = load_json_safe(s2_path / "m4_control_rail_conformance_snapshot.json") if s2 else {}
    s2_issues = [str(x) for x in s2_ctrl.get("issues", [])] if s2_ctrl else []
    new_ci = sorted(set(control_issues) - set(s2_issues))

    total = len(checks)
    fails = [r for r in checks if str(r.get("status", "FAIL")) != "PASS"]
    er = round((len(fails) / total) * 100.0, 4) if total else 100.0
    lat = [float(r.get("duration_ms", 0.0)) for r in checks]
    inj_expected = len(inj_results)
    inj_detected = sum(1 for x in inj_results if bool(x.get("detected")))

    if runtime_path_issues:
        blockers.append({"id": "M4-ST-B2", "severity": "S3", "status": "OPEN", "details": {"issues": runtime_path_issues}})
    b3_markers = ["s3_sfn_lookup", "s3_evidence_bucket", "s3_api_gateway", "s3_lambda", "s3_ddb", "s3_msk", "s3_flink_", "SR_READY_COMMIT_AUTHORITY"]
    b3_issues = [x for x in new_ci if any(y in x for y in b3_markers)]
    if b3_issues:
        blockers.append({"id": "M4-ST-B3", "severity": "S3", "status": "OPEN", "details": {"issues": sorted(set(b3_issues))}})
    b4_issues = [x for x in new_ci if "correlation" in x]
    if b4_issues:
        blockers.append({"id": "M4-ST-B4", "severity": "S3", "status": "OPEN", "details": {"issues": sorted(set(b4_issues))}})
    b5_details: dict[str, Any] = {}
    if er > 2.0:
        b5_details["error_rate_pct"] = er
    if b5_details:
        blockers.append({"id": "M4-ST-B5", "severity": "S3", "status": "OPEN", "details": b5_details})
    b7_details: dict[str, Any] = {}
    if inj_expected == 0:
        b7_details["injection_expected_count"] = 0
    if inj_detected != inj_expected:
        b7_details["injection_detected_count"] = inj_detected
        b7_details["injection_expected_count"] = inj_expected
    if inj_issues:
        b7_details["injection_issues"] = inj_issues
    if recovery_issues:
        b7_details["recovery_issues"] = recovery_issues
    if b7_details:
        blockers.append({"id": "M4-ST-B7", "severity": "S3", "status": "OPEN", "details": b7_details})

    wdec = any("--with-decryption" in str(r.get("command", "")) for r in checks)
    qval = any("Parameter.Value" in str(r.get("command", "")) or "SecretString" in str(r.get("command", "")) for r in checks)
    sus: list[str] = []
    rx = re.compile(r"(AKIA[0-9A-Z]{16}|BEGIN [A-Z ]*PRIVATE KEY|SECRET_ACCESS_KEY)")
    for r in checks:
        if rx.search((str(r.get("stdout", "")) + " " + str(r.get("stderr", ""))).strip()):
    ...
    leak = wdec or qval or bool(sus)
    if leak:
        blockers.append({"id": "M4-ST-B6", "severity": "S3", "status": "OPEN", "details": {"issues": ["plaintext leakage signal detected"]}})

    dur_s = max(1, int(round(time.perf_counter() - t0)))
    max_sp = float(pkt.get("M4_STRESS_MAX_SPEND_USD", 40))
    cost_within = True
    if not cost_within:
        blockers.append({"id": "M4-ST-B8", "severity": "S3", "status": "OPEN"})
    ...
    ctrl = {
        ...
        "phase_execution_id": phase_id,
        "stage_id": "M4-ST-S3",
        "overall_pass": len(new_ci) == 0 and len(inj_issues) == 0 and len(recovery_issues) == 0 and (inj_detected == inj_expected),
        "issues": control_issues,
        "s2_baseline_issues": s2_issues,
        "new_issues_vs_s2": new_ci,
        "s2_dependency_summary": s2.get("summary", {}) if s2 else {},
        "probe_failure_counts": fail_counts,
        "injection_results": [
            {
                "injection_id": x.get("injection_id"),
                "observed_classification": x.get("observed_classification"),
                "detected": bool(x.get("detected")),
            }
    ...
    cost = {
        ...
        "max_spend_usd": max_sp,
        "within_envelope": cost_within,
        "method": "bounded_failure_injection_readonly_v0",
    }
    dumpj(out / "m4_cost_outcome_receipt.json", cost)
    ...
    dlog = {
        ...
        "stage_id": "M4-ST-S3",
        "decisions": [
            "Validated S2 continuity and carried Stage-A artifacts forward.",
            "Executed bounded failure-injection set and deterministic classifier checks.",
            "Executed immediate and final recovery probes within recovery-budget gate.",
            "Compared S3 control posture against latest successful S2 baseline.",
            "Applied fail-closed blocker mapping for M4 S3.",
        ],
    }
    dumpj(out / "m4_decision_log.json", dlog)
    ...
    summ = {
        ...
        "stage_id": "M4-ST-S3",
        "overall_pass": len(blockers) == 0,
        "next_gate": "M4_ST_S4_READY" if len(blockers) == 0 else "BLOCKED",
        "required_artifacts": S3_ARTS,
        "probe_count": total,
        "error_rate_pct": er,
        "injection_expected_count": inj_expected,
        "injection_detected_count": inj_detected,
    ...
    miss = [n for n in S3_ARTS if not (out / n).exists()]
    if miss:
        blockers.append({"id": "M4-ST-B9", "severity": "S3", "status": "OPEN", "details": {"missing_artifacts": miss}})
        bref["overall_pass"] = False
        bref["open_blocker_count"] = len(blockers)
        bref["blockers"] = blockers
        summ["overall_pass"] = False
    ...
    print(f"[m4_s3] phase_execution_id={phase_id}")
    print(f"[m4_s3] output_dir={out.as_posix()}")
    print(f"[m4_s3] overall_pass={summ['overall_pass']}")
    print(f"[m4_s3] next_gate={summ['next_gate']}")
    print(f"[m4_s3] injection_detected={inj_detected}/{inj_expected}")
    print(f"[m4_s3] error_rate_pct={er}")
    print(f"[m4_s3] open_blockers={bref['open_blocker_count']}")
    return 0 if summ["overall_pass"] else 2
...
def run_s4(phase_id: str, out_root: Path) -> int:
    ...
    copy_errs = copy_stagea_from_stage(out_root, out, "m4_stress_s3_", "M4-ST-S3")
    if copy_errs:
        blockers.append({"id": "M4-ST-B9", "severity": "S4", "status": "OPEN", "details": {"copy_errors": copy_errs}})

    s3 = load_latest_successful_stage(out_root, "m4_stress_s3_", "M4-ST-S3")
    if not s3:
        blockers.append({"id": "M4-ST-B9", "severity": "S4", "status": "OPEN", "details": {"missing_dependency": "successful M4 S3"}})

    stage_snaps = {
        "S1": load_latest_stage_snapshot(out_root, "m4_stress_s1_"),
        "S2": load_latest_stage_snapshot(out_root, "m4_stress_s2_"),
        "S3": load_latest_stage_snapshot(out_root, "m4_stress_s3_"),
    }

    missing_stage_registers = [sid for sid, snap in stage_snaps.items() if not snap or not snap.get("blocker_register")]
    if missing_stage_registers:
        blockers.append(
            {
                "id": "M4-ST-B9",
                "severity": "S4",
                "status": "OPEN",
                "details": {"missing_stage_blocker_registers": missing_stage_registers},
            }
        )

    closure_states = {"CLOSED", "WAIVED", "POLICY_CLOSED"}
    unresolved: list[dict[str, Any]] = []
    for sid, snap in stage_snaps.items():
        if not snap:
            continue
        br = snap.get("blocker_register", {}) if isinstance(snap, dict) else {}
        items = br.get("blockers", []) if isinstance(br, dict) else []
        ...
        for b in items:
            ...
            st = str(b.get("status", "OPEN")).strip().upper()
            if not bid:
                continue
            if st in closure_states:
                continue
            unresolved.append(
                {
                    "source_stage": sid,
                    "phase_execution_id": str((snap.get("summary", {}) or {}).get("phase_execution_id", "")),
                    "id": bid,
                    "status": st or "OPEN",
                    "severity": str(b.get("severity", sid)),
                    "details": b.get("details", {}),
                }
            )

    lane_map = {
        "M4-ST-B1": "authority_handle_contract",
        "M4-ST-B2": "runtime_path_readiness",
        "M4-ST-B3": "identity_network_dependency",
        "M4-ST-B4": "correlation_continuity",
        "M4-ST-B5": "runtime_health_stability",
        "M4-ST-B6": "secret_safety",
        "M4-ST-B7": "failure_injection_recovery",
        "M4-ST-B8": "cost_envelope",
        "M4-ST-B9": "artifact_contract",
    }
    remediation_matrix: list[dict[str, Any]] = []
    for b in unresolved:
        sid = str(b.get("source_stage", ""))
        bid = str(b.get("id", ""))
        remediation_matrix.append(
            {
                "source_stage": sid,
                "source_phase_execution_id": str(b.get("phase_execution_id", "")),
                "blocker_id": bid,
                "severity": str(b.get("severity", sid)),
                "remediation_lane": lane_map.get(bid, "manual_triage"),
                "rerun_scope": sid,
                "closure_evidence_path": f"{out_root.as_posix()}/{{rerun_phase_execution_id}}/stress/m4_blocker_register.json",
                "details": b.get("details", {}),
            }
        )

    rerun_scopes = sorted({str(x.get("rerun_scope", "")) for x in remediation_matrix if str(x.get("rerun_scope", ""))})
    rerun_required = len(remediation_matrix) > 0

    remediation_receipt = {
        "generated_at_utc": now(),
        "phase_execution_id": phase_id,
        "stage_id": "M4-ST-S4",
        "s3_dependency_phase_execution_id": s3.get("summary", {}).get("phase_execution_id", "") if s3 else "",
        "remediation_required": rerun_required,
        "rerun_required": rerun_required,
        "rerun_scopes": rerun_scopes,
        "action_mode": "NOOP_RECEIPT" if not rerun_required else "RERUN_RECOMMENDATION_ONLY",
        "remediation_matrix": remediation_matrix,
        "note": "S4 emits deterministic no-op or rerun recommendation; no implicit infra mutation is executed by this lane.",
    }
    ...
    cost_within = True
    if not cost_within:
        blockers.append({"id": "M4-ST-B8", "severity": "S4", "status": "OPEN"})
    ...
    probe = {
        ...
        "stage_id": "M4-ST-S4",
        "window_seconds_observed": dur_s,
        "probe_count": estimated_api_call_count,
        "failure_count": 0,
        "error_rate_pct": 0.0,
        "latency_ms_p50": 0.0,
        "latency_ms_p95": 0.0,
        ...
        "rerun_required": rerun_required,
        "rerun_scopes": rerun_scopes,
        "s3_baseline_phase_execution_id": s3.get("summary", {}).get("phase_execution_id", "") if s3 else "",
    }
    dumpj(out / "m4_probe_latency_throughput_snapshot.json", probe)
    ...
    ctrl = {
        ...
        "phase_execution_id": phase_id,
        "stage_id": "M4-ST-S4",
        "overall_pass": len(unresolved) == 0,
        "issues": [f"unresolved blockers present: {len(unresolved)}"] if unresolved else [],
        "unresolved_blockers": unresolved,
        "remediation_matrix": remediation_matrix,
        "latest_stage_summaries": {
            sid: (snap.get("summary", {}) if snap else {}) for sid, snap in stage_snaps.items()
        },
        "s3_dependency_summary": s3.get("summary", {}) if s3 else {},
    }
    dumpj(out / "m4_control_rail_conformance_snapshot.json", ctrl)
    ...
    cost = {
        ...
        "max_spend_usd": max_sp,
        "within_envelope": cost_within,
        "method": "read_only_blocker_adjudication_v0",
    }
    dumpj(out / "m4_cost_outcome_receipt.json", cost)
    ...
    dlog = {
        ...
        "stage_id": "M4-ST-S4",
        "decisions": [
            "Validated S3 dependency and Stage-A carry-forward prerequisites.",
            "Aggregated latest S1/S2/S3 blocker posture for unresolved blockers.",
            "Built deterministic blocker-to-lane remediation matrix with minimal rerun scopes.",
            "Emitted explicit no-op remediation receipt when unresolved blockers are absent.",
            "Applied fail-closed blocker mapping for M4 S4.",
        ],
    ...
    if unresolved:
        blockers.extend(
            [
                {
                    "id": str(x.get("id", "")),
                    "severity": str(x.get("severity", "S4")),
                    "status": str(x.get("status", "OPEN")),
                    "details": {
                        "source_stage": str(x.get("source_stage", "")),
                        "source_phase_execution_id": str(x.get("phase_execution_id", "")),
                        "source_details": x.get("details", {}),
                    },
                }
                for x in unresolved
            ]
    ...
    summ = {
        ...
        "stage_id": "M4-ST-S4",
        "overall_pass": len(blockers) == 0,
        "next_gate": "M4_ST_S5_READY" if len(blockers) == 0 else "BLOCKED",
        "required_artifacts": S4_ARTS,
        "probe_count": estimated_api_call_count,
        "error_rate_pct": 0.0,
        "remediation_required": rerun_required,
        "rerun_required": rerun_required,
    ...
    miss = [n for n in S4_ARTS if not (out / n).exists()]
    if miss:
        blockers.append({"id": "M4-ST-B9", "severity": "S4", "status": "OPEN", "details": {"missing_artifacts": miss}})
        bref["overall_pass"] = False
        bref["open_blocker_count"] = len(blockers)
        bref["blockers"] = blockers
        summ["overall_pass"] = False
    ...
    print(f"[m4_s4] phase_execution_id={phase_id}")
    print(f"[m4_s4] output_dir={out.as_posix()}")
    print(f"[m4_s4] overall_pass={summ['overall_pass']}")
    print(f"[m4_s4] next_gate={summ['next_gate']}")
    print(f"[m4_s4] remediation_required={summ['remediation_required']}")
    print(f"[m4_s4] rerun_required={summ['rerun_required']}")
    print(f"[m4_s4] rerun_scopes={','.join(rerun_scopes) if rerun_scopes else 'NONE'}")
    print(f"[m4_s4] open_blockers={bref['open_blocker_count']}")
    return 0 if summ["overall_pass"] else 2
...
def run_s5(phase_id: str, out_root: Path) -> int:
    ...
    copy_errs = copy_stagea_from_stage(out_root, out, "m4_stress_s4_", "M4-ST-S4")
    if copy_errs:
        blockers.append({"id": "M4-ST-B9", "severity": "S5", "status": "OPEN", "details": {"copy_errors": copy_errs}})

    s4 = load_latest_successful_stage(out_root, "m4_stress_s4_", "M4-ST-S4")
    if not s4:
        blockers.append({"id": "M4-ST-B9", "severity": "S5", "status": "OPEN", "details": {"missing_dependency": "successful M4 S4"}})

    expected_next_gate = str(pkt.get("M4_STRESS_EXPECTED_NEXT_GATE_ON_PASS", "M5_READY")).strip() or "M5_READY"

    stage_defs = [
        ("S0", "m4_stress_s0_", "M4-ST-S0"),
        ("S1", "m4_stress_s1_", "M4-ST-S1"),
        ("S2", "m4_stress_s2_", "M4-ST-S2"),
        ("S3", "m4_stress_s3_", "M4-ST-S3"),
        ("S4", "m4_stress_s4_", "M4-ST-S4"),
    ]
    stage_rollup: dict[str, dict[str, Any]] = {}
    artifact_missing: dict[str, list[str]] = {}
    unresolved_blockers: list[dict[str, Any]] = []
    closure_states = {"CLOSED", "WAIVED", "POLICY_CLOSED"}

    for sid, pfx, stg in stage_defs:
        ref = load_latest_successful_stage(out_root, pfx, stg)
        if not ref:
            blockers.append({"id": "M4-ST-B9", "severity": "S5", "status": "OPEN", "details": {"missing_successful_stage": sid}})
            continue
        sp = Path(str(ref.get("path", "")))
        summ = ref.get("summary", {}) if isinstance(ref.get("summary", {}), dict) else {}
        br = load_json_safe(sp / "m4_blocker_register.json")
        sec = load_json_safe(sp / "m4_secret_safety_snapshot.json")
        cost = load_json_safe(sp / "m4_cost_outcome_receipt.json")
        probe = load_json_safe(sp / "m4_probe_latency_throughput_snapshot.json")

        missing = [n for n in S0_ARTS if not (sp / n).exists()]
        if missing:
            artifact_missing[sid] = missing

        br_items = br.get("blockers", []) if isinstance(br, dict) else []
        ...
        for b in br_items:
            ...
            bid = str(b.get("id", "")).strip()
            st = str(b.get("status", "OPEN")).strip().upper()
            if not bid or st in closure_states:
                continue
            open_count += 1
            unresolved_blockers.append(
                {
                    "source_stage": sid,
                    "source_phase_execution_id": str(summ.get("phase_execution_id", "")),
                    "id": bid,
                    "status": st or "OPEN",
                    "severity": str(b.get("severity", sid)),
                    "details": b.get("details", {}),
        ...
        stage_rollup[sid] = {
            "stage_id": stg,
            "path": sp.as_posix(),
            "phase_execution_id": str(summ.get("phase_execution_id", "")),
            "overall_pass": bool(summ.get("overall_pass") is True),
            "next_gate": str(summ.get("next_gate", "")),
            "open_blocker_count": open_count,
            "missing_artifacts": missing,
            "runtime_seconds": int(cost.get("window_seconds", 0)) if str(cost.get("window_seconds", "")).strip() else 0,
            "attributed_spend_usd": float(cost.get("attributed_spend_usd", 0.0)),
            "unattributed_spend_detected": bool(cost.get("unattributed_spend_detected", False)),
            "plaintext_leakage_detected": bool(sec.get("plaintext_leakage_detected", False)),
            "probe_count": int(probe.get("probe_count", 0)) if str(probe.get("probe_count", "")).strip() else 0,
        }

    if artifact_missing:
        blockers.append({"id": "M4-ST-B9", "severity": "S5", "status": "OPEN", "details": {"missing_artifacts_by_stage": artifact_missing}})
    ...
    if unresolved_blockers:
        blockers.extend(
            [
                {
                    "id": str(x.get("id", "")),
                    "severity": str(x.get("severity", "S5")),
                    "status": str(x.get("status", "OPEN")),
                    "details": {
                        "source_stage": str(x.get("source_stage", "")),
                        "source_phase_execution_id": str(x.get("source_phase_execution_id", "")),
                        "source_details": x.get("details", {}),
                    },
                }
                for x in unresolved_blockers
            ]
        )

    total_runtime_seconds = sum(int(v.get("runtime_seconds", 0)) for v in stage_rollup.values())
    total_runtime_minutes = round(total_runtime_seconds / 60.0, 3)
    max_runtime_minutes = float(pkt.get("M4_STRESS_MAX_RUNTIME_MINUTES", 240))
    runtime_within = total_runtime_minutes <= max_runtime_minutes
    ...
    total_spend_usd = round(sum(float(v.get("attributed_spend_usd", 0.0)) for v in stage_rollup.values()), 6)
    unattributed_spend_detected = any(bool(v.get("unattributed_spend_detected", False)) for v in stage_rollup.values())
    max_spend_usd = float(pkt.get("M4_STRESS_MAX_SPEND_USD", 40))
    spend_within = (total_spend_usd <= max_spend_usd) and (not unattributed_spend_detected)
    if not spend_within:
        blockers.append(
            {
                "id": "M4-ST-B8",
                ...
                "status": "OPEN",
                "details": {
                    "total_attributed_spend_usd": total_spend_usd,
    ...
    plaintext_leakage_detected = any(bool(v.get("plaintext_leakage_detected", False)) for v in stage_rollup.values())
    if plaintext_leakage_detected:
        blockers.append({"id": "M4-ST-B6", "severity": "S5", "status": "OPEN", "details": {"issues": ["plaintext leakage signal in stage rollup"]}})

    recommendation = "GO" if len(blockers) == 0 else "NO_GO"
    next_gate = expected_next_gate if recommendation == "GO" else "BLOCKED"

    dur_s = max(1, int(round(time.perf_counter() - t0)))
    total_probe_count = sum(int(v.get("probe_count", 0)) for v in stage_rollup.values())
    probe = {
        "generated_at_utc": now(),
        "phase_execution_id": phase_id,
        "stage_id": "M4-ST-S5",
        ...
        "failure_count": 0 if len(blockers) == 0 else len(blockers),
        "error_rate_pct": 0.0 if len(blockers) == 0 else 100.0,
        "latency_ms_p50": 0.0,
        "latency_ms_p95": 0.0,
        "latency_ms_p99": 0.0,
        "sample_failures": blockers[:10],
        "rollup_stage_count": len(stage_rollup),
        "total_runtime_minutes": total_runtime_minutes,
        "total_attributed_spend_usd": total_spend_usd,
        "runtime_within_envelope": runtime_within,
        "spend_within_envelope": spend_within,
    }
    dumpj(out / "m4_probe_latency_throughput_snapshot.json", probe)
    ...
    ctrl = {
        ...
        "stage_id": "M4-ST-S5",
        "overall_pass": len(blockers) == 0,
        "issues": [] if len(blockers) == 0 else [f"open blockers in rollup: {len(blockers)}"],
        "unresolved_blockers": unresolved_blockers,
        "artifact_missing_by_stage": artifact_missing,
        "stage_rollup": stage_rollup,
        "expected_next_gate_on_pass": expected_next_gate,
        "s4_dependency_summary": s4.get("summary", {}) if s4 else {},
    }
    dumpj(out / "m4_control_rail_conformance_snapshot.json", ctrl)
    ...
    sec = {
        ...
        "phase_execution_id": phase_id,
        "stage_id": "M4-ST-S5",
        "overall_pass": not plaintext_leakage_detected,
        "secret_probe_count": 0,
        "secret_failure_count": 0,
        "with_decryption_used": False,
        "queried_value_directly": False,
        "suspicious_output_probe_ids": [],
        "plaintext_leakage_detected": plaintext_leakage_detected,
    }
    dumpj(out / "m4_secret_safety_snapshot.json", sec)
    ...
    cost = {
        ...
        "window_seconds": dur_s,
        "estimated_api_call_count": 0,
        "attributed_spend_usd": total_spend_usd,
        "unattributed_spend_detected": unattributed_spend_detected,
        "max_spend_usd": max_spend_usd,
        "within_envelope": spend_within,
        "method": "read_only_closure_rollup_v0",
    }
    dumpj(out / "m4_cost_outcome_receipt.json", cost)
    ...
    dlog = {
        ...
        "stage_id": "M4-ST-S5",
        "decisions": [
            "Validated S4 dependency and Stage-A carry-forward prerequisites.",
            "Aggregated latest successful S0..S4 summaries and blocker registers.",
            "Audited artifact completeness and no-open-blocker posture across rollup.",
            "Checked runtime/spend envelopes against pinned M4 budgets.",
            f"Published deterministic handoff recommendation: {recommendation} ({next_gate}).",
        ],
        "handoff_recommendation": {
            ...
            "total_attributed_spend_usd": total_spend_usd,
            "max_spend_usd": max_spend_usd,
            "stage_rollup": {k: {"phase_execution_id": v.get("phase_execution_id", ""), "open_blocker_count": v.get("open_blocker_count", 0)} for k, v in stage_rollup.items()},
        },
    }
    dumpj(out / "m4_decision_log.json", dlog)
    ...
    summ = {
        ...
        "runtime_within_envelope": runtime_within,
        "spend_within_envelope": spend_within,
        "s4_baseline_phase_execution_id": s4.get("summary", {}).get("phase_execution_id", "") if s4 else "",
    }
    dumpj(out / "m4_blocker_register.json", bref)
    dumpj(out / "m4_execution_summary.json", summ)

    miss = [n for n in S5_ARTS if not (out / n).exists()]
    if miss:
        blockers.append({"id": "M4-ST-B9", "severity": "S5", "status": "OPEN", "details": {"missing_artifacts": miss}})
    ...
    print(f"[m4_s5] phase_execution_id={phase_id}")
    print(f"[m4_s5] output_dir={out.as_posix()}")
    print(f"[m4_s5] overall_pass={summ['overall_pass']}")
    print(f"[m4_s5] recommendation={summ['recommendation']}")
    print(f"[m4_s5] next_gate={summ['next_gate']}")
    print(f"[m4_s5] total_runtime_minutes={summ['total_runtime_minutes']}")
    print(f"[m4_s5] total_attributed_spend_usd={summ['total_attributed_spend_usd']}")
    print(f"[m4_s5] open_blockers={bref['open_blocker_count']}")
    return 0 if summ["overall_pass"] else 2
...
def main() -> int:
    ...
    a = ap.parse_args()
    pfx = {"S0": "m4_stress_s0", "S1": "m4_stress_s1", "S2": "m4_stress_s2", "S3": "m4_stress_s3", "S4": "m4_stress_s4", "S5": "m4_stress_s5"}[a.stage]
