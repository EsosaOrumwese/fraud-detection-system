#!/usr/bin/env python3
"""Dispatch M11.F managed lane and materialize local evidence artifacts."""
...
def main() -> int:
    env = dict(os.environ)
    execution_id = env.get("M11F_EXECUTION_ID", "").strip()
    run_dir_raw = env.get("M11F_RUN_DIR", "").strip()
    evidence_bucket = env.get("EVIDENCE_BUCKET", "").strip()
    upstream_m11e_execution = env.get("UPSTREAM_M11E_EXECUTION", "").strip()
    ...
    if not execution_id:
        raise SystemExit("M11F_EXECUTION_ID is required.")
    if not run_dir_raw:
        raise SystemExit("M11F_RUN_DIR is required.")
    if not evidence_bucket:
        raise SystemExit("EVIDENCE_BUCKET is required.")
    if not upstream_m11e_execution:
        raise SystemExit("UPSTREAM_M11E_EXECUTION is required.")
    ...
    rc, _, err = run_cmd(
        [
            ...
            "m11_subphase=F",
            "-f",
            f"upstream_m11e_execution={upstream_m11e_execution}",
            "-f",
            f"m11f_execution_id={execution_id}",
        ],
        timeout=150,
    )
    if rc != 0:
        blockers.append({"code": "M11-B6", "message": f"M11.F workflow dispatch failed ({err[:140]})."})
    ...
    if rc == 0:
        ...
        while time.time() < find_deadline:
            run_id, run_url = find_workflow_run(repo, branch, dispatch_start - timedelta(seconds=120), "M11.F")
            if run_id > 0:
                break
            time.sleep(5)
        if run_id <= 0:
            workflow_advisories.append("M11.F workflow run id not discoverable after dispatch.")
    ...
    if run_id > 0:
        ...
        if run_status.lower() != "completed" or run_conclusion.lower() != "success":
            workflow_advisories.append(f"M11.F workflow did not report success ({run_status}/{run_conclusion}).")

    s3 = boto3.client("s3", region_name=aws_region)
    snapshot_key = f"evidence/dev_full/run_control/{execution_id}/m11f_mlflow_lineage_snapshot.json"
    register_key = f"evidence/dev_full/run_control/{execution_id}/m11f_blocker_register.json"
    summary_key = f"evidence/dev_full/run_control/{execution_id}/m11f_execution_summary.json"

    snapshot: dict[str, Any] | None = None
    register: dict[str, Any] | None = None
    summary: dict[str, Any] | None = None
    for key, target in ((snapshot_key, "snapshot"), (register_key, "register"), (summary_key, "summary")):
        try:
            ...
            blockers.append({"code": "M11-B6", "message": f"M11.F artifact unreadable: {target}."})

    if summary is not None:
        if not bool(summary.get("overall_pass")):
            blockers.append({"code": "M11-B6", "message": "M11.F summary overall_pass is false."})
        if str(summary.get("next_gate", "")).strip() != "M11.G_READY":
            blockers.append({"code": "M11-B6", "message": "M11.F summary next_gate is not M11.G_READY."})
    ...
    if register and isinstance(register.get("blockers"), list):
        for row in register.get("blockers", []):
            ...
            if any(token in text for token in ("resourcelimit", "quota", "throttl", "accessdenied", "access denied")):
                blockers.append({"code": "M11-B19", "message": "M11.F indicates quota/capacity/access boundary pressure."})
                break

    lane_gate_pass = bool(summary and bool(summary.get("overall_pass")) and str(summary.get("next_gate", "")).strip() == "M11.G_READY")
    ...
    if workflow_advisories:
        if lane_gate_pass:
            ...
            for msg in workflow_advisories:
                blockers.append({"code": "M11-B6", "message": msg})
    ...
    write_local_artifacts(
        run_dir,
        {
            "m11f_mlflow_lineage_snapshot.json": snapshot,
            "m11f_blocker_register.json": register,
