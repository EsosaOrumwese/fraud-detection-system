#!/usr/bin/env python3
"""Dispatch M12.G managed lane and materialize local evidence artifacts."""
...
def main() -> int:
    env = dict(os.environ)
    execution_id = env.get("M12G_EXECUTION_ID", "").strip()
    run_dir_raw = env.get("M12G_RUN_DIR", "").strip()
    ...
    upstream_m12g_execution = env.get("M12_UPSTREAM_M12G_EXECUTION", "M12_S3_NOT_APPLICABLE").strip() or "M12_S3_NOT_APPLICABLE"
    upstream_m12h_execution = env.get("M12_UPSTREAM_M12H_EXECUTION", "M12_S3_NOT_APPLICABLE").strip() or "M12_S3_NOT_APPLICABLE"
    upstream_m12i_execution = env.get("M12_UPSTREAM_M12I_EXECUTION", "M12_S3_NOT_APPLICABLE").strip() or "M12_S3_NOT_APPLICABLE"

    if not execution_id:
        raise SystemExit("M12G_EXECUTION_ID is required.")
    if not run_dir_raw:
        raise SystemExit("M12G_RUN_DIR is required.")
    ...
    if rc != 0:
        blockers.append({"code": "M12-B7", "message": f"M12.G workflow dispatch failed ({err[:140]})."})
    ...
    if rc == 0:
        ...
        while time.time() < find_deadline:
            run_id, run_url = find_workflow_run(repo, branch, dispatch_start - timedelta(seconds=120), "M12.G")
            if run_id > 0:
                break
            time.sleep(5)
        if run_id <= 0:
            workflow_advisories.append("M12.G workflow run id not discoverable after dispatch.")
    ...
    if run_id > 0:
        ...
        if run_status.lower() != "completed" or run_conclusion.lower() != "success":
            workflow_advisories.append(f"M12.G workflow did not report success ({run_status}/{run_conclusion}).")

    s3 = boto3.client("s3", region_name=aws_region)
    snapshot_key = f"evidence/dev_full/run_control/{execution_id}/m12g_governance_append_snapshot.json"
    operability_key = f"evidence/dev_full/run_control/{execution_id}/m12_operability_acceptance_report.json"
    register_key = f"evidence/dev_full/run_control/{execution_id}/m12g_blocker_register.json"
    summary_key = f"evidence/dev_full/run_control/{execution_id}/m12g_execution_summary.json"

    snapshot: dict[str, Any] | None = None
    operability: dict[str, Any] | None = None
    register: dict[str, Any] | None = None
    summary: dict[str, Any] | None = None
    for key, target in (
        (snapshot_key, "snapshot"),
        (operability_key, "operability"),
        ...
        try:
            ...
            blockers.append({"code": "M12-B7", "message": f"M12.G artifact unreadable: {target}."})
    ...
    if register is not None and isinstance(register.get("blockers"), list):
        for row in register.get("blockers", []):
            ...
            if msg:
                blockers.append({"code": "M12-B7", "message": f"M12.G managed blocker: {msg}"})

    if summary is not None:
        if not bool(summary.get("overall_pass")):
            blockers.append({"code": "M12-B7", "message": "M12.G summary overall_pass is false."})
        if str(summary.get("next_gate", "")).strip() != "M12.H_READY":
            blockers.append({"code": "M12-B7", "message": "M12.G summary next_gate is not M12.H_READY."})
        if str(summary.get("verdict", "")).strip() != "ADVANCE_TO_M12_H":
            blockers.append({"code": "M12-B7", "message": "M12.G summary verdict is not ADVANCE_TO_M12_H."})

    lane_gate_pass = bool(
        summary
        and bool(summary.get("overall_pass"))
        and str(summary.get("next_gate", "")).strip() == "M12.H_READY"
        and str(summary.get("verdict", "")).strip() == "ADVANCE_TO_M12_H"
    )
    ...
    if workflow_advisories:
        if lane_authority_available:
            ...
            for msg in workflow_advisories:
                blockers.append({"code": "M12-B7", "message": msg})
    ...
    if snapshot is None:
        snapshot = {
            ...
            "execution_id": execution_id,
            "overall_pass": False,
            "note": "Governance append snapshot missing from managed output.",
            "upstream_refs": {
                "m11j_execution_id": upstream_m11j_execution,
                "m12a_execution_id": upstream_m12a_execution,
                "m12b_execution_id": upstream_m12b_execution,
    ...
    if operability is None:
        operability = {
            ...
            "execution_id": execution_id,
            "overall_pass": False,
            "note": "Operability acceptance report missing from managed output.",
            "source_refs": {
                "m12f_execution_id": upstream_m12f_execution,
                "m12e_execution_id": upstream_m12e_execution,
                "m12d_execution_id": upstream_m12d_execution,
    ...
    write_local_artifacts(
        run_dir,
        {
            "m12g_governance_append_snapshot.json": snapshot,
            "m12_operability_acceptance_report.json": operability,
            "m12g_blocker_register.json": register,
            "m12g_execution_summary.json": summary,
        },
    )
