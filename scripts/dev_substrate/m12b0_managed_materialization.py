#!/usr/bin/env python3
"""Dispatch M12.B0 managed lane and materialize local evidence artifacts."""
...
def main() -> int:
    env = dict(os.environ)
    execution_id = env.get("M12B0_EXECUTION_ID", "").strip()
    run_dir_raw = env.get("M12B0_RUN_DIR", "").strip()
    ...
    if not execution_id:
        raise SystemExit("M12B0_EXECUTION_ID is required.")
    if not run_dir_raw:
        raise SystemExit("M12B0_RUN_DIR is required.")
    ...
    if rc != 0:
        blockers.append({"code": "M12-B0", "message": f"M12.B0 workflow dispatch failed ({err[:140]})."})
    ...
    if rc == 0:
        ...
        while time.time() < find_deadline:
            ...
            time.sleep(5)
        if run_id <= 0:
            workflow_advisories.append("M12.B0 workflow run id not discoverable after dispatch.")
    ...
    if run_id > 0:
        ...
        if run_status.lower() != "completed" or run_conclusion.lower() != "success":
            workflow_advisories.append(f"M12.B0 workflow did not report success ({run_status}/{run_conclusion}).")

    s3 = boto3.client("s3", region_name=aws_region)
    snapshot_key = f"evidence/dev_full/run_control/{execution_id}/m12_managed_lane_materialization_snapshot.json"
    dispatch_key = f"evidence/dev_full/run_control/{execution_id}/m12_subphase_dispatchability_snapshot.json"
    register_key = f"evidence/dev_full/run_control/{execution_id}/m12b0_blocker_register.json"
    summary_key = f"evidence/dev_full/run_control/{execution_id}/m12b0_execution_summary.json"
    ...
    for key, target in (
        ...
        try:
            ...
            blockers.append({"code": "M12-B0", "message": f"M12.B0 artifact unreadable: {target}."})

    if summary is not None:
        if not bool(summary.get("overall_pass")):
            blockers.append({"code": "M12-B0", "message": "M12.B0 summary overall_pass is false."})
        if str(summary.get("next_gate", "")).strip() != "M12.A_READY":
            blockers.append({"code": "M12-B0", "message": "M12.B0 summary next_gate is not M12.A_READY."})
        if str(summary.get("verdict", "")).strip() != "ADVANCE_TO_M12_A":
            blockers.append({"code": "M12-B0", "message": "M12.B0 summary verdict is not ADVANCE_TO_M12_A."})

    lane_gate_pass = bool(
        summary
        and bool(summary.get("overall_pass"))
        and str(summary.get("next_gate", "")).strip() == "M12.A_READY"
        and str(summary.get("verdict", "")).strip() == "ADVANCE_TO_M12_A"
    )
    if workflow_advisories:
        if not lane_gate_pass:
            for msg in workflow_advisories:
                blockers.append({"code": "M12-B0", "message": msg})
    ...
    write_local_artifacts(
        run_dir,
        {
            ...
            "m12b0_blocker_register.json": register,
            "m12b0_execution_summary.json": summary,
        },
    )
