#!/usr/bin/env python3
"""Dispatch M11.C managed lane and materialize local evidence artifacts."""
...
WORKFLOW_FILE = "dev_full_m11_c_managed.yml"
...
def main() -> int:
    env = dict(os.environ)
    execution_id = env.get("M11C_EXECUTION_ID", "").strip()
    run_dir_raw = env.get("M11C_RUN_DIR", "").strip()
    evidence_bucket = env.get("EVIDENCE_BUCKET", "").strip()
    upstream_m11b_execution = env.get("UPSTREAM_M11B_EXECUTION", "").strip()
    ...
    if not execution_id:
        raise SystemExit("M11C_EXECUTION_ID is required.")
    if not run_dir_raw:
        raise SystemExit("M11C_RUN_DIR is required.")
    if not evidence_bucket:
        raise SystemExit("EVIDENCE_BUCKET is required.")
    if not upstream_m11b_execution:
        raise SystemExit("UPSTREAM_M11B_EXECUTION is required.")
    ...
    rc, _, err = run_cmd(
        [
            ...
            "-f",
            f"upstream_m11b_execution={upstream_m11b_execution}",
            "-f",
            f"m11c_execution_id={execution_id}",
        ],
        timeout=120,
    )
    if rc != 0:
        blockers.append({"code": "M11-B3", "message": f"M11.C workflow dispatch failed ({err[:140]})."})
    ...
    if rc == 0:
        ...
        while time.time() < find_deadline:
            ...
            time.sleep(5)
        if run_id <= 0:
            blockers.append({"code": "M11-B3", "message": "M11.C workflow run id not discoverable after dispatch."})
    ...
    if run_id > 0:
        ...
        if run_status.lower() != "completed" or run_conclusion.lower() != "success":
            blockers.append({"code": "M11-B3", "message": f"M11.C workflow did not succeed ({run_status}/{run_conclusion})."})

    s3 = boto3.client("s3", region_name=aws_region)
    snapshot_key = f"evidence/dev_full/run_control/{execution_id}/m11c_input_immutability_snapshot.json"
    register_key = f"evidence/dev_full/run_control/{execution_id}/m11c_blocker_register.json"
    summary_key = f"evidence/dev_full/run_control/{execution_id}/m11c_execution_summary.json"
    ...
    for key, target in ((snapshot_key, "snapshot"), (register_key, "register"), (summary_key, "summary")):
        try:
            ...
            blockers.append({"code": "M11-B3", "message": f"M11.C artifact unreadable: {target}."})

    if summary is not None:
        if not bool(summary.get("overall_pass")):
            blockers.append({"code": "M11-B3", "message": "M11.C summary overall_pass is false."})
        if str(summary.get("next_gate", "")).strip() != "M11.D_READY":
            blockers.append({"code": "M11-B3", "message": "M11.C summary next_gate is not M11.D_READY."})
    ...
    write_local_artifacts(
        run_dir,
        {
            "m11c_input_immutability_snapshot.json": snapshot,
            "m11c_blocker_register.json": register,
            "m11c_execution_summary.json": summary,
        },
    )
