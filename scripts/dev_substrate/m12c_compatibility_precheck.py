#!/usr/bin/env python3
"""Dispatch M12.C managed lane and materialize local evidence artifacts."""
...
def main() -> int:
    env = dict(os.environ)
    execution_id = env.get("M12C_EXECUTION_ID", "").strip()
    run_dir_raw = env.get("M12C_RUN_DIR", "").strip()
    ...
    if not execution_id:
        raise SystemExit("M12C_EXECUTION_ID is required.")
    if not run_dir_raw:
        raise SystemExit("M12C_RUN_DIR is required.")
    ...
    if rc != 0:
        blockers.append({"code": "M12-B3", "message": f"M12.C workflow dispatch failed ({err[:140]})."})
    ...
    if rc == 0:
        ...
        while time.time() < find_deadline:
            run_id, run_url = find_workflow_run(repo, branch, dispatch_start - timedelta(seconds=120), "M12.C")
            if run_id > 0:
                break
            time.sleep(5)
        if run_id <= 0:
            workflow_advisories.append("M12.C workflow run id not discoverable after dispatch.")
    ...
    if run_id > 0:
        ...
        if run_status.lower() != "completed" or run_conclusion.lower() != "success":
            workflow_advisories.append(f"M12.C workflow did not report success ({run_status}/{run_conclusion}).")

    s3 = boto3.client("s3", region_name=aws_region)
    snapshot_key = f"evidence/dev_full/run_control/{execution_id}/m12c_compatibility_precheck_snapshot.json"
    register_key = f"evidence/dev_full/run_control/{execution_id}/m12c_blocker_register.json"
    summary_key = f"evidence/dev_full/run_control/{execution_id}/m12c_execution_summary.json"
    ...
    if summary is not None:
        ...
        if str(summary.get("next_gate", "")).strip() != "M12.D_READY":
            blockers.append({"code": "M12-B3", "message": "M12.C summary next_gate is not M12.D_READY."})
        if str(summary.get("verdict", "")).strip() != "ADVANCE_TO_M12_D":
            blockers.append({"code": "M12-B3", "message": "M12.C summary verdict is not ADVANCE_TO_M12_D."})
    ...
    if snapshot is None:
        snapshot = {
            ...
            "execution_id": execution_id,
            "overall_pass": False,
            "note": "Compatibility precheck snapshot missing from managed output.",
            "upstream_refs": {
                "m11j_execution_id": upstream_m11j_execution,
                "m12a_execution_id": upstream_m12a_execution,
                "m12b_execution_id": upstream_m12b_execution,
    ...
    write_local_artifacts(
        run_dir,
        {
            "m12c_compatibility_precheck_snapshot.json": snapshot,
            "m12c_blocker_register.json": register,
            "m12c_execution_summary.json": summary,
        },
    )
