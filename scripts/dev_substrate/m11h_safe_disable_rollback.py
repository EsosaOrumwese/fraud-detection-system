#!/usr/bin/env python3
"""Dispatch M11.H managed lane and materialize local evidence artifacts."""
...
def main() -> int:
    env = dict(os.environ)
    execution_id = env.get("M11H_EXECUTION_ID", "").strip()
    run_dir_raw = env.get("M11H_RUN_DIR", "").strip()
    evidence_bucket = env.get("EVIDENCE_BUCKET", "").strip()
    upstream_m11g_execution = env.get("UPSTREAM_M11G_EXECUTION", "").strip()
    ...
    if not execution_id:
        raise SystemExit("M11H_EXECUTION_ID is required.")
    if not run_dir_raw:
        raise SystemExit("M11H_RUN_DIR is required.")
    if not evidence_bucket:
        raise SystemExit("EVIDENCE_BUCKET is required.")
    if not upstream_m11g_execution:
        raise SystemExit("UPSTREAM_M11G_EXECUTION is required.")
    ...
    rc, _, err = run_cmd(
        [
            ...
            "m11_subphase=H",
            "-f",
            f"upstream_m11g_execution={upstream_m11g_execution}",
            "-f",
            f"m11h_execution_id={execution_id}",
        ],
        timeout=150,
    )
    if rc != 0:
        blockers.append({"code": "M11-B8", "message": f"M11.H workflow dispatch failed ({err[:140]})."})
    ...
    if rc == 0:
        ...
        while time.time() < find_deadline:
            run_id, run_url = find_workflow_run(repo, branch, dispatch_start - timedelta(seconds=120), "M11.H")
            if run_id > 0:
                break
            time.sleep(5)
        if run_id <= 0:
            workflow_advisories.append("M11.H workflow run id not discoverable after dispatch.")
    ...
    if run_id > 0:
        ...
        if run_status.lower() != "completed" or run_conclusion.lower() != "success":
            workflow_advisories.append(f"M11.H workflow did not report success ({run_status}/{run_conclusion}).")

    s3 = boto3.client("s3", region_name=aws_region)
    reproducibility_key = f"evidence/dev_full/run_control/{execution_id}/m11_reproducibility_check.json"
    snapshot_key = f"evidence/dev_full/run_control/{execution_id}/m11h_safe_disable_rollback_snapshot.json"
    register_key = f"evidence/dev_full/run_control/{execution_id}/m11h_blocker_register.json"
    summary_key = f"evidence/dev_full/run_control/{execution_id}/m11h_execution_summary.json"
    ...
    for key, target in (
        ...
        try:
            ...
            blockers.append({"code": "M11-B8", "message": f"M11.H artifact unreadable: {target}."})

    if summary is not None:
        if not bool(summary.get("overall_pass")):
            blockers.append({"code": "M11-B8", "message": "M11.H summary overall_pass is false."})
        if str(summary.get("next_gate", "")).strip() != "M11.I_READY":
            blockers.append({"code": "M11-B8", "message": "M11.H summary next_gate is not M11.I_READY."})
    if reproducibility is not None and not bool(reproducibility.get("overall_pass")):
        blockers.append({"code": "M11-B8", "message": "M11.H reproducibility check is not pass posture."})
    ...
    if register and isinstance(register.get("blockers"), list):
        for row in register.get("blockers", []):
            ...
            if any(token in text for token in ("resourcelimit", "quota", "throttl", "accessdenied", "access denied")):
                blockers.append({"code": "M11-B19", "message": "M11.H indicates quota/capacity/access boundary pressure."})
                break

    lane_gate_pass = bool(summary and bool(summary.get("overall_pass")) and str(summary.get("next_gate", "")).strip() == "M11.I_READY")
    ...
    if workflow_advisories:
        if lane_gate_pass:
            ...
            for msg in workflow_advisories:
                blockers.append({"code": "M11-B8", "message": msg})
    ...
    if reproducibility is None:
        reproducibility = {
            ...
            "execution_id": execution_id,
            "overall_pass": False,
            "note": "Reproducibility check missing from managed output.",
        }
    ...
    write_local_artifacts(
        run_dir,
        {
            "m11_reproducibility_check.json": reproducibility,
            "m11h_safe_disable_rollback_snapshot.json": snapshot,
            "m11h_blocker_register.json": register,
            "m11h_execution_summary.json": summary,
        },
    )
