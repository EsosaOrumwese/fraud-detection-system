#!/usr/bin/env python3
"""Dispatch M11.G managed lane and materialize local evidence artifacts."""
...
def main() -> int:
    env = dict(os.environ)
    execution_id = env.get("M11G_EXECUTION_ID", "").strip()
    run_dir_raw = env.get("M11G_RUN_DIR", "").strip()
    evidence_bucket = env.get("EVIDENCE_BUCKET", "").strip()
    upstream_m11f_execution = env.get("UPSTREAM_M11F_EXECUTION", "").strip()
    ...
    if not execution_id:
        raise SystemExit("M11G_EXECUTION_ID is required.")
    if not run_dir_raw:
        raise SystemExit("M11G_RUN_DIR is required.")
    if not evidence_bucket:
        raise SystemExit("EVIDENCE_BUCKET is required.")
    if not upstream_m11f_execution:
        raise SystemExit("UPSTREAM_M11F_EXECUTION is required.")
    ...
    rc, _, err = run_cmd(
        [
            ...
            "m11_subphase=G",
            "-f",
            f"upstream_m11f_execution={upstream_m11f_execution}",
            "-f",
            f"m11g_execution_id={execution_id}",
        ],
        timeout=150,
    )
    if rc != 0:
        blockers.append({"code": "M11-B7", "message": f"M11.G workflow dispatch failed ({err[:140]})."})
    ...
    if rc == 0:
        ...
        while time.time() < find_deadline:
            run_id, run_url = find_workflow_run(repo, branch, dispatch_start - timedelta(seconds=120), "M11.G")
            if run_id > 0:
                break
            time.sleep(5)
        if run_id <= 0:
            workflow_advisories.append("M11.G workflow run id not discoverable after dispatch.")
    ...
    if run_id > 0:
        ...
        if run_status.lower() != "completed" or run_conclusion.lower() != "success":
            workflow_advisories.append(f"M11.G workflow did not report success ({run_status}/{run_conclusion}).")

    s3 = boto3.client("s3", region_name=aws_region)
    snapshot_key = f"evidence/dev_full/run_control/{execution_id}/m11g_candidate_bundle_snapshot.json"
    operability_key = f"evidence/dev_full/run_control/{execution_id}/m11_model_operability_report.json"
    register_key = f"evidence/dev_full/run_control/{execution_id}/m11g_blocker_register.json"
    summary_key = f"evidence/dev_full/run_control/{execution_id}/m11g_execution_summary.json"
    ...
    for key, target in (
        ...
        try:
            ...
            blockers.append({"code": "M11-B7", "message": f"M11.G artifact unreadable: {target}."})

    candidate_bundle: dict[str, Any] | None = None
    candidate_bundle_key = ""
    if summary is not None:
        if not bool(summary.get("overall_pass")):
            blockers.append({"code": "M11-B7", "message": "M11.G summary overall_pass is false."})
        if str(summary.get("next_gate", "")).strip() != "M11.H_READY":
            blockers.append({"code": "M11-B7", "message": "M11.G summary next_gate is not M11.H_READY."})
        art_keys = summary.get("artifact_keys", {})
        if isinstance(art_keys, dict):
            candidate_bundle_key = str(art_keys.get("mf_candidate_bundle_key", "")).strip()
    if not candidate_bundle_key and isinstance(snapshot, dict):
        candidate_block = snapshot.get("candidate_bundle", {})
        if isinstance(candidate_block, dict):
            candidate_bundle_key = str(candidate_block.get("bundle_key", "")).strip()
    if candidate_bundle_key:
        try:
            candidate_bundle = s3_get_json(s3, evidence_bucket, candidate_bundle_key)
        except (BotoCoreError, ClientError, ValueError, json.JSONDecodeError) as exc:
            read_errors.append({"surface": candidate_bundle_key, "error": type(exc).__name__})
            blockers.append({"code": "M11-B7", "message": "M11.G candidate bundle artifact unreadable."})
    else:
        blockers.append({"code": "M11-B7", "message": "M11.G candidate bundle key missing in lane evidence."})

    if operability is not None and not bool(operability.get("overall_pass")):
        blockers.append({"code": "M11-B7", "message": "M11.G model operability report is not pass posture."})
    ...
    if register and isinstance(register.get("blockers"), list):
        for row in register.get("blockers", []):
            ...
            if any(token in text for token in ("resourcelimit", "quota", "throttl", "accessdenied", "access denied")):
                blockers.append({"code": "M11-B19", "message": "M11.G indicates quota/capacity/access boundary pressure."})
                break

    lane_gate_pass = bool(summary and bool(summary.get("overall_pass")) and str(summary.get("next_gate", "")).strip() == "M11.H_READY")
    snapshot_notes: list[str] = []
    if isinstance(snapshot, dict) and isinstance(snapshot.get("notes"), list):
        snapshot_notes = [str(x) for x in snapshot.get("notes", [])]
    if workflow_advisories:
        if lane_gate_pass:
            snapshot_notes.extend(workflow_advisories)
            if isinstance(snapshot, dict):
                snapshot["notes"] = snapshot_notes
        else:
            for msg in workflow_advisories:
                blockers.append({"code": "M11-B7", "message": msg})
    ...
    if snapshot is None:
        snapshot = {
            ...
            "read_errors": read_errors,
            "notes": snapshot_notes if snapshot_notes else workflow_advisories,
        }
    if operability is None:
        operability = {
            ...
            "execution_id": execution_id,
            "overall_pass": False,
            "note": "Model operability report missing from managed output.",
        }
    ...
    artifacts: dict[str, dict[str, Any]] = {
        ...
        "m11_model_operability_report.json": operability,
        "m11g_blocker_register.json": register,
        "m11g_execution_summary.json": summary,
    }
    if candidate_bundle is not None:
        artifacts["m11_candidate_bundle.json"] = candidate_bundle
