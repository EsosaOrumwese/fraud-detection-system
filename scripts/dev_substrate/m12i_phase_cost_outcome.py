#!/usr/bin/env python3
"""Dispatch M12.I managed lane and materialize local evidence artifacts."""
...
def main() -> int:
    env = dict(os.environ)
    execution_id = env.get("M12I_EXECUTION_ID", "").strip()
    run_dir_raw = env.get("M12I_RUN_DIR", "").strip()
    ...
    if not execution_id:
        raise SystemExit("M12I_EXECUTION_ID is required.")
    if not run_dir_raw:
        raise SystemExit("M12I_RUN_DIR is required.")
    ...
    if rc != 0:
        blockers.append({"code": "M12-B10", "message": f"M12.I workflow dispatch failed ({err[:140]})."})
    ...
    if rc == 0:
        ...
        while time.time() < find_deadline:
            run_id, run_url = find_workflow_run(repo, branch, dispatch_start - timedelta(seconds=120), "M12.I")
            if run_id > 0:
                break
            time.sleep(5)
        if run_id <= 0:
            workflow_advisories.append("M12.I workflow run id not discoverable after dispatch.")
    ...
    if run_id > 0:
        ...
        if run_status.lower() != "completed" or run_conclusion.lower() != "success":
            workflow_advisories.append(f"M12.I workflow did not report success ({run_status}/{run_conclusion}).")

    s3 = boto3.client("s3", region_name=aws_region)
    envelope_key = f"evidence/dev_full/run_control/{execution_id}/m12_phase_budget_envelope.json"
    receipt_key = f"evidence/dev_full/run_control/{execution_id}/m12_phase_cost_outcome_receipt.json"
    register_key = f"evidence/dev_full/run_control/{execution_id}/m12i_blocker_register.json"
    summary_key = f"evidence/dev_full/run_control/{execution_id}/m12i_execution_summary.json"
    ...
    for key, target in (
        ...
        try:
            ...
            blockers.append({"code": "M12-B10", "message": f"M12.I artifact unreadable: {target}."})
    ...
    if register is not None and isinstance(register.get("blockers"), list):
        for row in register.get("blockers", []):
            ...
            if msg:
                blockers.append({"code": "M12-B10", "message": f"M12.I managed blocker: {msg}"})

    if summary is not None:
        if not bool(summary.get("overall_pass")):
            blockers.append({"code": "M12-B10", "message": "M12.I summary overall_pass is false."})
        if str(summary.get("next_gate", "")).strip() != "M12.J_READY":
            blockers.append({"code": "M12-B10", "message": "M12.I summary next_gate is not M12.J_READY."})
        if str(summary.get("verdict", "")).strip() != "ADVANCE_TO_M12_J":
            blockers.append({"code": "M12-B10", "message": "M12.I summary verdict is not ADVANCE_TO_M12_J."})

    lane_gate_pass = bool(
        summary
        and bool(summary.get("overall_pass"))
        and str(summary.get("next_gate", "")).strip() == "M12.J_READY"
        and str(summary.get("verdict", "")).strip() == "ADVANCE_TO_M12_J"
    )
    ...
    if workflow_advisories:
        if lane_authority_available:
            notes: list[str] = []
            if isinstance(receipt, dict) and isinstance(receipt.get("notes"), list):
                notes = [str(x) for x in receipt.get("notes", []) if str(x).strip()]
            ...
            if isinstance(receipt, dict):
                receipt["notes"] = deduped_notes
        elif not lane_gate_pass:
            for msg in workflow_advisories:
                blockers.append({"code": "M12-B10", "message": msg})
    ...
    if summary is None:
        summary = {
            ...
            "upstream_refs": {"m12h_execution_id": upstream_m12h_execution},
        }
    if register is None:
        register = {
            "captured_at_utc": captured_at,
    ...
    if receipt is None:
        receipt = {
            ...
            "upstream_m12h_execution": upstream_m12h_execution,
        }
    ...
    write_local_artifacts(
        run_dir,
        {
            "m12_phase_budget_envelope.json": envelope,
            "m12_phase_cost_outcome_receipt.json": receipt,
            "m12i_blocker_register.json": register,
            "m12i_execution_summary.json": summary,
        },
    )
