"""CLI runner for Segment 6B (S0 gate plus optional S1-S5)."""
...
from engine.scenario_runner.l3_seg_6B import Segment6BConfig, Segment6BOrchestrator, Segment6BResult

logger = logging.getLogger(__name__)


def _print_summary(result: Segment6BResult) -> None:
    logger.info("Segment6B completed: manifest=%s", result.manifest_fingerprint)
    logger.info("  sealed_inputs=%s", result.sealed_inputs_path)
    if result.s1_arrival_entities_paths:
        logger.info("  arrivals=%s", list(result.s1_arrival_entities_paths.values())[:1])
    if result.s5_bundle_index_path:
        logger.info("  validation_bundle=%s", result.s5_bundle_index_path)


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run Segment 6B S0 gate (and optional S1-S5).")
    ...
    parser.add_argument("--dictionary-path", type=Path, help="Override dictionary path for 6B")
    ...
    parser.add_argument("--validation-bundle-5b", type=Path, help="Override validation bundle path for 5B")
    parser.add_argument("--validation-bundle-6a", type=Path, help="Override validation bundle path for 6A")
    parser.add_argument("--no-s1", action="store_true", help="Skip S1 arrivals")
    parser.add_argument("--no-s2", action="store_true", help="Skip S2 baseline flows")
    parser.add_argument("--no-s3", action="store_true", help="Skip S3 fraud overlay")
    parser.add_argument("--no-s4", action="store_true", help="Skip S4 labels")
...
def main() -> None:
    ...
    config = Segment6BConfig(
        ...
        validation_bundle_5b=args.validation_bundle_5b,
        validation_bundle_6a=args.validation_bundle_6a,
    ...
    orchestrator = Segment6BOrchestrator()
    ...
    if args.result_json:
        ...
        logger.info("Segment6B CLI: wrote summary to %s", args.result_json)
