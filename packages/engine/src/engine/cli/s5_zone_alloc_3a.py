"""CLI for Segment 3A S5 zone allocation egress."""
...
from engine.layers.l1.seg_3A.s5_zone_alloc.runner import run_s5


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run Segment 3A S5 zone allocation egress + routing hash.")
...
def main() -> None:
    logger = get_logger("engine.layers.l1.seg_3A.s5_zone_alloc.cli")
    ...
    result = run_s5(cfg, run_id=args.run_id)
    logger.info(
        "S5 3A complete: run_id=%s parameter_hash=%s manifest_fingerprint=%s output=%s universe_hash=%s",
        result.run_id,
        result.parameter_hash,
        result.manifest_fingerprint,
        result.output_path,
        result.universe_hash_path,
    )
