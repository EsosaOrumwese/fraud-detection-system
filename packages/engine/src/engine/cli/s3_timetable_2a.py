"""CLI for Segment 2A S3 timetable cache."""
...
from engine.layers.l1.seg_2A.s3_timetable.runner import run_s3


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run Segment 2A S3 timetable cache.")
...
def main() -> None:
    logger = get_logger("engine.layers.l1.seg_2A.s3_timetable.cli")
    ...
    logger.info(
        "S3 2A complete: run_id=%s parameter_hash=%s manifest_fingerprint=%s",
        result.run_id,
