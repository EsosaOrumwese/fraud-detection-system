"""CLI for Segment 5A S2 weekly shape library."""
...
from engine.layers.l2.seg_5A.s2_weekly_shape_library.runner import run_s2


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run Segment 5A S2 weekly shape library.")
...
def main() -> None:
    logger = get_logger("engine.layers.l2.seg_5A.s2_weekly_shape_library.cli")
    ...
    logger.info(
        "S2 5A complete: run_id=%s parameter_hash=%s manifest_fingerprint=%s",
        result.run_id,
