...
from engine.layers.l1.seg_2A.s0_gate.runner import run_s0


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run Segment 2A S0 gate-in.")
...
def main() -> None:
    logger = get_logger("engine.layers.l1.seg_2A.s0_gate.cli")
    ...
    logger.info(
        "S0 2A complete: run_id=%s parameter_hash=%s manifest_fingerprint=%s",
        result.run_id,
