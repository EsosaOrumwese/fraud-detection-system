"""CLI for Segment 6A S3 instrument base."""
...
from engine.layers.l3.seg_6A.s3_instruments.runner import run_s3


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run Segment 6A S3 instrument base.")
...
def main() -> None:
    logger = get_logger("engine.layers.l3.seg_6A.s3_instruments.cli")
    ...
    result = run_s3(cfg, run_id=args.run_id)
    logger.info(
        "S3 6A complete: run_id=%s parameter_hash=%s manifest_fingerprint=%s instrument_base=%s",
        result.run_id,
        result.parameter_hash,
        result.manifest_fingerprint,
        result.instrument_base_path,
    )
