"""CLI for Segment 1B S7 site synthesis."""
...
from engine.layers.l1.seg_1B.s7_site_synthesis.runner import run_s7


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run Segment 1B S7 site synthesis.")
...
def main() -> None:
    logger = get_logger("engine.layers.l1.seg_1B.s7_site_synthesis.cli")
    ...
    result = run_s7(cfg, run_id=args.run_id)
    logger.info(
        "S7 1B complete: run_id=%s parameter_hash=%s manifest_fingerprint=%s s7_site_synthesis=%s run_summary=%s",
        result.run_id,
        result.parameter_hash,
        result.manifest_fingerprint,
        result.synthesis_root,
