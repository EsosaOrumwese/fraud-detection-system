"""CLI for Segment 5A S3 baseline intensity."""
...
from engine.layers.l2.seg_5A.s3_baseline_intensity.runner import run_s3


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run Segment 5A S3 baseline intensity.")
...
def main() -> None:
    logger = get_logger("engine.layers.l2.seg_5A.s3_baseline_intensity.cli")
    ...
    logger.info(
        "S3 5A complete: run_id=%s parameter_hash=%s manifest_fingerprint=%s",
        result.run_id,
