...
from engine.layers.l3.seg_6A.s0_gate.runner import run_s0


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run Segment 6A S0 gate-in.")
...
def main() -> None:
    logger = get_logger("engine.layers.l3.seg_6A.s0_gate.cli")
    ...
    logger.info(
        "S0 6A complete: run_id=%s parameter_hash=%s manifest_fingerprint=%s",
        result.run_id,
