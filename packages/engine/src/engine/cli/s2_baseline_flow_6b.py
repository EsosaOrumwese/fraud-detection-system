"""CLI for Segment 6B S2 baseline flow synthesis."""
...
from engine.layers.l3.seg_6B.s2_baseline_flow.runner import run_s2


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run Segment 6B S2 baseline flow synthesis.")
    ...
    parser.add_argument(
        "--batch-rows",
        type=int,
        default=int(os.getenv("ENGINE_6B_S2_BATCH_ROWS", "250000")),
        help="Rows per parquet batch when streaming arrivals.",
    )
    parser.add_argument(
        "--parquet-compression",
        default=os.getenv("ENGINE_6B_S2_PARQUET_COMPRESSION", "zstd"),
...
def main() -> None:
    logger = get_logger("engine.layers.l3.seg_6B.s2_baseline_flow.cli")
    ...
    logger.info(
        "S2 6B complete: run_id=%s parameter_hash=%s manifest_fingerprint=%s scenarios=%s flows=%s events=%s",
        result.run_id,
