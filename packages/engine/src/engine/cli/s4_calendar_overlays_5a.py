"""CLI for Segment 5A S4 calendar overlays."""
...
from engine.layers.l2.seg_5A.s4_calendar_overlays.runner import run_s4


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run Segment 5A S4 calendar overlays.")
...
def main() -> None:
    logger = get_logger("engine.layers.l2.seg_5A.s4_calendar_overlays.cli")
    ...
    logger.info(
        "S4 5A complete: run_id=%s parameter_hash=%s manifest_fingerprint=%s",
        result.run_id,
