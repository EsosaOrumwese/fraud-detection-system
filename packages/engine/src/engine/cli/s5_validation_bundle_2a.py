"""CLI for Segment 2A S5 validation bundle."""
...
from engine.layers.l1.seg_2A.s5_validation_bundle.runner import run_s5


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run Segment 2A S5 validation bundle.")
...
def main() -> None:
    logger = get_logger("engine.layers.l1.seg_2A.s5_validation_bundle.cli")
    ...
    logger.info(
        "S5 2A complete: run_id=%s parameter_hash=%s manifest_fingerprint=%s decision=%s",
        result.run_id,
