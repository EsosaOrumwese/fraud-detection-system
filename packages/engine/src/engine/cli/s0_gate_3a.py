...
from engine.layers.l1.seg_3A.s0_gate.runner import run_s0


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run Segment 3A S0 gate-in.")
...
def main() -> None:
    logger = get_logger("engine.layers.l1.seg_3A.s0_gate.cli")
    ...
    result = run_s0(cfg, run_id=args.run_id)
    logger.info(
        "S0 3A complete: run_id=%s parameter_hash=%s manifest_fingerprint=%s",
        result.run_id,
