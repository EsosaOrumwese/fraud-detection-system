"""CLI for Segment 1B S4 allocation plan."""
...
from engine.layers.l1.seg_1B.s4_alloc_plan.runner import run_s4


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run Segment 1B S4 allocation plan.")
...
def main() -> None:
    logger = get_logger("engine.layers.l1.seg_1B.s4_alloc_plan.cli")
    ...
    result = run_s4(cfg, run_id=args.run_id)
    logger.info(
        "S4 1B complete: run_id=%s parameter_hash=%s manifest_fingerprint=%s s4_alloc_plan=%s run_report=%s",
        result.run_id,
        result.parameter_hash,
        result.manifest_fingerprint,
        result.alloc_plan_path,
