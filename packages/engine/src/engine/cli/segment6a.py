"""CLI runner for Segment 6A (S0 gate plus optional S1-S5)."""
...
from engine.scenario_runner.l3_seg_6A import Segment6AConfig, Segment6AOrchestrator, Segment6AResult

logger = logging.getLogger(__name__)


def _print_summary(result: Segment6AResult) -> None:
    logger.info("Segment6A completed: manifest=%s", result.manifest_fingerprint)
    logger.info("  sealed_inputs=%s", result.sealed_inputs_path)
    if result.s1_party_base_path:
        logger.info("  party_base=%s", result.s1_party_base_path)
    if result.s5_bundle_index_path:
        logger.info("  validation_bundle=%s", result.s5_bundle_index_path)


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run Segment 6A S0 gate (and optional S1-S5).")
    ...
    parser.add_argument("--dictionary-path", type=Path, help="Override dictionary path for 6A")
    ...
    parser.add_argument("--no-s1", action="store_true", help="Skip S1 party base")
    parser.add_argument("--no-s2", action="store_true", help="Skip S2 account base")
    parser.add_argument("--no-s3", action="store_true", help="Skip S3 instrument base")
    parser.add_argument("--no-s4", action="store_true", help="Skip S4 network build")
    parser.add_argument("--no-s5", action="store_true", help="Skip S5 posture + validation")
...
def main() -> None:
    ...
    orchestrator = Segment6AOrchestrator()
    ...
    if args.result_json:
        ...
        logger.info("Segment6A CLI: wrote summary to %s", args.result_json)
