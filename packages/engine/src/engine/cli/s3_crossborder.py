"""CLI for Segment 1A S3 cross-border candidate set."""
...
from engine.layers.l1.seg_1A.s3_crossborder.runner import run_s3


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run Segment 1A S3 cross-border candidate set."
    )
    parser.add_argument(
        "--contracts-layout",
...
def main() -> None:
    logger = get_logger("engine.layers.l1.seg_1A.s3_crossborder.cli")
    ...
    logger.info(
        "S3 complete: run_id=%s parameter_hash=%s manifest_fingerprint=%s",
        result.run_id,
