"""CLI for Segment 5A S5 validation bundle."""
...
from engine.layers.l2.seg_5A.s5_validation_bundle.runner import run_s5


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run Segment 5A S5 validation bundle.")
...
def main() -> None:
    logger = get_logger("engine.layers.l2.seg_5A.s5_validation_bundle.cli")
    ...
    logger.info(
        "S5 5A complete: run_id=%s parameter_hash=%s manifest_fingerprint=%s",
        result.run_id,
