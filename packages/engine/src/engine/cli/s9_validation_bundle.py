"""CLI for Segment 1B S9 validation bundle."""
...
from engine.layers.l1.seg_1B.s9_validation_bundle.runner import run_s9


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run Segment 1B S9 validation bundle.",
    )
    parser.add_argument(
        "--contracts-layout",
    ...
    parser.add_argument(
        "--validate-only",
        action="store_true",
        help="Validate outputs without writing the bundle.",
    )
    return parser


def main() -> None:
    logger = get_logger("engine.layers.l1.seg_1B.s9_validation_bundle.cli")
    ...
    result = run_s9(cfg, run_id=args.run_id, validate_only=args.validate_only)
    logger.info(
        "S9 complete: run_id=%s parameter_hash=%s manifest_fingerprint=%s decision=%s",
        result.run_id,
