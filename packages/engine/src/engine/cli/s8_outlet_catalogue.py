"""CLI for Segment 1A S8 outlet catalogue."""
...
from engine.layers.l1.seg_1A.s8_outlet_catalogue.runner import run_s8


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run Segment 1A S8 outlet catalogue."
    )
    parser.add_argument(
        "--contracts-layout",
    ...
    parser.add_argument(
        "--validate-only",
        action="store_true",
        help="Validate outputs without writing new datasets.",
    )
    return parser


def main() -> None:
    logger = get_logger("engine.layers.l1.seg_1A.s8_outlet_catalogue.cli")
    ...
    result = run_s8(cfg, run_id=args.run_id, validate_only=args.validate_only)
    logger.info(
        "S8 complete: run_id=%s parameter_hash=%s manifest_fingerprint=%s",
        result.run_id,
