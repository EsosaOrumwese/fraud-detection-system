"""CLI for Segment 6A S4 device/IP graph."""
...
from engine.layers.l3.seg_6A.s4_device_graph.runner import run_s4


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run Segment 6A S4 device/IP graph.")
...
def main() -> None:
    logger = get_logger("engine.layers.l3.seg_6A.s4_device_graph.cli")
    ...
    logger.info(
        "S4 6A complete: run_id=%s parameter_hash=%s manifest_fingerprint=%s device_base=%s ip_base=%s",
        result.run_id,
        result.parameter_hash,
        result.manifest_fingerprint,
        result.device_base_path,
        result.ip_base_path,
    )
