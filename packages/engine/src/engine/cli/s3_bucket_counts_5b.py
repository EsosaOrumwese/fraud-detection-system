"""CLI for Segment 5B S3 bucket-level arrival counts."""
...
from engine.layers.l2.seg_5B.s3_bucket_counts.runner import run_s3


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run Segment 5B S3 bucket-level arrival counts."
    )
...
def main() -> None:
    logger = get_logger("engine.layers.l2.seg_5B.s3_bucket_counts.cli")
    ...
    logger.info(
        "S3 5B complete: run_id=%s parameter_hash=%s manifest_fingerprint=%s",
        result.run_id,
