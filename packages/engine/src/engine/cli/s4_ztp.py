"""CLI for Segment 1A S4 ZTP sampler."""
...
from engine.layers.l1.seg_1A.s4_ztp.runner import run_s4


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run Segment 1A S4 ZTP sampler.")
    parser.add_argument(
        "--contracts-layout",
...
def main() -> None:
    logger = get_logger("engine.layers.l1.seg_1A.s4_ztp.cli")
    ...
    logger.info(
        "S4 complete: run_id=%s parameter_hash=%s manifest_fingerprint=%s",
        result.run_id,
