"""CLI for Segment 3A S6 structural validation."""
...
from engine.layers.l1.seg_3A.s6_validation.runner import run_s6


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run Segment 3A S6 validation + receipt emission.")
...
def main() -> None:
    logger = get_logger("engine.layers.l1.seg_3A.s6_validation.cli")
    ...
    result = run_s6(cfg, run_id=args.run_id)
    logger.info(
        "S6 3A complete: run_id=%s parameter_hash=%s manifest_fingerprint=%s report=%s receipt=%s",
        result.run_id,
