"""CLI for Segment 2B S1 per-merchant weight freezing."""
...
from engine.layers.l1.seg_2B.s1_site_weights.runner import run_s1


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run Segment 2B S1 per-merchant weight freezing.")
...
def main() -> None:
    logger = get_logger("engine.layers.l1.seg_2B.s1_site_weights.cli")
    ...
    logger.info(
        "S1 2B complete: run_id=%s parameter_hash=%s manifest_fingerprint=%s",
        result.run_id,
