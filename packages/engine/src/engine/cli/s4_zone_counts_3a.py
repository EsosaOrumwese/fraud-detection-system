"""CLI for Segment 3A S4 zone counts."""
...
from engine.layers.l1.seg_3A.s4_zone_counts.runner import run_s4


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run Segment 3A S4 zone count integerisation.")
...
def main() -> None:
    logger = get_logger("engine.layers.l1.seg_3A.s4_zone_counts.cli")
    ...
    logger.info(
        "S4 3A complete: run_id=%s parameter_hash=%s manifest_fingerprint=%s output=%s",
        result.run_id,
