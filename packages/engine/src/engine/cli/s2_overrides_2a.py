"""CLI for Segment 2A S2 timezone overrides."""
...
from engine.layers.l1.seg_2A.s2_overrides.runner import run_s2


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run Segment 2A S2 timezone overrides.")
...
def main() -> None:
    logger = get_logger("engine.layers.l1.seg_2A.s2_overrides.cli")
    ...
    logger.info(
        "S2 2A complete: run_id=%s parameter_hash=%s manifest_fingerprint=%s",
        result.run_id,
