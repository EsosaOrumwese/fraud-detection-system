"""CLI for Segment 3A S2 country-zone priors."""
...
from engine.layers.l1.seg_3A.s2_priors.runner import run_s2


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run Segment 3A S2 country-zone priors.")
...
def main() -> None:
    logger = get_logger("engine.layers.l1.seg_3A.s2_priors.cli")
    ...
    logger.info(
        "S2 3A complete: run_id=%s parameter_hash=%s manifest_fingerprint=%s output=%s",
        result.run_id,
