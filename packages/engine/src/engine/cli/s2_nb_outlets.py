"""CLI for Segment 1A S2 NB outlets sampler."""
...
from engine.layers.l1.seg_1A.s2_nb_outlets.runner import run_s2


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run Segment 1A S2 NB outlets sampler."
    )
    parser.add_argument(
        "--contracts-layout",
...
def main() -> None:
    logger = get_logger("engine.layers.l1.seg_1A.s2_nb_outlets.cli")
    ...
    logger.info(
        "S2 complete: run_id=%s parameter_hash=%s manifest_fingerprint=%s",
        result.run_id,
