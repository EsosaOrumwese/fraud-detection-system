"""CLI for Segment 3A S3 zone shares."""
...
from engine.layers.l1.seg_3A.s3_zone_shares.runner import run_s3


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run Segment 3A S3 zone share sampling.")
...
def main() -> None:
    logger = get_logger("engine.layers.l1.seg_3A.s3_zone_shares.cli")
    ...
    logger.info(
        "S3 3A complete: run_id=%s parameter_hash=%s manifest_fingerprint=%s output=%s",
        result.run_id,
