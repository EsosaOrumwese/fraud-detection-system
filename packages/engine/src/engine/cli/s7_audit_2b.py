"""CLI for Segment 2B S7 audit/CI gate."""
...
from engine.layers.l1.seg_2B.s7_audit.runner import run_s7


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run Segment 2B S7 audit/CI gate.")
...
def main() -> None:
    logger = get_logger("engine.layers.l1.seg_2B.s7_audit.cli")
    ...
    logger.info(
        "S7 2B complete: run_id=%s parameter_hash=%s manifest_fingerprint=%s report=%s",
        result.run_id,
        result.parameter_hash,
        result.manifest_fingerprint,
        result.audit_report_path,
    )
