"""CLI for Segment 6A S1 party base."""
...
from engine.layers.l3.seg_6A.s1_party_base.runner import run_s1


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run Segment 6A S1 party base.")
...
def main() -> None:
    logger = get_logger("engine.layers.l3.seg_6A.s1_party_base.cli")
    ...
    logger.info(
        "S1 6A complete: run_id=%s parameter_hash=%s manifest_fingerprint=%s party_base=%s",
        result.run_id,
        result.parameter_hash,
        result.manifest_fingerprint,
        result.party_base_path,
    )
