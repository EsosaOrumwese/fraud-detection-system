"""CLI for Segment 1B S6 in-cell jitter."""
...
from engine.layers.l1.seg_1B.s6_site_jitter.runner import run_s6


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run Segment 1B S6 in-cell jitter."
    )
...
def main() -> None:
    logger = get_logger("engine.layers.l1.seg_1B.s6_site_jitter.cli")
    ...
    logger.info(
        "S6 1B complete: run_id=%s parameter_hash=%s manifest_fingerprint=%s s6_site_jitter=%s run_report=%s",
        result.run_id,
