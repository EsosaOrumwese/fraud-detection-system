"""CLI for Segment 1A S6 foreign set selection."""
...
from engine.layers.l1.seg_1A.s6_foreign_set.runner import run_s6


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run Segment 1A S6 foreign set selection."
    )
    parser.add_argument(
        "--contracts-layout",
    ...
    parser.add_argument(
        "--emit-membership-dataset",
        action="store_true",
        help="Assert membership emission matches policy (no override).",
    )
    parser.add_argument(
        "--log-all-candidates",
        action="store_true",
        help="Assert log_all_candidates matches policy (no override).",
    )
    parser.add_argument(
        "--fail-on-degrade",
        action="store_true",
        help="Fail the run if deterministic empties or cap diagnostics occur.",
    )
    parser.add_argument(
        "--validate-only",
        action="store_true",
        help="Validate and emit receipt without writing datasets.",
    )
    return parser


def main() -> None:
    logger = get_logger("engine.layers.l1.seg_1A.s6_foreign_set.cli")
    ...
    emit_membership = True if args.emit_membership_dataset else None
    log_all_candidates = True if args.log_all_candidates else None
    result = run_s6(
        cfg,
        run_id=args.run_id,
        emit_membership_dataset=emit_membership,
        log_all_candidates=log_all_candidates,
        fail_on_degrade=args.fail_on_degrade,
        validate_only=args.validate_only,
    )
    logger.info(
        "S6 complete: run_id=%s parameter_hash=%s manifest_fingerprint=%s",
        result.run_id,
