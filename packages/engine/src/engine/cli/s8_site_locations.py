"""CLI for Segment 1B S8 site_locations publish."""
...
from engine.layers.l1.seg_1B.s8_site_locations.runner import run_s8


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run Segment 1B S8 site_locations publish.")
...
def main() -> None:
    logger = get_logger("engine.layers.l1.seg_1B.s8_site_locations.cli")
    ...
    result = run_s8(cfg, run_id=args.run_id)
    logger.info(
        "S8 1B complete: run_id=%s parameter_hash=%s manifest_fingerprint=%s site_locations=%s run_summary=%s",
        result.run_id,
        result.parameter_hash,
        result.manifest_fingerprint,
        result.site_locations_root,
