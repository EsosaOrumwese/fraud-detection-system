"""CLI for Segment 1B S5 site-to-tile assignment."""
...
from engine.layers.l1.seg_1B.s5_site_tile_assignment.runner import run_s5


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run Segment 1B S5 site-to-tile assignment."
    )
...
def main() -> None:
    logger = get_logger("engine.layers.l1.seg_1B.s5_site_tile_assignment.cli")
    ...
    logger.info(
        "S5 1B complete: run_id=%s parameter_hash=%s manifest_fingerprint=%s s5_site_tile_assignment=%s run_report=%s",
        result.run_id,
