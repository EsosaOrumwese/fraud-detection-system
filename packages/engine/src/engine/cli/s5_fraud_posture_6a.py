"""CLI for Segment 6A S5 fraud posture."""
...
from engine.layers.l3.seg_6A.s5_fraud_posture.runner import run_s5


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run Segment 6A S5 fraud posture.")
...
def main() -> None:
    logger = get_logger("engine.layers.l3.seg_6A.s5_fraud_posture.cli")
    ...
    logger.info(
        "S5 6A complete: run_id=%s parameter_hash=%s manifest_fingerprint=%s report=%s",
        result.run_id,
