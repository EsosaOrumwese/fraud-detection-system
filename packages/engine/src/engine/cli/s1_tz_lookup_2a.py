"""CLI for Segment 2A S1 provisional tz lookup."""
...
from engine.layers.l1.seg_2A.s1_tz_lookup.runner import run_s1


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run Segment 2A S1 provisional tz lookup.")
...
def main() -> None:
    logger = get_logger("engine.layers.l1.seg_2A.s1_tz_lookup.cli")
    ...
    logger.info(
        "S1 2A complete: run_id=%s parameter_hash=%s manifest_fingerprint=%s",
        result.run_id,
