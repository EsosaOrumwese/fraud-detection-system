"""CLI for Segment 1A S7 integer allocation."""
...
from engine.layers.l1.seg_1A.s7_integerisation.runner import run_s7


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run Segment 1A S7 integer allocation."
    )
    parser.add_argument(
        "--contracts-layout",
...
def main() -> None:
    logger = get_logger("engine.layers.l1.seg_1A.s7_integerisation.cli")
    ...
    result = run_s7(cfg, run_id=args.run_id, validate_only=args.validate_only)
    logger.info(
        "S7 complete: run_id=%s parameter_hash=%s manifest_fingerprint=%s",
        result.run_id,
