"""CLI for Segment 6B S3 fraud overlay."""
...
from engine.layers.l3.seg_6B.s3_fraud_overlay.runner import run_s3


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run Segment 6B S3 fraud overlay.")
    ...
    parser.add_argument(
        "--batch-rows",
        type=int,
        default=int(os.getenv("ENGINE_6B_S3_BATCH_ROWS", "250000")),
        help="Rows per parquet batch when streaming flow/event overlays.",
    )
    parser.add_argument(
        "--parquet-compression",
        default=os.getenv("ENGINE_6B_S3_PARQUET_COMPRESSION", "zstd"),
...
def main() -> None:
    logger = get_logger("engine.layers.l3.seg_6B.s3_fraud_overlay.cli")
    ...
    logger.info(
        "S3 6B complete: run_id=%s parameter_hash=%s manifest_fingerprint=%s scenarios=%s flows=%s events=%s campaigns=%s",
        result.run_id,
