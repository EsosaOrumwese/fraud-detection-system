"""CLI for Segment 3A S7 validation bundle."""
...
from engine.layers.l1.seg_3A.s7_validation_bundle.runner import run_s7


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run Segment 3A S7 validation bundle + PASS flag.")
...
def main() -> None:
    logger = get_logger("engine.layers.l1.seg_3A.s7_validation_bundle.cli")
    ...
    logger.info(
        "S7 3A complete: run_id=%s parameter_hash=%s manifest_fingerprint=%s bundle=%s",
        result.run_id,
