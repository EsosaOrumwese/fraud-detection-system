"""CLI for Segment 3A S1 escalation queue."""
...
from engine.layers.l1.seg_3A.s1_escalation.runner import run_s1


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run Segment 3A S1 escalation queue.")
...
def main() -> None:
    logger = get_logger("engine.layers.l1.seg_3A.s1_escalation.cli")
    ...
    logger.info(
        "S1 3A complete: run_id=%s parameter_hash=%s manifest_fingerprint=%s output=%s",
        result.run_id,
