"""Dataset dictionary helpers for Segment 6A (Layer-3)."""
...
def default_dictionary_path() -> Path:
    """Return the canonical dictionary path for Segment 6A."""

    repo_root = _discover_repo_root()
    return repo_root / "contracts" / "dataset_dictionary" / "l3" / "seg_6A" / "layer3.6A.yaml"
...
def get_dataset_entry(
    ...
    for entry in _iter_entries(dictionary):
        if entry.get("id") == dataset_id:
            return entry
    raise DictionaryError(f"dataset '{dataset_id}' not present in the Segment 6A dictionary")
