...
from engine.layers.l3.seg_6A.shared.control_plane import compute_sealed_inputs_digest, parse_partition_keys
from engine.layers.l3.seg_6A.shared.dictionary import (
    DictionaryError,
    get_dataset_entry,
    load_dictionary,
    repository_root,
...
class S0Inputs:
    """Configuration required to execute 6A S0."""
    ...
    def __post_init__(self) -> None:
        ...
        if not self.run_id:
            raise ValueError("run_id must be provided for 6A S0")
        if self.seed < 0:
...
class S0GateRunner:
    """High-level helper that wires together the 6A S0 workflow."""

    _UPSTREAM_SEGMENTS = ("1A", "1B", "2A", "2B", "3A", "3B", "5A", "5B")
    ...
    _SEALED_CONFIG_DATASETS: tuple[str, ...] = (
        "prior_population_6A",
        "prior_segmentation_6A",
        "taxonomy_party_6A",
        "prior_account_per_party_6A",
        "prior_product_mix_6A",
        "taxonomy_account_types_6A",
        "prior_instrument_per_account_6A",
        "prior_instrument_mix_6A",
        "taxonomy_instrument_types_6A",
        "prior_device_counts_6A",
        "taxonomy_devices_6A",
        "prior_ip_counts_6A",
        "taxonomy_ips_6A",
        "taxonomy_fraud_roles_6A",
        "prior_party_roles_6A",
        "prior_account_roles_6A",
        "prior_merchant_roles_6A",
        "prior_device_roles_6A",
        "prior_ip_roles_6A",
            ...
            rows.append(
                {
                    ...
                    "owner_layer": 3,
                    "owner_segment": "6A",
                    "manifest_key": manifest_key or f"mlr.6A.config.{dataset_id}",
    ...
    def _seal_contracts(self, manifest_fingerprint: str) -> list[Mapping[str, object]]:
        ...
        contract_specs = (
            ...
            {
                "logical_id": "schemas.6A.yaml",
                "path": repo_root / "contracts/schemas/layer3/schemas.6A.yaml",
                "schema_ref": "schemas.6A.yaml",
                "role": "contract",
            },
            {
                "logical_id": "dataset_dictionary.layer3.6A.yaml",
                "path": repo_root / "contracts/dataset_dictionary/l3/seg_6A/layer3.6A.yaml",
                "schema_ref": "schemas.layer3.yaml",
                "role": "contract",
            },
            {
                "logical_id": "artefact_registry_6A.yaml",
                "path": repo_root / "contracts/artefact_registry/artefact_registry_6A.yaml",
                "schema_ref": "schemas.layer3.yaml",
                "role": "contract",
            },
        )
        ...
        for spec in contract_specs:
            path = Path(spec["path"])
            if not path.exists():
                raise FileNotFoundError(f"missing 6A contract file: {path}")
            sha256_hex = self._hash_paths([path])
            ...
            rows.append(
                {
                    ...
                    "owner_layer": 3,
                    "owner_segment": "6A",
                    "manifest_key": f"mlr.6A.contract.{spec['logical_id']}",
                    "path_template": path_template,
                    "partition_keys": [],
    ...
    def _write_sealed_inputs(
        ...
        *,
        ...
        if digest != expected_digest:
            raise RuntimeError("sealed_inputs_6A digest mismatch after write")
        return output_path
    ...
    def _write_receipt(
        ...
        *,
        ...
        payload = {
            "manifest_fingerprint": inputs.manifest_fingerprint,
            "parameter_hash": inputs.parameter_hash,
            "spec_version_6A": "1.0.0",
            "upstream_segments": upstream_bundles,
            "contracts_6A": self._contracts_payload(),
        ...
        logger.info(
            "Segment6A S0 gate: verified %s upstream segments in %sms",
            len(upstream_bundles),
            gate_verify_ms,
        )
        logger.info("Segment6A S0 gate: sealed_inputs_digest=%s", sealed_inputs_digest)
        logger.info("Segment6A S0 gate: started_at=%s", run_started_at.isoformat())
        return output_path
    ...
    def _contracts_payload(self) -> Mapping[str, Mapping[str, str]]:
        ...
        contracts = {
            ...
            "schemas.6A.yaml": repo_root / "contracts/schemas/layer3/schemas.6A.yaml",
            "dataset_dictionary.layer3.6A.yaml": repo_root / "contracts/dataset_dictionary/l3/seg_6A/layer3.6A.yaml",
            "artefact_registry_6A.yaml": repo_root / "contracts/artefact_registry/artefact_registry_6A.yaml",
        }
