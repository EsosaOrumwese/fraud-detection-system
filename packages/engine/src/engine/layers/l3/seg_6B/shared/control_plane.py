"""Shared helpers for loading Segment 6B control-plane artefacts."""
...
def load_control_plane(
    *,
    ...
    """Load `s0_gate_receipt_6B` and `sealed_inputs_6B` and validate digest."""

    dictionary = load_dictionary(dictionary_path)
    template_args = {"manifest_fingerprint": manifest_fingerprint}
    sealed_path = data_root / render_dataset_path(
        dataset_id="sealed_inputs_6B", template_args=template_args, dictionary=dictionary
    )
    receipt_path = data_root / render_dataset_path(
        dataset_id="s0_gate_receipt_6B", template_args=template_args, dictionary=dictionary
    )

    if not sealed_path.exists():
        raise FileNotFoundError(f"sealed_inputs_6B missing at {sealed_path}")
    if not receipt_path.exists():
        raise FileNotFoundError(f"s0_gate_receipt_6B missing at {receipt_path}")
    ...
    receipt = GateReceipt(
        ...
        sealed_inputs_digest=str(receipt_payload.get("sealed_inputs_digest_6B", "")),
...
def _assert_receipt_consistency(
    ...
    if receipt.get("manifest_fingerprint") != manifest_fingerprint:
        raise ValueError("s0_gate_receipt_6B manifest_fingerprint mismatch")
    if receipt.get("parameter_hash") != parameter_hash:
        raise ValueError("s0_gate_receipt_6B parameter_hash mismatch")
...
def _assert_inventory_consistency(
    ...
    if "manifest_fingerprint" not in sealed_df.columns:
        raise ValueError("sealed_inputs_6B missing manifest_fingerprint column")
    ...
    if mf_values and (len(mf_values) != 1 or mf_values[0] != manifest_fingerprint):
        raise ValueError("sealed_inputs_6B manifest_fingerprint mismatch")
...
def _validate_sealed_inputs_digest(
    ...
    expected = receipt.get("sealed_inputs_digest_6B")
    if expected and digest != expected:
        raise ValueError("sealed_inputs_6B digest mismatch with s0_gate_receipt_6B")
