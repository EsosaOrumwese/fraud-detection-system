"""Schema utilities for Segment 1B."""
...
def load_schema_document() -> Mapping[str, Any]:
    """Load the Segment 1B schema bundle."""

    schema_path = _schema_root() / "schemas.1B.yaml"
    if not schema_path.exists():
        raise err(
            "E_SCHEMA_RESOLUTION_FAILED",
            f"segment 1B schema bundle '{schema_path}' missing",
        )
    document = yaml.safe_load(schema_path.read_text(encoding="utf-8")) or {}
    if not isinstance(document, Mapping):
        raise err(
            "E_SCHEMA_RESOLUTION_FAILED",
            f"segment 1B schema bundle '{schema_path}' must decode to a mapping",
        )
    return document


def load_schema(ref: str) -> Mapping[str, Any]:
    """Resolve ``ref`` (JSON pointer) inside the Segment 1B schema bundle."""
