...
## Current Status
- `contexts.py` exposes immutable bundles for deterministic inputs (hyperparameters, merchant view, lineage).
- **L0** now contains frozen label literals plus `ZTPEventWriter`, which stamps all RNG envelopes and trace rows.
- **L1** provides link/regime computation, a Philox substream helper, and the attempt loop logic (`run_sampler`).
- **L2** introduces `S4ZTPTargetRunner`, deriving substreams per merchant and coordinating the L1 kernels.
- **L3** ships `expectations.py`, cataloguing dataset/schema bindings and the failure vocabulary that the validator will enforce.
