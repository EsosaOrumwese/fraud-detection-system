"""Dataset dictionary helpers for Segment 3A."""
...
def default_dictionary_path() -> Path:
    """Return the canonical dictionary path for Segment 3A."""
...
def get_dataset_entry(
    ...
    raise err(
        "E_DICTIONARY_RESOLUTION_FAILED",
        f"dataset '{dataset_id}' not present in the Segment 3A dictionary",
    )
...
def _format_template(template: str, tokens: Mapping[str, object], *, dataset_id: str) -> str:
    try:
