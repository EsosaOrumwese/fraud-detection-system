"""Schema utilities for Segment 3A."""
...
def _schema_root() -> Path:
    ...
    raise err("E_SCHEMA_RESOLUTION_FAILED", "unable to resolve schema root for Segment 3A")
...
def _load_shared_defs() -> Mapping[str, Any]:
    ...
    if _SHARED_DEFS is None:
        ...
        if not isinstance(defs, Mapping):
            raise err(
                "E_SCHEMA_RESOLUTION_FAILED",
                "layer1 schema bundle missing $defs mapping",
            )
        _SHARED_DEFS = defs
    return _SHARED_DEFS
...
def load_schema_document() -> Mapping[str, Any]:
    """Load the Segment 3A schema bundle."""

    schema_path = _schema_root() / "schemas.3A.yaml"
    if not schema_path.exists():
        raise err(
            "E_SCHEMA_RESOLUTION_FAILED",
            f"segment 3A schema bundle '{schema_path}' missing",
        )
    payload = yaml.safe_load(schema_path.read_text(encoding="utf-8")) or {}
    if not isinstance(payload, Mapping):
        raise err(
            "E_SCHEMA_RESOLUTION_FAILED",
            f"segment 3A schema bundle '{schema_path}' must decode to a mapping",
        )
    return payload
...
def _inline_layer1_refs(defs: MutableMapping[str, Any]) -> None:
    shared_defs = _load_shared_defs()
    for key, value in list(defs.items()):
        if isinstance(value, Mapping):
            ref = value.get("$ref")
            if isinstance(ref, str) and ref.startswith("schemas.layer1.yaml#/$defs/"):
                ref_name = ref.split("/")[-1]
                shared_value = shared_defs.get(ref_name)
                if isinstance(shared_value, Mapping):
                    defs[key] = copy.deepcopy(shared_value)
...
def load_schema(ref: str) -> Mapping[str, Any]:
    """Resolve ``ref`` (JSON pointer) inside the Segment 3A schema bundle."""
    ...
    if isinstance(defs_obj, MutableMapping):
        _inline_layer1_refs(defs_obj)
    _ensure_defs(resolved, _load_shared_defs())
