"""S0 gate-in runner for Segment 3A."""
...
def _abort(code: str, validator_id: str, message: str, context: dict, manifest_fingerprint: Optional[str]) -> None:
    logger = get_logger("engine.layers.l1.seg_3A.s0_gate.l2.runner")
...
def _check_catalogue_consistency(
    ...
    for dataset_id in cross_layer_ids:
        entry_3a = find_dataset_entry(dictionary_3a, dataset_id).entry
        upstream_entry = None
        ...
        if _maybe_catalogue_mismatch(entry_3a, upstream_entry, ("path", "schema_ref")):
            _abort(
                "E3A_S0_002_CATALOGUE_MALFORMED",
                "V-02",
                "dictionary_mismatch",
                {
                    "dataset_id": dataset_id,
                    "path_3A": entry_3a.get("path"),
                    "path_upstream": upstream_entry.get("path"),
                    "schema_3A": entry_3a.get("schema_ref"),
...
def _hash_for_asset(asset_id: str, resolved_path: Path, bundle_digest: Optional[str] = None) -> tuple[str, int]:
...
def run_s0(config: EngineConfig, run_id: Optional[str] = None) -> S0GateResult:
    ...
    logger.info(
        "S0: objective=gate 1A/1B/2A and seal inputs (policies + references + upstream egress) -> outputs (s0_gate_receipt_3A, sealed_inputs_3A)"
    )
    ...
    _check_catalogue_consistency(
        dictionary_3a,
        {"1A": dictionary_1a, "1B": dictionary_1b, "2A": dictionary_2a, "2B": dictionary_2b},
        logger,
        manifest_fingerprint,
    )
    ...
    for segment_key, (bundle_id, flag_id, schema_pack, index_anchor) in gate_map.items():
        bundle_entry = find_dataset_entry(dictionary_3a, bundle_id).entry
        flag_entry = find_dataset_entry(dictionary_3a, flag_id).entry
    ...
    for dataset_id in policy_ids:
        entry = find_dataset_entry(dictionary_3a, dataset_id).entry
        reg_entry = find_artifact_entry(registry_3a, dataset_id).entry
        ...
        if dataset_id == "day_effect_policy_v1":
            schema = _schema_from_pack(schema_2b, schema_ref)
        else:
            schema = _schema_from_pack(schema_3a, schema_ref)
        _inline_external_refs(schema, schema_layer1, "schemas.layer1.yaml#")
        errors = list(Draft202012Validator(schema).iter_errors(payload))
        if errors:
            _abort(
    ...
    for dataset_id in required_ids + optional_ids:
        ...
        digest_hex, _ = _hash_for_asset(dataset_id, resolved_path, bundle_digest)
        registry_digest = reg_entry.get("digest")
    ...
    receipt_entry = find_dataset_entry(dictionary_3a, "s0_gate_receipt_3A").entry
    sealed_entry = find_dataset_entry(dictionary_3a, "sealed_inputs_3A").entry
    ...
    receipt_registry = find_artifact_entry(registry_3a, "s0_gate_receipt_3A").entry
    ...
    receipt_payload = {
        ...
        "catalogue_versions": {
            "dictionary_1A": str(dictionary_1a.get("version", "unknown")),
            "dictionary_1B": str(dictionary_1b.get("version", "unknown")),
            "dictionary_2A": str(dictionary_2a.get("version", "unknown")),
            "dictionary_2B": str(dictionary_2b.get("version", "unknown")),
            "dictionary_3A": str(dictionary_3a.get("version", "unknown")),
            "registry_1A": str(registry_1a.get("version", "unknown")),
            "registry_1B": str(registry_1b.get("version", "unknown")),
            "registry_2A": str(registry_2a.get("version", "unknown")),
            "registry_2B": str(registry_2b.get("version", "unknown")),
            "registry_3A": str(registry_3a.get("version", "unknown")),
        },
        "sealed_policy_set": sealed_policy_set,
    }
    receipt_schema = _schema_from_pack(schema_3a, "validation/s0_gate_receipt_3A")
    ...
    _atomic_publish_json(sealed_inputs_path, sealed_inputs_rows, logger, "sealed_inputs_3A")
    _atomic_publish_json(receipt_path, receipt_payload, logger, "s0_gate_receipt_3A")

    utc_day = utc_now_rfc3339_micro()[:10]
    run_report_path = _segment_state_runs_path(run_paths, dictionary_3a, utc_day)
    _append_jsonl(
        run_report_path,
        {
            "layer": "layer1",
