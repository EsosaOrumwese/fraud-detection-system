"""Dataset dictionary helpers for Segment 5B (Layer-2)."""
...
def default_dictionary_path() -> Path:
    """Return the canonical dictionary path for Segment 5B."""

    repo_root = _discover_repo_root()
    return repo_root / "contracts" / "dataset_dictionary" / "l2" / "seg_5B" / "layer2.5B.yaml"
...
def load_dictionary(path: Path | None = None) -> Mapping[str, object]:
    ...
    if not isinstance(payload, MutableMapping):
        raise DictionaryError(f"dataset dictionary '{dictionary_path}' must decode to a mapping")
    return payload
...
def get_dataset_entry(dataset_id: str, *, dictionary: Mapping[str, object] | None = None) -> Mapping[str, object]:
        ...
        if isinstance(section, Mapping):
            ...
            if entry is not None:
                if not isinstance(entry, Mapping):
                    raise DictionaryError(f"dictionary entry '{dataset_id}' must be a mapping")
                return entry
            ...
            for item in section:
                ...
                if item.get("id") == dataset_id:
                    return item
    raise DictionaryError(f"dataset '{dataset_id}' not present in the Segment 5B dictionary")
