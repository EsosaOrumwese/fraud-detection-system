"""Dataset dictionary helpers for Segment 5A (Layer-2)."""
...
def default_dictionary_path() -> Path:
    """Return the canonical dictionary path for Segment 5A."""

    repo_root = _discover_repo_root()
    return repo_root / "contracts" / "dataset_dictionary" / "l2" / "seg_5A" / "layer2.5A.yaml"
        ...
        if isinstance(section, Mapping):
            ...
            for item in section:
                ...
                if item.get("id") == dataset_id:
                    return item
    raise DictionaryError(f"dataset '{dataset_id}' not present in the Segment 5A dictionary")
