...
- Given that we are undertaking this work as though we are in a role at a startup/company, we aren't just looking at the data at pre-ingestion, meaning that is hasn't passed through the platform at all. But we are currently at the Offline aspects of things, looking at our 3 months data, and assuming that these are the transactions or information that is available to us as an extract from our company's database for which we are to investigate and later on provide insights on to stakeholders.
   - This is to ensure we see our data in its use context and with meaning and not just as meaningless data with columns and rows
