...
def test_kafka_reader_performs_second_standard_poll_on_fresh_latest_boundary(monkeypatch) -> None:
    class _Message:
        def __init__(self, offset: int, payload: dict[str, object]):
            self._offset = offset
            self._payload = json.dumps(payload).encode("utf-8")
    ...
    reader = kafka.KafkaEventBusReader(
        kafka.KafkaReaderConfig(
            bootstrap_servers="localhost:9092",
            security_protocol="PLAINTEXT",
            client_id="test-standard-second-poll",
        )
    )

    rows = reader.read(topic="demo", partition=0, from_offset=None, limit=10, start_position="latest")

    assert [row["offset"] for row in rows] == [100]
    assert reader._consumer.poll_calls == 2
...
def test_kafka_reader_performs_second_oauth_poll_on_fresh_latest_boundary(monkeypatch) -> None:
    class _PyTopicPartition:
        def __init__(self, topic, partition):
            self.topic = topic
            self.partition = partition
    ...
    reader = kafka.KafkaEventBusReader(
        kafka.KafkaReaderConfig(
            ...
            sasl_mechanism="OAUTHBEARER",
            aws_region="eu-west-2",
            client_id="test-oauth-second-poll",
        )
    )
