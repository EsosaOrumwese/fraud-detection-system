...
from fraud_detection.scenario_runner.models import (
    ReemitGovernanceOverride,
    ReemitKind,
    ReemitRequest,
    RunPlan,
    RunStatusState,
...
def test_reemit_cross_run_requires_override(tmp_path: Path) -> None:
    wiring = _build_wiring(tmp_path)
    wiring.reemit_same_platform_run_only = True
    wiring.reemit_cross_run_override_required = True
    wiring.reemit_cross_run_reason_allowlist = ["INCIDENT_RECOVERY", "AUDIT_REPLAY", "OPERATOR_BACKFILL"]
    policy = _build_policy()
    runner = ScenarioRunner(wiring, policy, LocalEngineInvoker())
    source_platform_run_id = runner.platform_run_id

    run_id = run_id_from_equivalence_key("reemit-cross-run-requires-override")
    anchor_event = runner._event("RUN_ACCEPTED", run_id, {"intent_fingerprint": "x"})
    runner.ledger.anchor_run(run_id, anchor_event)

    facts_view = {
        "run_id": run_id,
        "platform_run_id": source_platform_run_id,
        "scenario_run_id": run_id,
        "pins": {
            "manifest_fingerprint": "a" * 64,
            "parameter_hash": "b" * 64,
    ...
    response = runner.reemit(
        ReemitRequest(
            run_id=run_id,
            reemit_kind=ReemitKind.READY_ONLY,
            emit_platform_run_id="platform_20260102T000000Z",
        )
    )
    assert response.published == []
    assert response.message == "Cross-run reemit blocked; governance override evidence required."

    events = runner.ledger.read_record_events(run_id)
    failed = [event for event in events if event.get("event_kind") == "REEMIT_FAILED"]
    assert failed
    assert failed[-1]["details"].get("reason") == "CROSS_RUN_OVERRIDE_REQUIRED"


def test_reemit_cross_run_with_override_allowed(tmp_path: Path) -> None:
    wiring = _build_wiring(tmp_path)
    wiring.reemit_same_platform_run_only = True
    wiring.reemit_cross_run_override_required = True
    wiring.reemit_cross_run_reason_allowlist = ["INCIDENT_RECOVERY", "AUDIT_REPLAY", "OPERATOR_BACKFILL"]
    ...
    runner = ScenarioRunner(wiring, policy, LocalEngineInvoker())
    source_platform_run_id = runner.platform_run_id
    target_platform_run_id = "platform_20260102T000000Z"

    run_id = run_id_from_equivalence_key("reemit-cross-run-override")
    anchor_event = runner._event("RUN_ACCEPTED", run_id, {"intent_fingerprint": "x"})
    runner.ledger.anchor_run(run_id, anchor_event)
    ...
    override = ReemitGovernanceOverride(
        override_id="ovr-001",
        reason_code="INCIDENT_RECOVERY",
        evidence_ref="s3://fraud-platform/dev_min/evidence/reemit/ovr-001.json",
        approved_by="ops-user",
        approved_at_utc=datetime(2026, 2, 11, tzinfo=timezone.utc),
    )
    response = runner.reemit(
        ReemitRequest(
            run_id=run_id,
            reemit_kind=ReemitKind.READY_ONLY,
            emit_platform_run_id=target_platform_run_id,
            cross_run_override=override,
        )
    )
    assert response.status_state == RunStatusState.READY
    assert response.published

    expected_key = hash_payload(f"ready|{target_platform_run_id}|{run_id}|{facts_view['bundle_hash']}")
    assert expected_key in response.published
    message = _read_bus_message(Path(wiring.control_bus_root), wiring.control_bus_topic, expected_key)
    payload = message["payload"]
    assert payload["platform_run_id"] == target_platform_run_id

    events = runner.ledger.read_record_events(run_id)
    override_events = [event for event in events if event.get("event_kind") == "GOV_REEMIT_OVERRIDE_APPROVED"]
    assert override_events
    assert override_events[-1]["details"].get("override_id") == "ovr-001"
